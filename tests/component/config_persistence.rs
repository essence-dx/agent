...
use dx_agents::config::{Config, MemoryConfig};
use std::fs;
...
fn config_toml_roundtrip_preserves_provider() {
    use dx_agents::config::{DeepseekModelProviderConfig, ModelProviderConfig};
    ...
    let parsed = dx_agents::config::migration::migrate_to_current(&toml_str)
...
fn config_file_write_read_roundtrip() {
    use dx_agents::config::{MistralModelProviderConfig, ModelProviderConfig};
    ...
    let parsed = dx_agents::config::migration::migrate_to_current(&read_back)
        .expect("TOML should round-trip through migration");
