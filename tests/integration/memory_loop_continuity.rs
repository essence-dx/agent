...
use dx_agents::memory::sqlite::SqliteMemory;
use dx_agents::memory::traits::{Memory, MemoryCategory};
use dx_agents::providers::ToolCall;
...
async fn compressor_with_memory_saves_summary() {
    use dx_agents::agent::context_compressor::{ContextCompressionConfig, ContextCompressor};
    use dx_agents::providers::traits::ChatMessage;
...
async fn agent_survives_large_tool_output() {
    use dx_agents::tools::{Tool, ToolResult};
...
async fn consolidation_extracts_facts_to_memory() {
    ...
    let result = dx_agents::memory::consolidation::consolidate_turn(
        &model_provider,
        "test-model",
        None,
        mem.as_ref(),
...
async fn memory_survives_rapid_consolidation() {
    ...
    for i in 0..10 {
        ...
        let _ = dx_agents::memory::consolidation::consolidate_turn(
            &model_provider,
            "test-model",
            None,
            mem.as_ref(),
...
async fn session_backend_persists_messages() {
    use dx_agents::channels::session_backend::SessionBackend;
    use dx_agents::channels::session_sqlite::SqliteSessionBackend;
    use dx_agents::providers::traits::ChatMessage;
