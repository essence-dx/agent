...
# ── DX Agents installer ──────────────────────────────────────────
# Builds and installs DX Agents from source.
...
BIN_NAME="dx-agents"
RELEASE_REPO="${DX_AGENTS_RELEASE_REPO:-millercarla211-ctrl/dx-agents}"
REPO_URL="${DX_AGENTS_REPO_URL:-https://github.com/${RELEASE_REPO}.git}"
ASSET_PREFIX="${DX_AGENTS_ASSET_PREFIX:-dx-agents}"
...
# fixed list: the desktop app needs the Tauri toolchain + webview deps,
...
# (the TUI) is the default app. Tauri-based apps
...
install_prebuilt() {
  ...
  version=$(curl -fsSL "https://api.github.com/repos/${RELEASE_REPO}/releases/latest" |
  ...
  asset_name="${ASSET_PREFIX}-${triple}.tar.gz"
  asset_url="https://github.com/${RELEASE_REPO}/releases/download/${version}/${asset_name}"
  sha256_url="https://github.com/${RELEASE_REPO}/releases/download/${version}/SHA256SUMS"

  echo
  printf "%s\n" "$(bold "Installing $PRODUCT_NAME ${version} (pre-built)")"
  ...
  *)
  ...
  if [ "$DRY_RUN" = true ]; then
    ...
    info "[dry-run] Would install to $CARGO_HOME/bin/$BIN_NAME"
  ...
  if [ ! -f "$tmp_dir/$BIN_NAME" ]; then
    warn "Release archive did not contain $BIN_NAME — falling back to source build"
    rm -rf "$tmp_dir"
    trap - EXIT
    return 1
  fi
  mkdir -p "$CARGO_HOME/bin"
  install -m 755 "$tmp_dir/$BIN_NAME" "$CARGO_HOME/bin/$BIN_NAME"
...
Options:
  ...
  --uninstall          Remove $PRODUCT_NAME binary and optionally config/data
...
Examples:
  ...
  $0 --uninstall                               # remove $PRODUCT_NAME
...
do_uninstall() {
  echo
  printf "%s\n" "$(bold "Uninstalling $PRODUCT_NAME")"
  echo

  local bin="$CARGO_HOME/bin/$BIN_NAME"
  ...
  # Check if another DX Agents binary still lurks in PATH
  local other_bin
  other_bin=$(PATH="$ORIGINAL_PATH" command -v "$BIN_NAME" 2>/dev/null || true)
  if [ -n "$other_bin" ]; then
    local other_version
    other_version=$("$other_bin" --version 2>/dev/null | awk '{print $NF}' || echo "unknown")
    echo
    warn "Another $BIN_NAME found at $other_bin (v$other_version)"
    warn "Remove it manually if you want a full uninstall"
  fi

  echo
  info "$PRODUCT_NAME uninstalled"
  exit 0
}
...
# re-install. Provider-only configs are not enough: a fresh operator can have
# credentials written but still no runnable `[agents.<alias>]` route.
config_has_model_provider() {
  cfg="$1"
  grep -qE '^[[:space:]]*\[providers\.models\.[^]]+\]' "$cfg" 2>/dev/null ||
    grep -qE '^[[:space:]]*\[model_providers(\.|])' "$cfg" 2>/dev/null ||
    grep -qE '^[[:space:]]*\[providers\.fallback\]|^[[:space:]]*fallback[[:space:]]*=|^[[:space:]]*default_provider[[:space:]]*=|^[[:space:]]*\[provider\]' "$cfg" 2>/dev/null
}

config_has_runnable_agent() {
  cfg="$1"
  awk '
    /^[[:space:]]*#/ { next }
    /^[[:space:]]*\[agents\.[[:alnum:]_-]+\][[:space:]]*$/ { in_agent = 1; next }
    /^[[:space:]]*\[agent\][[:space:]]*$/ { in_agent = 1; next }
    /^[[:space:]]*\[/ { in_agent = 0; next }
    in_agent && /^[[:space:]]*model_provider[[:space:]]*=/ { found = 1 }
    END { exit found ? 0 : 1 }
  ' "$cfg"
}
...
onboarding_needed() {
  ...
  return 0 # config exists but is incomplete → onboard
}
...
# Support DX Agents env vars first, then legacy env vars.
...
while [ $# -gt 0 ]; do
  ...
  -V | --version)
    if [ -f "Cargo.toml" ]; then
      ...
      echo "install.sh for $PRODUCT_NAME v$VERSION"
    else
...
if [ "$INSTALL_MODE" = "" ]; then
  ...
  if [ -n "$triple" ]; then
    if [ -t 0 ]; then
      echo
      printf "  %s\n" "$(bold "How would you like to install $PRODUCT_NAME?")"
...
if [ "${SOURCE_SKIPPED:-false}" != true ]; then
  ...
  echo
  printf "%s\n" "$(bold "$PRODUCT_NAME — source install")"
  if [ "$PREFIX" != "$HOME" ]; then
    printf "  prefix: %s\n" "$(bold "$PREFIX")"
  fi
  echo

  if [ -f "Cargo.toml" ] && grep -q 'name = "dx-agents"' "Cargo.toml" 2>/dev/null; then
    INSTALL_DIR="$(pwd)"
  ...
  if [ "$DRY_RUN" != true ]; then
    ...
    if ! version_gte "$RUST_VERSION" "$MSRV"; then
      die "Rust $RUST_VERSION is too old. $PRODUCT_NAME requires $MSRV+ (edition $EDITION). Run: rustup update stable"
    fi
...
See all available features:
  ...
  PATH_BIN=$(PATH="$ORIGINAL_PATH" command -v "$BIN_NAME" 2>/dev/null || true)
  if [ -n "$PATH_BIN" ]; then
    PATH_VERSION=$("$PATH_BIN" --version 2>/dev/null | awk '{print $NF}' || echo "unknown")
    TARGET_BIN="$CARGO_HOME/bin/$BIN_NAME"
    if [ "$PATH_BIN" != "$TARGET_BIN" ]; then
      warn "$BIN_NAME found at $PATH_BIN (v$PATH_VERSION)"
  ...
  echo
  printf "%s\n" "$(bold "Building $PRODUCT_NAME v$VERSION")"
  ...
  *--no-default-features*)
  ...
  # Apps connect to the runtime RPC server, so they need the
  ...
  *--no-default-features*)
  ...
  if [ "$DRY_RUN" != true ]; then
    ...
    if [ -f "$BIN" ]; then
      ...
      ACTIVE_BIN=$(PATH="$ORIGINAL_PATH" command -v "$BIN_NAME" 2>/dev/null || true)
      if [ -n "$ACTIVE_BIN" ] && [ "$ACTIVE_BIN" != "$BIN" ]; then
        ACTIVE_VERSION=$("$ACTIVE_BIN" --version 2>/dev/null | awk '{print $NF}' || echo "unknown")
        echo
        warn "$(bold "WARNING:") $BIN_NAME in your PATH is $ACTIVE_BIN (v$ACTIVE_VERSION)"
...
if [ "$SKIP_ONBOARD" = false ] && [ "$DRY_RUN" != true ] && [ -f "$BIN" ]; then
  ...
  if ! onboarding_needed; then
    ...
    echo
    printf "%s\n" "$(bold "$PRODUCT_NAME installed. How would you like to complete setup?")"
    printf "  [1] CLI quickstart  ($BIN_NAME quickstart)\n"
    printf "  [2] Open gateway in browser ($BIN_NAME daemon + dashboard)\n"
    ...
    1 | "")
      echo
      "$BIN" quickstart || warn "Quickstart exited with an error — run '$BIN_NAME quickstart' manually"
      ;;
    2)
      echo
      ...
      info "Stop the daemon with Ctrl+C when done; then run '$BIN_NAME service install' for always-on."
      "$BIN" daemon || warn "Daemon exited with an error — run '$BIN_NAME daemon' manually"
      ;;
    3)
      info "Skipped setup. Run '$BIN_NAME quickstart' (CLI) or '$BIN_NAME daemon' (browser) when ready."
      ;;
    *)
      warn "Unknown choice '$onboard_choice' — skipping. Run '$BIN_NAME quickstart' to configure."
      ;;
    esac
  else
    info "Non-interactive — skipping setup prompt. Run '$BIN_NAME quickstart' to configure."
  fi
fi
...
elif [ -f "$CARGO_HOME/bin/$BIN_NAME" ] && "$CARGO_HOME/bin/$BIN_NAME" --help 2>/dev/null | grep -q '\bdaemon\b'; then
  info "Done. Run $(bold "$BIN_NAME daemon") for the always-on daemon + web dashboard,"
  info "or $(bold "$BIN_NAME agent") for a one-off CLI chat."
else
  info "Done. Run $(bold "$BIN_NAME agent") to start chatting."
fi
echo
