...
DX_AGENT_providers__models__anthropic__home__api_key=sk-ant-...
...
DX_AGENT_providers__models__openrouter__prod_v2__api_key=sk-or-...

# Toggle and configure a channel
DX_AGENT_channels__matrix__enabled=true
DX_AGENT_channels__matrix__homeserver=https://matrix.example.org

# Override gateway runtime knobs
DX_AGENT_gateway__request_timeout_secs=120
DX_AGENT_gateway__long_running_request_timeout_secs=900

# Point the gateway at a built web dashboard (absolute path; no ~ / $HOME)
DX_AGENT_gateway__web_dist_dir=/srv/zeroclaw/web/dist

# Inject webhook signing secrets
DX_AGENT_channels__whatsapp__home__app_secret=...
DX_AGENT_channels__linq__home__signing_secret=...
DX_AGENT_channels__nextcloud_talk__home__webhook_secret=...

# Inject Qdrant memory backend connection
DX_AGENT_storage__qdrant__home__url=https://qdrant.example.com
DX_AGENT_storage__qdrant__home__collection=zeroclaw
DX_AGENT_storage__qdrant__home__api_key=...
```
...
| TOML | Env var |
|---|---|
| `[providers.models.anthropic.home] api_key = "..."` | `DX_AGENT_providers__models__anthropic__home__api_key=...` |
| `[channels.matrix] homeserver = "..."` | `DX_AGENT_channels__matrix__homeserver=...` |
| `[gateway] request_timeout_secs = 120` | `DX_AGENT_gateway__request_timeout_secs=120` |
| `[gateway] web_dist_dir = "/srv/zeroclaw/web/dist"` | `DX_AGENT_gateway__web_dist_dir=/srv/zeroclaw/web/dist` |
...
Unresolvable `DX_AGENT_<lowercase_*>` names (typos, paths that don't match any prop in the schema) abort startup with a hard error naming the offending env var. Env-var names without the `DX_AGENT_` prefix are not read by this override layer.
...
2. **Web Config editor** — every `ListEntry` carries an `is_env_overridden` bool. Env-overridden field rows render the 💉 badge and a persistent warning *"Edits here won't take effect — overridden by DX_AGENT_..."* so operators see the override without having to attempt an edit.
...
1. **Prefix the path with `DX_AGENT_`.** The dotted TOML path is the source of truth — find the field in your `config.toml` (or in `zeroclaw config schema`).
...
For example, `[providers.models.anthropic.home] api_key = "sk-..."` lives at the dotted path `providers.models.anthropic.home.api_key`. Apply the three rules and the env var is `DX_AGENT_providers__models__anthropic__home__api_key=sk-...`. Same mechanical mapping for any field in any section.
...
export DX_AGENT_providers__models__anthropic__home__api_key="$ANTHROPIC_API_KEY"
export DX_AGENT_providers__models__openai__home__api_key="$OPENAI_API_KEY"
export DX_AGENT_providers__models__openrouter__home__api_key="$OPENROUTER_API_KEY"
export DX_AGENT_storage__qdrant__home__url="$QDRANT_URL"
export DX_AGENT_storage__qdrant__home__api_key="$QDRANT_API_KEY"
export DX_AGENT_gateway__request_timeout_secs="$GATEWAY_TIMEOUT_SECS"
```

```powershell
# PowerShell — drop into $PROFILE
$env:DX_AGENT_providers__models__anthropic__home__api_key = $env:ANTHROPIC_API_KEY
$env:DX_AGENT_providers__models__openai__home__api_key    = $env:OPENAI_API_KEY
$env:DX_AGENT_storage__qdrant__home__url                  = $env:QDRANT_URL
```
