...
DX Agent's Signal channel talks to a running `signal-cli` HTTP daemon. Signal does not provide an official bot API, so DX Agent connects to `signal-cli` over local HTTP and lets `signal-cli` own the Signal account, device keys, and message transport.

Use this channel when you already operate a Signal account with `signal-cli`, or when you can run the daemon next to DX Agent. If you only have the Signal desktop or mobile app installed, that is not enough by itself; DX Agent needs the HTTP daemon endpoint.
...
- A DX Agent build with the `channel-signal` feature enabled.
...
Start the daemon first, then start DX Agent channels:
...
Use `zeroclaw channel doctor` to confirm DX Agent can load the configured channel. If the channel fails at runtime, check that `http_url` points at the daemon, the account is registered in `signal-cli`, and the build includes `channel-signal`.

## Common confusion

The `signal-cli` project is primarily known as a CLI, but DX Agent needs its HTTP daemon mode. If you installed only the command-line binary and never started the daemon, DX Agent has nothing to connect to.
