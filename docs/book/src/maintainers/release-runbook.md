...
That is the core release process. Docker still runs as a downstream release
job. External distribution dry-runs, website redeploys, and announcements are
fail-closed and require explicit DX repository variables before they run.
...
```
https://github.com/millercarla211-ctrl/dx-agents/actions/workflows/release-stable-manual.yml
```
...
Website redeploys, package-manager dry-runs, and announcements are not release
gates. They run only when the matching `DX_RELEASE_ENABLE_*` repository
...
You do not need to manually verify disabled downstream channels. Check Docker
only when its job is enabled and green, and check opt-in website, package
dry-run, or announcement jobs only when their repository variables enabled
...
DX Agents docs use a versioned structure on the `gh-pages` branch. When a tag
is pushed, the `Deploy mdBook docs to Pages` workflow automatically builds and
deploys the documentation for that version.
...
**A distribution channel dry-run failed (Scoop, AUR, Homebrew):** Each has a
corresponding manually-triggerable sub-workflow. Keep `dry_run: true` until
the DX-owned package manifest, repository, and credential story has been
reviewed. These are nice-to-have — a failed Scoop dry-run does not invalidate
the release itself.
