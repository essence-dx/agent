...
DX Agent has exactly one logging surface: the `dx_agent_log::record!` macro. Every emission in the workspace — agent loop activity, channel I/O, cron runs, tool calls, memory ops, session lifecycle, errors — flows through it. The macro feeds a single `LogCaptureLayer` that materializes structured `LogEvent` records and routes them to three sinks at once:

1. The terminal (via the `tracing-subscriber` fmt layer that `dx-agent-log` installs internally) so operators see colored, alias-prefixed lines on stderr.
...
The `tracing` crate is `dx-agent-log`'s implementation detail. No other workspace crate references `tracing`, `tracing-subscriber`, or `tracing-attributes`. Their Cargo.toml files do not depend on those crates, and no `.rs` file outside `crates/dx-agent-log/` names a tracing type.
...
```rust
use dx_agent_log::{record, Event, Action, EventCategory, EventOutcome};
...
`module_path!()` is the canonical source of the event name — it's the Rust module path of the call site (e.g. `dx_agent_channels::telegram`), so events are searchable, jump-to-source-able, and impossible to typo. The same convention is used at every `record!` site in the workspace.
...
All four are closed enums defined in `crates/dx-agent-log/src/event.rs`. Adding a value is the only point of change — call sites do not invent strings.
...
Lives in `crates/dx-agent-api/src/attribution.rs` so every crate can implement it without depending on `dx-agent-log`:
...
let span = dx_agent_log::attribution_span!(self);  // self impls Attributable
async move {
...
Field keys that match the alias-bound `ATTRIBUTION_FIELDS` / `COMPOSITE_PREFIXES` (in `crates/dx-agent-log/src/event.rs`) land in the typed attribution slot; everything else lands in the event `attributes` map for every descendant emission.

## Tool input/output propagation

The central tool executor (`crates/dx-agent-runtime/src/agent/tool_execution.rs::execute_one_tool`) wraps every `Tool::execute(args)` call with start/complete/fail events:
...
The layer in `crates/dx-agent-log/src/layer.rs` is a `tracing-subscriber` Layer that:

1. On span creation/record with target `"dx_agent_log_internal_attribution"` (the target the `attribution_span!` macro opens with): parses the role + alias fields into a `DxAgentAttribution` snapshot stored on the span's extensions.
2. On span creation/record with target `"dx_agent_log_internal_scope"` (`scope!`-opened): parses ad-hoc kvps and stashes them similarly.
3. On event emission with target `"dx_agent_log_event"` (the target the `record!` macro fires through): builds a `LogEvent` from the `zc_*` field set, walks the span scope leaf→root merging every attribution snapshot it finds, parses the `zc_attrs` JSON blob into the event `attributes`, attaches `_file`/`_line` from auto-captured source location, and writes the final event to:
...
`dx-agent-log` defines its own minimal `LogConfig` (in `crates/dx-agent-log/src/config.rs`) — `log_persistence`, `log_persistence_path`, `log_persistence_max_entries`, `log_tool_io`, `log_tool_io_truncate_bytes`, `log_tool_io_denylist`. This breaks what would otherwise be a dep cycle: `dx-agent-config::ObservabilityConfig` carries the full schema (with TOML deserialization and validation), and the runtime converts to `LogConfig` at startup via `crates/dx-agent-runtime/src/observability/runtime_trace.rs::to_log_config`. The result: `dx-agent-config` can `record!` without inverting the dep tree.
...
```rust
dx_agent_log::install_global_subscriber("info,matrix_sdk=warn,matrix_sdk_base=warn");
```

That single call sets up the agent-alias-prefixed terminal formatter + the `LogCaptureLayer` over a `tracing-subscriber::Registry`. `src/main.rs` is the only place that calls it. Tests use `dx_agent_log::try_install_capture_subscriber()` + `dx_agent_log::subscribe_or_install()` to drain emitted events through the broadcast hook without any tracing types named in the test crate.
