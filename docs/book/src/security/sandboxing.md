...
Firejail's default profile is fairly permissive; DX Agent applies a custom profile. Pass extra args with `firejail_args` on the risk profile.
...
Native macOS sandbox (`sandbox-exec`). Profiles are SBPL — DX Agent bundles one for tool runs. Works on macOS 10.11+.
...
No sandboxing. Tools run with the full privileges of the DX Agent service user. This is what YOLO mode enables. Loud, obvious, intentional.
...
- Detection: `crates/dx-agent-runtime/src/security/detect.rs`
- Backends: `crates/dx-agent-runtime/src/security/sandbox/` (one file per backend)
- Schema: `RiskProfileConfig` and `DockerRuntimeConfig` in `crates/dx-agent-config/src/schema.rs`
