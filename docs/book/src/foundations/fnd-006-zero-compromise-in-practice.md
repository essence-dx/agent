...
>
> This is the sixth document in DX Agent's maturity framework. The five before it
...
This RFC is the sixth in a set of documents that together form DX Agent's maturity
...
This distinction matters especially in this project's context. DX Agent is operated in
...
|--------|-------|-------------------|
| `dx-agent-config/src/schema.rs` | 16,800 lines | Now the largest file in the codebase; the original `loop_.rs` was called out at 9,500 lines in the architecture RFC — this surpasses it |
| `dx-agent-channels/src/orchestrator/mod.rs` | 11,813 lines | Second-largest file; a single module carrying concentrated responsibility |
| `dx-agent-runtime/src/onboard/wizard.rs` | 7,988 lines | A single workflow in a single file |
| `dx-agent-runtime/src/agent/loop_.rs` | 6,101 lines | Reduced from ~9,500 in the monolith — real, measurable progress; still large |
| `dx-agent-channels/src/orchestrator/telegram.rs` | 5,122 lines | One channel implementation; one file |
...
| Public functions in `dx-agent-api` | 371 | The entire foundational API surface — every other crate depends on this |
| Doc comment lines in `dx-agent-api` | ~27 | Roughly 14:1 ratio of undocumented public API — see §4.2 |
...
- Whether the public functions in `dx-agent-api` can be correctly implemented by someone
...
The trait layer in `dx-agent-api` is the right architecture. `Provider`, `Channel`,
...
start DX Agent because of a misconfiguration should leave the process with a clear
...
The `dx-agent-api` situation is specific enough to name directly. This is the one crate
...
At minimum, every public item in `dx-agent-api` should carry:
...
isolation. `dx-agent-tool-call-parser` should be testable with a `&str` input and no
runtime. `dx-agent-config` should be testable by constructing config structs directly.
Trait implementations in `dx-agent-api` should be testable against fake implementations
...
**Blast radius.** Debt in `dx-agent-api` — the foundation everything else depends on —
...
size, type, or content of data you did not produce. The DX Agent security model defines
...
development, but in the hands of users running DX Agent on hardware you will never see,
