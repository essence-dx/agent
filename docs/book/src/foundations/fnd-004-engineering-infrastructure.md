...
```
PR changes: crates/dx-agent-tool-call-parser/src/lib.rs

Affected crates:
  dx-agent-tool-call-parser     ← directly changed
  zeroclaw-misc                 ← depends on it
  zeroclawlabs (root)           ← depends on it

Not affected:
  dx-agent-channels             ← no dependency path
  dx-agent-memory               ← no dependency path
  dx-agent-providers            ← no dependency path
```
...
1. **Classify the advisory**: Is the affected crate a direct dependency or transitive? Does DX Agent call the vulnerable code path? Is there a fixed version available?
...
- Crates with `version.workspace = true` are bumped together; independently-versioned crates (`dx-agent-api`, hardware library crates) are handled separately per the versioning policy
...
For DX Agent's current scale and team size, **SLSA Level 2** is the appropriate target:
...
The categories that matter for DX Agent's changelog:
...
**Why this phase:** By v0.8.0 the workspace will have grown further. Running the full pipeline on every PR will be increasingly expensive. Contributors to `dx-agent-tool-call-parser` should not wait 30 minutes for a gateway rebuild.
...
- A PR touching only `dx-agent-tool-call-parser` runs tests for that crate and its dependents, not the full workspace
...
Configure `release-plz` for the workspace. Workspace application crates use `version.workspace = true`. `dx-agent-api` and hardware library crates are configured with independent release settings. The `version-sync.yml` workflow is retired.
