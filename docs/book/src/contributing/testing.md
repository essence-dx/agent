...
DX Agent uses a five-level testing taxonomy backed by filesystem layout. Each level has a different boundary and a different cost — pick the lowest level that proves what you need to prove.
...
- Read credentials from `env::var("DX_AGENT_TEST_*")`. Don't read from `~/.dx_agent/config.toml` — live tests should be hermetic.
