#!/usr/bin/env bash
# deploy-rpi.sh — cross-compile DX Agents for Raspberry Pi and deploy via SSH.
#
...
#   RPI_DIR         — remote deployment dir           (default: /home/$RPI_USER/dx-agents)
...
APP_NAME="dx-agents"
SERVICE_NAME="${DX_AGENTS_RPI_SERVICE_NAME:-dx-agents}"
LEGACY_SERVICE_NAME="zeroclaw"
RPI_DIR="${RPI_DIR:-/home/${RPI_USER}/${APP_NAME}}"
...
echo "==> Building DX Agents for Raspberry Pi (${TARGET})"
...
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo systemctl stop ${SERVICE_NAME} 2>/dev/null || true; sudo systemctl stop ${LEGACY_SERVICE_NAME} 2>/dev/null || true"
...
# ── 4. Deploy binary ──────────────────────────────────────────────────────────
echo ""
echo "==> Deploying binary to ${RPI_USER}@${RPI_HOST}:${RPI_DIR}/${APP_NAME}"
${SCP_CMD} ${SCP_OPTS} "${BINARY}" "${RPI_USER}@${RPI_HOST}:${RPI_DIR}/${APP_NAME}"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" "chmod 755 ${RPI_DIR}/${APP_NAME}"
...
sed \
  -e "s|@@RPI_USER@@|${RPI_USER}|g" \
  -e "s|@@RPI_DIR@@|${RPI_DIR}|g" \
  scripts/dx-agents.service > "${_RENDERED_SERVICE}"
${SCP_CMD} ${SCP_OPTS} "${_RENDERED_SERVICE}" "${RPI_USER}@${RPI_HOST}:/tmp/${SERVICE_NAME}.service"
...
# ── 8. Reset ACT LED trigger so DX Agents can control it ─────────────────────
echo ""
...
echo ""
echo "    DX Agents is running at http://${RPI_HOST}:8080"
