...
function ensure_config {
    CONFIG_DIR="$HOST_TARGET_DIR/.dx_agent"
    CONFIG_FILE="$CONFIG_DIR/config.toml"
    WORKSPACE_DIR="$CONFIG_DIR/workspace"

    if [ ! -f "$CONFIG_FILE" ]; then
        echo -e "${YELLOW}⚙️  Config file missing in target/.dx_agent. Creating default dev config from template...${NC}"
...
function print_help {
    echo -e "${YELLOW}DX Agent Development Environment Manager${NC}"
    ...
    echo -e "  ${GREEN}agent${NC}   Enter Agent (DX Agent CLI)"
...
case "$1" in
    up)
        ...
        echo -e "   - Config: target/.dx_agent/config.toml (Edit locally to apply changes)"
        ;;
    ...
    agent)
        echo -e "${GREEN}🤖 Entering Agent Container (DX Agent)... (Type 'exit' to leave)${NC}"
    ...
    clean)
        echo -e "${RED}⚠️  WARNING: This will delete 'target/.dx_agent' data and Docker volumes.${NC}"
        read -p "Are you sure? (y/N) " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            docker compose -f "$COMPOSE_FILE" down -v
            rm -rf "$HOST_TARGET_DIR/.dx_agent"
