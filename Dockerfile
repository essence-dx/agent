...
# `src/bin/dx-agents-acp-bridge.rs` is required because the `acp-bridge` feature
...
RUN mkdir -p src src/bin benches apps/tauri/src tools/fill-translations/src xtask/src/bin \
    ...
    && echo "" > src/lib.rs \
    && echo "fn main() {}" > src/bin/dx-agents-acp-bridge.rs \
...
RUN --mount=type=cache,id=zeroclaw-cargo-registry,target=/usr/local/cargo/registry,sharing=locked \
    ...
    cp target/release/dx-agents /app/dx-agents && \
    strip /app/dx-agents
RUN size=$(stat -c%s /app/dx-agents) && \
...
HEALTHCHECK --interval=60s --timeout=10s --retries=3 --start-period=10s \
    CMD ["dx-agents", "status", "--format=exit-code"]
ENTRYPOINT ["dx-agents"]
CMD ["daemon"]
