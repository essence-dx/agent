...
export const UNAUTHORIZED_EVENT = 'dx-agents-unauthorized';
export const LEGACY_UNAUTHORIZED_EVENT = 'zeroclaw-unauthorized';

function dispatchUnauthorized(): void {
  window.dispatchEvent(new Event(UNAUTHORIZED_EVENT));
  window.dispatchEvent(new Event(LEGACY_UNAUTHORIZED_EVENT));
}
...
export interface ListResponseEntry {
  ...
  /** Optional dashboard tab label emitted by the schema metadata. */
  tab?: string | null;
  /** Optional section label emitted by the schema metadata. */
  section?: string | null;
  /** True when an environment variable overrides this value at runtime. */
...
export function getReloadStatus(): Promise<ReloadStatusResponse> {
  return apiFetch<ReloadStatusResponse>('/api/config/reload-status');
}
...
export function deleteMapKey(path: string, key: string): Promise<MapKeyResponse> {
  return apiFetch<MapKeyResponse>(
    `/api/config/map-key?path=${encodeURIComponent(path)}&key=${encodeURIComponent(key)}`,
    { method: 'DELETE' },
  );
...
export interface CatalogResponse {
  ...
  catalog?: CatalogDiagnostic | null;
}

export async function getCatalog(): Promise<CatalogResponse> {
  const data = await apiFetch<
    CatalogResponse | { model_providers: CatalogProvider[]; catalog?: CatalogDiagnostic | null }
  >('/api/config/catalog');
  if ('providers' in data) return data;
  return { ...data, providers: data.model_providers };
}
...
export async function getCatalogModels(provider: string): Promise<ModelsResponse> {
  const data = await apiFetch<ModelsResponse>(
    `/api/config/catalog/models?provider=${encodeURIComponent(provider)}`,
  );
  return {
    ...data,
    provider: data.provider ?? data.model_provider ?? provider,
...
export interface SectionInfo {
  ...
  group: string;
  /** Editor shape emitted by the Rust section registry. */
  shape?: 'direct_form' | 'one_tier_alias_map' | 'typed_family_map' | 'backend_picker' | string | null;
  /** True for sections in the canonical quickstart flow. */
  is_quickstart?: boolean;
  /** Whether the section has enough usable config for first-run readiness. */
  ready?: boolean;
}
...
export async function getOnboardStatus(): Promise<OnboardStatusResponse> {
  ...
  return {
    ...status,
    needs_onboarding: status.needs_quickstart,
  };
}
...
export interface AgentOptionsResponse {
  ...
  mcp_bundles: string[];
  agents: string[];
  /** Retired in the V3 schema; retained as an empty list for legacy form code. */
  memory_namespaces: string[];
}

export async function getAgentOptions(): Promise<AgentOptionsResponse> {
  const data = await apiFetch<Omit<AgentOptionsResponse, 'memory_namespaces'> & {
    memory_namespaces?: string[];
  ...
  return {
    ...
    skill_bundles: data.skill_bundles ?? [],
    knowledge_bundles: data.knowledge_bundles ?? [],
    mcp_bundles: data.mcp_bundles ?? [],
    agents: data.agents ?? [],
    memory_namespaces: data.memory_namespaces ?? [],
  };
}

// ── Shared directory + agent workspace browsing ─────────────────────
...
export function browseShared(path = ''): Promise<BrowseResponse> {
  ...
  return apiFetch<BrowseResponse>(`/api/browse${params}`);
}

export function mkdirShared(path: string): Promise<void> {
  return apiFetch<unknown>('/api/browse/mkdir', {
    method: 'POST',
    body: JSON.stringify({ path }),
  }).then(() => undefined);
}

export function rmdirShared(path: string): Promise<void> {
  return apiFetch<unknown>('/api/browse/rmdir', {
    method: 'DELETE',
    body: JSON.stringify({ path }),
  }).then(() => undefined);
}
...
export function listAgentWorkspace(alias: string, path = ''): Promise<BrowseResponse> {
  ...
  return apiFetch<BrowseResponse>(
    `/api/agents/${encodeURIComponent(alias)}/workspace/list${params}`,
  );
}
...
export function deleteAgentWorkspacePath(alias: string, path: string): Promise<void> {
...
export function listSkillsInBundle(bundle: string): Promise<SkillsListResponse> {
  return apiFetch<SkillsListResponse>(
    `/api/skills/bundles/${encodeURIComponent(bundle)}/skills`,
  );
}

export function readSkill(bundle: string, name: string): Promise<SkillDocument> {
  return apiFetch<SkillDocument>(
...
export function writeSkill(
  bundle: string,
  name: string,
  doc: Pick<SkillDocument, 'frontmatter' | 'body'>,
): Promise<void> {
  return apiFetch<unknown>(
    `/api/skills/bundles/${encodeURIComponent(bundle)}/skills/${encodeURIComponent(name)}`,
    {
...
export function createSkill(
  ...
  return apiFetch<{ bundle: string; name: string; directory: string }>(
    `/api/skills/bundles/${encodeURIComponent(bundle)}/skills`,
    {
      method: 'POST',
...
export function deleteSkill(
  ...
  return apiFetch<unknown>(
    `/api/skills/bundles/${encodeURIComponent(bundle)}/skills/${encodeURIComponent(name)}?purge=${purge ? 'true' : 'false'}`,
    { method: 'DELETE' },
  ).then(() => undefined);
}
...
export function quickstartFields(body: {
  section: 'model_provider' | 'channel' | 'peer_group';
  type_key: string;
}): Promise<{ fields: QuickstartFieldDescriptor[] }> {
  return apiFetch<{ fields: QuickstartFieldDescriptor[] }>('/api/quickstart/fields', {
    method: 'POST',
    body: JSON.stringify(body),
  });
}

export function quickstartApply(body: unknown): Promise<QuickstartApplyResult> {
...
export function quickstartDismiss(body: {
  run_id: string;
  surface: 'web' | 'tui' | string;
  last_step?: QuickstartStep | null;
}): Promise<void> {
  return apiFetch<unknown>('/api/quickstart/dismiss', {
    method: 'POST',
    body: JSON.stringify(body),
  }).then(() => undefined);
}
...
export function deleteMemory(key: string, agent?: string): Promise<void> {
  ...
  return apiFetch<void>(`/api/memory/${encodeURIComponent(key)}${params}`, {
    method: 'DELETE',
  });
}
...
export function getCost(
  ...
  if (from) params.set('from', serializeDate(from));
  if (to) params.set('to', serializeDate(to));
  if (agent) params.set('agent', agent);
  const qs = params.toString();
  return apiFetch<CostSummary | { cost: CostSummary }>(`/api/cost${qs ? `?${qs}` : ''}`).then((data) =>
...
export function deleteSession(id: string): Promise<void> {
  return apiFetch<unknown>(`/api/sessions/${encodeURIComponent(id)}`, {
    method: 'DELETE',
  }).then(() => undefined);
}
...
export function getTuis(): Promise<TuiEntry[]> {
  return apiFetch<TuiEntry[] | { tuis: TuiEntry[] }>('/api/tuis').then((data) => {
