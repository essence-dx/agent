...
const PRIMARY_PREFIX = 'dx_agents_chat_history_v1:';
const LEGACY_PREFIXES = ['zeroclaw_chat_history_v1:'] as const;
...
function storageKey(sessionId: string): string {
  return `${PRIMARY_PREFIX}${sessionId}`;
}

function legacyStorageKeys(sessionId: string): string[] {
  return LEGACY_PREFIXES.map((prefix) => `${prefix}${sessionId}`);
}

export function loadChatHistory(sessionId: string): PersistedChatBubble[] {
  try {
    const raw = getCompatStorageItem(safeLocalStorage(), storageKey(sessionId), legacyStorageKeys(sessionId));
    if (!raw) return [];
