...
export const LOCALE_STORAGE_KEY = 'dx_agents-locale';
const LEGACY_LOCALE_STORAGE_KEYS = ['zeroclaw-locale'] as const;

export function loadLocale(): string {
  return getCompatStorageItem(safeLocalStorage(), LOCALE_STORAGE_KEY, LEGACY_LOCALE_STORAGE_KEYS) ?? 'en';
}

export function saveLocale(locale: string) {
  setCompatStorageItem(safeLocalStorage(), LOCALE_STORAGE_KEY, LEGACY_LOCALE_STORAGE_KEYS, locale);
}

// ── Theme storage (was themeStorage.ts) ──────────────────────────────────────

const STORAGE_KEY = 'dx_agents-theme';
const LEGACY_THEME_STORAGE_KEYS = ['zeroclaw-theme'] as const;
...
function loadStored(): StoredTheme {
  try {
    const raw = getCompatStorageItem(safeLocalStorage(), STORAGE_KEY, LEGACY_THEME_STORAGE_KEYS);
    if (raw) {
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const persist = useCallback((s: StoredTheme) => {
    setCompatStorageItem(safeLocalStorage(), STORAGE_KEY, LEGACY_THEME_STORAGE_KEYS, JSON.stringify(s));
  }, []);
