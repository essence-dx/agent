...
import { getCompatStorageItem, safeSessionStorage, setCompatStorageItem } from '@/lib/compatStorage';
...
const LOG_STORAGE_KEY = 'dx_agents_live_logs';
const LEGACY_LOG_STORAGE_KEYS = ['zeroclaw_live_logs'] as const;
const MAX_PERSISTED_LOGS = 200;

function loadPersistedLogs(): LogEntry[] {
  try {
    const raw = getCompatStorageItem(safeSessionStorage(), LOG_STORAGE_KEY, LEGACY_LOG_STORAGE_KEYS);
    if (!raw) return [];
...
function persistLogs(entries: LogEntry[]): void {
  try {
    setCompatStorageItem(safeSessionStorage(), LOG_STORAGE_KEY, LEGACY_LOG_STORAGE_KEYS, JSON.stringify(entries.slice(-MAX_PERSISTED_LOGS)));
  } catch { /* QuotaExceeded */ }
}
      ...
      {!infoDismissed && (
        <div className="flex items-start gap-3 px-6 py-3 border-b flex-shrink-0" style={{ borderColor: 'rgba(56, 189, 248, 0.2)', background: 'rgba(56, 189, 248, 0.05)' }}>
          <div className="flex-1 text-xs" style={{ color: 'var(--pc-text-secondary)' }}>
            ...
            {' '}To see tracing logs, run the daemon with <code>RUST_LOG=info dx-agents</code> and check your terminal.
          </div>
          <button
