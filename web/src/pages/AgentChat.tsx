...
export default function AgentChat() {
  ...
  const [compact, setCompact] = useState(() => {
    return getCompatStorageItem(safeLocalStorage(), 'dx_agents_chat_compact', ['zeroclaw_chat_compact']) === '1';
  });
  ...
  const [showToolActivity, setShowToolActivity] = useState(() => {
    return getCompatStorageItem(safeLocalStorage(), 'dx_agents_show_tool_activity', ['zeroclaw_show_tool_activity']) === '1';
  });
  ...
  const toggleCompact = useCallback(() => {
    setCompact((prev) => {
      const next = !prev;
      setCompatStorageItem(safeLocalStorage(), 'dx_agents_chat_compact', ['zeroclaw_chat_compact'], next ? '1' : '0');
      return next;
    });
  }, []);
  ...
  const toggleToolActivity = useCallback(() => {
    setShowToolActivity((prev) => {
      const next = !prev;
      setCompatStorageItem(safeLocalStorage(), 'dx_agents_show_tool_activity', ['zeroclaw_show_tool_activity'], next ? '1' : '0');
      return next;
    });
  }, []);
    ...
    <div className="flex flex-col h-[calc(100vh-3.5rem)]">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b" style={{ borderColor: 'var(--pc-border)', background: 'var(--pc-bg-surface)' }}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium" style={{ color: 'var(--pc-text-primary)' }}>DX Agents</span>
        </div>
      ...
      <div className={`flex-1 overflow-y-auto p-4 ${compact ? 'space-y-1.5' : 'space-y-4'}`}>
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center animate-fade-in" style={{ color: 'var(--pc-text-muted)' }}>
            ...
            <p className="text-lg font-semibold mb-1" style={{ color: 'var(--pc-text-primary)' }}>DX Agents</p>
