...
import { getAdminPairCode, getOnboardStatus, LEGACY_UNAUTHORIZED_EVENT, UNAUTHORIZED_EVENT } from './lib/api';
...
> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error('[DX Agents] Render error:', error, info.componentStack);
  }
    ...
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--pc-bg-base)' }}>
      ...
      <div className="relative surface-panel p-8 w-full max-w-md animate-fade-in-scale">
        ...
        <div className="text-center mb-8">
          ...
          />
          <h1 className="text-2xl font-bold mb-2 text-gradient-blue">DX Agents</h1>
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(LEGACY_UNAUTHORIZED_EVENT, logout);
    return () => {
      window.removeEventListener(UNAUTHORIZED_EVENT, logout);
      window.removeEventListener(LEGACY_UNAUTHORIZED_EVENT, logout);
    };
  }, [logout]);
