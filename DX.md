...
Update 2026-06-06 ACP/security launch hardening: `dx-agents-acp-bridge` now prefers `DX_AGENTS_ACP_BRIDGE_TOKEN` and `DX_AGENTS_ACP_PAIRING_CODE` with legacy `DX_AGENT_*` fallback, ACP initialize responses advertise `dx-agents-acp` and `_meta.dxAgents` while retaining `_meta.dx_agent`, and gateway pairing recovery/help copy points at `dx-agents`. File upload bundle/download headers plus Gmail push and voice-call secrets now encrypt on save. Docker release defaults keep gateway pairing required, Compose binds host ports to loopback by default, Kubernetes/Docker env examples are DX-first, Quick Start/Windows setup no longer send users to upstream DX Agent install/onboard paths, and a transient Groq smoke returned `dx-agents-provider-ok` with zero raw key hits in the temp config or repo scan.
...
Update 2026-06-05 onboarding brand: the desktop onboarding page now presents a static rainbow DX wordmark and ASCII train rail animation, then guards the final completion action while the train exits. Legacy DX Agent storage/protocol aliases remain compatibility data only.
