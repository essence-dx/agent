...
pub fn tooltip_for_state(connected: bool, status: AgentStatus) -> &'static str {
    if !connected {
        return "DX Agents - Disconnected";
    }
    match status {
        AgentStatus::Idle => "DX Agents - Idle",
        AgentStatus::Working => "DX Agents - Working",
        AgentStatus::Error => "DX Agents - Error",
    }
}
