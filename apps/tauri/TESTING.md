...
```sh
pkill -f 'target/debug/dx-agents-desktop'
rm "$HOME/Library/Application Support/ai.dxagents.desktop/settings.json"
tccutil reset All ai.dxagents.desktop      # for installed .app only — see notes
...
cp -R "target/release/bundle/macos/DX Agents.app" /Applications/
xattr -dr com.apple.quarantine "/Applications/DX Agents.app"
tccutil reset All ai.dxagents.desktop
open "/Applications/DX Agents.app"
```
...
- "Start DX Agents" closes onboarding, opens the main dashboard window pointed at the gateway, and fires `POST /api/devices/me/capabilities` (best-effort)
...
cargo run -p dx-agents -- gateway

# In another terminal, walk the wizard, click "Start DX Agents"
