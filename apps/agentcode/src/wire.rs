...
//! wire between `zerocode` and the DX Agent daemon.
//!
...
/// Wire view of `dx_agent_config::presets::RiskPreset` / `RuntimePreset`.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
...
mod quickstart_metadata_tests {
    ...
    #[test]
    fn quickstart_type_option_preserves_provider_metadata_on_wire() {
        ...
        let encoded = serde_json::to_value(row).expect("quickstart row must serialize");
...
/// Schema field-kind tag mirroring `dx_agent_config::traits::PropKind`.
...
impl PropKind {
    ...
    /// `dx_agent_config::traits::PropKind::wire_name`. Used by the
...
/// `dx_agent_config::traits::ConfigTab`. `Default` is `None` — the
...
/// Security classification for credential-shaped config fields. Mirrors
/// `dx_agent_config::traits::CredentialSurfaceClass`.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// `config/sections`. Mirrors `dx_agent_config::traits::ConfigFieldEntry`.
...
/// `dx_agent_config::sections::SectionShape`.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
