...
// It speaks JSON-RPC to whatever DX Agent daemon is at the configured
...
// `dx_agent_spawn::spawn!` rule is daemon-path only (see
...
#[command(
    name = "zerocode",
    about = "Interactive TUI config manager for DX Agent",
    version,
    long_version = concat!(
...
struct Cli {
    /// Path to the DX Agent config directory
    #[arg(long)]
