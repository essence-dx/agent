...
/// mirroring `dx_agent_config::traits::UNSET_DISPLAY`. zerocode talks to
/// the daemon over RPC and mirrors config types on the wire rather than
/// depending on `dx-agent-config`, so the sentinel is duplicated here.
...
fn provider_type_options(snapshot: Option<&QuickstartStateResult>) -> Vec<PickerOption> {
    ...
    // `dx_agent_runtime::quickstart::snapshot_state`, which maps the
    // canonical `dx_agent_providers::list_model_providers()` registry
    // into wire rows. Adding a model provider in
    // `dx-agent-providers` lights up here automatically — Quickstart
    ...
    snap.model_provider_types
        ...
        .map(|t| PickerOption {
            ...
            help: provider_type_help(t),
            use_existing: false,
        })
        .collect()
}

fn provider_type_help(provider: &crate::client::QuickstartTypeOption) -> String {
    if provider.local {
        return crate::i18n::t("zc-quickstart-provider-local");
    }

    let Some(freemium) = provider.freemium.as_ref() else {
        return crate::i18n::t("zc-quickstart-provider-cloud");
    };
    ...
    if freemium.free_model_ids.is_empty() {
        return format!("{provider_name}: free tier available.");
    }
...
mod tests {
    ...
    #[test]
    fn provider_type_options_renders_opencode_freemium_help() {
        let snapshot = QuickstartStateResult {
            ...
            storage: Vec::new(),
            model_provider_types: vec![crate::client::QuickstartTypeOption {
                kind: "opencode".into(),
                display_name: "OpenCode".into(),
                local: false,
                identity: Some(crate::client::ProviderIdentity {
                    canonical_id: "opencode-zen".into(),
                    display_name: "OpenCode Zen".into(),
                    aliases: vec!["opencode".into()],
                    database_ids: vec!["opencode".into()],
                    runtime_id: Some("opencode".into()),
                    exposure_status: crate::client::ExposureStatus::VerifiedWorking,
                }),
                freemium: Some(crate::client::FreemiumMetadata {
                    access: crate::client::AccessKind::FreeTier,
                    auth: vec![
                        crate::client::AuthKind::PublicKey,
                        crate::client::AuthKind::ApiKey,
                    ],
                    env_vars: Vec::new(),
        ...
        let rows = provider_type_options(Some(&snapshot));
        let opencode = rows
            .iter()
            .find(|row| row.value == "opencode")
            .expect("opencode provider option");
