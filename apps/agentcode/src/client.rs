...
//!
//! Wraps [`RpcOutbound`] from `dx-agent-api` — the same request/response
...
/// Resolve config dir: CLI flag > `$DX_AGENT_CONFIG_DIR` > home directory.
pub fn resolve_config_dir(cli_override: Option<&Path>) -> Result<PathBuf> {
    if let Some(dir) = cli_override {
        return Ok(dir.to_path_buf());
    }
    if let Ok(d) = std::env::var("DX_AGENT_CONFIG_DIR") {
        let d = d.trim();
        if !d.is_empty() {
    ...
    #[cfg(unix)]
    {
        ...
        Ok(PathBuf::from(home).join(".dx-agent"))
    }
    #[cfg(windows)]
    {
        let profile = std::env::var("USERPROFILE").context("USERPROFILE not set")?;
        Ok(PathBuf::from(profile).join(".dx-agent"))
    }
}
...
impl RpcClient {
    ...
    // `dx_agent_runtime::rpc::dispatch` and call into
    // `dx_agent_runtime::quickstart::{validate_only,apply}_with_surface`.
...
// `dx_agent_runtime::rpc::types` (the daemon-side single source of
// truth, which itself mirrors the gateway's HTTP route shapes). The
// types live in `dx-agent-runtime`, but that crate is not on the
...
/// Mirror of `dx_agent_runtime::quickstart::Surface` (`snake_case` on the wire).
...
/// Mirror of `dx_agent_runtime::quickstart::QuickstartStep`.
#[derive(Debug, Clone, Copy, serde::Serialize, serde::Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// Mirror of `dx_agent_runtime::quickstart::QuickstartError`.
...
/// Mirror of `dx_agent_runtime::quickstart::AppliedAgent`.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// Mirror of `dx_agent_runtime::quickstart::FieldSection`.
#[derive(Debug, Clone, Copy, serde::Serialize, serde::Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// Mirror of `dx_agent_config::traits::PropKind` (wire form).
#[derive(Debug, Clone, Copy, serde::Serialize, serde::Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// Mirror of `dx_agent_runtime::quickstart::FieldDescriptor`.
...
/// Mirror of `dx_agent_runtime::quickstart::QuickstartState`.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Mirror of `dx_agent_config::presets::RiskPreset` / `RuntimePreset`.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
/// Mirror of `dx_agent_runtime::rpc::types::QuickstartTypeOption`.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Mirror of `dx_agent_runtime::rpc::types::LogsGetResult`. Full log
...
/// Mirror of `dx_agent_runtime::rpc::types::MemoryGetResult`. Full
...
#[cfg(test)]
mod quickstart_metadata_tests {
    use super::*;

    #[test]
    fn quickstart_type_option_deserializes_provider_metadata() {
        let row: QuickstartTypeOption = serde_json::from_value(serde_json::json!({
            "kind": "opencode",
        ...
        assert_eq!(
            row.identity.expect("identity metadata").canonical_id,
            "opencode-zen"
        );
        assert!(
            row.freemium
                .expect("freemium metadata")
                .free_model_ids
