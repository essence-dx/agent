...
/// `DX_AGENT_CONFIG_DIR` and otherwise defaults to `~/.dx-agent`. This mirrors
/// the runtime loader's path (dx-agent-config::ftl_locale_dir) — kept inline
...
/// Resolve the DX Agent config directory with the same precedence as
/// `client::resolve_config_dir`: the `--config-dir` flag (passed to `init` and
/// cached in `CONFIG_DIR`) first, then `DX_AGENT_CONFIG_DIR`, then `~/.dx-agent`.
/// This keeps the FTL read path aligned with the flag the rest of zerocode uses.
fn config_dir() -> PathBuf {
    if let Some(dir) = CONFIG_DIR.get() {
        return dir.clone();
    }
    if let Ok(custom) = std::env::var("DX_AGENT_CONFIG_DIR") {
    ...
    directories::BaseDirs::new()
        .map(|b| b.home_dir().join(".dx-agent"))
        .unwrap_or_else(|| PathBuf::from(".dx-agent"))
}
...
/// then `DX_AGENT_CONFIG_DIR`, then `~/.dx-agent`). Reading and writing the
