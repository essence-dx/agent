# DX Agents OpenShift deployment

Deploy a minimal DX Agents agent on OpenShift with an external LLM
...
1. Update the `image` field in `deployment.yaml` to point to your
   registry (e.g., `ghcr.io/millercarla211-ctrl/dx-agents:latest`)
...
```bash
oc -n dx-agents get pods
oc -n dx-agents get route dx-agents
```

Test the health endpoint:

```bash
ROUTE=$(oc -n dx-agents get route dx-agents -o jsonpath='{.spec.host}')
...
oc -n dx-agents rollout restart deployment dx-agents
```
