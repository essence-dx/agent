...
fn default_config_dir() -> Result<PathBuf> {
    if let Some((_name, custom)) =
        non_empty_env(&["DX_AGENTS_CONFIG_DIR", "ZEROCLAW_CONFIG_DIR"], true)
    {
...
/// > legacy `ZEROCLAW_*` aliases > defaults.
...
async fn resolve_runtime_config_dirs(
        ...
        non_empty_env(&["DX_AGENTS_CONFIG_DIR", "ZEROCLAW_CONFIG_DIR"], true)
    {
        ...
        if let Some((data_env_name, _)) =
            non_empty_env(&["DX_AGENTS_DATA_DIR", "ZEROCLAW_DATA_DIR"], true)
        {
            ::zeroclaw_log::record!(
                WARN,
                ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Note)
        ...
        if let Some((workspace_env_name, _)) =
            non_empty_env(&["DX_AGENTS_WORKSPACE", "ZEROCLAW_WORKSPACE"], false)
        {
            ::zeroclaw_log::record!(
                WARN,
                ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Note)
...
"#;
    ...
    async fn resolve_runtime_config_dirs_prefers_dx_agents_config_dir() {
        ...
        unsafe { std::env::set_var("ZEROCLAW_CONFIG_DIR", &legacy_config_dir) };
        // SAFETY: tests serialize on `env_override_lock()`.
