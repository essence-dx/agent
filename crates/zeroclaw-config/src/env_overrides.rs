...
//! alias key.
//! The legacy `ZEROCLAW_` prefix remains accepted for migration, but the DX
...
//! `DX_AGENTS_WORKSPACE`, and their legacy `ZEROCLAW_*` aliases keep their
...
const LEGACY_PREFIX: &str = "ZEROCLAW_";
const SEP: &str = "__";
...
mod tests {
    ...
    async fn dx_agents_schema_mirror_prefix_wins_over_legacy_zeroclaw_prefix() {
        let _guard = super::env_test_lock().await;
        let _legacy = EnvVarGuard::set("ZEROCLAW_gateway__request_timeout_secs", "120");
