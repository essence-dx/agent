...
fn format_paircode_recovery_command(host: &str, port: u16) -> String {
    let mut cmd = format!("dx-agents gateway get-paircode --new --port {port}");
...
mod tests {
    ...
    fn paircode_recovery_command_includes_alternate_port() {
        assert_eq!(
            ...
            "dx-agents gateway get-paircode --new --port 42617"
        );
    }
    ...
    fn paircode_recovery_command_includes_specific_host_when_needed() {
        assert_eq!(
            ...
            "dx-agents gateway get-paircode --new --port 42617 --host 192.168.1.20"
        );
    }
