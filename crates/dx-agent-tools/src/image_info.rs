...
impl Tool for ImageInfoTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // registration time (see dx-agent-runtime::tools::mod). Successful
...
mod tests {
    ...
    /// `dx-agent-runtime::tools::mod`.  Use this in tests that exercise
    ...
    async fn wrapped_normalizes_workspace_prefixed_relative_path() {
        ...
        let workspace = root.path().join("dx-agent-data").join("workspace");
