...
use std::sync::Arc;
use dx_agent_api::attribution::{Attributable, Role};
...
///
/// `FileReadTool` (`dx-agent-runtime::tools::file_read`) and `PdfReadTool` in
...
impl<T: Tool> PathGuardedTool<T> {
    ...
    fn extract_path_string(&self, args: &serde_json::Value) -> Option<String> {
        ...
        // Default: check common argument names used across DX Agent tools.
...
mod tests {
    ...
    dx_agent_api::mock_tool_attribution!(CountingTool);
    ...
    async fn rate_limited_does_not_consume_budget_on_failure() {
        ...
        struct AlwaysFails;
        impl ::dx_agent_api::attribution::Attributable for AlwaysFails {
            fn role(&self) -> ::dx_agent_api::attribution::Role {
                ::dx_agent_api::attribution::Role::Tool(
                    ::dx_agent_api::attribution::ToolKind::Plugin,
                )
            }
            fn alias(&self) -> &str {
