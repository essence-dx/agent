...
impl WebSearchTool {
    ...
    fn resolve_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        if let Some(ref key) = self.boot_brave_api_key
            && !key.is_empty()
            && !dx_agent_config::secrets::SecretStore::is_encrypted(key)
        {
            return Ok(key.clone());
        }
    ...
    fn reload_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        let config: dx_agent_config::schema::Config = toml::from_str(&contents).map_err(|e| {
            ::dx_agent_log::record!(
                ERROR,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail)
                    .with_outcome(::dx_agent_log::EventOutcome::Failure)
        ...
        if dx_agent_config::secrets::SecretStore::is_encrypted(&raw_key) {
            let dx_agent_dir = self.config_path.parent().unwrap_or_else(|| Path::new("."));
            let store =
                dx_agent_config::secrets::SecretStore::new(dx_agent_dir, self.secrets_encrypt);
    ...
    async fn search_duckduckgo_at(
        ...
        let builder =
            dx_agent_config::schema::apply_runtime_proxy_to_builder(builder, "tool.web_search");
    ...
    async fn search_jina(&self, query: &str) -> anyhow::Result<String> {
        ...
        let builder = reqwest::Client::builder()
            .timeout(Duration::from_secs(self.timeout_secs))
            .user_agent("DX Agent/1.0 (https://zeroclaw.ai)");
        let builder =
            dx_agent_config::schema::apply_runtime_proxy_to_builder(builder, "tool.web_search");
        let client = builder.build()?;
...
mod tests {
    ...
    fn test_resolve_brave_api_key_decrypts_encrypted_key() {
        let tmp = tempfile::TempDir::new().unwrap();
        let store = dx_agent_config::secrets::SecretStore::new(tmp.path(), true);
