...
use dx_agent_config::schema::{ClassificationRule, Config, ModelRouteConfig};
...
impl ModelRoutingConfigTool {
    ...
    fn load_config_without_env(&self) -> anyhow::Result<Config> {
        ...
        let mut parsed =
            dx_agent_config::migration::migrate_to_current(&contents).map_err(|error| {
                ::dx_agent_log::record!(
                    ERROR,
                    ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail)
                        .with_outcome(::dx_agent_log::EventOutcome::Failure)
    ...
    async fn handle_set_default(&self, args: &Value) -> anyhow::Result<ToolResult> {
        ...
        {
            if dx_agent_providers::reliable::is_non_retryable(&probe_err) {
            ...
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model": model_name, "probe_err": probe_err.to_string()})), "Model probe returned retryable error (keeping new config)");
        }
    ...
    async fn probe_model(&self, provider_name: &str, model: &str) -> anyhow::Result<()> {
        ...
        let model_provider = match dx_agent_providers::create_model_provider_with_url(
            provider_name,
            api_key,
...
mod tests {
    ...
    use dx_agent_config::autonomy::AutonomyLevel;
    use dx_agent_config::policy::SecurityPolicy;
    ...
    fn read_saved_provider_entry(
        ...
        family: &str,
        alias: &str,
    ) -> Option<dx_agent_config::schema::ModelProviderConfig> {
        let contents = std::fs::read_to_string(cfg_path).ok()?;
        let cfg = dx_agent_config::migration::migrate_to_current(&contents).ok()?;
