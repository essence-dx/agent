...
use dx_agent_config::schema::LinkedInImageConfig;
    ...
    pub async fn generate(&self, prompt: &str) -> anyhow::Result<PathBuf> {
        ...
        for provider_name in &self.config.providers {
            ...
            match result {
                ...
                Err(e) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", e), "provider_name": provider_name})), "Image model_provider '' failed");
                }
            }
        }
