...
/// DX Agent-domain attribution. Every field is alias-bound where
...
mod tests {
    ...
    fn merge_from_fills_missing_only() {
        let mut child = DxAgentAttribution::default();
        child.set("agent_alias", "clamps");
        let mut parent = DxAgentAttribution::default();
