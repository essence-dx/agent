//! Unified log emission surface for the DX Agent workspace.
//!
...
//!
//! Schema is an OTel/ECS hybrid with a DX Agent-domain `zeroclaw.*`
...
/// reach `tracing` types via `dx_agent_log::*`. Do NOT use directly
...
pub use event::{
    ...
    Severity, DxAgentAttribution, is_attribution_field, severity_text_from_number,
...
/// the type) but the public path is `dx_agent_log::Span` — no `tracing`
