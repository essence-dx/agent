...
//! [`dx_agent_api::observability_traits::ObserverEvent`] variants when a
...
use dx_agent_api::observability_traits::{Observer, ObserverEvent};
...
fn project(event: &LogEvent) -> Option<ObserverEvent> {
    ...
    let attribution = &event.dx_agent;
...
mod tests {
    ...
    fn projects_llm_request() {
        ...
        event.dx_agent.set("model", "claude-sonnet-4-6");
    ...
    fn projects_tool_call_success() {
        ...
        event.dx_agent.set("tool", "shell");
        event.dx_agent.duration_ms = Some(120);
