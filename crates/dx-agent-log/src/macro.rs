...
//!
//! ```ignore
//! use dx_agent_log::{record, Event, Action, EventOutcome};
//!
...
/// Emit a structured DX Agent log event. The single positional `Event`
...
macro_rules! attribution_span {
    ($attributable:expr) => {{
        ...
        let __zc_role = ::dx_agent_api::attribution::Attributable::role(__zc_thing);
        let __zc_alias = ::dx_agent_api::attribution::Attributable::alias(__zc_thing);
        $crate::__private::tracing::info_span!(
            target: "dx_agent_log_internal_attribution",
