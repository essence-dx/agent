...
//!
//! `dx-agent-log` defines its own minimal [`LogConfig`] shape so it does
//! not depend on `dx-agent-config`. Callers convert the full
//! `dx_agent_config::schema::ObservabilityConfig` into a [`LogConfig`]
...
/// `dx_agent_config::schema::ObservabilityConfig`.
#[derive(Debug, Clone)]
