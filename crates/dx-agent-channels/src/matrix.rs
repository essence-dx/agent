...
use dx_agent_config::schema::{MatrixConfig, StreamMode, TranscriptionConfig};
...
mod client {
    ...
    use super::session;
    use dx_agent_config::schema::MatrixConfig;
        ...
        if recovery_attempts > 1 {
            bail!(
                "matrix: corruption recovery looped — aborting to avoid an infinite restart cycle. \
                 Wipe ~/.dx-agent/state/matrix/ manually and restart."
            );
        }
    ...
    async fn password_login(client: &Client, config: &MatrixConfig, password: &str) -> Result<()> {
        ...
        let mut login = client
            ...
            .initial_device_display_name("DX Agent");
...
mod inbound {
    ...
    use dx_agent_config::schema::{MatrixConfig, TranscriptionConfig};

    #[derive(Clone)]
    pub(super) struct HandlerCtx {
        pub config: Arc<MatrixConfig>,
        /// DX Agent alias for `[channels.matrix.<alias>]` so session_key
    ...
    async fn handle_message(
        ...
        if let Some(info) = media_kind {
            ...
            match room.event(&reply_target, None).await {
                ...
                Err(e) => {
                    ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"error": format!("{}", e), "reply_target": reply_target})), "matrix: could not fetch in_reply_to parent")
                }
            }
        }
        ...
        let Some(workspace) = workspace else {
            ::dx_agent_log::record!(
                ...
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                    .with_outcome(::dx_agent_log::EventOutcome::Unknown),
                &format!(
                    "matrix: cannot persist {} — channels.matrix workspace_dir not configured. Set DX_AGENT_DIR or run via the orchestrator.",
                    info.file_name
                )
            );
            return Ok(None);
...
mod outbound {
    ...
    use dx_agent_api::{channel::SendMessage, media::MediaAttachment};
    ...
    /// streaming-cap pattern in `crates/dx-agent-tools/src/web_fetch.rs`.
...
impl ::dx_agent_api::attribution::Attributable for MatrixChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(::dx_agent_api::attribution::ChannelKind::Matrix)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
                                    ...
                                    .await
                            {
                                ...
                                match outbound::send(&self.outbox(client), &fallback).await {
                                    ...
                                    Err(send_err) if any_attachment_landed => {
                                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"send_err": send_err.to_string()})), "matrix: partial finalize cleaned text fallback failed after attachment upload: ; suppressing error to avoid duplicate attachment retry");
                                    }
                                    Err(send_err) => {
...
mod tests {
    ...
    mod mention {
        ...
        fn body_fallback_display_name() {
            let bot = user_id!("@bot:example.org");
            assert!(is_mentioned(bot, Some("DX Agent"), None, "hi zeroclaw!"));
        }
    ...
    mod live_smoke {
        ...
        use dx_agent_config::schema::{MatrixConfig, StreamMode};
        ...
        async fn same_room_partial_draft_lifecycle_uses_real_draft_ids() {
            let homeserver = env_first(
                "DX_AGENT_MATRIX_SMOKE_HOMESERVER",
                "DX_AGENT_MATRIX_HOMESERVER",
            );
            let room_id = env_first("DX_AGENT_MATRIX_SMOKE_ROOM_ID", "DX_AGENT_MATRIX_ROOM_ID");
            let access_token = env_first(
                "DX_AGENT_MATRIX_SMOKE_ACCESS_TOKEN",
                "DX_AGENT_MATRIX_ACCESS_TOKEN",
            );
