...
use dx_agent_config::schema::{Config, LineDmPolicy, LineGroupPolicy};
            ...
            match state.dm_policy {
                ...
                LineDmPolicy::Pairing => {
                    if !is_line_user_allowed(&*state, user_id) {
                            ...
                            if let Some(ref guard) = state.pairing {
                                match guard.try_pair(code, user_id).await {
                                    Ok(Some(_)) => {
                                        ...
                                        {
                                            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"user_id": user_id, "e": e.to_string()})), "paired userId= but persist failed");
                                        } else {
                                            ::dx_agent_log::record!(
                                                INFO,
                                                ::dx_agent_log::Event::new(
                                    ...
                                    Err(wait_ms) => {
                                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"user_id": user_id, "wait_ms": wait_ms})), "bind rate-limited for userId=, retry after ms");
                                    }
                                }
                            }
                        ...
                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"user_id": user_id, "LINE_BIND_COMMAND": LINE_BIND_COMMAND})), "ignoring message from unpaired user: . Send ` <code>` to pair.");
                        continue;
                    }
                }
            }
...
impl LineChannel {
    pub fn new(
        ...
        Self {
            ...
            client: dx_agent_config::schema::build_channel_proxy_client("channel.line", None),
    ...
    /// Construct a `LineChannel` directly from a [`dx_agent_config::schema::LineConfig`].
    ///
    ...
    pub fn from_config(
        config: &dx_agent_config::schema::LineConfig,
        alias: impl Into<String>,
        peer_resolver: Arc<dyn Fn() -> Vec<String> + Send + Sync>,
    ) -> Self {
        Self::new(
    ...
    pub fn with_proxy_url(mut self, proxy_url: Option<String>) -> Self {
        self.client = dx_agent_config::schema::build_channel_proxy_client(
            "channel.line",
            proxy_url.as_deref(),
        );
        self
...
impl ::dx_agent_api::attribution::Attributable for LineChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(::dx_agent_api::attribution::ChannelKind::Line)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
...
mod tests {
    ...
    async fn spawn_webhook(
        ...
        let jh = dx_agent_spawn::spawn!(async move {
    ...
    async fn with_transcription_disabled_config_is_noop() {
        ...
        .with_transcription(dx_agent_config::schema::TranscriptionConfig {
            enabled: false,
            ..Default::default()
        });
    ...
    async fn webhook_audio_transcribed_and_forwarded() {
        ...
        let transcription_config = dx_agent_config::schema::TranscriptionConfig {
            enabled: true,
            api_key: None,
            api_url: String::new(),
