...
impl ::dx_agent_api::attribution::Attributable for DingTalkChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(
            ::dx_agent_api::attribution::ChannelKind::DingTalk,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl Channel for DingTalkChannel {
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        let title = message.subject.as_deref().unwrap_or("DX Agent");
        let body = serde_json::json!({
            "msgtype": "markdown",
            "markdown": {
                "title": title,
