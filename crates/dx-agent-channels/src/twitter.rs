...
impl TwitterChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        dx_agent_config::schema::build_runtime_proxy_client("channel.twitter")
    }
...
impl ::dx_agent_api::attribution::Attributable for TwitterChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(
            ::dx_agent_api::attribution::ChannelKind::Twitter,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
        ...
        loop {
                ...
                .await
            {
                ...
                Ok(resp) => {
                    ...
                    let err = resp.text().await.unwrap_or_default();
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", err), "status": status.to_string()})), "mentions request failed");
                }
                Err(e) => {
                    ::dx_agent_log::record!(
                        WARN,
...
bearer_token = "AAAA"
"#;
        let config: dx_agent_config::schema::TwitterConfig = toml::from_str(toml_str).unwrap();
