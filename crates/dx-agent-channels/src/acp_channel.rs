...
//!
//! Bridges DX Agent's [`Channel`] abstraction onto an active ACP session so
...
impl ::dx_agent_api::attribution::Attributable for AcpChannel {
...
mod tests {
    ...
    async fn request_choice_emits_request_permission_and_resolves_selection() {
        ...
        let task = dx_agent_spawn::spawn!(async move {
    ...
    async fn request_approval_emits_request_permission_and_resolves_approve() {
        ...
        let task = dx_agent_spawn::spawn!(async move { ch.request_approval("", &request).await });
