...
use dx_agent_config::schema::VoiceWakeConfig;
...
impl ::dx_agent_api::attribution::Attributable for VoiceWakeChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(
            ::dx_agent_api::attribution::ChannelKind::VoiceWake,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl Channel for VoiceWakeChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> Result<()> {
        ...
        // ── Initialise cpal stream ────────────────────────────
        {
            ...
            ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"device": device.name().unwrap_or_default(), "sample_rate": sample_rate, "channels": channels_count})), "VoiceWake: opening audio input");
...
mod tests {
    use super::*;
    use dx_agent_config::traits::ChannelConfig;
