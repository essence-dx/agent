...
impl ::dx_agent_api::attribution::Attributable for LinqChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(::dx_agent_api::attribution::ChannelKind::Linq)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        if resp.status() == reqwest::StatusCode::NOT_FOUND {
            ...
            if !create_resp.status().is_success() {
                ...
                ::dx_agent_log::record!(ERROR, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_outcome(::dx_agent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"status": status.to_string(), "error_body": error_body})), "create chat failed:");
