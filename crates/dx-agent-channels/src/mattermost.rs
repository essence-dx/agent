    ...
    async fn try_transcribe_audio_attachment(&self, post: &serde_json::Value) -> Option<String> {
        ...
        if let Some(duration_ms) = audio_file.get("duration").and_then(|d| d.as_u64()) {
            ...
            if duration_secs > config.max_duration_secs {
                ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"duration_secs": duration_secs, "max": config.max_duration_secs})), "audio attachment exceeds max duration, skipping");
                return None;
            }
        }
...
impl ::dx_agent_api::attribution::Attributable for MattermostChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(
            ::dx_agent_api::attribution::ChannelKind::Mattermost,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
