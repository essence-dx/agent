...
pub use dx_agent_config::scattered_types::GmailPushConfig;
...
impl ::dx_agent_api::attribution::Attributable for GmailPushChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(
            ::dx_agent_api::attribution::ChannelKind::GmailPush,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl Channel for GmailPushChannel {
    ...
    async fn send(&self, message: &SendMessage) -> Result<()> {
        ...
        let subject = message.subject.as_deref().unwrap_or("DX Agent Message");
