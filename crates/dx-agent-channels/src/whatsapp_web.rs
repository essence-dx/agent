...
//! session_path = "~/.dx-agent/whatsapp-session.db"  # Required for Web mode
...
/// ```toml
/// [channels_config.whatsapp]
/// session_path = "~/.dx-agent/whatsapp-session.db"
...
pub struct WhatsAppWebChannel {
    ...
    mode: dx_agent_config::schema::WhatsAppWebMode,
    /// DM policy when mode = personal
    dm_policy: dx_agent_config::schema::WhatsAppChatPolicy,
    /// Group policy when mode = personal
    group_policy: dx_agent_config::schema::WhatsAppChatPolicy,
...
impl WhatsAppWebChannel {
    ...
    #[cfg(feature = "whatsapp-web")]
    pub fn new(
        config: &dx_agent_config::schema::WhatsAppConfig,
    ...
    #[cfg(feature = "whatsapp-web")]
    pub fn with_tts(mut self, config: &dx_agent_config::schema::Config) -> Self {
    ...
    async fn try_transcribe_voice_note(
        ...
        transcription_config: Option<&dx_agent_config::schema::TranscriptionConfig>,
...
#[cfg(feature = "whatsapp-web")]
impl ::dx_agent_api::attribution::Attributable for WhatsAppWebChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(
            ::dx_agent_api::attribution::ChannelKind::WhatsappWeb,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
            ...
            let mut builder = Bot::builder()
                ...
                .on_event({
                        ...
                        match &*event {
                            Event::Message(msg, info) => {
                                ...
                                if reply_target != chat {
                                    ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"from": chat, "to": reply_target})), "LID→phone reply target");
                                }

                                // ── Personal-mode chat-type policy filtering ──
                                if wa_mode == dx_agent_config::schema::WhatsAppWebMode::Personal {
                                    ...
                                    if is_self_chat {
                                        if !wa_self_chat_mode {
                                            ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "ignoring self-chat message (self_chat_mode=false)");
                                            return;
                                        }
                                        ...
                                        )
                                    {
                                        ...
                                        ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"chat": chat, "sender": sender})), "ignoring fromMe message outside self-chat thread (chat=, sender=)");
                                        return;
                                    } else if is_group {
                                        match wa_group_policy {
                                            dx_agent_config::schema::WhatsAppChatPolicy::Ignore => {
                                                ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "ignoring group message (group_policy=ignore)");
                                                return;
                                            }
                                            dx_agent_config::schema::WhatsAppChatPolicy::All => {
                                                // allow unconditionally
                                            }
                                            dx_agent_config::schema::WhatsAppChatPolicy::Allowlist => {
                                                if normalized.is_none() {
                                                    let lid_diag = Self::lid_rejection_diagnostic(
                                                        &sender_jid,
                                                        mapped_phone.as_deref(),
                                                    );
                                                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown), &format!("message from unrecognized sender not in allowed list (candidates_count={}){}", sender_candidates.len(), lid_diag));
                                                    return;
                                                }
                                            }
                                        }
                                        ...
                                        match wa_dm_policy {
                                            dx_agent_config::schema::WhatsAppChatPolicy::Ignore => {
                                                ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "ignoring DM (dm_policy=ignore)");
                                                return;
                                            }
                                            dx_agent_config::schema::WhatsAppChatPolicy::All => {
                                                // allow unconditionally
                                ...
                                let voice_text = if let Some(ref audio) = msg.audio_message {
                                    ...
                                    if is_ptt || non_ptt_enabled {
                                        ...
                                        ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), &format!("ignoring non-PTT audio message from {}", normalized));
                                        None
                                    }
                                } else {
                                    None
                                ...
                                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), &format!("WhatsApp Web message received (sender_len={}, chat_len={}, content_len={})", sender.len(), chat.len(), content.len()));
                                ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), &format!("WhatsApp Web message content: {}", content));

                                if content.is_empty() {
                                    ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), &format!("ignoring empty or non-text message from {}", normalized));
                                    return;
                                }
                                ...
                                if mention_only && is_group {
                                    ...
                                    if bot_phone.is_some() || bot_lid.is_some() {
                                        ...
                                        if !Self::contains_bot_mention(
                                            ...
                                            ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "ignoring group message without bot mention");
                                            return;
                                        }
                                    } else {
                                        ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "mention_only active but bot identity unknown, skipping group msg");
                                        return;
                                    }
                                }
                                ...
                                let content =
                                    match super::whatsapp::WhatsAppChannel::apply_mention_gating(
                                        ...
                                        Some(c) => c,
                                        None => {
                                            ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"normalized": normalized})), "message from did not match mention patterns, dropping");
                                            return;
                                        }
                                    };
                    ...
                    attachments: vec![],
                                    ...
                                    .await
                                {
                                    ::dx_agent_log::record!(ERROR, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_outcome(::dx_agent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"error": format!("{}", e)})), "failed to send message to channel");
                                }
                            }
                            Event::Connected(_) => {
                                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "connected successfully");
                                ...
                                if mention_only {
                                    ...
                                    {
                                        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), &format!("resolved bot identity from device: +{}", digits));
                                    }
                                    if let Some(ref lid) = device.lid
                                        && let Some(digits) =
                                            Self::store_jid_digits(&bot_lid_inner, lid.user())
                                    {
                                        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), &format!("resolved bot LID from device: {}", digits));
                                    }
                                }
                            }
                            Event::LoggedOut(_) => {
                                session_revoked.store(true, std::sync::atomic::Ordering::Relaxed);
                                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown), "WhatsApp Web was logged out — will clear session and reconnect");
                                let _ = logout_tx.send(());
                            }
                            Event::StreamError(stream_error) => {
                                ::dx_agent_log::record!(ERROR, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_outcome(::dx_agent_log::EventOutcome::Failure), &format!("stream error: {:?}", stream_error));
                            }
                            Event::PairingCode { code, .. } => {
                                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "pair code received");
                                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "Link your phone by entering this code in WhatsApp > Linked Devices");
                                eprintln!();
                                eprintln!("pair code: {code}");
                                eprintln!();
                            }
                            Event::PairingQrCode { code, .. } => {
                                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "WhatsApp Web QR code received (scan with WhatsApp > Linked Devices)");
                                match Self::render_pairing_qr(code) {
                                    ...
                                    Err(err) => {
                                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown), &format!("failed to render pairing QR in terminal: {}", err));
                                        eprintln!();
            ...
            *self.bot_handle.lock() = Some(bot_handle);
            ...
            let should_reconnect = select! {
                ...
                _ = tokio::signal::ctrl_c() => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "channel received Ctrl+C");
                    false
                }
            };
...
impl WhatsAppWebChannel {
    pub fn new(
        ...
        _mention_only: bool,
        _mode: dx_agent_config::schema::WhatsAppWebMode,
        _dm_policy: dx_agent_config::schema::WhatsAppChatPolicy,
        _group_policy: dx_agent_config::schema::WhatsAppChatPolicy,
        _self_chat_mode: bool,
    ) -> Self {
        Self { _private: () }
    }

    pub fn with_transcription(self, _config: dx_agent_config::schema::TranscriptionConfig) -> Self {
        self
    }

    pub fn with_tts(self, _config: dx_agent_config::schema::TtsConfig) -> Self {
        self
    }
}
...
mod tests {
    ...
    fn whatsapp_web_channel_name() {
        ...
        let cfg = dx_agent_config::schema::WhatsAppConfig {
            enabled: true,
    ...
    fn with_transcription_ignores_when_disabled() {
        let tc = dx_agent_config::schema::TranscriptionConfig::default(); // enabled = false
        let mention_only = false;
        let self_chat_mode = false;
        let cfg = dx_agent_config::schema::WhatsAppConfig {
            enabled: true,
