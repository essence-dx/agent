...
/// and forwards PRIVMSG messages to the `DX Agent` message bus.
...
impl ::dx_agent_api::attribution::Attributable for IrcChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(::dx_agent_api::attribution::ChannelKind::Irc)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
...
impl Channel for IrcChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        Self::send_raw(
            &mut writer,
            &format!("USER {} 0 * :DX Agent", self.username),
        )
        .await?;
...
mod tests {
    ...
    fn irc_config_serde_roundtrip() {
        use dx_agent_config::schema::IrcConfig;
