...
use dx_agent_config::schema::{Config, StreamMode};
    ...
    pending_approvals: Arc<
        tokio::sync::Mutex<
            std::collections::HashMap<
                String,
                tokio::sync::oneshot::Sender<dx_agent_api::channel::ChannelApprovalResponse>,
            >,
        >,
    >,
...
impl TelegramChannel {
    ...
    fn try_add_ack_reaction_nonblocking(&self, chat_id: String, message_id: i64) {
        ...
        dx_agent_spawn::spawn!(async move {
            let response = match client.post(&url).json(&body).send().await {
                Ok(resp) => resp,
                Err(err) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"chat_id": chat_id, "message_id": message_id, "err": err.to_string()})), "failed to add ACK reaction to chat_id=, message_id=");
                    return;
                }
            };
            ...
            if !response.status().is_success() {
                ...
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"chat_id": chat_id, "message_id": message_id, "status": status.to_string(), "err_body": err_body})), "add ACK reaction failed for chat_id=, message_id=: status=, body=");
            }
        });
    }
    ...
    pub fn with_voice_peer_prefs(
        ...
        use dx_agent_config::multi_agent::OutputModality;
    ...
    async fn register_bot_commands(&self) {
        ...
        if let Some(ref workspace_dir) = self.workspace_dir {
            let skills = dx_agent_runtime::skills::load_skills(workspace_dir);
        ...
        if total_before_cap > TELEGRAM_MAX_BOT_COMMANDS {
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"TELEGRAM_MAX_BOT_COMMANDS": TELEGRAM_MAX_BOT_COMMANDS, "total_before_cap": total_before_cap})), "Telegram limits bots to commands; configured, registering first . Reduce installed skills to expose more commands.");
        }
    ...
    async fn synthesize_and_send_voice(
        ...
        let client = dx_agent_config::schema::build_runtime_proxy_client("channel.telegram");
                                ...
                                Ok(()) => {
                                    let _ = self
                                        .send(&SendMessage::new(
                                            "✅ Telegram account bound successfully. You can talk to DX Agent now.",
                                            &chat_id,
                                        ))
                                        .await;
                                    ::dx_agent_log::record!(
...
impl ::dx_agent_api::attribution::Attributable for TelegramChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(
            ::dx_agent_api::attribution::ChannelKind::Telegram,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl Channel for TelegramChannel {
    ...
    async fn update_draft(
        ...
        if resp.status().is_success() {
            ...
            let status = resp.status();
            let err = resp.text().await.unwrap_or_default();
            ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"error": format!("{}", err), "status": status.to_string()})), "editMessageText failed");
        }
                        ...
                        Ok(data) => {
                            ...
                            if error_code == 409 {
                                ...
                                let desc = data
                                    ...
                                    .and_then(serde_json::Value::as_str)
                                    .unwrap_or("unknown");
                                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error_code": error_code, "desc": desc})), "Startup probe: API error : ; retrying in 5s");
                            }
                            tokio::time::sleep(std::time::Duration::from_secs(5)).await;
                        }
                    }
                    ...
                    if let Some(cb) = update.get("callback_query") {
                            ...
                            && let Some((approval_id, action)) = rest.rsplit_once(':')
                        {
                            let response = match action {
                                ...
                                "always" => Some(
                                    dx_agent_api::channel::ChannelApprovalResponse::AlwaysApprove,
                                ),
                                "deny" => {
...
mod tests {
    ...
    fn with_voice_peer_prefs_seeds_static_voice_peers_for_matching_groups_only() {
        use dx_agent_config::multi_agent::{OutputModality, PeerGroupConfig, PeerUsername};
    ...
    async fn telegram_with_transcription_binds_sole_provider_alias() {
        ...
        let config = dx_agent_config::schema::TranscriptionConfig {
            ...
            ..dx_agent_config::schema::TranscriptionConfig::default()
        };
    ...
    async fn stop_typing_clears_handle() {
        ...
        // Manually insert a dummy handle
        {
            ...
            *guard = Some(dx_agent_spawn::spawn!(async {
    ...
    fn markdown_attachment_not_detected_by_multimodal_image_markers() {
        ...
        let messages = vec![dx_agent_providers::ChatMessage::user(content)];
        assert_eq!(
            dx_agent_providers::multimodal::count_image_markers(&messages),
            0,
    ...
    fn photo_image_marker_detected_by_multimodal() {
        ...
        let messages = vec![dx_agent_providers::ChatMessage::user(
            photo_content.to_string(),
        )];
        let count = dx_agent_providers::multimodal::count_image_markers(&messages);
        assert_eq!(
            count, 1,
    ...
    fn e2e_attachment_saves_file_and_formats_content() {
        ...
        let doc_msgs = vec![dx_agent_providers::ChatMessage::user(doc_content)];
        assert_eq!(
            dx_agent_providers::multimodal::count_image_markers(&doc_msgs),
            0,
        ...
        let photo_msgs = vec![dx_agent_providers::ChatMessage::user(photo_content.clone())];
        assert_eq!(
            dx_agent_providers::multimodal::count_image_markers(&photo_msgs),
            1,
        ...
        let cap_msgs = vec![dx_agent_providers::ChatMessage::user(captioned.clone())];
        assert_eq!(
            dx_agent_providers::multimodal::count_image_markers(&cap_msgs),
            1,
        ...
        let md_msgs = vec![dx_agent_providers::ChatMessage::user(md_content)];
        assert_eq!(
            dx_agent_providers::multimodal::count_image_markers(&md_msgs),
            0,
            "markdown file must not trigger image marker detection"
        );
    }
    ...
    fn groq_provider_rejects_photo_with_vision_error() {
        ...
        use dx_agent_providers::compatible::{AuthStyle, OpenAiCompatibleModelProvider};
        ...
        let marker_count = dx_agent_providers::multimodal::count_image_markers(&messages);
