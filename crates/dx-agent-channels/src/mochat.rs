...
impl MochatChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        dx_agent_config::schema::build_runtime_proxy_client("channel.mochat")
    }
...
impl ::dx_agent_api::attribution::Attributable for MochatChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(::dx_agent_api::attribution::ChannelKind::MoChat)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
        ...
        loop {
                ...
                .await
            {
                ...
                Ok(resp) => {
                    let status = resp.status();
                    let err = resp.text().await.unwrap_or_default();
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", err), "status": status.to_string()})), "poll request failed");
                }
                Err(e) => {
                    ::dx_agent_log::record!(
                        WARN,
