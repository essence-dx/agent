...
use uuid::Uuid;
use dx_agent_api::channel::{
    Channel, ChannelApprovalRequest, ChannelApprovalResponse, ChannelMessage, SendMessage,
};
use dx_agent_api::media::MediaAttachment;
...
pub struct DiscordChannel {
    ...
    archive_memory: Option<std::sync::Arc<dyn dx_agent_memory::Memory>>,
    ...
    stream_mode: dx_agent_config::schema::StreamMode,
...
impl DiscordChannel {
    ...
    pub fn with_archive_memory(mut self, mem: std::sync::Arc<dyn dx_agent_memory::Memory>) -> Self {
    ...
    async fn thread_parent(&self, client: &reqwest::Client, channel_id: &str) -> Option<String> {
        ...
        let result = match tokio::time::timeout(THREAD_LOOKUP_TIMEOUT, lookup).await {
            ...
            Err(_) => {
                ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"channel_id": channel_id, "timeout_secs": THREAD_LOOKUP_TIMEOUT.as_secs()})), "channel lookup timed out");
                return None;
            }
        };
    ...
    for att in attachments {
        ...
        if ct.starts_with("text/") {
            match client.get(url).send().await {
                ...
                Ok(resp) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"name": name, "status": resp.status().to_string()})), "attachment fetch failed");
                }
                Err(e) => {
                    ::dx_agent_log::record!(
                        WARN,
        ...
        let marker_target = match workspace_dir {
            Some(dir) => match save_attachment_bytes_to_workspace(dir, name, &bytes).await {
                ...
                Err(e) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"name": name, "kind": marker_kind, "error": format!("{}", e)})), "attachment save failed, falling back to url");
                    url.to_string()
                }
            },
            None => url.to_string(),
    ...
    for attachment in attachments {
        match validate_marker_target(&attachment.target, workspace_dir) {
            ...
            Err(e) => {
                ...
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"kind": attachment.kind.marker_name(), "target": attachment.target, "reason": kind_label, "error": format!("{}", e)})), "dropping unresolved outbound attachment marker");
...
fn chunks_for_send(
    ...
    let mut chunks = match stream_mode {
        dx_agent_config::schema::StreamMode::MultiMessage => {
...
impl ::dx_agent_api::attribution::Attributable for DiscordChannel {
...
impl Channel for DiscordChannel {
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        let inter_chunk_delay_ms =
            if self.stream_mode == dx_agent_config::schema::StreamMode::MultiMessage {
    ...
    async fn listen(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        let watchdog = if self.stall_timeout_secs > 0 {
            Some(dx_agent_infra::stall_watchdog::StallWatchdog::new(
        ...
        loop {
            tokio::select! {
                _ = stall_rx.recv() => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "breaking listen loop due to stall watchdog");
                    break;
                }
                _ = hb_rx.recv() => {
                ...
                msg = read.next() => {
                    let msg = match msg {
                        ...
                        Some(Ok(Message::Ping(payload))) => {
                            if write.send(Message::Pong(payload)).await.is_err() {
                                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown), "pong send failed, reconnecting");
                                break;
                            }
                            continue;
                        }
                        Some(Ok(Message::Close(frame))) => {
                            if let Some(frame) = frame {
                                ...
                                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"code": code, "reason": reason, "had_ready": had_ready, "sequence": sequence})), "discord gateway closed; reconnecting");
                            }
                            break;
                        }
                        None => {
                            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"had_ready": had_ready, "sequence": sequence})), "discord gateway stream ended; reconnecting");
                            break;
                        }
                        Some(Err(e)) => {
                            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", e), "had_ready": had_ready, "sequence": sequence})), "websocket read error, reconnecting");
                            break;
                        }
                        _ => continue,
                    };
                    ...
                    match op {
                        ...
                        // Op 7: Reconnect
                        7 => {
                            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"had_ready": had_ready, "sequence": sequence})), "received Reconnect (op 7), closing for restart");
                            break;
                        }
                        ...
                        9 => {
                            ...
                            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"resumable": resumable, "had_ready": had_ready, "sequence": sequence})), "received Invalid Session (op 9), closing for restart");
                            break;
                        }
                        _ => {}
                    }
                    ...
                    if !self.is_user_allowed(author_id) {
                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"author_id": author_id})), "ignoring message from unauthorized user");
                        continue;
                    }
                        ...
                        if !content_raw.is_empty() {
                            ...
                            if let Err(e) = archive_mem
                                .store(
                                    &mem_key,
                                    &mem_content,
                                    dx_agent_memory::MemoryCategory::Custom(
                                        "discord".to_string(),
                                    ),
                                    session,
                                )
                                .await
                            {
                                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", e)})), "archive store failed");
                            }
                        }
                    }
                    ...
                    if !message_id.is_empty() && !channel_id.is_empty() {
                        ...
                        dx_agent_spawn::spawn!(async move {
                            if let Err(err) = reaction_channel
                                ...
                                .await
                            {
                                ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"reaction_message_id": reaction_message_id, "err": err.to_string()})), "failed to add ACK reaction for message");
                            }
                        });
                    }
    ...
    fn supports_draft_updates(&self) -> bool {
        self.stream_mode != dx_agent_config::schema::StreamMode::Off
    }

    fn supports_multi_message_streaming(&self) -> bool {
        self.stream_mode == dx_agent_config::schema::StreamMode::MultiMessage
    }
    ...
    async fn send_draft(&self, message: &SendMessage) -> anyhow::Result<Option<String>> {
        use dx_agent_config::schema::StreamMode;
