...
impl ::dx_agent_api::attribution::Attributable for WhatsAppChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(
            ::dx_agent_api::attribution::ChannelKind::WhatsappBusiness,
        )
    }
    fn alias(&self) -> &str {
        &self.alias
...
impl Channel for WhatsAppChannel {
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        if !resp.status().is_success() {
            ...
            ::dx_agent_log::record!(ERROR, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_outcome(::dx_agent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"status": status.to_string(), "error_body": error_body})), "send failed:");
...
mod tests {
    ...
    fn whatsapp_compile_skips_invalid_patterns() {
        let patterns =
            WhatsAppChannel::compile_mention_patterns(&["@?DX Agent".into(), "[invalid".into()]);
    ...
    fn whatsapp_compile_skips_empty_patterns() {
        let patterns =
            WhatsAppChannel::compile_mention_patterns(&["@?DX Agent".into(), "  ".into()]);
        assert_eq!(patterns.len(), 1);
    }
    ...
    fn whatsapp_text_matches_at_name() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?DX Agent".into()]);
    ...
    fn whatsapp_group_mention_rejects_group_message_without_match() {
        ...
        .with_group_mention_patterns(vec!["@?DX Agent".into()]);
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
    ...
    fn whatsapp_group_mention_admits_and_preserves_in_group() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "@DX Agent what is the weather?")]
                    }
                }]
            }]
        });
        let msgs = ch.parse_webhook_payload(&payload);
        assert_eq!(msgs.len(), 1);
        assert_eq!(msgs[0].content, "@DX Agent what is the weather?");
    }
    ...
    fn whatsapp_group_mention_preserves_mid_sentence_mention() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "Hey @DX Agent tell me a joke")]
                    }
                }]
            }]
        });
        let msgs = ch.parse_webhook_payload(&payload);
        assert_eq!(msgs.len(), 1);
        assert_eq!(msgs[0].content, "Hey @DX Agent tell me a joke");
    }
    ...
    fn whatsapp_group_mention_admits_mention_only_group_message() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "@DX Agent")]
                    }
                }]
            }]
        });
        let msgs = ch.parse_webhook_payload(&payload);
        assert_eq!(msgs.len(), 1);
        assert_eq!(msgs[0].content, "@DX Agent");
    }
    ...
    fn whatsapp_group_mention_mixed_group_messages() {
                ...
                "changes": [{
                    "value": {
                        "messages": [
                            ...
                            group_msg("222", "2", "@DX Agent help me"),
        ...
        let msgs = ch.parse_webhook_payload(&payload);
        assert_eq!(msgs.len(), 1);
        assert_eq!(msgs[0].content, "@DX Agent help me");
    ...
    fn whatsapp_group_mention_dm_not_stripped() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [dm_msg("111", "1", "@DX Agent what is the weather?")]
                    }
                }]
            }]
        });
        ...
        assert_eq!(msgs.len(), 1);
        assert_eq!(
            msgs[0].content, "@DX Agent what is the weather?",
    ...
    fn whatsapp_dm_mention_rejects_dm_without_match() {
        ...
        .with_dm_mention_patterns(vec!["@?DX Agent".into()]);
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
