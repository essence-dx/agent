...
fn build_resolved_approval_card(
    tool_name: &str,
    arguments_summary: &str,
    decision: dx_agent_api::channel::ChannelApprovalResponse,
) -> serde_json::Value {
    use dx_agent_api::channel::ChannelApprovalResponse;
...
struct PendingApproval {
    sender: tokio::sync::oneshot::Sender<dx_agent_api::channel::ChannelApprovalResponse>,
...
pub struct LarkChannel {
    ...
    receive_mode: dx_agent_config::schema::LarkReceiveMode,
    ...
    transcription: Option<dx_agent_config::schema::TranscriptionConfig>,
...
impl LarkChannel {
    ...
    fn new_with_platform(
        ...
        Self {
            ...
            mention_only,
            platform,
            receive_mode: dx_agent_config::schema::LarkReceiveMode::default(),
    ...
    pub fn from_config(
        config: &dx_agent_config::schema::LarkConfig,
        alias: impl Into<String>,
        peer_resolver: Arc<dyn Fn() -> Vec<String> + Send + Sync>,
    ) -> Self {
    ...
    pub fn with_transcription(
        mut self,
        config: dx_agent_config::schema::TranscriptionConfig,
    ) -> Self {
        if !config.enabled {
            return self;
        }
    ...
    fn http_client(&self) -> reqwest::Client {
        dx_agent_config::schema::build_channel_proxy_client(
    ...
    async fn try_add_ack_reaction(&self, message_id: &str, emoji_type: &str) {
        ...
        loop {
                ...
                .await
            {
                Ok(resp) => resp,
                Err(err) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", err), "message_id": message_id})), "failed to add reaction for");
                    return;
                }
            };
            ...
            if response.status().as_u16() == 401 && !retried {
                ...
                token = match self.get_tenant_access_token().await {
                    ...
                    Err(err) => {
                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"message_id": message_id, "err": err.to_string()})), "failed to refresh token for reaction on");
                        return;
                    }
                };
                retried = true;
            ...
            if !response.status().is_success() {
                ...
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"message_id": message_id, "status": status.to_string(), "err_body": err_body})), "add reaction failed for : status=, body=");
                return;
            }
            ...
            let payload: serde_json::Value = match response.json().await {
                Ok(v) => v,
                Err(err) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", err), "message_id": message_id})), "add reaction decode failed for");
                    return;
                }
            };
            ...
            if code != 0 {
                ...
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"code": code.to_string(), "message_id": message_id, "msg": msg.to_string()})), "add reaction returned code= for");
            }
            return;
        }
    }
    ...
    async fn listen_ws(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        let (ws_stream, _) = dx_agent_config::schema::ws_connect_with_proxy(
            &wss_url,
            "channel.lark",
            self.proxy_url.as_deref(),
        )
        ...
        loop {
            tokio::select! {
                ...
                _ = hb_interval.tick() => {
                    ...
                    if write.send(WsMsg::Binary(ping.encode_to_vec().into())).await.is_err() {
                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown), "ping failed, reconnecting");
                        break;
                    }
                ...
                _ = timeout_check.tick() => {
                    if last_recv.elapsed() > WS_HEARTBEAT_TIMEOUT {
                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown), "heartbeat timeout, reconnecting");
                        break;
                    }
                }
                ...
                msg = read.next() => {
                    let raw = match msg {
                        Some(Ok(ws_msg)) => {
                            ...
                            match ws_msg {
                                ...
                                WsMsg::Close(_) => { ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "WS closed — reconnecting"); break; }
                                _ => continue,
                            }
                        }
                        None => { ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "WS closed — reconnecting"); break; }
                        Some(Err(e)) => { ::dx_agent_log::record!(ERROR, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_outcome(::dx_agent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"error": format!("{}", e)})), "WS read error"); break; }
                    };

                    let frame = match PbFrame::decode(&raw[..]) {
                        Ok(f) => f,
                        Err(e) => { ::dx_agent_log::record!(ERROR, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_outcome(::dx_agent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"error": format!("{}", e)})), "proto decode"); continue; }
                    };
                                ...
                                && let Ok(cfg) = serde_json::from_slice::<WsClientConfig>(p)
                                    && let Some(secs) = cfg.ping_interval {
                                        ...
                                        if secs != ping_secs {
                                            ...
                                            ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"ping_secs": ping_secs})), "ping_interval → s");
                                        }
                                    }
                        continue;
                    }
                    ...
                    let event: LarkEvent = match serde_json::from_slice(&payload) {
                        Ok(e) => e,
                        Err(e) => { ::dx_agent_log::record!(ERROR, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_outcome(::dx_agent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"error": format!("{}", e)})), "event JSON"); continue; }
                    };
                    ...
                    let recv: MsgReceivePayload = match serde_json::from_value(event_payload.clone()) {
                        Ok(r) => r,
                        Err(e) => { ::dx_agent_log::record!(ERROR, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_outcome(::dx_agent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"error": format!("{}", e)})), "payload parse"); continue; }
                    };
                    ...
                    if !self.is_user_allowed(sender_open_id) {
                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"sender_open_id": sender_open_id})), "WS: ignoring (not in peer group)");
                        continue;
                    }
                    ...
                    // Dedup
                    {
                        ...
                        if seen.contains_key(&lark_msg.message_id) {
                            ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), &format!("WS: dup {}", lark_msg.message_id));
                            continue;
                        }
                    ...
                    let (text, post_mentioned_open_ids) = match lark_msg.message_type.as_str() {
                        ...
                        "image" => {
                            ...
                            let image_key = match v.get("image_key").and_then(|k| k.as_str()) {
                                Some(k) => k.to_string(),
                                None => { ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "WS: image message missing image_key"); continue; }
                            };
                            match self.download_image_as_marker(&image_key).await {
                                Some(marker) => (marker, Vec::new()),
                                None => {
                                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"image_key": image_key})), "WS: failed to download image");
                        ...
                        "file" => {
                            ...
                            let file_key = match v.get("file_key").and_then(|k| k.as_str()) {
                                Some(k) => k.to_string(),
                                None => { ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "WS: file message missing file_key"); continue; }
                            };
                            ...
                            match self.download_file_as_content(&lark_msg.message_id, &file_key, &file_name).await {
                                ...
                                None => {
                                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"file_key": file_key})), "WS: failed to download file");
                        ...
                        "audio" => {
                            let Some(manager) = self.transcription_manager.as_deref() else {
                                ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), &format!("WS: audio message in {} (transcription not configured)", lark_msg.chat_id));
                                continue;
                            };
                        ...
                        "list" => match parse_list_content(&lark_msg.content) {
                            Some(t) => (t, Vec::new()),
                            None => { ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "WS: list message with no extractable text"); continue; }
                        },
                        _ => { ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), &format!("WS: skipping unsupported type '{}'", lark_msg.message_type)); continue; }
                    };
            ...
            channel_alias: Some(self.alias.clone()),
                    ...
                    ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), &format!("WS: message in {}", lark_msg.chat_id));
...
impl ::dx_agent_api::attribution::Attributable for LarkChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(::dx_agent_api::attribution::ChannelKind::Lark)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
...
impl Channel for LarkChannel {
    ...
    async fn listen(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        use dx_agent_config::schema::LarkReceiveMode;
    ...
    async fn request_approval(
        &self,
        recipient: &str,
        request: &dx_agent_api::channel::ChannelApprovalRequest,
...
impl LarkChannel {
    ...
    async fn wait_for_decision(
        &self,
        rx: tokio::sync::oneshot::Receiver<dx_agent_api::channel::ChannelApprovalResponse>,
        approval_id: &str,
    ) -> dx_agent_api::channel::ChannelApprovalResponse {
        use dx_agent_api::channel::ChannelApprovalResponse;
    ...
    async fn handle_card_action_event(
        ...
        //
        //   RUST_LOG=info,dx_agent_log_event=debug
        //
        // Captured payloads should land in
        // `crates/dx-agent-channels/tests/fixtures/lark/` and are replayed
        // by the integration test in `tests/lark_approval_live_evidence.rs`.
        ::dx_agent_log::record!(
            DEBUG,
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Receive).with_attrs(
                ::serde_json::json!({
...
mod tests {
    ...
    async fn lark_parse_valid_text_message() {
        ...
        assert_eq!(msgs[0].content, "Hello DX Agent!");
    ...
    fn lark_config_serde() {
        use dx_agent_config::schema::{LarkConfig, LarkReceiveMode};
        let lc = LarkConfig {
            enabled: true,
    ...
    fn lark_reaction_url_matches_region() {
        ...
        let feishu_cfg = dx_agent_config::schema::LarkConfig {
            enabled: true,
            app_id: "cli_app123".into(),
            app_secret: "secret456".into(),
            encrypt_key: None,
            ...
            mention_only: false,
            use_feishu: true,
            receive_mode: dx_agent_config::schema::LarkReceiveMode::Webhook,
            port: Some(9898),
            proxy_url: None,
            excluded_tools: vec![],
        };
    ...
    fn lark_manager_none_when_disabled() {
        let tc = dx_agent_config::schema::TranscriptionConfig {
            enabled: false,
            ..Default::default()
        };
    ...
    async fn lark_audio_file_key_missing_returns_none() {
        ...
        let tc = dx_agent_config::schema::TranscriptionConfig {
            enabled: true,
            local_whisper: Some(dx_agent_config::schema::LocalWhisperConfig {
    ...
    async fn lark_audio_routes_through_transcription_manager() {
        ...
        let config = dx_agent_config::schema::TranscriptionConfig {
            enabled: true,
            local_whisper: Some(dx_agent_config::schema::LocalWhisperConfig {
    ...
    async fn lark_send_via_from_config_emits_post_to_messages_endpoint() {
        ...
        let config = dx_agent_config::schema::LarkConfig {
            enabled: true,
            use_feishu: false,
