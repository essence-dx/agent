...
/// Extract the Slack message timestamp from a DX Agent message ID.
///
...
impl SlackChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        dx_agent_config::schema::build_channel_proxy_client_with_timeouts(
            "channel.slack",
    ...
    pub async fn post_message(&self, channel: &str, text: &str) -> anyhow::Result<String> {
        ...
        if !status.is_success() {
            let sanitized = dx_agent_providers::sanitize_api_error(&raw);
    ...
    async fn fetch_sender_display_name(&self, user_id: &str) -> Option<String> {
        ...
        if !status.is_success() {
            let sanitized = dx_agent_providers::sanitize_api_error(&body);
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"user_id": user_id, "status": status.to_string(), "sanitized": sanitized})), "users.info failed for");
            return None;
        }
    ...
    async fn fetch_permalink_message(
        ...
        if payload.get("ok") == Some(&serde_json::Value::Bool(false)) {
            ...
            return match err {
                ...
                _ => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown), &format!("Slack permalink resolver: conversations.history returned error for channel={} ts={}: {}", channel_id, message_ts, err));
                    SlackPermalinkLookup::NotFound
                }
            };
        }
    ...
    async fn fetch_file_info(&self, file_id: &str) -> Option<serde_json::Value> {
        ...
        if !status.is_success() {
            let sanitized = dx_agent_providers::sanitize_api_error(&body);
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"file_id": file_id, "status": status.to_string(), "sanitized": sanitized})), "files.info failed for");
            return None;
        }
    ...
    fn validate_slack_private_file_url(raw_url: &str) -> Option<reqwest::Url> {
        let parsed = match reqwest::Url::parse(raw_url) {
            ...
            Err(err) => {
                ...
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", err), "redacted_raw": redacted_raw})), "file URL parse failed for");
                return None;
            }
        };
    ...
    fn slack_media_http_client_no_redirect(&self) -> anyhow::Result<reqwest::Client> {
        let builder = dx_agent_config::schema::apply_channel_proxy_to_builder(
    ...
    fn parse_block_action_as_command(
        ...
        let command = match action_id {
            "dx_agent_config_provider" => format!("/models {selected_value}"),
            "dx_agent_config_model" => format!("/model {selected_value}"),
            _ => return None,
        };
    ...
    async fn listen_socket_mode(
        ...
        loop {
            ...
            let (ws_stream, _) = match dx_agent_config::schema::ws_connect_with_proxy(
                &ws_url,
                "channel.slack",
                self.proxy_url.as_deref(),
            )
                        ...
                        if reaction == cancel_emoji.as_str() {
                            ...
                            if !user.is_empty() && self.is_user_allowed(user) {
                                ...
                                if !item_channel.is_empty() && !item_ts.is_empty() {
                                    ...
                                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"cancel_emoji": cancel_emoji, "user": user, "item_channel": item_channel, "item_ts": item_ts})), ":: reaction from on / — sending /stop");
    ...
    async fn fetch_history_with_retry(
        ...
        for attempt in 0..=SLACK_HISTORY_MAX_RETRIES {
                ...
                .await
            {
                Ok(r) => r,
                Err(e) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", e), "channel_id": channel_id})), "poll error for channel");
                    return None;
                }
            };
            ...
            if payload.get("ok") == Some(&serde_json::Value::Bool(false)) {
                let err = payload
                    ...
                    .and_then(|e| e.as_str())
                    .unwrap_or("unknown");
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"error": format!("{}", err), "channel_id": channel_id})), "history error for channel");
                return None;
            }
    ...
    async fn fetch_thread_replies_with_retry(
        ...
        for attempt in 0..=SLACK_HISTORY_MAX_RETRIES {
                ...
                .await
            {
                Ok(r) => r,
                Err(e) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"thread_ts": thread_ts, "channel_id": channel_id, "e": e.to_string()})), "Slack conversations.replies error for thread in");
                    return None;
                }
            };
...
impl ::dx_agent_api::attribution::Attributable for SlackChannel {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Channel(::dx_agent_api::attribution::ChannelKind::Slack)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
...
impl Channel for SlackChannel {
    ...
    async fn add_reaction(
        ...
        if !status.is_success() {
            let sanitized = dx_agent_providers::sanitize_api_error(&text);
...
mod tests {
    ...
    fn approval_block_action_non_approval_returns_none() {
        let envelope = serde_json::json!({
            "payload": {
                "type": "block_actions",
                "actions": [{ "action_id": "dx_agent_config_provider", "selected_option": { "value": "anthropic" } }]
            }
        });
