...
//! DX Agent to external platforms. Each channel implements the [`Channel`] trait
...
//! `dx-agent-channels/src/`, declare it in `lib.rs` behind the appropriate feature
...
pub use dx_agent_api::channel::{Channel, ChannelMessage, SendMessage};
// Local channel types (in misc, not dx-agent-channels)
...
pub use dx_agent_infra::debounce::MessageDebouncer;
pub use dx_agent_infra::session_backend::SessionBackend;
pub use dx_agent_infra::session_sqlite::SqliteSessionBackend;
pub use dx_agent_infra::stall_watchdog::StallWatchdog;
...
use dx_agent_api::session_keys::sanitize_session_key;
use dx_agent_config::schema::Config;
use dx_agent_memory::{self, MEMORY_CONTEXT_CLOSE, MEMORY_CONTEXT_OPEN, Memory};
use dx_agent_providers::reliable::{scope_provider_fallback, take_last_provider_fallback};
use dx_agent_providers::{self, ChatMessage, ModelProvider};
use dx_agent_runtime::agent::loop_::{
...
use dx_agent_runtime::approval::ApprovalManager;
use dx_agent_runtime::observability::traits::{ObserverEvent, ObserverMetric};
use dx_agent_runtime::observability::{self, Observer};
use dx_agent_runtime::platform;
use dx_agent_runtime::security::{AutonomyLevel, SecurityPolicy};
use dx_agent_runtime::tools::{self, Tool};
use dx_agent_runtime::util::truncate_with_ellipsis;
...
// System prompt functions live in `dx_agent_runtime::agent::system_prompt`.
#[allow(unused_imports)]
pub use dx_agent_runtime::agent::system_prompt::{
...
struct ChannelRuntimeDefaults {
    ...
    reliability: dx_agent_config::schema::ReliabilityConfig,
}
...
struct ChannelCostTrackingState {
    tracker: Arc<dx_agent_runtime::cost::CostTracker>,
    model_provider_pricing: Arc<dx_agent_runtime::agent::cost::ModelProviderPricing>,
...
struct ChannelRuntimeContext {
    ...
    agent_cfg: Arc<dx_agent_config::schema::AliasedAgentConfig>,
    prompt_config: Arc<dx_agent_config::schema::Config>,
    ...
    api_key: Option<String>,
    api_url: Option<String>,
    reliability: Arc<dx_agent_config::schema::ReliabilityConfig>,
    provider_runtime_options: dx_agent_providers::ModelProviderRuntimeOptions,
    workspace_dir: Arc<PathBuf>,
    message_timeout_secs: u64,
    interrupt_on_new_message: InterruptOnNewMessageConfig,
    multimodal: dx_agent_config::schema::MultimodalConfig,
    media_pipeline: dx_agent_config::schema::MediaPipelineConfig,
    transcription_config: dx_agent_config::schema::TranscriptionConfig,
    ...
    hooks: Option<Arc<dx_agent_runtime::hooks::HookRunner>>,
    non_cli_excluded_tools: Arc<Vec<String>>,
    autonomy_level: AutonomyLevel,
    tool_call_dedup_exempt: Arc<Vec<String>>,
    model_routes: Arc<Vec<dx_agent_config::schema::ModelRouteConfig>>,
    query_classification: dx_agent_config::schema::QueryClassificationConfig,
    ack_reactions: bool,
    show_tool_calls: bool,
    session_store: Option<Arc<dyn dx_agent_infra::session_backend::SessionBackend>>,
    ...
    activated_tools:
        Option<std::sync::Arc<std::sync::Mutex<dx_agent_runtime::tools::ActivatedToolSet>>>,
    cost_tracking: Option<ChannelCostTrackingState>,
    pacing: dx_agent_config::schema::PacingConfig,
    max_tool_result_chars: usize,
    context_token_budget: usize,
    debouncer: Arc<dx_agent_infra::debounce::MessageDebouncer>,
    ...
    receipt_generator: Option<dx_agent_runtime::agent::tool_receipts::ReceiptGenerator>,
...
fn conversation_memory_key(msg: &dx_agent_api::channel::ChannelMessage) -> String {
...
pub fn conversation_history_key(msg: &dx_agent_api::channel::ChannelMessage) -> String {
...
fn followup_thread_id(msg: &dx_agent_api::channel::ChannelMessage) -> Option<String> {
...
fn interruption_scope_key(msg: &dx_agent_api::channel::ChannelMessage) -> String {
...
fn build_channel_system_prompt_for_message(
    base_prompt: &str,
    msg: &dx_agent_api::channel::ChannelMessage,
...
async fn load_runtime_config_and_defaults(
    ...
    if let Some(dx_agent_dir) = path.parent() {
        let store =
            dx_agent_runtime::security::SecretStore::new(dx_agent_dir, parsed.secrets.encrypt);
...
async fn maybe_apply_runtime_config_update(ctx: &ChannelRuntimeContext) -> Result<()> {
    ...
    let next_options = dx_agent_providers::options_for_provider_ref(
        &next_config,
        &next_defaults.default_model_provider,
        &ctx.provider_runtime_options,
    );
    let next_default_model_provider = dx_agent_providers::create_resilient_model_provider_from_ref(
        &next_config,
        &next_defaults.default_model_provider,
    ...
    if let Err(err) = next_default_model_provider.warmup().await {
        if dx_agent_providers::reliable::is_non_retryable(&err) {
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model_provider": next_defaults.default_model_provider, "model": next_defaults.model, "err": err.to_string()})), "Rejecting config reload: model not available (non-retryable)");
            return Ok(());
        }
...
fn refreshed_new_session_system_prompt(ctx: &ChannelRuntimeContext) -> String {
    let refreshed_skills = dx_agent_runtime::skills::skills_to_prompt_with_mode(
        &dx_agent_runtime::skills::load_skills_with_config(
...
fn should_rollback_failed_user_turn(error: &anyhow::Error) -> bool {
    if error
        .downcast_ref::<dx_agent_providers::ProviderCapabilityError>()
        .is_some_and(|capability| capability.capability.eq_ignore_ascii_case("vision"))
    {
        return true;
    }

    dx_agent_providers::reliable::is_non_retryable(error)
}

fn should_skip_memory_context_entry(key: &str, content: &str) -> bool {
    if dx_agent_memory::is_assistant_autosave_key(key) {
        return true;
    }
    ...
    if dx_agent_memory::is_user_autosave_key(key) {
        return true;
    }

    if dx_agent_memory::should_skip_autosave_content(content) {
        return true;
    }
...
async fn create_resilient_model_provider_nonblocking(
    config: Arc<dx_agent_config::schema::Config>,
    provider_name: &str,
    api_key: Option<String>,
    api_url: Option<String>,
    reliability: dx_agent_config::schema::ReliabilityConfig,
    ...
    tokio::task::spawn_blocking(move || {
        let options = dx_agent_providers::options_for_provider_ref(
            &config,
            &provider_name,
            &provider_runtime_options,
        );
        dx_agent_providers::create_resilient_model_provider_from_ref(
            &config,
            &provider_name,
            api_key.as_deref(),
            api_url.as_deref(),
...
fn build_models_help_response(
    ...
    workspace_dir: &Path,
    model_routes: &[dx_agent_config::schema::ModelRouteConfig],
) -> String {
...
fn build_providers_help_response(current: &ChannelRouteSelection) -> String {
    ...
    for model_provider in dx_agent_providers::list_model_providers() {
...
fn build_config_text_response(
    ...
    for p in dx_agent_providers::list_model_providers() {
...
fn build_config_block_kit(
    ...
    let provider_options: Vec<serde_json::Value> = dx_agent_providers::list_model_providers()
        .iter()
        .map(|p| {
            serde_json::json!({
    ...
    let mut provider_select = serde_json::json!({
        "type": "static_select",
        "action_id": "dx_agent_config_provider",
    ...
    let mut model_select = serde_json::json!({
        "type": "static_select",
        "action_id": "dx_agent_config_model",
    ...
    let response = match command {
            ...
            match canonical_model_provider_name(&raw_model_provider) {
                Some(provider_name) => {
                    match get_or_create_provider(ctx, &provider_name, None).await {
                        ...
                        Err(err) => {
                            let safe_err = dx_agent_providers::sanitize_api_error(&err.to_string());
                            format!(
        ...
        ChannelRuntimeCommand::ShowConfig => {
            if msg.channel == "slack" {
                ...
                format!("__DX_AGENT_BLOCK_KIT__{blocks_json}")
            } else {
...
fn append_recalled_memory_entries(
    entries: &mut Vec<dx_agent_memory::MemoryEntry>,
    seen_keys: &mut HashSet<String>,
    recalled: Result<Vec<dx_agent_memory::MemoryEntry>>,
) {
...
fn format_memory_context(
    entries: &[dx_agent_memory::MemoryEntry],
    min_relevance_score: f64,
) -> String {
...
async fn classify_channel_reply_intent(
    ...
    for msg in history.iter().filter(|m| m.role != "system") {
        ...
        let safe_content = dx_agent_providers::multimodal::strip_media_markers(&msg.content);
...
async fn resolve_classifier_route(
    ctx: &ChannelRuntimeContext,
    provider_ref: &dx_agent_config::providers::ModelProviderRef,
        ...
        .await
    {
        Ok(p) => p,
        Err(e) => {
            let safe_err = dx_agent_providers::sanitize_api_error(&e.to_string());
            ::dx_agent_log::record!(
                WARN,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                    .with_outcome(::dx_agent_log::EventOutcome::Unknown)
...
fn starts_with_visible_tool_call_tag_example(response: &str) -> bool {
    ...
    starts_with_tool_tag && dx_agent_tool_call_parser::looks_like_tool_protocol_example(response)
}
...
fn should_suppress_top_level_tool_protocol_response(
    ...
    if dx_agent_tool_call_parser::looks_like_tool_protocol_example(response) {
        return false;
    }

    if dx_agent_tool_call_parser::looks_like_malformed_tool_protocol_envelope_for_known_tools(
        response,
        known_tool_names,
    ) {
        return true;
    ...
    if let Some(kind) = dx_agent_tool_call_parser::classify_tool_protocol_envelope(response) {
        return matches!(
            kind,
            dx_agent_tool_call_parser::ToolProtocolEnvelopeKind::TaggedToolCall
        ) || (!known_tool_names.is_empty()
            && (matches!(
                kind,
                dx_agent_tool_call_parser::ToolProtocolEnvelopeKind::ToolResult
            ) || dx_agent_tool_call_parser::tool_protocol_envelope_mentions_known_tool(
                response,
                known_tool_names,
            )));
    }
    ...
    dx_agent_tool_call_parser::looks_like_tool_protocol_envelope(response)
}
...
fn sanitize_channel_response(response: &str, tools: &[Box<dyn Tool>]) -> String {
    ...
    match dx_agent_runtime::security::LeakDetector::new().scan(&sanitized) {
        dx_agent_runtime::security::LeakResult::Clean => sanitized,
        dx_agent_runtime::security::LeakResult::Detected { patterns, redacted } => {
            ::dx_agent_log::record!(
                WARN,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                    .with_outcome(::dx_agent_log::EventOutcome::Unknown)
...
fn sanitize_tool_json_value(
    ...
    if let Some(kind) =
        dx_agent_tool_call_parser::classify_tool_protocol_envelope(&value.to_string())
    {
        ...
        if !dx_agent_tool_call_parser::tool_protocol_envelope_mentions_known_tool(
            &value.to_string(),
            known_tool_names,
        ) {
            return None;
...
fn safe_protocol_envelope_content(value: &serde_json::Value) -> String {
    ...
    if content.is_empty()
        || dx_agent_tool_call_parser::looks_like_tool_protocol_envelope(content)
        || dx_agent_tool_call_parser::looks_like_malformed_tool_protocol_envelope(content)
    {
        return String::new();
    }
...
fn strip_fenced_tool_protocol_artifacts(
    ...
    if dx_agent_tool_call_parser::looks_like_tool_protocol_example(message) {
    ...
    while let Some(rel_open) = message[cursor..].find("```") {
        ...
        let should_strip = if language.eq_ignore_ascii_case("json") {
            ...
            is_tool_protocol_fence_language(language)
                && dx_agent_tool_call_parser::contains_tool_protocol_tag_call(fence_block)
        };
...
fn spawn_supervised_listener(
    ch: Arc<dyn Channel>,
    alias: Option<String>,
    tx: tokio::sync::mpsc::Sender<dx_agent_api::channel::ChannelMessage>,
...
fn spawn_supervised_listener_with_health_interval(
    ...
    let span = dx_agent_log::attribution_span!(&*ch);
    dx_agent_spawn::spawn!(
        async move {
            ...
            loop {
                dx_agent_runtime::health::mark_component_ok(&component);
                ...
                match result {
                    Ok(()) => {
                        ...
                        dx_agent_runtime::health::mark_component_error(
                            &component,
                    ...
                    Err(e) => {
                        if is_non_retryable_channel_listener_error(ch.name(), &e) {
                            ...
                            dx_agent_runtime::health::mark_component_error(&component, e.to_string());
                            tokio::select! {
                                () = cancel.cancelled() => return,
                ...
                dx_agent_runtime::health::bump_component_restart(&component);
                tokio::select! {
                    () = cancel.cancelled() => return,
...
fn spawn_scoped_typing_task(
    ...
    dx_agent_spawn::spawn!(async move {
            ...
            tokio::select! {
                ...
                _ = interval.tick() => {
                    if let Err(e) = channel.start_typing(&recipient).await {
                        ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"error": format!("{}", e)})), "failed to start typing");
                    }
                }
            }
        }
...
async fn process_channel_message_body(
    ...
    ::dx_agent_log::record!(
        INFO,
        ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Inbound).with_attrs(
            ::serde_json::json!({
                "sender": msg.sender,
                "message_id": msg.id,
    ...
    let mut msg = if let Some(hooks) = &ctx.hooks {
        match hooks.run_on_message_received(msg).await {
            dx_agent_runtime::hooks::HookResult::Cancel(reason) => {
                ::dx_agent_log::record!(
                    INFO,
                    ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
            ...
            dx_agent_runtime::hooks::HookResult::Continue(modified) => modified,
        }
    } else {
        msg
    };
    ...
    if let Some(channel) = target_channel.as_ref() {
        ...
        if dx_agent_runtime::peers::should_drop_self_loop(
            &msg.sender,
    ...
    if let Some(ref store) = ctx.session_store {
        ...
        let context = dx_agent_infra::session_backend::SessionContext {
    ...
    if let Some(hint) =
        dx_agent_runtime::agent::classifier::classify(&ctx.query_classification, &msg.content)
        && let Some(matched_route) = ctx
            .model_routes
            .iter()
            .find(|r| r.hint.eq_ignore_ascii_case(&hint))
    {
        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"hint": hint.as_str(), "model_provider": matched_route.model_provider.as_str(), "model": matched_route.model.as_str()})), "Channel message classified — overriding route");
    ...
    if ctx.auto_save_memory
        ...
        && !dx_agent_memory::should_skip_autosave_content(&msg.content)
    {
        ...
        let _ = ctx
            ...
            .store(
                &autosave_key,
                &msg.content,
                dx_agent_memory::MemoryCategory::Conversation,
                Some(&history_key),
            )
            .await;
    }
    ...
    if !active_model_provider.supports_vision() && prior_turns.len() > 1 {
        ...
        for turn in &mut prior_turns[..last_idx] {
            if turn.content.contains("[IMAGE:") {
                let (cleaned, _refs) =
                    dx_agent_providers::multimodal::parse_image_markers(&turn.content);
    ...
    if dropped > 0 {
        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"sender": msg.sender, "dropped_turns": dropped, "remaining_turns": prior_turns.len()})), "Proactively trimmed conversation history to fit context budget");
    }
    ...
    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"mem_recall_ms": mem_recall_ms, "sender_empty": sender_memory.is_empty(), "group_empty": group_memory.is_empty()})), "memory recall completed");
    ...
    // and preserving key decisions through LLM-driven summarization.
    {
        ...
        let compressor = dx_agent_runtime::agent::context_compressor::ContextCompressor::new(
            cc_config,
            ...
            .await
        {
            Ok(result) if result.compressed => {
                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"sender": msg.sender, "tokens_before": result.tokens_before, "tokens_after": result.tokens_after, "passes": result.passes_used})), "Proactive context compression applied before LLM call");
            }
            Err(e) => {
                ::dx_agent_log::record!(
                    WARN,
    ...
    let is_acp_channel = target_channel
        ...
        .map(|c| {
            matches!(
                ::dx_agent_api::attribution::Attributable::role(c.as_ref()),
                ::dx_agent_api::attribution::Role::Channel(
                    ::dx_agent_api::attribution::ChannelKind::AcpChannel
                )
            )
        })
        .unwrap_or(false);
        ...
        } = classifier_intent
    {
        ::dx_agent_log::record!(
            DEBUG,
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(
                ::serde_json::json!({
                    "kind": format!("{kind:?}"),
    ...
    if let AssistantChannelOutcome::NoReply { kind, reason } = reply_intent {
        ...
        ::dx_agent_log::record!(
            INFO,
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Skip)
    ...
    ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"has_target_channel": target_channel.is_some(), "use_draft_streaming": use_draft_streaming})), "Streaming decision");

    // Partial mode: delta channel for draft updates (progress + text).
    let (delta_tx, delta_rx) = if use_draft_streaming {
        let (tx, rx) = tokio::sync::mpsc::channel::<dx_agent_runtime::agent::loop_::DraftEvent>(64);
        (Some(tx), Some(rx))
    } else {
        (None, None)
    };
    ...
    let draft_updater = if use_draft_streaming {
        ...
        if let (Some(mut rx), Some(draft_id_ref), Some(channel_ref)) = (
            ...
            Some(dx_agent_spawn::spawn!(async move {
                use dx_agent_runtime::agent::loop_::StreamDelta;
    ...
    let cost_tracking_context = ctx.cost_tracking.clone().map(|state| {
        dx_agent_runtime::agent::loop_::ToolLoopCostTrackingContext::new(
            state.tracker,
    ...
    let receipt_scope = ctx.receipt_generator.as_ref().map(|generator| {
        dx_agent_runtime::agent::tool_receipts::ReceiptScope {
            ...
            let loop_result = tokio::select! {
                ...
                result = tokio::time::timeout(
                    ...
                    scope_session_key(
                        ...
                        dx_agent_runtime::agent::loop_::TOOL_LOOP_COST_TRACKING_CONTEXT.scope(
                            cost_tracking_context.clone(),
                        dx_agent_runtime::agent::tool_receipts::TOOL_LOOP_RECEIPT_CONTEXT.scope(
                            receipt_scope.clone(),
                        run_tool_call_loop(
                        active_model_provider.as_ref(),
                        &mut history,
    ...
    match llm_result {
        LlmExecutionResult::Cancelled => {
            ...
            ::dx_agent_log::record!(
                INFO,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Cancel)
                    .with_outcome(::dx_agent_log::EventOutcome::Failure)
                    .with_duration(
        ...
        LlmExecutionResult::Completed(Ok(Ok(response))) => {
            ...
            if let Some(hooks) = &ctx.hooks {
                    ...
                    .await
                {
                    ...
                    dx_agent_runtime::hooks::HookResult::Continue((
                        hook_channel,
                        hook_recipient,
                        mut modified_content,
                    )) => {
                        if hook_channel != msg.channel || hook_recipient != msg.reply_target {
                            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"from_channel": channel_composite, "from_recipient": msg.reply_target, "to_channel": hook_channel, "to_recipient": hook_recipient})), "on_message_sending attempted to rewrite channel routing; only content mutation is applied");
                        }

                        let modified_len = modified_content.chars().count();
                        if modified_len > CHANNEL_HOOK_MAX_OUTBOUND_CHARS {
                            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"limit": CHANNEL_HOOK_MAX_OUTBOUND_CHARS, "attempted": modified_len})), "hook-modified outbound content exceeded limit; truncating");
                        ...
                        if modified_content != outbound_response {
                            ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"sender": msg.sender, "before_len": outbound_response.chars().count(), "after_len": modified_content.chars().count()})), "outgoing message content modified by hook");
                        }
            ...
            ::dx_agent_log::record!(
                INFO,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Outbound)
            ...
            if ctx.auto_save_memory && msg.content.chars().count() >= AUTOSAVE_MIN_MESSAGE_CHARS {
                ...
                dx_agent_spawn::spawn!(async move {
                    if let Err(e) = dx_agent_memory::consolidation::consolidate_turn(
        ...
        LlmExecutionResult::Completed(Ok(Err(e))) => {
            if dx_agent_runtime::agent::loop_::is_tool_loop_cancelled(&e)
                || cancellation_token.is_cancelled()
            {
                ...
                if dx_agent_providers::reliable::is_auth_error(&e) {
                    let cache_key =
                ...
                let safe_error = dx_agent_providers::sanitize_api_error(&e.to_string());
                ::dx_agent_log::record!(
                    WARN,
                    ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail)
                        .with_outcome(::dx_agent_log::EventOutcome::Failure)
        ...
        LlmExecutionResult::Completed(Err(_)) => {
            ...
            ::dx_agent_log::record!(
                WARN,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Timeout)
                    .with_outcome(::dx_agent_log::EventOutcome::Failure)
                    .with_duration(
                        u64::try_from(started_at.elapsed().as_millis()).unwrap_or(u64::MAX),
                    )
...
async fn run_message_dispatch_loop(
    mut rx: tokio::sync::mpsc::Receiver<dx_agent_api::channel::ChannelMessage>,
    router: AgentRouter,
    ...
    while let Some(msg) = rx.recv().await {
        let Some(ctx) = router.resolve(&msg) else {
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"channel_alias": msg.channel_alias, "sender": msg.sender})), "dropping inbound message: no agent owns this channel");
            continue;
        };
        ...
        let msg = if msg.channel != "cli" && ctx.debouncer.enabled() {
            ...
            match ctx.debouncer.debounce(&debounce_key, &msg.content).await {
                dx_agent_infra::debounce::DebounceResult::Pending(rx) => {
                    ...
                    workers.spawn(async move {
                        ...
                        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"channel": debounce_msg.channel, "sender": debounce_msg.sender})), "Debounced message ready — dispatching combined message");
                ...
                dx_agent_infra::debounce::DebounceResult::Passthrough(content) => {
                    let mut m = msg;
                    m.content = content;
                    m
                }
...
pub async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    use dx_agent_config::multi_agent::{PeerGroupConfig, PeerUsername};
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        let plist = home
            .join("Library")
            .join("LaunchAgents")
            .join("com.dx-agent.daemon.plist");
        if !plist.exists() {
            return Ok(false);
        }
        ...
        if !listed.contains("com.dx-agent.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.dx-agent.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.dx-agent.daemon"])
            .output()
        ...
        "matrix" => {
            #[cfg(feature = "channel-matrix")]
            {
                ...
                let state_dir = config
                    ...
                    .unwrap_or_else(|| std::path::PathBuf::from(".dx-agent/state/matrix"));
                let alias = "default".to_string();
                let peer_resolver: Arc<dyn Fn() -> Vec<String> + Send + Sync> = {
                    let cfg_arc = config_arc.clone();
                    let alias = alias.clone();
...
struct ConfiguredChannel {
    ...
    /// DX Agent channel alias (the `<alias>` half of `[channels.<type>.<alias>]`).
...
pub fn build_channel_map(
    config: &Config,
) -> HashMap<String, Arc<dyn dx_agent_api::channel::Channel>> {
    let config_arc = Arc::new(RwLock::new(config.clone()));
...
fn collect_configured_channels(
    ...
    for (alias, dc) in &config.channels.discord {
        ...
        .with_approval_timeout_secs(dc.approval_timeout_secs);
        if dc.archive {
            match dx_agent_memory::SqliteMemory::new_named("sqlite", &config.data_dir, "discord") {
                Ok(mem) => {
    ...
    for (alias, wechat) in &config.channels.wechat {
        ...
        match WeChatChannel::new(
            ...
            Err(err) => {
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"matrix_skip_context": matrix_skip_context, "err": err.to_string()})), "WeChat channel configuration is invalid; skipping WeChat");
            }
        }
    }
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    println!("🩺 DX Agent Channel Doctor");
    println!();
...
pub async fn start_channels(
    config: Config,
    canvas_store: Option<dx_agent_runtime::tools::CanvasStore>,
    cancel: tokio_util::sync::CancellationToken,
) -> Result<()> {
    ...
    let i18n_locale = config
        ...
        .filter(|s| !s.is_empty())
        .map(ToString::to_string)
        .unwrap_or_else(dx_agent_runtime::i18n::detect_locale);
    dx_agent_runtime::i18n::init(&i18n_locale);
    ...
    let shared_session_store: Option<Arc<dyn dx_agent_infra::session_backend::SessionBackend>> =
        if config.channels.session_persistence {
            match dx_agent_infra::make_session_backend(
                &config.data_dir,
    ...
    let mut rx_holder: Option<tokio::sync::mpsc::Receiver<dx_agent_api::channel::ChannelMessage>> =
        None;
    ...
    for agent_alias in &enabled_agents {
        ...
        let provider_runtime_options =
            dx_agent_providers::provider_runtime_options_for_agent(&config, agent_alias);
        ...
        let model = agent_provider_entry
            .and_then(|e| e.model.clone())
            .or_else(|| {
                ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"agent": agent_alias})), "model_provider has no `model` set; falling back to resolved_default_model");
        ...
        let mem: Arc<dyn Memory> = dx_agent_memory::create_memory_for_agent(
            &config,
            agent_alias,
        ...
        let skills =
            dx_agent_runtime::skills::load_skills_for_agent(&workspace, &config, agent_alias);
        ...
        let peripheral_tools =
            dx_agent_runtime::agent::loop_::load_peripheral_tools(config.peripherals.clone()).await;
        ...
        let mut ch_activated_handle: Option<
            std::sync::Arc<std::sync::Mutex<dx_agent_runtime::tools::ActivatedToolSet>>,
        > = None;
        // Resolution-only MCP wrappers for skill MCP elevation (kind = "mcp").
        let mut ch_mcp_elevation_arcs: Vec<std::sync::Arc<dyn dx_agent_runtime::tools::Tool>> =
            Vec::new();
        if config.mcp.enabled && !config.mcp.servers.is_empty() {
            ...
            match dx_agent_runtime::tools::McpRegistry::connect_all(&config.mcp.servers).await {
                Ok(registry) => {
                    let registry = std::sync::Arc::new(registry);
                    ch_mcp_elevation_arcs =
                        dx_agent_runtime::tools::collect_mcp_elevation_arcs(&registry).await;
                    if config.mcp.deferred_loading {
                        let deferred_set =
                            dx_agent_runtime::tools::DeferredMcpToolSet::from_registry(
                        ...
                        deferred_section =
                            dx_agent_runtime::tools::build_deferred_tools_section(&deferred_set);
                        let activated = std::sync::Arc::new(std::sync::Mutex::new(
                            dx_agent_runtime::tools::ActivatedToolSet::new(),
                        ));
                        ch_activated_handle = Some(std::sync::Arc::clone(&activated));
                        built_tools.push(Box::new(dx_agent_runtime::tools::ToolSearchTool::new(
                            deferred_set,
                            activated,
                        )));
                    } else {
                        ...
                        for name in names {
                            if let Some(def) = registry.get_tool_def(&name).await {
                                let wrapper: std::sync::Arc<dyn Tool> = std::sync::Arc::new(
                                    dx_agent_runtime::tools::McpToolWrapper::new(
                                        name,
                                        def,
                                        std::sync::Arc::clone(&registry),
                                    ),
                                ...
                                built_tools
                                    .push(Box::new(dx_agent_runtime::tools::ArcToolRef(wrapper)));
                                registered += 1;
                            }
                        }
                        ::dx_agent_log::record!(
                ...
                Err(e) => {
                    ::dx_agent_log::record!(ERROR, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_outcome(::dx_agent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"agent_alias": agent_alias, "error": format!("{}", e)})), "MCP registry failed to initialize");
                }
            }
        }
        ...
        let skill_resolution_registry: Vec<std::sync::Arc<dyn dx_agent_runtime::tools::Tool>> =
            all_tools_result_ch
                ...
                .collect();
        dx_agent_runtime::tools::register_skill_tools_with_context(
            &mut built_tools,
            &skills,
            security.clone(),
        ...
        if matches!(
            ...
            dx_agent_config::schema::SkillsPromptInjectionMode::Compact
        ) {
            tool_descs.push((
                "read_skill",
        ...
        if channels_by_name_shared.is_none() {
            ...
            println!("🦀 DX Agent Channel Server");
            ...
            dx_agent_runtime::health::mark_component_ok("channels");
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<dx_agent_api::channel::ChannelMessage>(100);
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            hooks: if config.hooks.enabled {
                let mut runner = dx_agent_runtime::hooks::HookRunner::new();
                if config.hooks.builtin.command_logger {
                    runner.register(Box::new(
                        dx_agent_runtime::hooks::builtin::CommandLoggerHook::new(),
                    ));
                }
                if config.hooks.builtin.webhook_audit.enabled {
                    runner.register(Box::new(
                        dx_agent_runtime::hooks::builtin::WebhookAuditHook::new(
            ...
            cost_tracking: dx_agent_runtime::cost::CostTracker::get_or_init_global(
                config.cost.clone(),
                &config.data_dir,
            )
            .map(|tracker| {
                // The cost tracker's lookup site (`record_tool_loop_cost_usage`
                // in dx-agent-runtime) receives the bare provider type — the
            ...
            debouncer: Arc::new(dx_agent_infra::debounce::MessageDebouncer::new(
                Duration::from_millis(config.channels.debounce_ms),
            )),
            receipt_generator: if agent.resolved.tool_receipts.enabled {
                Some(dx_agent_runtime::agent::tool_receipts::ReceiptGenerator::new())
            } else {
                None
            },
    ...
    if let Some(ref store) = shared_session_store {
        ...
        for m in metadata {
            ...
            let pruned =
                dx_agent_runtime::agent::history_pruner::remove_orphaned_tool_messages(&mut msgs);
            if !pruned.is_empty() {
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"category": "agent", "agent_alias": owner_agent.as_deref().unwrap_or(""), "channel": m.channel_id.as_deref().unwrap_or(""), "session_key": m.key, "removed": pruned.removed, "orphan_tool_call_ids": pruned.orphan_tool_call_ids})), "removed orphaned tool messages from restored history (tool_use/tool_result pairing inconsistency auto-healed)");
            }
...
pub async fn deliver_announcement(
    config: &dx_agent_config::schema::Config,
    channel: &str,
    target: &str,
    thread_id: Option<String>,
    output: &str,
) -> anyhow::Result<()> {
    use dx_agent_api::channel::SendMessage;
    let _ = config;

    // Scan for credential leaks before delivering
    let leak_detector = dx_agent_runtime::security::LeakDetector::new();
    let safe_output = match leak_detector.scan(output) {
        dx_agent_runtime::security::LeakResult::Detected { redacted, .. } => redacted,
        dx_agent_runtime::security::LeakResult::Clean => output.to_string(),
    };
    ...
    match channel_type.as_str() {
        ...
        "telegram" => {
            ...
            dx_agent_api::channel::Channel::send(&ch, &make_msg(&safe_output)).await?;
        }
        #[cfg(not(feature = "channel-telegram"))]
        "telegram" => {
            anyhow::bail!("Telegram channel requires the `channel-telegram` feature");
...
mod tests {
    ...
    use dx_agent_memory::{Memory, MemoryCategory, SqliteMemory};
    use dx_agent_providers::{ChatMessage, ModelProvider};
    use dx_agent_runtime::agent::loop_::build_tool_instructions;
    use dx_agent_runtime::observability::NoopObserver;
    use dx_agent_runtime::tools::{Tool, ToolResult};

    fn make_workspace() -> TempDir {
        let tmp = TempDir::new().unwrap();
        // Create minimal workspace files
        std::fs::write(tmp.path().join("SOUL.md"), "# Soul\nBe helpful.").unwrap();
        std::fs::write(tmp.path().join("IDENTITY.md"), "# Identity\nName: DX Agent").unwrap();
    ...
    impl ::dx_agent_api::attribution::Attributable for NamedMockChannel {
    ...
    impl Channel for NamedMockChannel {
        ...
        async fn send(&self, _message: &dx_agent_api::channel::SendMessage) -> anyhow::Result<()> {
            Ok(())
        }
        async fn listen(
            &self,
            _tx: tokio::sync::mpsc::Sender<dx_agent_api::channel::ChannelMessage>,
        ) -> anyhow::Result<()> {
            Ok(())
        }
    }
    ...
    impl ::dx_agent_api::attribution::Attributable for MentionMockChannel {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Channel(
                ::dx_agent_api::attribution::ChannelKind::Discord,
            )
        }
        fn alias(&self) -> &str {
            "test"
    ...
    fn channel_message(
        channel: &str,
        alias: Option<&str>,
    ) -> dx_agent_api::channel::ChannelMessage {
        dx_agent_api::channel::ChannelMessage {
            id: "m1".into(),
            sender: "u1".into(),
    ...
    fn router_test_ctx() -> Arc<ChannelRuntimeContext> {
        Arc::new(ChannelRuntimeContext {
            ...
            agent_cfg: Arc::new(dx_agent_config::schema::AliasedAgentConfig::default()),
            ...
            api_key: None,
            api_url: None,
            reliability: Arc::new(dx_agent_config::schema::ReliabilityConfig::default()),
            interrupt_on_new_message: InterruptOnNewMessageConfig {
                telegram: false,
                slack: false,
                discord: false,
            ...
            multimodal: dx_agent_config::schema::MultimodalConfig::default(),
            media_pipeline: dx_agent_config::schema::MediaPipelineConfig::default(),
            transcription_config: dx_agent_config::schema::TranscriptionConfig::default(),
            agent_transcription_provider: String::new(),
            hooks: None,
            provider_runtime_options: dx_agent_providers::ModelProviderRuntimeOptions::default(),
            workspace_dir: Arc::new(std::env::temp_dir()),
            prompt_config: Arc::new(dx_agent_config::schema::Config::default()),
            message_timeout_secs: CHANNEL_MESSAGE_TIMEOUT_SECS,
            non_cli_excluded_tools: Arc::new(Vec::new()),
            autonomy_level: AutonomyLevel::default(),
            tool_call_dedup_exempt: Arc::new(Vec::new()),
            model_routes: Arc::new(Vec::new()),
            query_classification: dx_agent_config::schema::QueryClassificationConfig::default(),
            ack_reactions: true,
            show_tool_calls: true,
            session_store: None,
            approval_manager: Arc::new(ApprovalManager::for_non_interactive(
                &dx_agent_config::schema::RiskProfileConfig::default(),
            )),
            activated_tools: None,
            cost_tracking: None,
            pacing: dx_agent_config::schema::PacingConfig::default(),
    ...
    async fn resolve_classifier_route_returns_none_for_empty_ref() {
        ...
        let empty = dx_agent_config::providers::ModelProviderRef::default();
    ...
    async fn resolve_classifier_route_returns_none_for_unresolvable_ref() {
        let ctx = router_test_ctx();
        let bogus = dx_agent_config::providers::ModelProviderRef::from("custom.does-not-exist");
    ...
    async fn resolve_classifier_route_returns_alias_temperature() {
        ...
        let mut cfg = dx_agent_config::schema::Config::default();
        cfg.providers.models.openai.insert(
            "my-classifier".to_string(),
            dx_agent_config::schema::OpenAIModelProviderConfig {
                base: dx_agent_config::schema::ModelProviderConfig {
        ...
        let alias_ref = dx_agent_config::providers::ModelProviderRef::from("openai.my-classifier");
    ...
    fn pacing_config_defaults_preserve_existing_behavior() {
        let pacing = dx_agent_config::schema::PacingConfig::default();
...
<tool_result name="http_request">{"status":200}</tool_result>"#;
    ...
    fn rollback_orphan_user_turn_also_removes_from_session_store() {
        ...
        let store: Arc<dyn dx_agent_infra::session_backend::SessionBackend> =
            Arc::new(dx_agent_infra::session_store::SessionStore::new(tmp.path()).unwrap());
    ...
    impl ::dx_agent_api::attribution::Attributable for DummyModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
    ...
    impl ::dx_agent_api::attribution::Attributable for FormatErrorModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for TelegramRecordingChannel {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Channel(
                ::dx_agent_api::attribution::ChannelKind::Webhook,
            )
    ...
    impl ::dx_agent_api::attribution::Attributable for SlackRecordingChannel {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Channel(
                ::dx_agent_api::attribution::ChannelKind::Webhook,
            )
    ...
    impl ::dx_agent_api::attribution::Attributable for RecordingChannel {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Channel(
                ::dx_agent_api::attribution::ChannelKind::Webhook,
            )
    ...
    fn test_runtime_ctx_with_config_agent_and_default_provider(
        channel: Arc<dyn Channel>,
        model_provider: Arc<dyn ModelProvider>,
        prompt_config: dx_agent_config::schema::Config,
        agent_cfg: dx_agent_config::schema::AliasedAgentConfig,
    ...
    impl ::dx_agent_api::attribution::Attributable for SlowModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
...
{"name":"mock_price","arguments":{"symbol":"BTC"}}
    ...
    impl ::dx_agent_api::attribution::Attributable for ToolCallingModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
...
{"name":"sessions_current","arguments":{}}
    ...
    impl ::dx_agent_api::attribution::Attributable for SessionsCurrentModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for ToolCallingAliasModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    impl ::dx_agent_api::attribution::Attributable for RawToolArtifactModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for IterativeToolModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for HistoryCaptureModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for DelayedHistoryCaptureModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for MockPriceTool {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Tool(::dx_agent_api::attribution::ToolKind::Plugin)
        }
        fn alias(&self) -> &str {
            <Self as ::dx_agent_api::tool::Tool>::name(self)
        }
    }
    ...
    impl ::dx_agent_api::attribution::Attributable for ModelCaptureModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for PrecheckProbeModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for NamedMockTool {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Tool(::dx_agent_api::attribution::ToolKind::Plugin)
        }
        fn alias(&self) -> &str {
    ...
    async fn process_channel_message_scopes_sender_session_key_for_sessions_current_tool() {
        ...
        let tmp = TempDir::new().unwrap();
        let session_store: Arc<dyn dx_agent_infra::session_backend::SessionBackend> =
            Arc::new(dx_agent_infra::session_store::SessionStore::new(tmp.path()).unwrap());
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            tools_registry: Arc::new(vec![Box::new(
                dx_agent_runtime::tools::SessionsCurrentTool::new(Arc::clone(&session_store)),
            )]),
            observer: Arc::new(NoopObserver),
            system_prompt: Arc::new("test-system-prompt".to_string()),
            model: Arc::new("test-model".to_string()),
            ...
            approval_manager: Arc::new(ApprovalManager::for_non_interactive(&{
                let mut profile = dx_agent_config::schema::RiskProfileConfig::default();
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            approval_manager: Arc::new(ApprovalManager::for_non_interactive(
                &dx_agent_config::schema::RiskProfileConfig {
                    level: dx_agent_config::autonomy::AutonomyLevel::Full,
            ...
            receipt_generator: Some(
                dx_agent_runtime::agent::tool_receipts::ReceiptGenerator::new(),
            ),
    ...
    async fn process_channel_message_uses_classifier_provider_for_precheck_model_selection() {
        ...
        let mut prompt_config = dx_agent_config::schema::Config::default();
        ...
        let agent_cfg = dx_agent_config::schema::AliasedAgentConfig {
            classifier_provider: dx_agent_config::providers::ModelProviderRef::from(
    ...
    async fn process_channel_message_respects_configured_max_tool_iterations_above_default() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            pacing: dx_agent_config::schema::PacingConfig {
                ...
                ..dx_agent_config::schema::PacingConfig::default()
            },
            max_tool_result_chars: 0,
            context_token_budget: 0,
            debouncer: Arc::new(dx_agent_infra::debounce::MessageDebouncer::new(
    ...
    impl Memory for NoopMemory {
        ...
        async fn store(
            ...
            _key: &str,
            _content: &str,
            _category: dx_agent_memory::MemoryCategory,
        ...
        async fn recall(
            ...
            _since: Option<&str>,
            _until: Option<&str>,
        ) -> anyhow::Result<Vec<dx_agent_memory::MemoryEntry>> {
            Ok(Vec::new())
        }

        async fn get(&self, _key: &str) -> anyhow::Result<Option<dx_agent_memory::MemoryEntry>> {
            Ok(None)
        }

        async fn list(
            &self,
            _category: Option<&dx_agent_memory::MemoryCategory>,
            _session_id: Option<&str>,
        ) -> anyhow::Result<Vec<dx_agent_memory::MemoryEntry>> {
            Ok(Vec::new())
        }
    ...
    impl ::dx_agent_api::attribution::Attributable for NoopMemory {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Memory(
                ::dx_agent_api::attribution::MemoryKind::InMemory,
            )
        }
        fn alias(&self) -> &str {
            "NoopMemory"
    ...
    impl Memory for RecallMemory {
        ...
        async fn recall(
            ...
            Ok(vec![dx_agent_memory::MemoryEntry {
                id: "entry-1".to_string(),
                key: "memory_key_1".to_string(),
                content: "Age is 45".to_string(),
                category: dx_agent_memory::MemoryCategory::Conversation,
    ...
    impl ::dx_agent_api::attribution::Attributable for RecallMemory {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Memory(
                ::dx_agent_api::attribution::MemoryKind::InMemory,
            )
    ...
    impl ::dx_agent_api::attribution::Attributable for ConcurrencyTrackingProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    async fn message_dispatch_processes_messages_in_parallel() {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<dx_agent_api::channel::ChannelMessage>(4);
        tx.send(dx_agent_api::channel::ChannelMessage {
            id: "1".to_string(),
            sender: "alice".to_string(),
    ...
    async fn message_dispatch_interrupts_in_flight_telegram_request_and_preserves_context() {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<dx_agent_api::channel::ChannelMessage>(8);
        let send_task = dx_agent_spawn::spawn!(async move {
            tx.send(dx_agent_api::channel::ChannelMessage {
                id: "msg-1".to_string(),
                sender: "alice".to_string(),
                reply_target: "chat-1".to_string(),
    ...
    fn channel_strict_non_native_prompt_hides_text_tool_protocol() {
        ...
        let mut prompt = build_system_prompt_with_mode_and_autonomy(
            ...
            None,
            false,
            dx_agent_config::schema::SkillsPromptInjectionMode::Full,
            false,
            0,
            false,
        );
    ...
    fn prompt_skills_include_instructions_and_tools() {
        let ws = make_workspace();
        let skills = vec![dx_agent_runtime::skills::Skill {
            name: "code-review".into(),
            description: "Review code for bugs".into(),
            version: "1.0.0".into(),
            author: None,
            tags: vec![],
            tools: vec![dx_agent_runtime::skills::SkillTool {
                name: "lint".into(),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from DX Agent 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";

        // Reproduces the production crash path where channel logs truncate at 80 chars.
        let result =
            std::panic::catch_unwind(|| dx_agent_runtime::util::truncate_with_ellipsis(msg, 80));
        assert!(
            result.is_ok(),
    ...
    fn full_autonomy_prompt_executes_allowed_tools_without_extra_approval() {
        let ws = make_workspace();
        let config = dx_agent_config::schema::RiskProfileConfig {
            level: dx_agent_runtime::security::AutonomyLevel::Full,
            ..dx_agent_config::schema::RiskProfileConfig::default()
        };
    ...
    fn readonly_prompt_explains_policy_blocks_without_fake_approval() {
        let ws = make_workspace();
        let config = dx_agent_config::schema::RiskProfileConfig {
            level: dx_agent_runtime::security::AutonomyLevel::ReadOnly,
            ..dx_agent_config::schema::RiskProfileConfig::default()
        };
        let prompt = build_system_prompt_with_mode_and_autonomy(
            ws.path(),
    ...
    fn channel_notify_observer_truncates_utf8_arguments_safely() {
        ...
        observer.record_event(
            &dx_agent_runtime::observability::traits::ObserverEvent::ToolCallStart {
    ...
    fn conversation_memory_key_uses_message_id() {
        let msg = dx_agent_api::channel::ChannelMessage {
            id: "msg_abc123".into(),
            sender: "U123".into(),
    ...
    fn matrix_root_conversation_history_key_omits_event_id() {
        let first = dx_agent_api::channel::ChannelMessage {
        ...
        let second = dx_agent_api::channel::ChannelMessage {
    ...
    fn conversation_memory_key_is_unique_per_message() {
        let msg1 = dx_agent_api::channel::ChannelMessage {
            id: "msg_1".into(),
            sender: "U123".into(),
            reply_target: "C456".into(),
            content: "first".into(),
        ...
        let msg2 = dx_agent_api::channel::ChannelMessage {
            id: "msg_2".into(),
            sender: "U123".into(),
            reply_target: "C456".into(),
            content: "second".into(),
    ...
    async fn autosaved_group_conversation_memory_stays_session_scoped() {
        let tmp = TempDir::new().unwrap();
        let mem = SqliteMemory::new("test", tmp.path()).unwrap();
        let group_a_msg = dx_agent_api::channel::ChannelMessage {
            id: "msg_1".into(),
            sender: "U123".into(),
        ...
        let group_b_msg = dx_agent_api::channel::ChannelMessage {
            id: "msg_2".into(),
            sender: "U123".into(),
    ...
    async fn process_channel_message_refreshes_available_skills_after_new_session() {
        ...
        let initial_skills =
            dx_agent_runtime::skills::load_skills_with_config(workspace.path(), &config);
        assert!(initial_skills.is_empty());

        let default_identity = dx_agent_config::schema::IdentityConfig::default();
...
This is an example JSON object for profile settings."#;
    ...
    fn aieos_identity_from_file() {
        use tempfile::TempDir;
        use dx_agent_config::schema::IdentityConfig;
    ...
    fn collect_configured_channels_includes_mattermost_when_configured() {
        ...
        config.channels.mattermost.insert(
            "default".to_string(),
            dx_agent_config::schema::MattermostConfig {
                enabled: true,
    ...
    fn collect_configured_channels_skips_unreferenced_email() {
        ...
        config.channels.email.insert(
            "default".to_string(),
            dx_agent_config::scattered_types::EmailConfig::default(),
        );
    ...
    fn collect_configured_channels_skips_unreferenced_voice_call() {
        ...
        config.channels.voice_call.insert(
            "default".to_string(),
            dx_agent_config::scattered_types::VoiceCallConfig::default(),
        );
    ...
    impl ::dx_agent_api::attribution::Attributable for AlwaysFailChannel {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Channel(
                ::dx_agent_api::attribution::ChannelKind::Webhook,
            )
    ...
    impl ::dx_agent_api::attribution::Attributable for BlockUntilClosedChannel {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Channel(
                ::dx_agent_api::attribution::ChannelKind::Webhook,
            )
    ...
    impl ::dx_agent_api::attribution::Attributable for FailOnceChannel {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Channel(
                ::dx_agent_api::attribution::ChannelKind::Discord,
            )
    ...
    async fn supervised_listener_marks_error_and_restarts_on_failures() {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<dx_agent_api::channel::ChannelMessage>(1);
        ...
        let snapshot = dx_agent_runtime::health::snapshot_json();
    ...
    async fn supervised_listener_refreshes_health_while_running() {
        ...
        let first_last_ok = dx_agent_runtime::health::snapshot_json()["components"]
        ...
        let second_last_ok = dx_agent_runtime::health::snapshot_json()["components"]
            [&component_name]["last_ok"]
            .as_str()
            .unwrap_or("")
            .to_string();
    ...
    async fn non_retryable_listener_error_does_not_stop_other_listener_health() {
        ...
        let (failing_tx, failing_rx) =
            tokio::sync::mpsc::channel::<dx_agent_api::channel::ChannelMessage>(1);
    ...
    async fn process_channel_message_applies_query_classification_route() {
        ...
        let classification_config = dx_agent_config::schema::QueryClassificationConfig {
            enabled: true,
            rules: vec![dx_agent_config::schema::ClassificationRule {
                hint: "vision".into(),
        ...
        let model_routes = vec![dx_agent_config::schema::ModelRouteConfig {
            hint: "vision".into(),
    ...
    async fn process_channel_message_classification_priority_selects_highest() {
        ...
        let classification_config = dx_agent_config::schema::QueryClassificationConfig {
            enabled: true,
            rules: vec![
                dx_agent_config::schema::ClassificationRule {
                    hint: "fast".into(),
        ...
        let model_routes = vec![
            dx_agent_config::schema::ModelRouteConfig {
                hint: "fast".into(),
    ...
    fn build_channel_by_id_configured_telegram_succeeds() {
        ...
        config.channels.telegram.insert(
            "default".to_string(),
            dx_agent_config::schema::TelegramConfig {
                enabled: true,
                bot_token: "test-token".to_string(),
                stream_mode: dx_agent_config::schema::StreamMode::Off,
    ...
    fn build_channel_by_id_configured_voice_call_succeeds() {
        ...
        config.channels.voice_call.insert(
            "default".to_string(),
            dx_agent_config::scattered_types::VoiceCallConfig {
                enabled: true,
                model_provider: dx_agent_config::scattered_types::VoiceProvider::Twilio,
...
Done."#;
    ...
    fn default_keep_tool_context_turns_is_two() {
        let config = dx_agent_config::schema::AliasedAgentConfig::default();
    ...
    async fn deliver_announcement_routes_lark_to_lark_arm() {
        ...
        let config = dx_agent_config::schema::Config::default();
    ...
    async fn deliver_announcement_rejects_feishu_value_when_use_feishu_false() {
        ...
        let mut config = dx_agent_config::schema::Config::default();
        config.channels.lark.insert(
            "work".to_string(),
            dx_agent_config::schema::LarkConfig {
                enabled: true,
                use_feishu: false,
