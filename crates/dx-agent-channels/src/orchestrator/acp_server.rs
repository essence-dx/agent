...
use uuid::Uuid;
pub use dx_agent_api::jsonrpc::RpcOutbound;
...
use dx_agent_api::model_provider::ConversationMessage;
use dx_agent_config::schema::Config;
use dx_agent_infra::acp_session_store::AcpSessionStore;
use dx_agent_runtime::agent::agent::{Agent, TurnEvent};
...
impl AcpServer {
    ...
    pub async fn run(self: Arc<Self>) -> Result<()> {
        ...
        dx_agent_spawn::spawn!(writer_task(writer_rx));
    ...
    fn handle_initialize(&self, _params: &Value) -> RpcResult {
        ...
        let mut dx_agents_meta = serde_json::json!({
            "maxSessions": self.acp_config.max_sessions,
            "sessionTimeoutSecs": self.acp_config.session_timeout_secs,
        });
        if let Some(model) = default_model {
            dx_agents_meta["defaultModel"] = serde_json::json!(model);
        }
        let legacy_zeroclaw_meta = dx_agents_meta.clone();
        ...
        Ok(serde_json::json!({
            ...
            "authMethods": [],
            "_meta": {
                "dxAgents": dx_agents_meta,
                "zeroclaw": legacy_zeroclaw_meta,
            }
        }))
    }
    ...
    async fn handle_session_prompt(&self, params: &Value, _request_id: &Value) -> RpcResult {
        ...
        ::dx_agent_log::record!(
            INFO,
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Start).with_category(::dx_agent_log::EventCategory::Channel)
                .with_attrs(::serde_json::json!({
        ...
        let turn_handle = dx_agent_spawn::spawn!(async move {
        ...
        while let Some(event) = event_rx.recv().await {
            ...
            match &event {
                ...
                TurnEvent::ToolResult { id, name, output } => {
                    ::dx_agent_log::record!(
                        DEBUG,
                        ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Complete).with_category(::dx_agent_log::EventCategory::Channel)
                            .with_outcome(::dx_agent_log::EventOutcome::Success)
                            .with_attrs(::serde_json::json!({
                                "tool_call_id": id,
                                "tool": name,
        ...
        let was_cancelled = match &turn_result {
            Err(e) => dx_agent_runtime::agent::loop_::is_tool_loop_cancelled(e),
            Ok(_) => false,
        };
        ...
        let (result_text, new_turn_msgs) = turn_result.map_err(|e| {
            ::dx_agent_log::record!(
                ERROR,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_category(::dx_agent_log::EventCategory::Channel)
                    .with_outcome(::dx_agent_log::EventOutcome::Failure)
                    .with_attrs(::serde_json::json!({
                        "error": e.to_string(),
                    })),
            ...
            && !new_turn_msgs.is_empty()
        {
            ...
            if let Some(detail) = error {
                ::dx_agent_log::record!(
                    WARN,
                    ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_category(::dx_agent_log::EventCategory::Channel)
                        .with_outcome(::dx_agent_log::EventOutcome::Unknown)
                        .with_attrs(::serde_json::json!({
                            "error": detail,
                        })),
...
/// DX Agent's internal tool args use `old_string`/`new_string` (snake_case) for
...
fn map_tool_kind(name: &str) -> &'static str {
    match name {
        ...
        // DX Agent tools return user-visible text, so use `other` to keep the
...
mod tests {
    ...
    fn initialize_response_uses_acp_v1_shape() {
        ...
        assert_eq!(result["agentInfo"]["name"], "dx-agent-acp");
        assert_eq!(result["agentInfo"]["title"], "DX Agents ACP");
        ...
        assert!(result["_meta"].get("dxAgents").is_some());
        assert!(result["_meta"].get("zeroclaw").is_some());
    ...
    fn initialize_advertises_load_session_when_store_present() {
        ...
        let store =
            Arc::new(dx_agent_infra::acp_session_store::AcpSessionStore::new(cwd.path()).unwrap());
    ...
    async fn session_new_does_not_wait_for_configured_mcp_servers() {
        ...
        let mut config = Config {
            ...
            providers: {
                let mut p = dx_agent_config::providers::Providers::default();
                p.models.openrouter.insert(
                    "default".to_string(),
                    dx_agent_config::schema::OpenRouterModelProviderConfig {
            ...
            mcp: dx_agent_config::schema::McpConfig {
                enabled: true,
                servers: vec![dx_agent_config::schema::McpServerConfig {
                    name: "slow".to_string(),
                    transport: dx_agent_config::schema::McpTransport::Stdio,
    ...
    fn handle_initialize_default_model_absent_when_unconfigured() {
        ...
        let result = server.handle_initialize(&serde_json::json!({})).unwrap();
        assert!(
            result["_meta"]["dxAgents"].get("defaultModel").is_none(),
            "defaultModel must be absent when no model_provider is configured, got: {}",
            result["_meta"]["dxAgents"]["defaultModel"]
        );
        assert!(result["_meta"]["zeroclaw"].get("defaultModel").is_none());
    }

    #[test]
    fn handle_initialize_default_model_reflects_configured_provider() {
        use dx_agent_config::schema::{ModelProviderConfig, OllamaModelProviderConfig};
        let mut config = Config::default();
        ...
        let server = AcpServer::new(config, AcpServerConfig::default());
        let result = server.handle_initialize(&serde_json::json!({})).unwrap();
        assert_eq!(result["_meta"]["dxAgents"]["defaultModel"], "llama3.2");
    ...
    async fn session_load_restores_history_and_streams_notifications() {
        use dx_agent_api::model_provider::{ChatMessage, ConversationMessage};
        let cwd = tempfile::tempdir().unwrap();
        let store =
            Arc::new(dx_agent_infra::acp_session_store::AcpSessionStore::new(cwd.path()).unwrap());
