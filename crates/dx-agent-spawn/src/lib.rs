//! `dx-agent-spawn` — the sanctioned interface against `tokio::spawn` for
//! the DX Agent workspace.
//!
...
//!    through the standard `dx_agent_log::record!` pipeline:
...
//! `dx-agent-spawn` adds telemetry, not control flow.
//!
//! ## Layering
//!
//! `dx-agent-spawn` depends on `dx-agent-log`. Crates that already depend
//! on `dx-agent-log` (i.e. almost everything in the workspace) can add
//! `dx-agent-spawn` as a peer dependency without inverting the graph.
//! The lowest-level crate (`dx-agent-api`) intentionally does NOT depend
//! on `dx-agent-spawn` — its small handful of internal spawn needs go
...
/// callers needing `tokio`, `tracing`, or `dx_agent_log` as direct
/// dependencies.
#[doc(hidden)]
pub mod __private {
    pub use ::serde_json;
...
macro_rules! spawn {
    ($body:expr) => {{
        ...
        // entered.
        $crate::__private::dx_agent_log::record!(
            INFO,
            $crate::__private::dx_agent_log::Event::new(
                $crate::TASK_EVENT_NAME,
                $crate::__private::dx_agent_log::Action::Spawn,
            )
        ...
        let __zc_task_future = async move {
            ...
            $crate::__private::dx_agent_log::record!(
                ...
                $crate::__private::dx_agent_log::Event::new(
                    $crate::TASK_EVENT_NAME,
                    $crate::__private::dx_agent_log::Action::Complete,
                )
                .with_outcome($crate::__private::dx_agent_log::EventOutcome::Success)
...
mod tests {
    ...
    /// without depending on `dx-agent-log` formatting.
