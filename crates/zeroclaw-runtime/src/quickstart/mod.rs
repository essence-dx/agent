...
pub struct QuickstartTypeOption {
    ...
    pub identity: Option<zeroclaw_providers::provider_metadata::ProviderIdentity>,
    /// Free-tier metadata when the provider has a documented freemium
    /// surface. Channel rows omit it.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub freemium: Option<zeroclaw_providers::provider_metadata::FreemiumMetadata>,
}
...
pub fn snapshot_state(cfg: &Config) -> QuickstartState {
    let model_provider_types =
        zeroclaw_providers::provider_catalog::list_model_provider_catalog_entries()
            .into_iter()
            .map(|entry| {
...
fn resolve_model_provider_type(type_key: &str) -> Option<ResolvedModelProviderType> {
    let entry =
        zeroclaw_providers::provider_catalog::find_model_provider_catalog_entry(type_key.trim())?;
    let catalog_only = !entry.is_native();
    let default_uri = if catalog_only {
        zeroclaw_providers::provider_catalog::find_dx_provider_catalog_summary(&entry.name)
            .ok()
...
fn set_model_provider_field(
    ...
    if let Err(err) = config.set_prop_persistent(&format!("{prefix}.{field}"), value) {
        errors.push(QuickstartError::new(
            ...
            zeroclaw_config::helpers::kebab_to_snake(field),
            err.to_string(),
        ));
        return false;
    }
...
pub async fn model_catalog(model_provider: &str) -> (Vec<String>, bool) {
    ...
    match zeroclaw_providers::catalog::list_models_for_family(model_provider).await {
        ...
        _ => match zeroclaw_providers::provider_catalog::list_models_for_catalog_provider(
            model_provider,
        ) {
            Ok(models) if !models.is_empty() => (models, true),
            _ => (Vec::new(), false),
...
pub fn model_provider_is_local(model_provider: &str) -> bool {
    zeroclaw_providers::provider_catalog::find_model_provider_catalog_entry(model_provider)
...
mod tests {
    ...
    async fn model_catalog_reads_dx_archive_for_catalog_only_provider_when_available() {
        if zeroclaw_providers::provider_catalog::discover_dx_providers_catalog().is_none() {
            return;
        }
    ...
    fn catalog_only_quickstart_provider() -> Option<(String, String, String)> {
        ...
        zeroclaw_providers::provider_catalog::list_model_provider_catalog_entries()
            ...
            .filter(|entry| {
                entry.source
                    == zeroclaw_providers::provider_catalog::ModelProviderCatalogSource::DxProvidersCatalog
            })
            .find_map(|entry| {
                let provider =
                    zeroclaw_providers::provider_catalog::find_dx_provider_catalog_summary(
                        &entry.name,
                    )
                    .ok()?;
