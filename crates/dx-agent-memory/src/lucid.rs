    ...
    async fn recall(
        ...
        match self.recall_from_lucid(recall_query).await {
            ...
            Err(error) => {
                ...
                ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"command": self.lucid_cmd, "error": format!("{}", error)})), "Lucid context unavailable; using local sqlite results");
                Ok(local_results)
            }
        }
    }
...
impl ::dx_agent_api::attribution::Attributable for LucidMemory {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Memory(::dx_agent_api::attribution::MemoryKind::Lucid)
    }
    fn alias(&self) -> &str {
        &self.alias
    }
