    ...
    F: FnMut() -> anyhow::Result<SqliteMemory>,
{
    match classify_memory_backend(backend_name) {
        ...
        MemoryBackendKind::Unknown => {
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"backend_name": backend_name, "unknown_context": unknown_context})), "Unknown memory backend '', falling back to markdown");
            Ok(Box::new(MarkdownMemory::new("markdown", workspace_dir)))
        }
    }
}
...
pub async fn create_memory_for_agent(
    ...
    use dx_agent_config::multi_agent::MemoryBackendKind as ConfigBackend;
...
mod tests {
    ...
    use dx_agent_config::schema::EmbeddingRouteConfig;
    ...
    fn factory_postgres_without_feature_gives_clear_error() {
        use dx_agent_config::schema::PostgresStorageConfig;
        let tmp = TempDir::new().unwrap();
        let cfg = MemoryConfig {
