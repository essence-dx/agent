...
const DEFAULT_CODEX_INSTRUCTIONS: &str =
    "You are DX Agent, a concise and helpful coding assistant.";
...
impl OpenAiCodexModelProvider {
    pub fn new(
        ...
        let state_dir = options
            .dx_agent_dir
            .clone()
            .unwrap_or_else(default_dx_agent_dir);
...
fn default_dx_agent_dir() -> PathBuf {
    directories::UserDirs::new().map_or_else(
        || PathBuf::from(".dx-agent"),
        |dirs| dirs.home_dir().join(".dx-agent"),
    )
}
...
impl ::dx_agent_api::attribution::Attributable for OpenAiCodexModelProvider {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Provider(
            ::dx_agent_api::attribution::ProviderKind::Model(
                ::dx_agent_api::attribution::ModelProviderKind::OpenAiCodex,
            ),
        )
    }
    fn alias(&self) -> &str {
...
mod tests {
    ...
    async fn mock_codex_provider(
        ...
        let options = ModelProviderRuntimeOptions {
            ...
            dx_agent_dir: Some(temp_dir.path().to_path_buf()),
