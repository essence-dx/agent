...
impl OpenAiCompatibleModelProvider {
    ...
    fn http_client(&self) -> Client {
        ...
        if has_user_agent || has_extra_headers {
            ...
            let builder = dx_agent_config::schema::apply_runtime_proxy_to_builder(
                builder,
    ...
    fn streaming_http_client(&self) -> Client {
        ...
        let builder =
            dx_agent_config::schema::apply_runtime_proxy_to_builder(builder, "provider.compatible");
...
struct ToolCall {
    ...
    /// See [`dx_agent_api::ToolCall::extra_content`].
    #[serde(default, skip_serializing_if = "Option::is_none")]
                ...
                Ok(bytes) => {
                    ...
                    while let Some(pos) = buffer.find('\n') {
                        ...
                        if let Some(usage) = chunk.usage.as_ref() {
                            let token_usage = dx_agent_api::model_provider::TokenUsage {
...
async fn stream_status_error(response: reqwest::Response) -> StreamError {
    let status = response.status();
    let error = match response.text().await {
        Ok(text) => super::sanitize_api_error(&text),
        Err(error) => super::format_error_chain(&error),
    };
    StreamError::ModelProvider(format!("{status}: {error}"))
}
...
impl OpenAiCompatibleModelProvider {
    ...
    fn convert_tool_specs(
        tools: Option<&[dx_agent_api::tool::ToolSpec]>,
    ) -> Option<Vec<serde_json::Value>> {
        tools.map(|items| {
            items
                .iter()
                .map(|tool| {
                    let params = dx_agent_api::schema::SchemaCleanr::clean_for_openai(
        ...
        Some(
            converted
                ...
                .map(|mut tool| {
                    ...
                    let cleaned = dx_agent_api::schema::SchemaCleanr::clean(
                        raw_parameters,
                        dx_agent_api::schema::CleaningStrategy::Conservative,
                    );
    ...
    async fn normalize_messages_for_upstream(
        ...
        let config = dx_agent_config::schema::MultimodalConfig::default();
    ...
    fn with_prompt_guided_tool_instructions(
        ...
        let instructions = dx_agent_api::model_provider::build_tool_instructions_text(tools);
...
impl ModelProvider for OpenAiCompatibleModelProvider {
    fn capabilities(&self) -> dx_agent_api::model_provider::ProviderCapabilities {
        dx_agent_api::model_provider::ProviderCapabilities {
    ...
    async fn chat(
        ...
        if !response.status().is_success() {
            let status = response.status();
            let retry_after = super::retry_after_hint(&response);
            let error = response.text().await?;
            let sanitized = super::sanitize_api_error(&error);
            ...
            anyhow::bail!(
                "{} API error ({status}): {}",
                self.name,
                super::append_retry_after_hint(sanitized, retry_after.as_deref())
            );
        }
    ...
    fn stream_chat(
        ...
        let tools_owned: Option<Vec<dx_agent_api::tool::ToolSpec>> =
            request.tools.map(<[dx_agent_api::tool::ToolSpec]>::to_vec);
        ...
        let handle = ::dx_agent_spawn::spawn!(async move {
            ...
            if !response.status().is_success() {
                let _ = tx.send(Err(stream_status_error(response).await)).await;
                return;
            }
...
impl ::dx_agent_api::attribution::Attributable for OpenAiCompatibleModelProvider {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Provider(
            ::dx_agent_api::attribution::ProviderKind::Model(
                ::dx_agent_api::attribution::ModelProviderKind::Plugin,
            ),
        )
    }
    fn alias(&self) -> &str {
...
mod tests {
    ...
    fn request_serializes_correctly() {
        let req = ApiChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: MessageContent::Text("You are DX Agent".to_string()),
                },
                Message {
                    role: "user".to_string(),
    ...
    fn prompt_guided_tool_fallback_injects_system_instruction() {
        ...
        let tools = vec![dx_agent_api::tool::ToolSpec {
    ...
    #[tokio::test]
    async fn stream_chat_failed_status_redacts_provider_error_body() {
        use axum::Router;
        ...
        let leaked_groq_key = "gsk_test_stream_status_secret";
        let leaked_google_key = "AIzaStreamStatusSecret";
        let body = format!(
            r#"{{"error":"bad key {leaked_groq_key}","url":"https://generativelanguage.googleapis.com/v1beta/models?key={leaked_google_key}"}}"#
        );
        ...
        let provider = OpenAiCompatibleModelProvider::new(
            ...
            "test",
            &format!("http://{addr}"),
            Some("configured-provider-secret"),
            AuthStyle::Bearer,
        );
        ...
        let first = stream
            .next()
            .await
            .expect("status failure must surface as the first stream item")
            .expect_err("401 response must produce a stream error");
        server.abort();
        ...
        assert!(
            !message.contains(leaked_groq_key),
            "stream status error leaked Groq-shaped credential: {message}"
        );
        assert!(
            !message.contains(leaked_google_key),
            "stream status error leaked Google-shaped credential: {message}"
        );
    }
