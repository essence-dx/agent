...
fn sanitize_stream_error(error: StreamError) -> StreamError {
    match error {
        StreamError::Http(message) => StreamError::Http(super::sanitize_api_error(&message)),
        StreamError::InvalidSse(message) => {
            StreamError::InvalidSse(super::sanitize_api_error(&message))
        }
        StreamError::ModelProvider(message) => {
            StreamError::ModelProvider(super::sanitize_api_error(&message))
        }
        StreamError::Json(error) => StreamError::Json(error),
...
fn parse_retry_after_ms(err: &anyhow::Error) -> Option<u64> {
    ...
    for prefix in &[
        ...
        if let Some(pos) = lower.find(prefix) {
            ...
            if let Some(value) = parse_retry_after_value_ms(after, chrono::Utc::now()) {
                return Some(value);
            }
        }
    }
...
fn parse_retry_after_value_ms(value: &str, now: chrono::DateTime<chrono::Utc>) -> Option<u64> {
    ...
    let date_value = trimmed
        .split_once('\n')
        .map_or(trimmed, |(line, _)| line)
        .trim_end_matches('.');
    let retry_at = chrono::DateTime::parse_from_rfc2822(date_value)
        .ok()?
    ...
    u64::try_from(delay.to_std().ok()?.as_millis()).ok()
}
...
fn unapplied_key_rotation_message() -> &'static str {
    "Rate limited; key rotation selected another configured credential but cannot apply it \
     because ModelProvider has no set_api_key operation. Retrying with the original credential."
}
    ...
    async fn chat_with_system(
        ...
        for current_model in &models {
            for (provider_name, model_provider) in &self.model_providers {
                ...
                for attempt in 0..=self.max_retries {
                        ...
                        .await
                    {
                        Ok(resp) => {
                            ...
                            {
                                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"model_provider": provider_name, "model": *current_model, "attempt": attempt, "original_model": model})), "ModelProvider recovered (failover/retry)");
                                let primary = self
                                    .model_providers
                                    .first()
                                    .map(|(n, _)| n.as_str())
                        ...
                        Err(e) => {
                            ...
                            {
                                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model_provider": provider_name, "error": error_detail})), unapplied_key_rotation_message());
                            }

                            if non_retryable {
                                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model_provider": provider_name, "model": *current_model, "error": error_detail})), "Non-retryable error, moving on");
                                break;
                            }

                            if attempt < self.max_retries {
                                let wait = self.compute_backoff(backoff_ms, &e);
                                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model_provider": provider_name, "model": *current_model, "attempt": attempt + 1, "backoff_ms": wait, "reason": failure_reason, "error": error_detail})), "ModelProvider call failed, retrying");
                ...
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model_provider": provider_name, "model": *current_model})), "Exhausted retries, trying next model_provider/model");
            }

            if *current_model != model {
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"original_model": model, "fallback_model": *current_model})), "Model fallback exhausted all model_providers, trying next fallback model");
            }
        }
                        ...
                        Ok(resp) => {
                            ...
                            if attempt > 0
                                ...
                                || self.model_providers.first().map(|(n, _)| n.as_str())
                                    != Some(provider_name.as_str())
                            {
                                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"model_provider": provider_name, "model": *current_model, "attempt": attempt, "original_model": model, "context_truncated": context_truncated})), "ModelProvider recovered (failover/retry)");
                                let primary = self
                                    .model_providers
                                    .first()
                                    .map(|(n, _)| n.as_str())
                        ...
                        Err(e) => {
                            ...
                            if is_context_window_exceeded(&e) && !context_truncated {
                                ...
                                if dropped > 0 {
                                    ...
                                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model_provider": provider_name, "model": *current_model, "dropped": dropped, "remaining": effective_messages.len()})), "Context window exceeded; truncated history and retrying");
        ...
        for (provider_name, model_provider) in &self.model_providers {
            ...
            let handle = ::dx_agent_spawn::spawn!(async move {
                ...
                while let Some(event) = stream.next().await {
                    let event = event.map_err(sanitize_stream_error);
                    if let Err(ref e) = event {
                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model_provider": provider_clone, "model": current_model, "e": e.to_string()})), "Streaming error: ");
                    }
        ...
        for (provider_name, model_provider) in &self.model_providers {
            ...
            let handle = ::dx_agent_spawn::spawn!(async move {
                ...
                while let Some(chunk) = stream.next().await {
                    let chunk = chunk.map_err(sanitize_stream_error);
...
impl ::dx_agent_api::attribution::Attributable for ReliableModelProvider {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Provider(
            ::dx_agent_api::attribution::ProviderKind::Model(
                ::dx_agent_api::attribution::ModelProviderKind::Reliable,
            ),
        )
    }
    fn alias(&self) -> &str {
...
mod tests {
    ...
    #[test]
    fn sanitize_stream_error_scrubs_string_variants() {
        let model_provider_error = sanitize_stream_error(StreamError::ModelProvider(
            r#"Authorization: Bearer plain-stream-secret {"api_key":"plain-json-secret"}"#
                .to_string(),
        ));
        let http_error = sanitize_stream_error(StreamError::Http(
            "request failed: https://example.test?refresh_token=plain-refresh-secret".to_string(),
        ));
        let invalid_sse_error = sanitize_stream_error(StreamError::InvalidSse(
            "data contained x-api-key: plain-header-secret".to_string(),
        ));

        for error in [model_provider_error, http_error, invalid_sse_error] {
            let message = error.to_string();
            assert!(!message.contains("plain-stream-secret"));
            assert!(!message.contains("plain-json-secret"));
            assert!(!message.contains("plain-refresh-secret"));
            assert!(!message.contains("plain-header-secret"));
    ...
    impl ::dx_agent_api::attribution::Attributable for ModelAwareMock {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for EmptyThenTextMock {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    #[test]
    fn unapplied_key_rotation_message_omits_credential_material() {
        let message = unapplied_key_rotation_message();

        assert!(message.contains("Rate limited"));
        assert!(
            !message.contains("ending"),
            "rotation warning must not describe key suffixes: {message}"
        );
        assert!(
            !message.contains("..."),
            "rotation warning must not reserve space for credential fragments: {message}"
        );
    }
    ...
    #[test]
    fn non_retryable_rate_limit_detects_google_quota_exceeded() {
        let err = anyhow::Error::msg(
            "Gemini API error (429 Too Many Requests): { \"error\": { \"code\": 429, \"message\": \"You exceeded your current quota. Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_requests, limit: 0, model: gemini-2.0-flash\" } }",
        );
        assert!(
            is_non_retryable_rate_limit(&err),
            "Google quota-exceeded 429 should skip retries"
        );
    }
    ...
    #[tokio::test]
    async fn non_retryable_rate_limit_skips_retries_for_google_quota_errors() {
        let calls = Arc::new(AtomicUsize::new(0));
        let model_provider = ReliableModelProvider::new(
            "test",
            vec![(
                ...
                Box::new(MockModelProvider {
                    ...
                    fail_until_attempt: usize::MAX,
                    response: "never",
                    error: "Gemini API error (429 Too Many Requests): { \"error\": { \"code\": 429, \"message\": \"You exceeded your current quota. Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_requests, limit: 0, model: gemini-2.0-flash\" } }",
                }),
            )],
            5,
            1,
        ...
        assert!(
            result.is_err(),
            "Google quota-exceeded 429 should fail quickly without retrying"
        );
        assert_eq!(
            calls.load(Ordering::SeqCst),
            1,
            "must not retry non-retryable Google quota errors"
        );
    }
    ...
    impl ::dx_agent_api::attribution::Attributable for NativeToolMock {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for NativeModelAwareMock {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for ContextOverflowMock {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for StreamingToolEventMock {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for StreamingHistoryMock {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    fn supports_vision_reflects_first_provider_not_any_fallback() {
        ...
        impl ::dx_agent_api::attribution::Attributable for VisionMock {
            fn role(&self) -> ::dx_agent_api::attribution::Role {
                ::dx_agent_api::attribution::Role::Provider(
                    ::dx_agent_api::attribution::ProviderKind::Model(
                        ::dx_agent_api::attribution::ModelProviderKind::Custom,
