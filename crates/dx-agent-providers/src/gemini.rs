...
//! - DX Agent auth-profiles OAuth tokens
...
impl GeminiModelProvider {
    ...
    /// 1. Explicit API key passed in from config
    ...
    pub fn new(alias: &str, api_key: Option<&str>) -> Self {
        ...
        let resolved_auth =
            Self::resolve_config_or_environment_api_key(api_key, |name| std::env::var(name).ok())
                .map(GeminiAuth::ExplicitKey)
                .or_else(|| {
                    Self::try_load_gemini_cli_token(oauth_cred_paths.first()).map(|state| {
    ...
    /// 1. Explicit API key passed in from config
    /// 2. `GEMINI_API_KEY` or `GOOGLE_API_KEY` environment variable
    /// 3. Managed OAuth from auth-profiles.json (if auth_service provided)
    ...
    fn resolve_config_or_environment_api_key(
        ...
        configured_key
            ...
            .or_else(|| {
                env_lookup(GEMINI_API_KEY_ENV).and_then(|value| Self::normalize_non_empty(&value))
            })
            .or_else(|| {
                env_lookup(GOOGLE_API_KEY_ENV).and_then(|value| Self::normalize_non_empty(&value))
            })
    }
...
impl ::dx_agent_api::attribution::Attributable for GeminiModelProvider {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Provider(
            ::dx_agent_api::attribution::ProviderKind::Model(
                ::dx_agent_api::attribution::ModelProviderKind::Gemini,
            ),
        )
    }
    fn alias(&self) -> &str {
...
mod tests {
    ...
    #[test]
    fn api_key_resolution_prefers_configured_key() {
        let key = GeminiModelProvider::resolve_config_or_environment_api_key(
            Some(" config-key "),
            |name| match name {
                "GEMINI_API_KEY" => Some("gemini-env-key".to_string()),
                "GOOGLE_API_KEY" => Some("google-env-key".to_string()),
                _ => None,
            },
        );
    ...
    #[test]
    fn api_key_resolution_uses_gemini_environment_key() {
        let key =
            GeminiModelProvider::resolve_config_or_environment_api_key(None, |name| match name {
                "GEMINI_API_KEY" => Some(" gemini-env-key ".to_string()),
                _ => None,
            });

        assert_eq!(key.as_deref(), Some("gemini-env-key"));
    }

    #[test]
    fn api_key_resolution_uses_google_environment_key_alias() {
        let key =
            GeminiModelProvider::resolve_config_or_environment_api_key(None, |name| match name {
        ...
        assert_eq!(key.as_deref(), Some("google-env-key"));
    }

    #[test]
    fn api_key_resolution_ignores_empty_environment_keys() {
        let key =
            GeminiModelProvider::resolve_config_or_environment_api_key(None, |name| match name {
                "GEMINI_API_KEY" | "GOOGLE_API_KEY" => Some(" \t ".to_string()),
                _ => None,
            });
    ...
    #[test]
    fn oauth_quota_exhaustion_does_not_rotate_or_retry() {
        let quota_body = r#"{
            "error": {
                "code": 429,
        ...
        assert!(!GeminiModelProvider::should_rotate_oauth_on_error(
    ...
    #[test]
    fn oauth_transient_server_errors_still_rotate() {
        assert!(GeminiModelProvider::should_rotate_oauth_on_error(
