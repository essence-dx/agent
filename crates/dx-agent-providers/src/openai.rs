...
impl ::dx_agent_api::attribution::Attributable for OpenAiModelProvider {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Provider(
            ::dx_agent_api::attribution::ProviderKind::Model(
                ::dx_agent_api::attribution::ModelProviderKind::OpenAi,
            ),
        )
    }
    fn alias(&self) -> &str {
...
impl ::dx_agent_api::attribution::Attributable for OpenAiResponsesModelProvider {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Provider(
            ::dx_agent_api::attribution::ProviderKind::Model(
                ::dx_agent_api::attribution::ModelProviderKind::OpenAi,
...
mod tests {
    ...
    async fn chat_with_system_fails_without_key() {
        let p = OpenAiModelProvider::new("test", None);
        let result = p
            .chat_with_system(Some("You are DX Agent"), "test", "gpt-4o", Some(0.5))
            .await;
        assert!(result.is_err());
    }
    ...
    fn request_serializes_with_system_message() {
        let req = ChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: "You are DX Agent".to_string(),
                },
                Message {
                    role: "user".to_string(),
