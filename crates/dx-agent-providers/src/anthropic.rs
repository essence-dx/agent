...
impl AnthropicModelProvider {
    ...
    pub fn with_base_url(alias: &str, credential: Option<&str>, base_url: Option<&str>) -> Self {
        ...
        Self {
            ...
            base_url,
            max_tokens: dx_agent_api::model_provider::BASELINE_MAX_TOKENS,
        }
    }
    ...
    fn auth_log_attributes(credential: &str) -> serde_json::Value {
        let is_setup = Self::is_setup_token(credential);
        serde_json::json!({
            "header": if is_setup { "Authorization" } else { "x-api-key" },
            "credential_kind": if is_setup { "setup_token" } else { "api_key" },
            "credential_present": true,
        })
    }

    fn sanitized_stream_provider_error(message: &str) -> StreamError {
    ...
    fn apply_auth(
        ...
        ::dx_agent_log::record!(
            ...
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                .with_attrs(Self::auth_log_attributes(credential)),
    ...
    fn resolve_thinking(
        &self,
        thinking: Option<dx_agent_api::model_provider::NativeThinkingParams>,
        temperature: Option<f64>,
        model: &str,
    ...
    fn http_client(&self) -> Client {
        dx_agent_config::schema::build_runtime_proxy_client_with_timeouts(
            ...
            }
        {
            ...
            match event_type {
                "message_start" => {
                    ...
                    ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"model": model, "input_tokens": observed_input, "cached_input_tokens": observed_cached, "cache_creation_input_tokens": observed_cache_create})), "stream: message_start");
                }
                "content_block_start" => {
                ...
                "message_delta" => {
                    ...
                    if stop_reason == "max_tokens" {
                        ...
                        ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"stop_reason": stop_reason, "output_tokens": observed_output})), "stream: message_delta");
                    }
                }
                "message_stop" => {
                    ::dx_agent_log::record!(
                ...
                "error" => {
                    ...
                    let _ = tx
                        .send(Err(Self::sanitized_stream_provider_error(msg)))
                        .await;
                    return;
                }
                _ => {}
...
impl ModelProvider for AnthropicModelProvider {
    ...
    async fn chat(
        ...
        let tool_choice_override = dx_agent_api::TOOL_CHOICE_OVERRIDE
    ...
    fn stream_chat(
        ...
        if thinking_config.is_some() {
            ...
            return stream::once(async move {
                ...
                if !response.status().is_success() {
                    ...
                    return Err(Self::sanitized_stream_provider_error(&format!(
                        "{status}: {body}"
                    )));
                }
        ...
        ::dx_agent_log::record!(
            DEBUG,
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Spawn)
                .with_category(::dx_agent_log::EventCategory::Provider)
                .with_attrs(::serde_json::json!({
        ...
        let parser_handle = ::dx_agent_spawn::spawn!(async move {
            let mut req = client
                .post(&url)
                .header("anthropic-version", "2023-06-01")
                .header("content-type", "application/json")
            ...
            if !response.status().is_success() {
                ...
                let error = response
                    ...
                    .unwrap_or_else(|_| format!("HTTP error: {status}"));
                let _ = tx
                    .send(Err(Self::sanitized_stream_provider_error(&format!(
                        "{status}: {error}"
                    ))))
                    .await;
                return;
...
impl ::dx_agent_api::attribution::Attributable for AnthropicModelProvider {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Provider(
            ::dx_agent_api::attribution::ProviderKind::Model(
                ::dx_agent_api::attribution::ModelProviderKind::Anthropic,
            ),
        )
    }
    fn alias(&self) -> &str {
...
data: {\"type\":\"message_start\",\"message\":{\"id\":\"m\",\"type\":\"message\",\"role\":\"assistant\",\"model\":\"claude\",\"usage\":{\"input_tokens\":1}}}\n\n"
        ...
        let parser = ::dx_agent_spawn::spawn!(async move {
            AnthropicModelProvider::parse_anthropic_sse_from_reader(reader, &tx).await;
        });
...
data: {\"type\":\"message_stop\"}\n\n";
    ...
    #[tokio::test]
    async fn streaming_error_sanitizes_credential_material() {
        use std::io::Cursor;

        let bytes = b"event: error\n\
data: {\"type\":\"error\",\"error\":{\"type\":\"api_error\",\"message\":\"Authorization: Bearer sk-ant-api03-secret-body and {\\\"api_key\\\":\\\"sk-ant-api03-json-secret\\\"}\"}}\n\n";
        let reader = tokio::io::BufReader::new(Cursor::new(bytes.as_slice()));
        let (tx, mut rx) = tokio::sync::mpsc::channel::<StreamResult<StreamEvent>>(8);
        AnthropicModelProvider::parse_anthropic_sse_from_reader(reader, &tx).await;

        let error = tokio::time::timeout(std::time::Duration::from_millis(50), rx.recv())
            .await
            .expect("stream should emit an error")
            .expect("stream should not close before error")
            .expect_err("stream error event must be returned as an error");
        let rendered = error.to_string();

        assert!(
            rendered.contains("[REDACTED]"),
            "sanitized error should show redaction marker: {rendered}"
        );
        assert!(
            !rendered.contains("sk-ant-api03"),
            "sanitized error must not contain credential prefixes: {rendered}"
        );
        assert!(
            !rendered.contains("secret-body") && !rendered.contains("json-secret"),
            "sanitized error must not contain credential bodies: {rendered}"
        );
    }
    ...
    #[test]
    fn auth_log_attributes_never_include_credential_material() {
        for credential in [
            "sk-ant-api03-secret-head-and-tail-1234",
            "sk-ant-oat01-secret-head-and-tail-5678",
        ] {
            let attrs = AnthropicModelProvider::auth_log_attributes(credential);
            let serialized = attrs.to_string();

            assert!(
                !serialized.contains("sk-ant-"),
                "log attributes must not contain credential prefixes: {serialized}"
            );
            assert!(
                !serialized.contains("secret-head"),
                "log attributes must not contain credential body: {serialized}"
            );
            assert!(
                !serialized.contains("1234") && !serialized.contains("5678"),
                "log attributes must not contain credential tails: {serialized}"
            );
            assert_eq!(
                attrs
        ...
        let api_key_attrs =
            AnthropicModelProvider::auth_log_attributes("sk-ant-api03-secret-head-and-tail-1234");
        assert_eq!(
            api_key_attrs.get("header").and_then(|value| value.as_str()),
            Some("x-api-key")
        );
        assert_eq!(
            api_key_attrs
        ...
        let setup_attrs =
            AnthropicModelProvider::auth_log_attributes("sk-ant-oat01-secret-head-and-tail-5678");
        assert_eq!(
            setup_attrs.get("header").and_then(|value| value.as_str()),
            Some("Authorization")
        );
        assert_eq!(
            setup_attrs
    ...
    fn chat_request_serializes_with_system() {
        let req = ChatRequest {
            model: "claude-3-opus".to_string(),
            max_tokens: 4096,
            system: Some("You are DX Agent".to_string()),
            messages: vec![Message {
                role: "user".to_string(),
                content: "hello".to_string(),
            }],
        ...
        let json = serde_json::to_string(&req).unwrap();
        assert!(json.contains("\"system\":\"You are DX Agent\""));
    }
    ...
    fn resolve_thinking_drops_native_for_opus_4_7() {
        ...
        let params = dx_agent_api::model_provider::NativeThinkingParams {
