...
//! `dx-agent-config::providers::ModelProviders` declares its own construction
...
//!
//! [`for_each_model_provider_slot!`]: dx_agent_config::providers::for_each_model_provider_slot
...
pub fn dispatch_family_factory(
        ...
        ($(($field:ident, $type_str:literal, $cfg_ty:ty)),+ $(,)?) => {
            match family {
                "openai-compatible" | "openai_compatible" => {
                    let default_cfg = dx_agent_config::schema::ModelProviderConfig::default();
                    let cfg = config
    ...
    dx_agent_config::for_each_model_provider_slot!(emit_dispatch)
}

// ════════════════════════════════════════════════════════════════════════
// Per-family impls — grouped by category. Adding a family means: one row
// in `for_each_model_provider_slot!` (dx-agent-config) plus one impl
...
impl FamilyProviderFactory for OpenAIModelProviderConfig {
    fn create_provider(
        ...
        if self.base.wire_api == Some(dx_agent_config::schema::WireApi::Responses) {
...
impl FamilyProviderFactory for GeminiModelProviderConfig {
    fn create_provider(
        ...
        let state_dir = opts.dx_agent_dir.clone().unwrap_or_else(|| {
            directories::UserDirs::new().map_or_else(
                || std::path::PathBuf::from(".dx-agent"),
...
impl FamilyProviderFactory for dx_agent_config::schema::ModelProviderConfig {
    fn create_provider(
        &self,
        alias: &str,
        key: Option<&str>,
...
mod tests {
    use super::*;
    use dx_agent_config::schema::ModelProviderConfig;
    ...
    fn llamacpp_factory_routes_to_responses_provider_when_wire_api_responses() {
        use dx_agent_config::schema::{LlamacppModelProviderConfig, ModelProviderConfig, WireApi};
    ...
    fn llamacpp_factory_defaults_to_chat_completions_without_wire_api() {
        use dx_agent_config::schema::LlamacppModelProviderConfig;
