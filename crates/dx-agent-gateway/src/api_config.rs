...
use dx_agent_config::field_visibility;
use dx_agent_config::sections::section_for_path;
use dx_agent_config::traits::MaskSecrets;
...
///
/// `comment` is a DX Agent extension — when provided it accompanies the
...
pub struct PatchResponse {
    ...
    pub warnings: Vec<dx_agent_config::validation_warnings::ValidationWarning>,
}
...
pub struct ListEntry {
    ...
    pub populated: bool,
    pub is_secret: bool,
    /// Whether this field was populated by a `DX_AGENT_*` env-var override
    /// at load time. The dashboard renders the 💉 badge and a persistent
    /// warning *"Edits here won't take effect — overridden by DX_AGENT_..."*
    /// when this is `true`.
...
fn prop_kind_wire(kind: dx_agent_config::traits::PropKind) -> &'static str {
...
// Typed-value coercion lives in `dx_agent_config::typed_value` — both the
...
use dx_agent_config::typed_value::coerce_for_set_prop as json_to_setprop_string;
...
fn lookup_prop_field(
    ...
    path: &str,
) -> Option<dx_agent_config::traits::PropFieldInfo> {
    config
        .prop_fields()
        .into_iter()
...
fn scoped_validate(
    working: &dx_agent_config::schema::Config,
) -> Result<Vec<dx_agent_config::validation_warnings::ValidationWarning>, ConfigApiError> {
    if let Err(e) = working.validate() {
        ...
        return Ok(vec![
            dx_agent_config::validation_warnings::ValidationWarning::new(
...
pub async fn compute_drift(in_memory: &dx_agent_config::schema::Config) -> Vec<DriftEntry> {
    ...
    let on_disk: dx_agent_config::schema::Config =
        match toml::from_str::<dx_agent_config::schema::Config>(&raw) {
            Ok(mut cfg) => {
    ...
    let in_memory_props: std::collections::HashMap<String, dx_agent_config::traits::PropFieldInfo> =
        in_memory
            .prop_fields()
            .into_iter()
            .map(|p| (p.name.clone(), p))
            .collect();
    let on_disk_props: std::collections::HashMap<String, dx_agent_config::traits::PropFieldInfo> =
        on_disk
            .prop_fields()
            .into_iter()
            .map(|p| (p.name.clone(), p))
    ...
    for name in all_names {
        ...
        let mem_display = mem
            ...
            .unwrap_or(dx_agent_config::traits::UNSET_DISPLAY);
...
pub async fn handle_prop_get(
    ...
    if info.is_secret || info.derived_from_secret {
        let populated = info.display_value != dx_agent_config::traits::UNSET_DISPLAY;
...
pub async fn handle_prop_put(
    ...
    if is_sensitive
        && (value_str == dx_agent_config::traits::MASKED_SECRET
            || value_str == "****"
            || value_str.is_empty())
    {
    ...
    if let Some(comment) = body.comment.as_ref() {
        ...
        if let Err(e) =
            dx_agent_config::comment_writer::apply_comments(&config_path, &annotations).await
        {
            ::dx_agent_log::record!(
                WARN,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
...
pub async fn handle_templates(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let templates: Vec<TemplateEntry> = dx_agent_config::schema::Config::map_key_sections()
        .into_iter()
        .map(|s| TemplateEntry {
            path: s.path,
            kind: match s.kind {
                dx_agent_config::traits::MapKeyKind::Map => "map",
                dx_agent_config::traits::MapKeyKind::List => "list",
            },
        ...
        if q.path == "agents" {
                ...
                && let Some(parent) = workspace.parent()
            {
                ...
                if let Err(err) = tokio::fs::create_dir_all(&archive_root).await {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": q.key, "archive": archive_root.display().to_string(), "err": err.to_string()})), "agent alias removed from config but archive dir creation failed");
                } else if let Err(err) = tokio::fs::rename(&workspace, &archive_dir).await {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": q.key, "from": workspace.display().to_string(), "to": archive_dir.display().to_string(), "err": err.to_string()})), "agent alias removed from config but workspace archive failed");
                } else {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"agent": q.key, "archive": archive_dir.display().to_string()})), "agent workspace archived after alias removal");
                }
            }
            match state.mem.purge_agent(&q.key).await {
                Ok(rows) if rows > 0 => ::dx_agent_log::record!(
                    INFO,
                    ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
...
pub async fn handle_map_key(
    ...
    if created {
        ...
        if path == "skill_bundles" {
            ...
            if let Ok(dir) =
                dx_agent_config::skill_bundles::resolve_directory(&working, &install_root, &key)
...
pub async fn handle_patch(
    ...
    // `X-DX Agent-Override-Drift: true` header. The opt-in surface keeps
                ...
                return error_response(ConfigApiError::new(
                    ...
                    format!(
                        "on-disk config has drifted from in-memory state on \
                         {} path(s) being patched: {}. Send `X-DX Agent-Override-Drift: true` \
    ...
    // `dx_agent_log::record!` from `validate()`, surfaced structured so dashboard
    // callers see it.
...
pub async fn handle_migrate(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let migrated = match dx_agent_config::migration::migrate_file(&raw) {
        Ok(out) => out,
        Err(e) => {
            return error_response(ConfigApiError::new(
                ConfigApiCode::ValidationFailed,
    ...
    match migrated {
        Some(new_content) => {
            ...
            let new_cfg: dx_agent_config::schema::Config = match toml::from_str(&new_content) {
                Ok(c) => c,
                Err(e) => {
                    return error_response(ConfigApiError::new(
                        ConfigApiCode::ReloadFailed,
            ...
            *state.config.write() = new_cfg;
            ...
            axum::Json(MigrateResponse {
                ...
                schema_version: dx_agent_config::migration::CURRENT_SCHEMA_VERSION,
            })
            .into_response()
        }
...
pub async fn handle_options_config(headers: HeaderMap) -> Response {
    ...
    schema_response("dx_agent_config_schema_full")
}
...
fn schema_body_value() -> serde_json::Value {
    let schema = schemars::schema_for!(dx_agent_config::schema::Config);
...
mod tests {
    ...
    // typed-value coercion tests live in dx_agent_config::typed_value
    // — shared helper, single source of truth.
    //
    // build_comment_prefix tests live in dx_agent_config::comment_writer
    // — same reason.
    ...
    async fn compute_drift_returns_empty_when_in_memory_matches_disk() {
        ...
        let cfg = dx_agent_config::schema::Config {
    ...
    async fn compute_drift_surfaces_mismatched_non_secret_field() {
        let (_tmp, path) = temp_config_path();
        let mut cfg = dx_agent_config::schema::Config {
            config_path: path.clone(),
            ..Default::default()
        };
    ...
    fn scrub_credentials_catches_credential_shaped_strings() {
        ...
        use dx_agent_runtime::agent::loop_::scrub_credentials;
