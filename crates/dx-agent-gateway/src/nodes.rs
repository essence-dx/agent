...
fn extract_node_ws_token(headers: &HeaderMap) -> Option<&str> {
...
pub(crate) fn check_node_auth(
    nodes_config: &dx_agent_config::schema::NodesConfig,
    ...
    if let Some(ref expected_token) = nodes_config.auth_token {
        let token = extract_node_ws_token(headers).unwrap_or("");
...
pub async fn handle_ws_nodes(
    ...
    if let Some((status, body)) = check_node_auth(&nodes_config, &state.pairing, &headers) {
...
mod tests {
    ...
    use dx_agent_config::schema::NodesConfig;
    use dx_agent_runtime::security::pairing::PairingGuard;
    ...
    fn nodes_disabled_returns_404() {
        ...
        let result = check_node_auth(&cfg, &make_pairing(false), &empty_headers());
    ...
    fn nodes_auth_token_correct_token_passes() {
        ...
        let result = check_node_auth(&cfg, &make_pairing(false), &headers);
    ...
    fn nodes_pairing_required_wrong_token_returns_401() {
        ...
        let result = check_node_auth(&cfg, &make_pairing(true), &empty_headers());
        assert_eq!(result.map(|(s, _)| s), Some(StatusCode::UNAUTHORIZED));
    }
    ...
    fn extract_node_ws_token_from_header() {
        ...
        assert_eq!(extract_node_ws_token(&headers), Some("node_tok_123"));
    }

    #[test]
    fn extract_node_ws_token_from_subprotocol() {
        let mut headers = HeaderMap::new();
        headers.insert(
            "sec-websocket-protocol",
            "zeroclaw.nodes.v1, bearer.node_tok_456".parse().unwrap(),
        );
        assert_eq!(extract_node_ws_token(&headers), Some("node_tok_456"));
    }

    #[test]
    fn extract_node_ws_token_returns_none_without_trusted_source() {
        let headers = HeaderMap::new();
        assert_eq!(
            extract_node_ws_token(&headers),
            None,
            "node WebSocket bearer tokens require Authorization or bearer subprotocol"
        );
    }
    ...
    fn extract_node_ws_token_none_when_empty() {
        let headers = HeaderMap::new();
        assert_eq!(extract_node_ws_token(&headers), None);
    }
}
