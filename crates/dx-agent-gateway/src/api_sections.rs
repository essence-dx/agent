...
use dx_agent_runtime::rpc::types::{
    CatalogModelsResult, CatalogResponse, ConfigSectionEntry, ConfigSectionsResult,
    ConfigStatusResult, PickerItem, PickerResponse, SelectItemResponse,
};

use super::AppState;
use super::api::require_auth;

/// `GET /api/config/catalog` — list native model providers plus the shared
/// DX providers archive when it is available. Native providers stay first and
/// keep priority over catalog duplicates.
pub async fn handle_catalog(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    axum::Json(CatalogResponse::from(listing)).into_response()
}
...
/// (`dx_agent_providers::create_model_provider(...).list_models()`), which goes
...
pub async fn handle_catalog_models(
    ...
    let _ = state;
    let local = dx_agent_runtime::quickstart::model_provider_is_local(&q.model_provider);
    let (models, live) = dx_agent_runtime::quickstart::model_catalog(&q.model_provider).await;
...
/// that state.
#[must_use]
pub fn derive_section_status(cfg: &dx_agent_config::schema::Config) -> ConfigStatusResult {
...
fn quickstart_missing_requirements(cfg: &dx_agent_config::schema::Config) -> Vec<String> {
...
fn quickstart_agent_missing_requirements(
    cfg: &dx_agent_config::schema::Config,
    alias: &str,
    agent: &dx_agent_config::schema::AliasedAgentConfig,
) -> Vec<String> {
    let mut missing = Vec::new();
    if !agent.enabled {
    ...
    if model_ref.is_empty() {
        ...
        if !has_model {
            ...
            provider,
            dx_agent_runtime::quickstart::model_provider_is_local(family),
        ) {
            missing.push(format!(
...
/// `crates/dx-agent-macros/src/lib.rs:366` builds lookup arms with
...
pub fn build_agent_options(cfg: &dx_agent_config::schema::Config) -> AgentOptionsResponse {
    fn dotted_aliases(cfg: &dx_agent_config::schema::Config, prefix: &str) -> Vec<String> {
...
pub async fn handle_sections(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let all_map_paths: Vec<&'static str> = dx_agent_config::schema::Config::map_key_sections()
        .iter()
        .map(|s| s.path)
        .collect();
    ...
    for s in dx_agent_config::sections::QUICKSTART_SECTIONS {
    ...
    // (single source of truth in `dx_agent_config::sections`), then
    ...
    ordered.sort_by(|a, b| {
        match (
            dx_agent_config::sections::section_index_for_key(a),
            dx_agent_config::sections::section_index_for_key(b),
        ) {
    ...
    let sections: Vec<ConfigSectionEntry> = ordered
        ...
        .map(|key| {
            ...
            let wizard = dx_agent_config::sections::Section::from_key(&key);
            let has_picker = match wizard {
                Some(w) => !matches!(
                    w,
                    dx_agent_config::sections::Section::Hardware
                        | dx_agent_config::sections::Section::Mcp
                        | dx_agent_config::sections::Section::Skills
                ),
            ...
            ConfigSectionEntry {
                ...
                shape: wizard.map(dx_agent_config::sections::Section::shape),
                key,
            }
        })
        .collect();
...
fn section_ready(cfg: &dx_agent_config::schema::Config, key: &str, completed_marker: bool) -> bool {
    use dx_agent_config::sections::Section;
...
/// Help text for a section. Delegates to `dx_agent_config::sections::section_help`
...
fn section_help(key: &str) -> &'static str {
    dx_agent_config::sections::section_help(key)
}
...
/// * `providers` → `dx_agent_providers::list_model_providers()` (CLI's catalog).
/// * `memory` → `dx_agent_memory::selectable_memory_backends()`.
...
fn picker_items_for(
    section: dx_agent_config::sections::Section,
    cfg: &dx_agent_config::schema::Config,
) -> PickerDispatch {
    use dx_agent_config::sections::Section;
    match section {
...
fn providers_picker(cfg: &dx_agent_config::schema::Config) -> Vec<PickerItem> {
    dx_agent_providers::list_model_providers()
        ...
        .map(|provider| PickerItem {
            ...
            description: provider_picker_description(&provider),
            badge: provider_type_badge(cfg, provider.name, provider.local),
        })
        .collect()
}

fn provider_picker_description(provider: &dx_agent_providers::ModelProviderInfo) -> Option<String> {
    if provider.local {
        return Some("Local — no API key required".to_string());
    }

    let freemium = provider.freemium()?;
    let provider_name = provider
        .identity()
        .map(|identity| identity.display_name)
        .unwrap_or_else(|| provider.display_name.to_string());

    if freemium.free_model_ids.is_empty() {
        return Some(format!("{provider_name}: free tier available."));
    }

    Some(format!(
        "{provider_name}: free models {}.",
        summarized_model_ids(&freemium.free_model_ids, 3)
    ))
}

fn summarized_model_ids(model_ids: &[String], visible_count: usize) -> String {
    let visible = model_ids
        .iter()
        .take(visible_count)
        .map(String::as_str)
        .collect::<Vec<_>>()
        .join(", ");
    let remaining = model_ids.len().saturating_sub(visible_count);
...
fn any_usable_model_provider(cfg: &dx_agent_config::schema::Config) -> bool {
    cfg.providers
        .models
        .iter_entries()
        .any(|(family, _, base)| {
...
fn storage_picker(cfg: &dx_agent_config::schema::Config) -> Vec<PickerItem> {
...
fn memory_picker(cfg: &dx_agent_config::schema::Config) -> Vec<PickerItem> {
    ...
    dx_agent_memory::selectable_memory_backends()
        .iter()
        .map(|b| PickerItem {
            key: b.key.to_string(),
...
fn schema_walk_picker(cfg: &dx_agent_config::schema::Config, section: &str) -> Vec<PickerItem> {
    ...
    let all: std::collections::BTreeSet<String> =
        dx_agent_config::schema::Config::map_key_sections()
            .into_iter()
            .filter_map(|s| {
                s.path
...
fn agents_picker(cfg: &dx_agent_config::schema::Config) -> Vec<PickerItem> {
...
fn apply_first_run_agent_defaults(cfg: &mut dx_agent_config::schema::Config, alias: &str) {
...
fn mark_section_completed(cfg: &mut dx_agent_config::schema::Config, section: &str) {
    if !cfg
        .onboard_state
        .completed_sections
        .iter()
    ...
    let (fields_prefix, created) = match section_enum {
        ...
        | Section::RuntimeProfiles => {
            ...
            if created && matches!(section_enum, Section::Agents) {
                ...
                if let Err(err) =
                    dx_agent_config::schema::ensure_bootstrap_files(&workspace_dir).await
                {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": key, "workspace": workspace_dir.display().to_string(), "err": err.to_string()})), "agent workspace scaffolded but bootstrap files seed failed (continuing)");
                }
            }
...
mod tests {
    ...
    fn memory_section_ready_tracks_onboarding_progress_not_default_backend() {
        let cfg = dx_agent_config::schema::Config::default();
        assert!(
    ...
    fn empty_cfg() -> dx_agent_config::schema::Config {
    ...
    fn handle_sections_derives_every_top_level_field_from_schema() {
        ...
        // populated entries).
        for s in dx_agent_config::schema::Config::map_key_sections() {
    ...
    fn providers_picker_sources_from_list_providers() {
        // Single source of truth: dx_agent_providers::list_model_providers().
    ...
    #[test]
    fn providers_picker_exposes_opencode_freemium_metadata() {
        let cfg = empty_cfg();
        let items = providers_picker(&cfg);
        let opencode = items
            .iter()
            .find(|item| item.key == "opencode")
            .expect("opencode provider picker item");
        let description = opencode
            .description
            .as_deref()
            .expect("opencode freemium description");
        ...
        assert!(
            ...
            "picker description should not expose env var setup details"
        );
        assert!(
            !description.contains("paid Zen models"),
    ...
    fn memory_picker_sources_from_selectable_backends() {
        ...
        // Mirrors dx_agent_memory::selectable_memory_backends() exactly.
