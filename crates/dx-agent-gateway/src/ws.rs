...
//! tool args through `dx_agent_runtime::approval::summarize_args` before
...
///
/// Browsers cannot set custom headers on `new WebSocket(url)`, so browser
/// clients must use the bearer subprotocol instead of URL query parameters.
...
pub async fn handle_ws_chat(
    ...
    // Auth: check header, then bearer subprotocol.
...
async fn handle_socket(
    ...
    let mut memory_session_id = dx_agent_api::session_keys::sanitize_session_key(&session_id);
            ...
            Ok(Message::Text(text)) => {
                if let Ok(cp) = serde_json::from_str::<ConnectParams>(&text) {
                    if cp.msg_type == "connect" {
                        ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"session_id": cp.session_id, "device_name": cp.device_name, "capabilities": cp.capabilities, "cwd": cp.cwd})), "WebSocket connect params received");
                        if let Some(sid) = &cp.session_id {
                            memory_session_id =
                                dx_agent_api::session_keys::sanitize_session_key(sid);
                            ::dx_agent_log::record!(
                                DEBUG,
    ...
    let mut agent =
        match dx_agent_runtime::agent::Agent::from_config_with_session_cwd_and_mcp_backchannel(
            &config,
            &agent_alias,
            Some(&session_cwd),
            true,
    ...
    let (approval_event_tx, mut approval_event_rx) =
        tokio::sync::mpsc::channel::<dx_agent_api::agent::TurnEvent>(8);
        ...
        tokio::select! {
            ...
            client_msg = receiver.next() => {
                ...
                if msg_type == "approval_response" {
                    ...
                    if let Some(tx) = pending_approvals.lock().remove(request_id) {
                        ...
                        ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"request_id": request_id})), "approval_response with no matching pending request");
                    }
                    continue;
                }
            ...
            approval_event = approval_event_rx.recv() => {
                ...
                let frame = match event {
                    dx_agent_api::agent::TurnEvent::ApprovalRequest {
                        request_id,
                        tool_name,
                        arguments_summary,
                        timeout_secs,
                    ...
                    other => {
                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"kind": format!("{:?}", other)})), "non-ApprovalRequest event leaked into approval channel");
                        continue;
                    }
                };
...
fn persist_conversation_messages(
    backend: &dyn dx_agent_infra::session_backend::SessionBackend,
    session_key: &str,
    messages: &[dx_agent_providers::ConversationMessage],
) {
    for message in messages {
        let dx_agent_providers::ConversationMessage::Chat(message) = message else {
            continue;
        };
...
fn has_assistant_chat_message(messages: &[dx_agent_providers::ConversationMessage]) -> bool {
    messages.iter().any(|message| {
        matches!(
            message,
            dx_agent_providers::ConversationMessage::Chat(message)
...
async fn process_chat_message(
    state: &AppState,
    agent: &mut dx_agent_runtime::agent::Agent,
    sender: &mut futures_util::stream::SplitSink<WebSocket, Message>,
    receiver: &mut futures_util::stream::SplitStream<WebSocket>,
    approval_event_rx: &mut tokio::sync::mpsc::Receiver<dx_agent_api::agent::TurnEvent>,
    pending_approvals: &PendingApprovals,
    content: &str,
    session_key: &str,
) {
    use futures_util::StreamExt as _;
    use dx_agent_runtime::agent::TurnEvent;
    ...
    let turn_fut = async {
        use ::dx_agent_log::Instrument as _;
        let span = ::dx_agent_log::info_span!(
        ...
        dx_agent_runtime::agent::loop_::scope_session_key(
                    ...
                    match parsed["type"].as_str() {
                        Some("approval_response") => {
                            ...
                            if let Some(tx) = pending_approvals.lock().remove(request_id) {
                                ...
                                ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"request_id": request_id})), "approval_response with no matching pending request (mid-turn)");
                            }
                        }
                        Some("message") => {
                            let content = parsed["content"].as_str().unwrap_or("").to_string();
    ...
    let was_cancelled = match &result {
        Err(e) => dx_agent_runtime::agent::loop_::is_tool_loop_cancelled(&e.error),
        Ok(_) => false,
    };
            ...
            match &result {
                Err(error) if !error.new_messages.is_empty() => {
                    ...
                    if !has_assistant_chat_message(&error.new_messages) {
                        ...
                        let assistant_msg = dx_agent_providers::ChatMessage::assistant(&truncated);
    ...
    match result {
        ...
        Err(e) => {
            ...
            let sanitized = dx_agent_providers::sanitize_api_error(&e.error.to_string());
...
fn record_turn_cost(
    ...
    let usage = dx_agent_runtime::cost::types::TokenUsage::new(
        model,
        input,
        output,
        cached_input,
    ...
    if let Err(error) = tracker.record_usage(usage) {
        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"provider": provider_name, "model": model, "error": format!("{}", error)})), "Failed to record gateway turn cost");
    }
    Some(cost_usd)
}
...
mod tests {
    ...
    fn extract_ws_token_from_authorization_header() {
        ...
        assert_eq!(extract_ws_token(&headers), Some("zc_test123"));
    }
    ...
    fn extract_ws_token_from_subprotocol() {
        ...
        assert_eq!(extract_ws_token(&headers), Some("zc_sub456"));
    }
    ...
    fn extract_ws_token_returns_none_without_trusted_source() {
        ...
        assert_eq!(
            ...
            None,
            "WebSocket bearer tokens require Authorization or bearer subprotocol"
        );
    }
    ...
    #[test]
    fn extract_ws_token_uses_subprotocol_without_authorization() {
        let mut headers = HeaderMap::new();
        headers.insert("sec-websocket-protocol", "bearer.zc_sub".parse().unwrap());
    ...
    fn extract_ws_token_skips_empty_header_value() {
        ...
        assert_eq!(
            extract_ws_token(&headers),
            None,
            "empty Authorization must not fall back to query-string tokens"
        );
    }

    #[test]
    fn ws_query_ignores_legacy_token_field() {
        let parsed: WsQuery = serde_json::from_value(serde_json::json!({
            "token": "zc_secret",
        ...
        assert_eq!(parsed.name.as_deref(), Some("Operator"));
    }
    ...
    fn extract_ws_token_subprotocol_with_multiple_entries() {
        ...
        assert_eq!(extract_ws_token(&headers), Some("zc_tok"));
    }
