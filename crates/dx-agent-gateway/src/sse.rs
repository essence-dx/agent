...
impl dx_agent_runtime::observability::Observer for BroadcastObserver {
    fn record_event(&self, event: &dx_agent_runtime::observability::ObserverEvent) {
        ...
        let json = match event {
            ...
            dx_agent_runtime::observability::ObserverEvent::ToolCall {
                tool,
                duration,
                success,
                ..
            ...
            dx_agent_runtime::observability::ObserverEvent::ToolCallStart { tool, .. } => {
                serde_json::json!({
            ...
            dx_agent_runtime::observability::ObserverEvent::Error { component, message } => {
                serde_json::json!({
                    "type": "error",
                    "component": component,
                    "message": message,
    ...
    fn record_metric(&self, _metric: &dx_agent_runtime::observability::traits::ObserverMetric) {
...
mod tests {
    use super::*;
    use dx_agent_runtime::observability::{Observer, ObserverEvent};
    ...
    fn factory_observer_events_reach_broadcast_hook() {
        ...
        dx_agent_runtime::observability::clear_broadcast_hook();

        let (tx, mut rx) = tokio::sync::broadcast::channel(16);
        let buffer = Arc::new(EventBuffer::new(16));
        let bo: Arc<dyn Observer> = Arc::new(BroadcastObserver::new(tx, buffer.clone()));
        dx_agent_runtime::observability::set_broadcast_hook(bo);

        // Same factory call site as `process_message` in the agent loop.
        let cfg = dx_agent_config::schema::ObservabilityConfig {
            backend: "noop".into(),
            ..Default::default()
        };
        let observer = dx_agent_runtime::observability::create_observer(&cfg);
