...
//!
//! Thin HTTP adapter over [`dx_agent_log::load_page`]. Pagination is
...
//! [`dx_agent_log::is_attribution_field`]. Adding a new attribution
...
pub async fn handle_api_logs(
    ...
    let Some(path) = dx_agent_log::current_log_path() else {
    ...
    let LogPage {
        ...
        next_cursor,
        at_end,
    } = match dx_agent_log::load_page(&path, &filter, limit) {
        Ok(page) => page,
        Err(err) => {
            return (
