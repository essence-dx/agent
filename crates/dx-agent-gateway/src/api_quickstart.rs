...
//!
//! Thin wrapper over `dx_agent_runtime::quickstart::{validate_only, apply}`.
//! Routes:
//!
...
//!
//! All business logic lives in `dx-agent-runtime`; this module is route
//! plumbing only.
...
use dx_agent_config::presets::BuilderSubmission;
...
/// `dx_agent_runtime::quickstart::QuickstartState`; both transports
...
pub async fn handle_state(State(state): State<AppState>, headers: HeaderMap) -> impl IntoResponse {
    ...
    let body = dx_agent_runtime::quickstart::snapshot_state(&cfg);
    (StatusCode::OK, Json(body)).into_response()
}

#[derive(Debug, Deserialize)]
pub struct FieldsRequest {
    pub section: dx_agent_runtime::quickstart::FieldSection,
    pub type_key: String,
}

#[derive(Debug, Serialize)]
pub struct FieldsResult {
    pub fields: Vec<dx_agent_runtime::quickstart::FieldDescriptor>,
}
...
pub async fn handle_fields(
    ...
    let body = FieldsResult {
        fields: dx_agent_runtime::quickstart::field_shape(req.section, &req.type_key),
    };
    (StatusCode::OK, Json(body)).into_response()
}
...
// `dx_agent_runtime::quickstart::snapshot_state` so both transports
