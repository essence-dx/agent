...
//!
//! DX Agent's agent loop calls [`dx_agent_hardware::boot`] on **every** request,
//! which re-reads `~/.dx-agent/hardware/` from disk.  Writing to those files
...
/// Return `~/.dx-agent/hardware/` or an error string.
fn hardware_dir() -> Result<PathBuf, String> {
    directories::BaseDirs::new()
        .map(|b| b.home_dir().join(".dx-agent").join("hardware"))
...
/// `POST /api/hardware/pin` — register a single GPIO pin assignment.
///
/// Appends one line to `~/.dx-agent/hardware/devices/<device>.md`:
/// ```text
...
pub async fn handle_hardware_pin(
    ...
    match append_to_file(&device_path, &line).await {
        Ok(()) => {
            ...
            ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"device": req.device, "pin": req.pin, "component": component})), &format!("{}", message));
            (
                StatusCode::OK,
...
///
/// Since [`dx_agent_hardware::boot`] re-reads from disk on every agent invocation,
...
pub async fn handle_hardware_reload(
    ...
    let context = dx_agent_hardware::load_hardware_context_prompt(&[]);
