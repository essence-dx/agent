...
pub async fn handle_canvas_post(
    ...
    if !dx_agent_runtime::tools::ALLOWED_CONTENT_TYPES.contains(&content_type) {
        return (
            ...
            Json(serde_json::json!({
                "error": format!(
                    ...
                    content_type,
                    dx_agent_runtime::tools::ALLOWED_CONTENT_TYPES
                )
            })),
        )
            .into_response();
    ...
    if body.content.len() > dx_agent_runtime::tools::MAX_CONTENT_SIZE {
        return (
            StatusCode::PAYLOAD_TOO_LARGE,
            Json(serde_json::json!({
                "error": format!(
                    "Content exceeds maximum size of {} bytes",
                    dx_agent_runtime::tools::MAX_CONTENT_SIZE
                )
            })),
        )
            .into_response();
