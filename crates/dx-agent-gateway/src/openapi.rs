...
/// committed snapshot at `crates/dx-agent-gateway/openapi.json`. CI
...
pub fn build_spec() -> serde_json::Value {
    ...
    use dx_agent_config::api_error::ConfigApiError;
    ...
    let components = serde_json::json!({
        "schemas": {
            ...
            "Config":           schema_value::<dx_agent_config::schema::Config>(),
        },
        "securitySchemes": {
            "bearerAuth": {
                "type": "http",
    ...
    let mut spec = serde_json::json!({
        "openapi": "3.1.0",
        "info": {
            "title": "DX Agent Gateway — Config CRUD",
