...
use dx_agent_config::schema::{ChannelAliasInfo, Config};
...
pub struct CronAddBody {
    ...
    pub delivery: Option<dx_agent_runtime::cron::DeliveryConfig>,
...
fn cron_schedule_from_api(
    expr: String,
    tz: Option<String>,
) -> Result<dx_agent_runtime::cron::Schedule, (StatusCode, Json<serde_json::Value>)> {
    let schedule = dx_agent_runtime::cron::Schedule::Cron { expr, tz };
    dx_agent_runtime::cron::validate_schedule(&schedule, chrono::Utc::now())
...
pub async fn handle_api_status(
    ...
    let health = dx_agent_runtime::health::snapshot();
    ...
    let process = dx_agent_runtime::process_stats::sample();
    ...
    let body = serde_json::json!({
        ...
        "daemon_started_at": dx_agent_runtime::health::daemon_started_at(),
...
pub async fn handle_api_cron_list(
    ...
    let config = state.config.read().clone();
    match dx_agent_runtime::cron::list_jobs(&config) {
...
pub async fn handle_api_cron_add(
    ...
    if let Err(e) = dx_agent_runtime::cron::validate_delivery_config(delivery.as_ref()) {
        return (
            StatusCode::BAD_REQUEST,
    ...
    let result = if is_agent {
        ...
        let session_target = session_target
            .as_deref()
            .map(dx_agent_runtime::cron::SessionTarget::parse)
            .unwrap_or_default();

        let default_delete = matches!(schedule, dx_agent_runtime::cron::Schedule::At { .. });
        let delete_after_run = delete_after_run.unwrap_or(default_delete);

        dx_agent_runtime::cron::add_agent_job(
            &config,
            &agent_alias,
            name,
            schedule,
        ...
        dx_agent_runtime::cron::add_shell_job_with_approval(
            &config,
            &agent_alias,
            name,
            schedule,
...
pub async fn handle_api_cron_runs(
    ...
    if let Err(e) = dx_agent_runtime::cron::get_job(&config, &id) {
        return (
    ...
    match dx_agent_runtime::cron::list_runs(&config, &id, limit) {
        Ok(runs) => {
...
pub async fn handle_api_cron_run(
    ...
    let job = match dx_agent_runtime::cron::get_job(&config, &id) {
        Ok(job) => job,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
    ...
    let (mut success, output) =
        dx_agent_runtime::cron::scheduler::execute_job_now(&config, &job).await;
    let finished_at = chrono::Utc::now();
    let duration_ms = (finished_at - started_at).num_milliseconds();
    let outcome = dx_agent_runtime::cron::scheduler::deliver_and_classify_run_result(
        &config,
        &job,
        success,
        output,
        dx_agent_runtime::cron::scheduler::CronDeliveryContext::GatewayManual,
    )
    .await;
    success = outcome.success;

    if let Err(e) = dx_agent_runtime::cron::record_run(
        &config,
        &job.id,
        started_at,
        finished_at,
    ...
    if let Err(e) = dx_agent_runtime::cron::record_last_run_with_status(
        &config,
        &job.id,
        finished_at,
        &outcome.status,
...
pub async fn handle_api_cron_patch(
    ...
    let existing = match dx_agent_runtime::cron::get_job(&config, &id) {
        Ok(j) => j,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
        ...
        let (expr, existing_tz) = match (&existing.schedule, new_expr) {
            (_, Some(expr)) => {
                let existing_tz = match &existing.schedule {
                    dx_agent_runtime::cron::Schedule::Cron { tz, .. } => tz.clone(),
                    _ => None,
                };
                (expr, existing_tz)
            }
            (dx_agent_runtime::cron::Schedule::Cron { expr, tz }, None) => {
                (expr.clone(), tz.clone())
            }
            (_, None) => {
    ...
    let is_agent = matches!(existing.job_type, dx_agent_runtime::cron::JobType::Agent);
    ...
    let patch = dx_agent_runtime::cron::CronJobPatch {
        name,
        schedule,
        command: patch_command,
        prompt: patch_prompt,
        ..dx_agent_runtime::cron::CronJobPatch::default()
    };

    match dx_agent_runtime::cron::update_shell_job_with_approval(
        &config,
        &agent_alias,
        &id,
        patch,
...
pub async fn handle_api_cron_delete(
    ...
    let config = state.config.read().clone();
    match dx_agent_runtime::cron::remove_job(&config, &id) {
...
pub async fn handle_api_integrations(
    ...
    let config = state.config.read().clone();
    let entries = dx_agent_runtime::integrations::registry::all_integrations(&config);
...
pub async fn handle_api_integrations_settings(
    ...
    for entry in &entries {
        let enabled = matches!(
            entry.status,
            dx_agent_runtime::integrations::IntegrationStatus::Active
        );
        settings.insert(
            entry.name.clone(),
            serde_json::json!({
...
pub async fn handle_api_doctor(
    ...
    let config = state.config.read().clone();
    let results = dx_agent_runtime::doctor::diagnose(&config);

    let ok_count = results
        .iter()
        .filter(|r| r.severity == dx_agent_runtime::doctor::Severity::Ok)
        .count();
    let warn_count = results
        .iter()
        .filter(|r| r.severity == dx_agent_runtime::doctor::Severity::Warn)
        .count();
    let error_count = results
        .iter()
        .filter(|r| r.severity == dx_agent_runtime::doctor::Severity::Error)
        .count();
...
/// `dx_agent_memory::create_memory_for_agent` so SQL backends filter by
...
async fn resolve_memory_handle(
...
) -> Result<std::sync::Arc<dyn dx_agent_memory::Memory>, (StatusCode, Json<serde_json::Value>)> {
    ...
    dx_agent_memory::create_memory_for_agent(&config, alias, api_key.as_deref())
        .await
        .map_err(|e| {
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn handle_api_memory_list(
    ...
    if params.query.is_some() || params.since.is_some() || params.until.is_some() {
        ...
        let category = params.category.as_deref().map(|cat| match cat {
            "core" => dx_agent_memory::MemoryCategory::Core,
            "daily" => dx_agent_memory::MemoryCategory::Daily,
            "conversation" => dx_agent_memory::MemoryCategory::Conversation,
            other => dx_agent_memory::MemoryCategory::Custom(other.to_string()),
        });
...
pub async fn handle_api_memory_store(
    ...
    let category = body
        ...
        .unwrap_or(dx_agent_memory::MemoryCategory::Core);
...
pub async fn handle_api_cli_tools(
    ...
    let tools = dx_agent_tools::cli_discovery::discover_cli_tools(&[], &[]);
...
pub async fn handle_api_channels(
    ...
    let channels: Vec<serde_json::Value> = config
        ...
        .map(|info| {
            ...
            let compiled = dx_agent_channels::listing::is_channel_type_compiled(compiled_key);
...
fn channel_readiness(
    ...
    info: &dx_agent_config::schema::ChannelAliasInfo,
    health: &dx_agent_runtime::health::HealthSnapshot,
    state: &AppState,
) -> ChannelReadiness {
...
fn component_health_ok_and_fresh(component: &dx_agent_runtime::health::ComponentHealth) -> bool {
...
pub async fn handle_api_health(
    ...
    let snapshot = dx_agent_runtime::health::snapshot();
...
pub async fn handle_api_session_message_post(
    ...
    let message = dx_agent_providers::ChatMessage::assistant(&body.content);
...
pub async fn handle_claude_code_hook(
    State(state): State<AppState>,
    Json(payload): Json<dx_agent_tools::claude_code_runner::ClaudeCodeHookEvent>,
) -> impl IntoResponse {
    ...
    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"session_id": payload.session_id, "event_type": payload.event_type, "tool_name": payload.tool_name, "summary": payload.summary})), "Claude Code hook event received");
...
mod tests {
    ...
    use dx_agent_infra::session_store::SessionStore;
    use dx_agent_memory::{Memory, MemoryCategory, MemoryEntry};
    use dx_agent_providers::ModelProvider;
    use dx_agent_runtime::security::pairing::PairingGuard;
    ...
    fn with_test_agent(
        mut config: dx_agent_config::schema::Config,
    ) -> dx_agent_config::schema::Config {
    ...
    fn test_state(config: dx_agent_config::schema::Config) -> AppState {
        AppState {
    ...
    fn api_channels_readiness_key_tracks_whatsapp_backend_type() {
        ...
        config.channels.whatsapp.insert(
            "web".to_string(),
            dx_agent_config::schema::WhatsAppConfig {
                enabled: true,
        ...
        let web = dx_agent_config::schema::ChannelAliasInfo {
            channel_type: "whatsapp".to_string(),
            alias: "web".to_string(),
            owning_agent: None,
            enabled: true,
        };
        let cloud = dx_agent_config::schema::ChannelAliasInfo {
            channel_type: "whatsapp".to_string(),
            alias: "cloud".to_string(),
            owning_agent: None,
            enabled: true,
        };
        let ambiguous = dx_agent_config::schema::ChannelAliasInfo {
            channel_type: "whatsapp".to_string(),
            alias: "ambiguous".to_string(),
            owning_agent: None,
            enabled: true,
        };
        let discord = dx_agent_config::schema::ChannelAliasInfo {
    ...
    async fn api_channels_marks_configured_uncompiled_channel_unavailable() {
        ...
        config.channels.nextcloud_talk.insert(
            "default".to_string(),
            dx_agent_config::schema::NextcloudTalkConfig {
                enabled: true,
    ...
    fn config_with_webhook(
        ...
        config.channels.webhook.insert(
            alias.to_string(),
            dx_agent_config::schema::WebhookConfig {
                enabled,
                port,
        ...
        if bound {
            config.agents.insert(
                "rowan".to_string(),
                dx_agent_config::schema::AliasedAgentConfig {
                    channels: vec![dx_agent_config::providers::ChannelRef::new(format!(
                        "webhook.{alias}"
                    ))],
                    ..Default::default()
                },
    ...
    fn config_with_telegram(alias: &str) -> dx_agent_config::schema::Config {
        let mut config = dx_agent_config::schema::Config::default();
    ...
    fn first_channel_info(
        config: &dx_agent_config::schema::Config,
    ) -> dx_agent_config::schema::ChannelAliasInfo {
        config
            .channels_by_alias()
            .into_iter()
            .next()
    ...
    fn channel_readiness_webhook_uses_supervised_listener_health_for_custom_path() {
        ...
        dx_agent_runtime::health::mark_component_ok("channel:webhook.supervised");
        let state = test_state(config.clone());
        let health = dx_agent_runtime::health::snapshot();
        let info = first_channel_info(&config);
        let readiness = channel_readiness(&config, &info, &health, &state);
    ...
    fn channel_readiness_webhook_rejects_stale_listener_health() {
        ...
        .to_rfc3339();
        let health = dx_agent_runtime::health::HealthSnapshot {
            pid: std::process::id(),
            updated_at: chrono::Utc::now().to_rfc3339(),
            uptime_seconds: 1,
            components: std::collections::BTreeMap::from([(
                component,
                dx_agent_runtime::health::ComponentHealth {
                    status: "ok".to_string(),
                    updated_at: old,
                    last_ok: None,
                    last_error: None,
    ...
    fn channel_readiness_webhook_uses_live_pairing_guard_for_auth() {
        ...
        dx_agent_runtime::health::mark_component_ok("channel:webhook.paired");
    ...
    async fn api_channels_serializes_readiness_without_duplicate_summary_fields() {
        ...
        let webhook_compiled = dx_agent_channels::listing::is_channel_type_compiled("webhook");
    ...
    async fn session_message_post_persists_and_broadcasts_to_session() {
        ...
        let config = dx_agent_config::schema::Config {
            data_dir: tmp.path().join("workspace"),
            config_path: tmp.path().join("config.toml"),
            ..dx_agent_config::schema::Config::default()
        };
        ...
        backend
            .append(
                "gw_operator-1",
                &dx_agent_providers::ChatMessage::assistant("existing"),
            )
            .unwrap();
    ...
    async fn cron_api_accepts_agent_jobs() {
        ...
        let config = state.config.read().clone();
        let jobs = dx_agent_runtime::cron::list_jobs(&config).unwrap();
        assert_eq!(jobs.len(), 1);
        assert_eq!(jobs[0].job_type, dx_agent_runtime::cron::JobType::Agent);
    ...
    async fn cron_api_timezone_add_persists_explicit_timezone() {
        ...
        assert_eq!(
            jobs[0].schedule,
            dx_agent_runtime::cron::Schedule::Cron {
                expr: "0 9 * * *".to_string(),
    ...
    async fn cron_api_timezone_patch_schedule_preserves_existing_timezone() {
        ...
        std::fs::create_dir_all(&config.data_dir).unwrap();
        let state = test_state(with_test_agent(config));
        let job = dx_agent_runtime::cron::add_shell_job_with_approval(
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let updated = dx_agent_runtime::cron::get_job(&state.config.read().clone(), &job.id)
            .expect("updated job");
        assert_eq!(
            updated.schedule,
            dx_agent_runtime::cron::Schedule::Cron {
    ...
    async fn cron_api_rejects_announce_delivery_without_target() {
        ...
        let config = state.config.read().clone();
        assert!(
            dx_agent_runtime::cron::list_jobs(&config)
                .unwrap()
                .is_empty()
        );
    }
    ...
    async fn cron_api_run_executes_shell_job_and_records_run() {
        ...
        let runs = dx_agent_runtime::cron::list_runs(&state.config.read().clone(), &job.id, 10)
            .expect("runs listed");
    ...
    async fn cron_api_run_records_best_effort_delivery_failure_as_degraded() {
        ...
        let job = dx_agent_runtime::cron::add_shell_job_with_approval(
            ...
            "echo hello-from-manual-trigger",
            Some(dx_agent_runtime::cron::DeliveryConfig {
                mode: "announce".into(),
        ...
        let config = state.config.read().clone();
        let updated = dx_agent_runtime::cron::get_job(&config, &job.id).expect("updated job");
        ...
        let runs = dx_agent_runtime::cron::list_runs(&config, &job.id, 10).expect("runs listed");
        assert_eq!(runs.len(), 1);
