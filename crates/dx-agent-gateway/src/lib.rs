...
use dx_agent_api::channel::{Channel, SendMessage};
use dx_agent_api::tool::ToolSpec;
#[cfg(feature = "channel-email")]
use dx_agent_channels::gmail_push::GmailPushChannel;
#[cfg(feature = "channel-linq")]
use dx_agent_channels::linq::LinqChannel;
#[cfg(feature = "channel-nextcloud")]
use dx_agent_channels::nextcloud_talk::NextcloudTalkChannel;
#[cfg(feature = "channel-wati")]
use dx_agent_channels::wati::WatiChannel;
#[cfg(feature = "channel-whatsapp-cloud")]
use dx_agent_channels::whatsapp::WhatsAppChannel;
use dx_agent_config::policy::SecurityPolicy;
use dx_agent_config::schema::Config;
use dx_agent_infra::session_backend::SessionBackend;
use dx_agent_memory::{self, Memory, MemoryCategory};
use dx_agent_providers::{self, ModelProvider};
use dx_agent_runtime::cost::CostTracker;
use dx_agent_runtime::i18n;
use dx_agent_runtime::platform;
use dx_agent_runtime::security::pairing::{PairingGuard, constant_time_eq, is_public_bind};
use dx_agent_runtime::tools;
use dx_agent_runtime::tools::CanvasStore;
...
pub fn gateway_request_timeout_secs(cfg: &dx_agent_config::schema::GatewayConfig) -> u64 {
...
pub fn gateway_long_running_request_timeout_secs(
    cfg: &dx_agent_config::schema::GatewayConfig,
) -> u64 {
...
#[cfg(feature = "channel-whatsapp-cloud")]
fn whatsapp_memory_key(msg: &dx_agent_api::channel::ChannelMessage) -> String {
    format!("whatsapp_{}_{}", msg.sender, msg.id)
}

#[cfg(feature = "channel-linq")]
fn linq_memory_key(msg: &dx_agent_api::channel::ChannelMessage) -> String {
    format!("linq_{}_{}", msg.sender, msg.id)
}

#[cfg(feature = "channel-wati")]
fn wati_memory_key(msg: &dx_agent_api::channel::ChannelMessage) -> String {
    format!("wati_{}_{}", msg.sender, msg.id)
}

#[cfg(feature = "channel-nextcloud")]
fn nextcloud_talk_memory_key(msg: &dx_agent_api::channel::ChannelMessage) -> String {
...
fn sender_session_id(channel: &str, msg: &dx_agent_api::channel::ChannelMessage) -> String {
    match &msg.thread_ts {
...
pub struct AppState {
    ...
    pub observer: Arc<dyn dx_agent_runtime::observability::Observer>,
    ...
    pub tui_registry: Option<Arc<dx_agent_runtime::rpc::tui_identity::TuiRegistry>>,
}
...
pub async fn run_gateway(
    ...
    tui_registry: Option<Arc<dx_agent_runtime::rpc::tui_identity::TuiRegistry>>,
    ...
    // ── Hooks ──────────────────────────────────────────────────────
    let hooks: Option<std::sync::Arc<dx_agent_runtime::hooks::HookRunner>> = if config.hooks.enabled
    {
        Some(std::sync::Arc::new(
            dx_agent_runtime::hooks::HookRunner::new(),
        ))
    } else {
        None
    };
    ...
    let (model_provider, boot_provider_failed): (Arc<dyn ModelProvider>, bool) =
        match dx_agent_providers::create_resilient_model_provider_from_ref(
            &config,
            model_provider_name,
            fallback.and_then(|e| e.api_key.as_deref()),
            fallback.and_then(|e| e.uri.as_deref()),
            &config.reliability,
            &dx_agent_providers::provider_runtime_options_for_alias(
                &config,
                &boot_family,
                &boot_alias,
            ),
    ...
    let model = if boot_provider_failed {
            ...
            .filter(|m| !m.is_empty())
        {
            ...
            None => match config.resolve_default_model() {
                Some(m) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model_provider": model_provider_name, "model": m})), "first model_provider has no `model` set; using first configured \
    ...
    let mem: Arc<dyn Memory> = if config.agents.is_empty() {
        ...
        match dx_agent_memory::create_memory_with_storage_and_routes(
            &config.memory,
    ...
    let agent_setup: Option<(
        ...
        let Some(risk_profile) = config.risk_profile_for_agent(agent_alias) else {
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": agent_alias, "agent_alias": agent_alias})), "Gateway: agents..risk_profile does not name a configured risk_profiles entry; booting with empty tools registry. Fix via /admin/reload or /quickstart.");
            return None;
        };
        ...
        let security = match SecurityPolicy::for_agent(&config, agent_alias) {
            Ok(s) => Arc::new(s),
            Err(e) => {
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": agent_alias, "error": format!("{}", e), "agent_alias": agent_alias})), "Gateway: agent SecurityPolicy failed to build; booting with empty tools registry. Fix [agents.] via /admin/reload or /quickstart.");
                return None;
            }
        };
    ...
    let (mut tools_registry_raw, delegate_handle_gw) = match (&agent_alias_opt, agent_setup) {
        (Some(agent_alias), Some((risk_profile, security))) => {
            ...
            let channel_names = dx_agent_channels::orchestrator::register_channels_for_tools(
                &config,
    ...
    // ── Tunnel ────────────────────────────────────────────────
    let tunnel = match dx_agent_runtime::tunnel::create_tunnel(&config.tunnel) {
        Ok(t) => t,
        Err(e) => {
            ::dx_agent_log::record!(
                WARN,
    ...
    let auto_detect_web_dist = || -> Option<std::path::PathBuf> {
        let mut candidates = vec![
            ...
            std::path::PathBuf::from("/dx-agent-data/web/dist"),
            // AUR / system package
    ...
    println!("🦀 DX Agent Gateway listening on http://{display_addr}{pfx}");
    ...
    dx_agent_runtime::health::mark_component_ok("gateway");
    ...
    let broadcast_layer: Arc<dyn dx_agent_runtime::observability::Observer> = Arc::new(
        sse::BroadcastObserver::new(event_tx.clone(), event_buffer.clone()),
    );
    let broadcast_hook_guard =
        dx_agent_runtime::observability::set_scoped_broadcast_hook(broadcast_layer);

    // Install the same broadcast sender as dx-agent-log's canonical
    ...
    dx_agent_log::set_broadcast_hook(event_tx.clone());
    ...
    let state_observer: Arc<dyn dx_agent_runtime::observability::Observer> = Arc::from(
        dx_agent_runtime::observability::create_observer(&config.observability),
    );
    ...
    let state = AppState {
        ...
        webauthn: if config.security.webauthn.enabled {
            let secret_store = Arc::new(dx_agent_runtime::security::SecretStore::new(
                &config.data_dir,
                true,
            ));
            let wa_config = dx_agent_runtime::security::webauthn::WebAuthnConfig {
                enabled: true,
            ...
            Some(Arc::new(api_webauthn::WebAuthnState {
                manager: dx_agent_runtime::security::webauthn::WebAuthnManager::new(
                    wa_config,
                    secret_store,
                    &config.data_dir,
                ),
    ...
    if let Some(tls_acceptor) = tls_acceptor {
                ...
                conn = listener.accept() => {
                    ...
                    dx_agent_spawn::spawn!(async move {
                        let tls_stream = match tls_acceptor.accept(tcp_stream).await {
                            Ok(s) => s,
                            Err(e) => {
                                ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"error": format!("{}", e), "remote_addr": remote_addr})), "TLS handshake failed from");
                                return;
                            }
                        };
                        ...
                        .await
                        {
                            ::dx_agent_log::record!(DEBUG, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"error": format!("{}", e), "remote_addr": remote_addr})), "connection error from");
                        }
                    });
                }
                _ = shutdown_signal.changed() => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "DX Agent Gateway shutting down");
                    break;
                }
            }
        }
        ...
        .with_graceful_shutdown(async move {
            ...
            ::dx_agent_log::record!(
                INFO,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note),
                "DX Agent Gateway shutting down"
            );
        })
        .await?;
    }
...
fn format_paircode_recovery_command(host: &str, port: u16) -> String {
    let mut cmd = format!("dx-agent gateway get-paircode --new --port {port}");
...
async fn handle_health(State(state): State<AppState>) -> impl IntoResponse {
    let body = serde_json::json!({
        ...
        "runtime": dx_agent_runtime::health::snapshot_json(),
    });
    Json(body)
}
...
fn prometheus_observer_from_state(
    observer: &dyn dx_agent_runtime::observability::Observer,
) -> Option<&dx_agent_runtime::observability::PrometheusObserver> {
    ...
    observer
        .as_any()
        .downcast_ref::<dx_agent_runtime::observability::PrometheusObserver>()
}
...
impl ::dx_agent_api::attribution::Attributable for UnconfiguredModelProvider {
...
/// `crates/dx-agent-runtime/locales/<locale>/cli.ftl`) so non-English
...
async fn run_gateway_chat_with_tools(
    ...
    #[cfg(not(test))]
    {
        ...
        let cost_tracking_context = state.cost_tracker.as_ref().map(|tracker| {
            let pricing: dx_agent_runtime::agent::cost::ModelProviderPricing = config
                .providers
                .models
                .iter_entries()
                ...
                .collect();
            dx_agent_runtime::agent::cost::ToolLoopCostTrackingContext::new(
                tracker.clone(),
        ...
        let response = Box::pin(
            dx_agent_runtime::agent::cost::TOOL_LOOP_COST_TRACKING_CONTEXT.scope(
                cost_tracking_context,
                dx_agent_runtime::agent::process_message(config, &agent_alias, message, session_id),
            ),
        )
        .await?;
...
async fn handle_webhook(
    ...
    if state.auto_save && !dx_agent_memory::should_skip_autosave_content(message) {
    ...
    state.observer.record_event(
        &dx_agent_runtime::observability::ObserverEvent::AgentStart {
            model_provider: provider_label.clone(),
            model: model_label.clone(),
        },
    );
    state.observer.record_event(
        &dx_agent_runtime::observability::ObserverEvent::LlmRequest {
            model_provider: provider_label.clone(),
            model: model_label.clone(),
            messages_count: 1,
        },
    ...
    match run_gateway_chat_with_tools(&state, message, session_id.as_deref()).await {
        Ok(GatewayChatOutcome {
            ...
            let tokens_used = input_tokens
                ...
                .or(output_tokens);
            state.observer.record_event(
                &dx_agent_runtime::observability::ObserverEvent::LlmResponse {
                    model_provider: provider_label.clone(),
                    model: model_label.clone(),
                    duration,
                    success: true,
            ...
            state.observer.record_metric(
                &dx_agent_runtime::observability::traits::ObserverMetric::RequestLatency(duration),
            );
            state.observer.record_event(
                &dx_agent_runtime::observability::ObserverEvent::AgentEnd {
        ...
        Err(e) => {
            let duration = started_at.elapsed();
            let sanitized = dx_agent_providers::sanitize_api_error(&e.to_string());
            ...
            state
                .observer
                .record_event(&dx_agent_runtime::observability::ObserverEvent::Error {
...
async fn handle_whatsapp_message(
    ...
    for msg in &messages {
        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"channel": "whatsapp", "sender": msg.sender, "content": msg.content})), "inbound webhook message");

        // Route approval replies to pending approval requests before dispatching to agent
        if let Some((token, response)) = dx_agent_channels::util::parse_approval_reply(&msg.content)
        {
        ...
        // Auto-save to memory
        if state.auto_save && !dx_agent_memory::should_skip_autosave_content(&msg.content) {
...
async fn handle_linq_webhook(
    ...
    if let Some(ref signing_secret) = state.linq_signing_secret {
        ...
        if !dx_agent_channels::linq::verify_linq_signature(
            signing_secret,
            &body_str,
            timestamp,
            signature,
    ...
    // Process each message
    for msg in &messages {
        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"channel": "linq", "sender": msg.sender, "content": msg.content})), "inbound webhook message");
...
async fn handle_wati_webhook(State(state): State<AppState>, body: Bytes) -> impl IntoResponse {
    ...
    // Process each message
    for msg in &messages {
        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"channel": "wati", "sender": msg.sender, "content": msg.content})), "inbound webhook message");
...
async fn handle_nextcloud_talk_webhook(
    ...
    if let Some(ref webhook_secret) = state.nextcloud_talk_webhook_secret {
        ...
        if !dx_agent_channels::nextcloud_talk::verify_nextcloud_talk_signature(
            webhook_secret,
            random,
            &body_str,
            signature,
    ...
    for msg in messages {
        ...
        dx_agent_spawn::spawn!(async move {
            ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"channel": "nextcloud_talk", "sender": msg.sender, "content": msg.content})), "inbound webhook message");
            ...
            .await
            {
                ...
                Err(e) => {
                    let reply = if is_needs_quickstart_err(&e) {
                        ...
                        ::dx_agent_log::record!(ERROR, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_outcome(::dx_agent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"channel": "nextcloud_talk", "error": format!("{}", e)})), "LLM error");
                        "Sorry, I couldn't process your message right now.".to_string()
                    };
...
async fn handle_gmail_push_webhook(
    ...
    let body_str = String::from_utf8_lossy(&body);
    let envelope: dx_agent_channels::gmail_push::PubSubEnvelope =
...
mod tests {
    ...
    #[cfg(feature = "channel-whatsapp-cloud")]
    use dx_agent_api::channel::ChannelMessage;
    use dx_agent_memory::{Memory, MemoryCategory, MemoryEntry};
    use dx_agent_providers::ModelProvider;
    ...
    fn gateway_timeout_uses_typed_config_default() {
        let cfg = dx_agent_config::schema::GatewayConfig::default();
    ...
    fn paircode_recovery_command_includes_alternate_port() {
        assert_eq!(
            ...
            "dx-agent gateway get-paircode --new --port 42617"
        );
    }
    ...
    fn paircode_recovery_command_includes_specific_host_when_needed() {
        assert_eq!(
            ...
            "dx-agent gateway get-paircode --new --port 42617 --host 192.168.1.20"
        );
    }
    ...
    async fn run_gateway_starts_with_unresolved_agent_risk_profile() {
        use dx_agent_config::schema::AliasedAgentConfig;
    ...
    async fn run_gateway_starts_with_mismatched_provider_api_key() {
        ...
        config.providers.models.anthropic.insert(
            "default".to_string(),
            dx_agent_config::schema::AnthropicModelProviderConfig {
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        ...
        let prom = dx_agent_runtime::observability::PrometheusObserver::new();
        dx_agent_runtime::observability::Observer::record_event(
            &prom,
            &dx_agent_runtime::observability::ObserverEvent::HeartbeatTick,
        );

        let observer: Arc<dyn dx_agent_runtime::observability::Observer> = Arc::new(prom);
        let state = AppState {
            config: Arc::new(RwLock::new(Config::default())),
            model_provider: Arc::new(MockModelProvider::default()),
            model: "test-model".into(),
    ...
    async fn persist_pairing_tokens_writes_config_tokens() {
        ...
        assert!(
            dx_agent_runtime::security::SecretStore::is_encrypted(on_disk),
    ...
    impl ::dx_agent_api::attribution::Attributable for MockMemory {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
    ...
    impl ::dx_agent_api::attribution::Attributable for TrackingMemory {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Memory(
                ::dx_agent_api::attribution::MemoryKind::InMemory,
            )
    ...
    #[cfg(feature = "channel-nextcloud")]
    impl ::dx_agent_api::attribution::Attributable for SlowProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
