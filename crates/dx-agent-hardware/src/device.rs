...
pub enum DeviceKind {
    ...
    Nucleo,
    /// Unknown VID that passed the DX Agent firmware ping handshake.
    Generic,
...
impl DeviceRegistry {
    ...
    ///    if the device responds with DX Agent firmware.
    ...
    pub async fn discover() -> Self {
        ...
        for info in scan_serial_devices() {
            ...
            let probe_transport = if !is_known_vid {
                ...
                if !probe.ping_handshake().await {
                    ::dx_agent_log::record!(
                        ...
                        "skipping unknown device: no DX Agent firmware response"
                    );
                    continue;
                }
                Some(probe)
            ...
            ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"alias": alias, "port": info.port_path, "vid": info.vid})), "device registered");
        }
...
impl DeviceRegistry {
    ...
    /// DX Agent firmware is alive, and re-attaches the transport.
    ///
