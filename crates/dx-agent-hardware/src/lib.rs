...
pub use dx_agent_config::schema::{HardwareConfig, HardwareTransport};
...
pub fn merge_hardware_tools(
    tools: &mut Vec<Box<dyn dx_agent_api::tool::Tool>>,
    ...
    if !hw_boot.tools.is_empty() {
        ...
        let new_hw_tools: Vec<Box<dyn dx_agent_api::tool::Tool>> = hw_boot
            .tools
            .into_iter()
...
pub struct HardwareBootResult {
    ...
    pub tools: Vec<Box<dyn dx_agent_api::tool::Tool>>,
    /// Human-readable device summary for the LLM system prompt.
    pub device_summary: String,
    /// Content of `~/.dx-agent/hardware/` context files (HARDWARE.md, device
    /// profiles, and skills) for injection into the system prompt.
    pub context_files_prompt: String,
}

/// Load hardware context files from `~/.dx-agent/hardware/` and return them
/// concatenated as a single markdown string ready for system-prompt injection.
///
/// Reads (if they exist):
/// 1. `~/.dx-agent/hardware/HARDWARE.md`
/// 2. `~/.dx-agent/hardware/devices/<alias>.md` for each discovered alias
/// 3. All `~/.dx-agent/hardware/skills/*.md` files (sorted by name)
///
...
pub fn load_hardware_context_prompt(aliases: &[&str]) -> String {
    ...
    load_hardware_context_from_dir(&home.join(".dx-agent").join("hardware"), aliases)
}
...
///
/// Without the feature: loads plugin tools from `~/.dx-agent/tools/` only,
...
pub async fn boot(
    peripherals: &dx_agent_config::schema::PeripheralsConfig,
    ...
    if uf2::find_rpi_rp2_mount().is_some() {
        ...
        ::dx_agent_log::record!(
            INFO,
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note),
            "Say \"flash my pico\" to install DX Agent firmware automatically"
        );
    }
...
#[allow(unused_mut)] // tools and context_files_prompt are mutated on Linux+peripheral-rpi
pub async fn boot(
    _peripherals: &dx_agent_config::schema::PeripheralsConfig,
) -> anyhow::Result<HardwareBootResult> {
