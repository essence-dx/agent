//! Deploy DX Agent Bridge app to Arduino Uno Q.
...
fn deploy_remote(host: &str, bridge_dir: &std::path::Path) -> Result<()> {
    ...
    println!("DX Agent Bridge app started. Add to config.toml:");
...
fn deploy_local(bridge_dir: Option<&std::path::Path>) -> Result<()> {
    ...
    println!("DX Agent Bridge app started.");
    Ok(())
}
...
mod tests {
    ...
    fn write_embedded_bridge_main_py_contains_zeroclaw() {
        ...
        let main_py = std::fs::read_to_string(dest.join("python").join("main.py")).unwrap();
        assert!(
            main_py.contains("DX Agent") || main_py.contains("zeroclaw"),
            "main.py should contain DX Agent marker"
        );
    }
