...
use dx_agent_api::attribution::ToolKind;
use dx_agent_api::tool::{Tool, ToolResult};
use dx_agent_api::tool_attribution;
...
impl Tool for DeviceReadCodeTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"alias": alias, "port": port, "runtime": format!("{:?}", runtime)})), "reading main.py from device");
...
impl Tool for DeviceWriteCodeTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"alias": alias, "port": port, "runtime": format!("{:?}", runtime), "code_len": code.len()})), "writing main.py to device");
...
impl Tool for DeviceExecTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"alias": alias, "port": port, "runtime": format!("{:?}", runtime), "code_len": code.len()})), "executing snippet on device");
