//! Plugin manifest loader — scans `~/.dx-agent/tools/` at startup.
//!
...
/// Scan `~/.dx-agent/tools/` and return all valid plugins.
///
...
/// Return the path `~/.dx-agent/tools/` using the `directories` crate.
pub fn plugin_tools_dir() -> Result<PathBuf> {
    ...
    Ok(base.home_dir().join(".dx-agent").join("tools"))
}
...
"#;
    ...
    fn scan_plugin_dir_skips_broken_manifests_without_panicking() {
        ...
        // always uses ~/.dx-agent/tools), but we can verify load_one_plugin
    ...
    #[test]
    fn plugin_tools_dir_returns_path_ending_in_dx_agent_tools() {
        let path = plugin_tools_dir().expect("should resolve");
        let display = path.to_string_lossy();
        let expected = std::path::Path::new(".dx-agent").join("tools");
