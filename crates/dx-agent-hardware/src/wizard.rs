...
//!
//! Provides [`run_setup`] — the hardware step of the DX Agent onboarding
...
use dx_agent_config::schema::{HardwareConfig, HardwareTransport};
...
pub fn run_setup() -> Result<HardwareConfig> {
    println!(
        "  {} {}",
        style("ℹ").dim(),
        style("DX Agent can talk to physical hardware (LEDs, sensors, motors).").dim()
    );
    println!(
        "  {} {}",
        style("ℹ").dim(),
    ...
    let choice = Select::new()
        .with_prompt("  How should DX Agent interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
