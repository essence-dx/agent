...
//!
//! Wire protocol (DX Agent serial JSON):
//! ```text
...
/// Default baud rate for DX Agent serial devices.
...
/// Serial transport for DX Agent hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify DX Agent firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "zeroclaw"`.
    ///
    /// Returns `true` if a DX Agent device responds, `false` otherwise.
