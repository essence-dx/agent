...
use dx_agent_config::schema::v2::V2Config;
use dx_agent_config::secrets::SecretStore;
...
[extra_headers]
"User-Agent" = "DX Agent-V1-Test/1.0"
"#;
...
"#;
    ...
    let raw_value: toml::Value = toml::from_str(
        &dx_agent_config::migration::migrate_file(raw)
            .expect("migrate_file succeeds")
...
nested_value = 42
"#;
    let migrated = dx_agent_config::migration::migrate_file(raw)
        .expect("migrate_file succeeds")
        .expect("migration ran");
...
fn v3_channel_types_covers_every_typed_channel_slot() {
    ...
    use dx_agent_config::schema::Config;
    use dx_agent_config::schema::v2::V3_CHANNEL_TYPES;
    ...
    assert!(
        ...
        "ChannelsConfig has typed channel slots that V3_CHANNEL_TYPES does \
         ...
         the const in `crates/dx-agent-config/src/schema/v2.rs`, or add a \
...
fn map_key_sections_exposes_typed_family_slots() {
    ...
    let paths: std::collections::HashSet<&'static str> = Config::map_key_sections()
        .iter()
        .filter(|s| matches!(s.kind, dx_agent_config::traits::MapKeyKind::Map))
        .map(|s| s.path)
        .collect();
    for required in [
        "providers.models.anthropic",
