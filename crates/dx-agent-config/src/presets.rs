...
//! hand to `dx-agent-runtime`'s apply path. The runtime validates and
...
/// `dx_agent_config::multi_agent::MemoryBackendKind` lights up in
...
/// `dx-agent-runtime::quickstart::apply` (and pre-validate via
