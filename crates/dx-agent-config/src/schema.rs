...
use dx_agent_macros::Configurable;
...
/// Top-level DX Agent configuration, loaded from `config.toml`.
///
/// Resolution order: `DX_AGENT_CONFIG_DIR` env → `DX_AGENT_WORKSPACE` env → `~/.dx-agent/config.toml`.
#[derive(Debug, Clone, Serialize, Deserialize, Configurable)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
pub struct Config {
    /// Shared instance data directory (databases, hygiene state, cost
    /// records, daemon state files). Computed from `DX_AGENT_CONFIG_DIR`
    /// / `DX_AGENT_DATA_DIR` / `DX_AGENT_WORKSPACE` (deprecated) at
    ...
    /// Dotted prop-paths overridden by `DX_AGENT_*` env vars at load time.
    ...
    ///
    /// If omitted or empty, the locale is auto-detected from `DX_AGENT_LOCALE`,
...
//
// The `ModelProviders` container in `crates/dx-agent-config/src/model_providers.rs`
// holds a typed slot per family; the runtime impls in dx-agent-providers
...
pub struct GeminiModelProviderConfig {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub auth_mode: Option<AuthMode>,
    /// Google OAuth app `client_id`, used when this alias drives DX Agent's
...
pub struct AliasedAgentConfig {
    ...
    /// (`classify_channel_reply_intent` in dx-agent-channels). When non-empty,
...
impl Config {
    fn encrypt_route_api_keys(
        &mut self,
        store: &crate::secrets::SecretStore,
    ) -> anyhow::Result<()> {
        for (index, route) in self.model_routes.iter_mut().enumerate() {
            route.encrypt_api_key(store, &format!("model_routes[{index}].api_key"))?;
        }
        for (index, route) in self.embedding_routes.iter_mut().enumerate() {
            route.encrypt_api_key(store, &format!("embedding_routes[{index}].api_key"))?;
        }
        Ok(())
    }

    fn decrypt_route_api_keys(
        &mut self,
        store: &crate::secrets::SecretStore,
    ) -> anyhow::Result<()> {
        for (index, route) in self.model_routes.iter_mut().enumerate() {
            route.decrypt_api_key(store, &format!("model_routes[{index}].api_key"))?;
        }
        for (index, route) in self.embedding_routes.iter_mut().enumerate() {
            route.decrypt_api_key(store, &format!("embedding_routes[{index}].api_key"))?;
        }
        Ok(())
    }

    pub fn mask_route_api_keys(&mut self) {
        for route in &mut self.model_routes {
            route.mask_api_key();
        }
        for route in &mut self.embedding_routes {
            route.mask_api_key();
        }
    }

    pub fn restore_route_api_keys_from(&mut self, current: &Self) {
        for (index, route) in self.model_routes.iter_mut().enumerate() {
            if route
                .api_key
                .as_deref()
                .is_some_and(crate::traits::is_masked_secret)
            {
                route.api_key = restore_model_route_api_key(&current.model_routes, index, route);
            }
        }
        for (index, route) in self.embedding_routes.iter_mut().enumerate() {
            if route
            ...
            {
                route.api_key =
                    restore_embedding_route_api_key(&current.embedding_routes, index, route);
            }
        }
    }
    ...
    // `crates/dx-agent-runtime/src/tools/delegate.rs` already applies for
    ...
    /// `crates/dx-agent-runtime/src/tools/delegate.rs::resolve_brain` for
    ...
    /// `start_channels()` in dx-agent-channels: prefer the migration-
...
pub struct HardwareConfig {
    ...
    #[serde(default)]
    pub enabled: bool,
    /// How DX Agent reaches the hardware: `native` = Linux SBC with direct GPIO access (Raspberry Pi, Orange Pi); `serial` = USB-tethered microcontroller speaking over a TTY; `probe` = SWD/JTAG debug probe driving a target chip via probe-rs; `none` = disabled.
    #[serde(default)]
...
/// execution events back to DX Agent's gateway, updating a Slack message
...
pub enum ProxyScope {
    ...
    Environment,
    /// Apply proxy to all DX Agent-managed HTTP traffic (default).
    #[default]
    Zeroclaw,
    ...
    pub fn apply_to_reqwest_builder(
        ...
        if let Some(url) = normalize_proxy_url_option(self.all_proxy.as_deref()) {
            match reqwest::Proxy::all(&url) {
                ...
                Err(error) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"proxy_url": url, "service_key": service_key, "error": format!("{}", error)})), "Ignoring invalid all_proxy URL: ");
                }
            }
        }
        ...
        if let Some(url) = normalize_proxy_url_option(self.http_proxy.as_deref()) {
            match reqwest::Proxy::http(&url) {
                ...
                Err(error) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"proxy_url": url, "service_key": service_key, "error": format!("{}", error)})), "Ignoring invalid http_proxy URL: ");
                }
            }
        }
        ...
        if let Some(url) = normalize_proxy_url_option(self.https_proxy.as_deref()) {
            match reqwest::Proxy::https(&url) {
                ...
                Err(error) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"proxy_url": url, "service_key": service_key, "error": format!("{}", error)})), "Ignoring invalid https_proxy URL: ");
                }
            }
        }
...
fn build_explicit_proxy_client(
    ...
    let client = builder.build().unwrap_or_else(|error| {
        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"service_key": service_key, "proxy_url": proxy_url, "error": format!("{}", error)})), "Failed to build channel proxy client: ");
        reqwest::Client::new()
    });
    set_runtime_proxy_cached_client(cache_key, client.clone());
    client
...
fn apply_explicit_proxy_to_builder(
    ...
    match reqwest::Proxy::all(proxy_url) {
        ...
        Err(error) => {
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"proxy_url": proxy_url, "service_key": service_key, "error": format!("{}", error)})), "Ignoring invalid channel proxy_url: ");
        }
    }
    builder
}
...
pub struct MemoryConfig {
    ...
    pub backend: String,
    /// Auto-save what *you* tell DX Agent into memory as conversation history — the agent's own replies are not saved. Turn off if you want memory to only hold things you explicitly record via the memory tool.
...
pub struct ObservabilityConfig {
    ...
    /// Controls whether every event passing through `dx_agent_log::record!`
...
impl RiskProfileConfig {
    ...
    /// `dx-agent-runtime::security::detect`) can call this helper.
    #[must_use]
...
impl ModelRouteConfig {
    fn encrypt_api_key(
        ...
        crate::traits::SecretField::encrypt_in_place(&mut self.api_key, store, path)
    }
    ...
    fn decrypt_api_key(
        ...
        crate::traits::SecretField::decrypt_in_place(&mut self.api_key, store, path)
    }

    fn mask_api_key(&mut self) {
        crate::traits::SecretField::mask(&mut self.api_key);
    }
}
...
fn restore_model_route_api_key(
    current: &[ModelRouteConfig],
    index: usize,
    route: &ModelRouteConfig,
) -> Option<String> {
    unique_route_api_key(current, |existing| {
        existing.hint == route.hint
            && existing.model_provider == route.model_provider
            && existing.model == route.model
    })
    .or_else(|| unique_route_api_key(current, |existing| existing.hint == route.hint))
    .or_else(|| {
        current
            .get(index)
...
fn restore_embedding_route_api_key(
    current: &[EmbeddingRouteConfig],
    index: usize,
    ...
    unique_route_api_key(current, |existing| {
        existing.hint == route.hint
            && existing.model_provider == route.model_provider
            && existing.model == route.model
            && existing.dimensions == route.dimensions
    })
    .or_else(|| unique_route_api_key(current, |existing| existing.hint == route.hint))
    .or_else(|| {
        current
...
fn unique_route_api_key<Route>(
    routes: &[Route],
    mut matches_route: impl FnMut(&Route) -> bool,
) -> Option<String>
where
    Route: RouteApiKey,
{
    let mut matches = routes.iter().filter(|route| matches_route(route));
    let first = matches.next()?;
    if matches.next().is_some() {
        return None;
    }
    first.api_key().cloned()
}
...
impl RouteApiKey for ModelRouteConfig {
    fn api_key(&self) -> Option<&String> {
        self.api_key.as_ref()
    }
}

impl RouteApiKey for EmbeddingRouteConfig {
    fn api_key(&self) -> Option<&String> {
        self.api_key.as_ref()
    }
}
...
impl ChannelsConfig {
    ...
    /// `dx_agent_channels::listing::compiled_channels` instead.
...
pub struct MatrixConfig {
    ...
    /// When set, DX Agent recovers room keys and cross-signing secrets on startup.
    #[secret]
    #[credential_class = "encrypted_secret"]
    #[tab(Connection)]
    #[cfg_attr(feature = "schema-export", schemars(extend("x-secret" = true)))]
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `DX_AGENT_WHATSAPP_APP_SECRET` environment variable
    /// Only used in Cloud API mode
    #[serde(default)]
    #[secret]
    #[tab(Connection)]
...
pub struct NextcloudTalkConfig {
    ...
    ///
    /// Can also be set via `DX_AGENT_NEXTCLOUD_TALK_WEBHOOK_SECRET`.
    #[serde(default)]
    #[secret]
    #[tab(Connection)]
    #[cfg_attr(feature = "schema-export", schemars(extend("x-secret" = true)))]
...
/// How DX Agent receives events from Feishu / Lark.
///
...
pub enum LineDmPolicy {
    ...
    /// DX Agent prints the bind code on startup; send it once to unlock access.
    #[default]
    Pairing,
...
pub struct WebAuthnConfig {
    ...
    /// Relying Party display name. Default: "DX Agent".
...
///
/// When `enabled` is true, DX Agent validates incoming requests against a Nevis
...
pub struct NevisConfig {
    ...
    /// Nevis role to DX Agent permission mappings.
    #[serde(default)]
...
/// Maps a Nevis role to DX Agent tool permissions and workspace access.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
#[serde(deny_unknown_fields)]
pub struct NevisRoleMappingConfig {
    ...
    #[serde(default)]
    pub dx_agent_permissions: Vec<String>,
...
#[derive(Debug, Clone, Serialize, Deserialize, dx_agent_macros::Configurable)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
#[prefix = "voice_wake"]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let dx_agent_dir = home.join(".dx-agent");

        Self {
            data_dir: dx_agent_dir.join("data"),
            config_path: dx_agent_dir.join("config.toml"),
...
fn default_config_dir() -> Result<PathBuf> {
    if let Some((_name, custom)) =
        non_empty_env(&["DX_AGENTS_CONFIG_DIR", "DX_AGENT_CONFIG_DIR"], true)
    {
        return Ok(expand_tilde_path(custom.trim()));
    }

    if let Ok(home) = std::env::var("HOME")
        && !home.is_empty()
    {
        return Ok(PathBuf::from(home).join(".dx-agent"));
    }

    let home = UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    Ok(home.join(".dx-agent"))
}
...
/// `<config_dir>` honors `DX_AGENT_CONFIG_DIR` and otherwise defaults to
/// `~/.dx-agent`. The zerocode binary mirrors this path inline (it carries no
/// `zeroclaw-*` dependency).
...
pub const FTL_CATALOGS: &[(&str, &str, &str)] = &[
    (
        "cli",
        "crates/dx-agent-runtime/locales/{locale}/cli.ftl",
        "cli.ftl",
    ),
    (
        "tools",
        "crates/dx-agent-runtime/locales/{locale}/tools.ftl",
        "tools.ftl",
    ),
    (
        "zerocode",
...
/// (`/home/<user>/.dx-agent/<relative>` on Linux,
/// `C:\Users\<user>\.dx-agent\<relative>` on Windows, etc.) instead of a
/// literal `~/...` token that doesn't expand on Windows. Falls back to
/// `~/.dx-agent/<relative>` if the platform dir can't be resolved (rare —
...
fn default_path_under_config_dir(relative: &str) -> String {
    match default_config_dir() {
        ...
        Err(_) => format!("~/.dx-agent/{relative}"),
    }
}
...
pub fn resolve_config_dir_for_data(data_dir: &Path) -> (PathBuf, PathBuf) {
    ...
    let legacy_config_dir = data_dir.parent().map(|parent| parent.join(".dx-agent"));
...
/// `DX_AGENTS_CONFIG_DIR` > `DX_AGENTS_DATA_DIR` > `DX_AGENTS_WORKSPACE`
/// > legacy `DX_AGENT_*` aliases > defaults.
pub async fn resolve_runtime_dirs() -> Result<(PathBuf, PathBuf)> {
    let (default_dx_agent_dir, default_data_dir) = default_config_and_data_dirs()?;
    let (config_dir, data_dir, _) =
        resolve_runtime_config_dirs(&default_dx_agent_dir, &default_data_dir).await?;
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::EnvConfigDir => "DX_AGENTS_CONFIG_DIR",
            Self::EnvDataDir => "DX_AGENTS_DATA_DIR",
            Self::EnvWorkspaceLegacy => "DX_AGENTS_WORKSPACE",
...
fn non_empty_env(names: &[&'static str], trim: bool) -> Option<(&'static str, String)> {
...
///
/// Homebrew can execute DX Agent from `<prefix>/Cellar/zeroclaw/<version>/bin/`,
...
async fn resolve_runtime_config_dirs(
    default_dx_agent_dir: &Path,
    default_data_dir: &Path,
) -> Result<(PathBuf, PathBuf, ConfigResolutionSource)> {
    if let Some((config_env_name, custom_config_dir)) =
        non_empty_env(&["DX_AGENTS_CONFIG_DIR", "DX_AGENT_CONFIG_DIR"], true)
    {
        let custom_config_dir = custom_config_dir.trim();
        // If the operator ALSO set DATA_DIR or WORKSPACE, CONFIG_DIR wins;
        // surface the collision so they know which one took effect.
        if let Some((data_env_name, _)) =
            non_empty_env(&["DX_AGENTS_DATA_DIR", "DX_AGENT_DATA_DIR"], true)
        {
            ::dx_agent_log::record!(
                WARN,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                    ...
                    .with_attrs(::serde_json::json!({
                        ...
                        "ignored_env": data_env_name,
                    })),
                "Config directory env var is set; data directory env var is ignored \
                 because CONFIG_DIR pins both the config directory and the data \
                 directory under it."
            );
        }
        if let Some((workspace_env_name, _)) =
            non_empty_env(&["DX_AGENTS_WORKSPACE", "DX_AGENT_WORKSPACE"], false)
        {
            ::dx_agent_log::record!(
                WARN,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                    ...
                    .with_attrs(::serde_json::json!({
                        "config_env": config_env_name,
                        "ignored_env": workspace_env_name,
                    })),
                "Config directory env var is set; workspace env var is ignored. \
                 Use DX_AGENTS_DATA_DIR for data-directory overrides."
            );
        }
        let dx_agent_dir = expand_tilde_path(custom_config_dir);
        return Ok((
            dx_agent_dir.clone(),
            dx_agent_dir.join("data"),
            ConfigResolutionSource::EnvConfigDir,
        ));
    }

    if let Some((_data_env_name, custom_data)) =
        non_empty_env(&["DX_AGENTS_DATA_DIR", "DX_AGENT_DATA_DIR"], true)
    {
        if let Some((workspace_env_name, _)) =
            non_empty_env(&["DX_AGENTS_WORKSPACE", "DX_AGENT_WORKSPACE"], false)
        {
            ::dx_agent_log::record!(
                ...
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                    .with_outcome(::dx_agent_log::EventOutcome::Unknown)
                    .with_attrs(::serde_json::json!({"ignored_env": workspace_env_name})),
                "Data directory env var is set; workspace env var is ignored. \
                 Use DX_AGENTS_DATA_DIR for data-directory overrides."
            );
        }
        let expanded = expand_tilde_path(&custom_data);
        let (dx_agent_dir, data_dir) = resolve_config_dir_for_data(&expanded);
        return Ok((dx_agent_dir, data_dir, ConfigResolutionSource::EnvDataDir));
    }

    if let Some((_workspace_env_name, custom_workspace)) =
        non_empty_env(&["DX_AGENTS_WORKSPACE", "DX_AGENT_WORKSPACE"], false)
    {
        ::dx_agent_log::record!(
            WARN,
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                .with_outcome(::dx_agent_log::EventOutcome::Unknown),
            "Workspace env var is deprecated; use DX_AGENTS_DATA_DIR instead."
        );
    ...
    Ok((
        default_dx_agent_dir.to_path_buf(),
    ...
    let defaults: &[(&str, &str)] = &[
        (
            ...
            "# IDENTITY.md — Who Am I?\n\n\
             I am DX Agent, an autonomous AI agent.\n\n\
             ## Traits\n\
        ...
        (
            "SOUL.md",
            "# SOUL.md — Who You Are\n\n\
             You are DX Agent, an autonomous AI agent.\n\n\
             ## Core Principles\n\
...
impl Config {
    ...
    /// Returns `true` if `path` was populated by a `DX_AGENT_*` env-var
    ...
    pub async fn load_or_init() -> Result<Self> {
        let (default_dx_agent_dir, default_workspace_dir) = default_config_and_data_dirs()?;

        // Resolve env overrides FIRST so the migration runs against
        // the install root the operator actually uses. Running the
        // migration against `default_dx_agent_dir` would silently skip
        // any install reached via `DX_AGENT_CONFIG_DIR` or
        // `DX_AGENT_WORKSPACE`.
        let (dx_agent_dir, _legacy_workspace_dir, resolution_source) =
            resolve_runtime_config_dirs(&default_dx_agent_dir, &default_workspace_dir).await?;
        ...
        // the real error.
        let config_toml_path = dx_agent_dir.join("config.toml");
        ...
        if needs_fs_migration
            && let Err(e) = crate::schema::v2::migrate_v2_to_v3_install_filesystem(&dx_agent_dir)
        {
            ::dx_agent_log::record!(
                WARN,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                    .with_outcome(::dx_agent_log::EventOutcome::Unknown)
                    .with_attrs(::serde_json::json!({
                        "install": dx_agent_dir.display().to_string(),
                        "error": format!("{}", e),
                    })),
            ...
            && let Err(e) =
                crate::schema::v2::relocate_default_agent_skills_to_shared(&dx_agent_dir)
        {
            ::dx_agent_log::record!(
                WARN,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
        ...
        let config_path = dx_agent_dir.join("config.toml");
        ...
        let data_dir = dx_agent_dir.join("data");
        ...
        let shared_dir = dx_agent_dir.join("shared");
        ...
        fs::create_dir_all(&dx_agent_dir)
            .await
            .with_context(|| config_dir_creation_error(&dx_agent_dir))?;
        ...
        if config_path.exists() {
            ...
            let store = crate::secrets::SecretStore::new(&dx_agent_dir, config.secrets.encrypt);
            // Decrypt all #[secret]-annotated fields via Configurable derive
            config.decrypt_secrets(&store)?;
            config.decrypt_route_api_keys(&store)?;

            // Apply DX_AGENT_<lowercase_path> env-var overrides. Hard-errors
            ...
            ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"path": config.config_path.display().to_string(), "workspace": config.data_dir.display().to_string(), "source": resolution_source.as_str(), "initialized": true})), "Config loaded");
            Ok(config)
        } else {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        for (type_key, alias_key, profile) in self.providers.models.iter_entries() {
            ...
            if !has_uri && !has_api_key && !has_model {
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"model_provider": profile_name, "profile_name": profile_name})), "providers.models. is empty (no uri / api_key / model). \
        ...
        for (alias, cfg) in &self.providers.models.ollama {
            ...
            {
                anyhow::bail!(
                    "providers.models.ollama.{alias}.model uses ':cloud', but no API key is configured. Set api_key on [providers.models.ollama.{alias}] (or via the schema-mirror grammar: DX_AGENT_providers__models__ollama__{alias}__api_key=<value>)."
                );
            }
        }
        ...
        for alias in agent_aliases {
            ...
            // crates/dx-agent-macros/src/lib.rs:1056); field name matches
    ...
    async fn resolve_config_path_for_save(&self) -> Result<PathBuf> {
        ...
        let (default_dx_agent_dir, default_workspace_dir) = default_config_and_data_dirs()?;
        let (dx_agent_dir, _workspace_dir, source) =
            resolve_runtime_config_dirs(&default_dx_agent_dir, &default_workspace_dir).await?;
        let file_name = self
            .config_path
            .file_name()
        ...
        let resolved = dx_agent_dir.join(file_name);
        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"path": self.config_path.display().to_string(), "resolved": resolved.display().to_string(), "source": source.as_str()})), "Config path missing parent directory; resolving from runtime environment");
        Ok(resolved)
    }
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let dx_agent_dir = config_path
            .parent()
            .context("Config path must have a parent directory")?;
        let store = crate::secrets::SecretStore::new(dx_agent_dir, self.secrets.encrypt);

        // Restore env-overridden paths to their pre-override snapshots before
        // encryption, so values supplied via `DX_AGENT_*` env vars never reach
        ...
        config_to_save.encrypt_route_api_keys(&store)?;
...
mod tests {
    ...
    async fn expand_tilde_path_expands_tilde_when_home_set() {
        ...
        let path = expand_tilde_path("~/.dx-agent");
...
"#,
    ...
    fn capture_log_events() -> tokio::sync::broadcast::Receiver<serde_json::Value> {
        ::dx_agent_log::try_install_capture_subscriber();
        ::dx_agent_log::subscribe_or_install()
    }
...
"#;
    ...
    async fn config_save_encrypts_nested_credentials() {
        ...
        config.model_routes.push(ModelRouteConfig {
            ...
            api_key: Some("model-route-credential".into()),
        });
        ...
        config.file_upload_bundle.headers = HashMap::from([(
            "Authorization".to_string(),
            "Bearer upload-bundle-credential".to_string(),
        )]);
        config.file_download.headers = HashMap::from([(
            "Authorization".to_string(),
            "Bearer download-credential".to_string(),
        )]);
        config.channels.gmail_push.insert(
            "default".to_string(),
            crate::scattered_types::GmailPushConfig {
                webhook_secret: "gmail-webhook-credential".to_string(),
                ..Default::default()
            },
        );
        config.channels.voice_call.insert(
            ...
            crate::scattered_types::VoiceCallConfig {
                auth_token: "voice-auth-credential".to_string(),
                ..Default::default()
            },
        );
        ...
        for plaintext in [
            ...
            "Bearer upload-bundle-credential",
            "Bearer download-credential",
            "model-route-credential",
            "embedding-route-credential",
            "gmail-webhook-credential",
            "voice-auth-credential",
            "mcp-env-credential",
            "Bearer mcp-cred",
            "tenant-42",
        ] {
        ...
        let model_route_key = stored.model_routes[0].api_key.as_deref().unwrap();
        assert!(crate::secrets::SecretStore::is_encrypted(model_route_key));
        assert_eq!(
            store.decrypt(model_route_key).unwrap(),
            "model-route-credential"
        );

        let embedding_route_key = stored.embedding_routes[0].api_key.as_deref().unwrap();
        assert!(crate::secrets::SecretStore::is_encrypted(
            embedding_route_key
        ));
        assert_eq!(
            store.decrypt(embedding_route_key).unwrap(),
            "embedding-route-credential"
        );
        ...
        assert_eq!(
            store.decrypt(upload_bundle_auth).unwrap(),
            "Bearer upload-bundle-credential"
        );

        let download_auth = stored.file_download.headers.get("Authorization").unwrap();
        assert!(crate::secrets::SecretStore::is_encrypted(download_auth));
        assert_eq!(
            store.decrypt(download_auth).unwrap(),
            "Bearer download-credential"
        );

        let gmail_push = stored.channels.gmail_push.get("default").unwrap();
        assert!(crate::secrets::SecretStore::is_encrypted(
            &gmail_push.webhook_secret
        ));
        assert_eq!(
            store.decrypt(&gmail_push.webhook_secret).unwrap(),
            "gmail-webhook-credential"
        );

        let voice_call = stored.channels.voice_call.get("default").unwrap();
        assert!(crate::secrets::SecretStore::is_encrypted(
            &voice_call.auth_token
        ));
        assert_eq!(
            store.decrypt(&voice_call.auth_token).unwrap(),
            "voice-auth-credential"
        );
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            app_secret: None,
            session_path: Some("~/.dx-agent/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            ws_url: None,
            mention_only: false,
...
"#;
    ...
    async fn env_override_lock() -> MutexGuard<'static, ()> {
        ...
        // this, tests across the two modules race on `DX_AGENT_*` vars.
...
"#;
    ...
    async fn save_repairs_bare_config_filename_using_runtime_resolution() {
        ...
        let resolved_config_path = temp_home.join(".dx-agent").join("config.toml");

        let original_home = std::env::var("HOME").ok();
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("HOME", &temp_home) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DX_AGENT_WORKSPACE", &workspace_dir) };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DX_AGENT_WORKSPACE") };
...
"#;
    ...
    async fn resolve_runtime_config_dirs_accepts_legacy_zeroclaw_workspace() {
        ...
        // DX_AGENT_WORKSPACE is the deprecated alias for DX_AGENT_DATA_DIR.
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DX_AGENT_CONFIG_DIR", &explicit_config_dir) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DX_AGENT_WORKSPACE") };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DX_AGENT_CONFIG_DIR") };
        let _ = fs::remove_dir_all(default_config_dir).await;
    }

    #[test]
    async fn resolve_runtime_config_dirs_prefers_dx_agents_config_dir() {
        let _env_guard = env_override_lock().await;
        let default_config_dir = std::env::temp_dir().join(uuid::Uuid::new_v4().to_string());
        let default_workspace_dir = default_config_dir.join("workspace");
        let legacy_config_dir = default_config_dir.join("legacy-config");
        let dx_config_dir = default_config_dir.join("dx-config");

        fs::create_dir_all(&default_config_dir).await.unwrap();

        // SAFETY: tests serialize on `env_override_lock()`.
        unsafe { std::env::set_var("DX_AGENT_CONFIG_DIR", &legacy_config_dir) };
        // SAFETY: tests serialize on `env_override_lock()`.
        unsafe { std::env::set_var("DX_AGENTS_CONFIG_DIR", &dx_config_dir) };
        ...
        assert_eq!(source, ConfigResolutionSource::EnvConfigDir);
        assert_eq!(config_dir, dx_config_dir);
        assert_eq!(resolved_workspace_dir, dx_config_dir.join("data"));

        // SAFETY: tests serialize on `env_override_lock()`.
        unsafe { std::env::remove_var("DX_AGENT_CONFIG_DIR") };
        // SAFETY: tests serialize on `env_override_lock()`.
        unsafe { std::env::remove_var("DX_AGENTS_CONFIG_DIR") };
        let _ = fs::remove_dir_all(default_config_dir).await;
    }

    #[test]
    async fn resolve_runtime_config_dirs_accepts_dx_agents_data_dir() {
        let _env_guard = env_override_lock().await;
        let default_config_dir = std::env::temp_dir().join(uuid::Uuid::new_v4().to_string());
        let default_workspace_dir = default_config_dir.join("workspace");
        let data_dir = default_config_dir.join("profile-a");

        fs::create_dir_all(&default_config_dir).await.unwrap();

        // SAFETY: tests serialize on `env_override_lock()`.
        unsafe { std::env::set_var("DX_AGENTS_DATA_DIR", &data_dir) };
        ...
        assert_eq!(source, ConfigResolutionSource::EnvDataDir);
        assert_eq!(config_dir, data_dir);
        assert_eq!(resolved_workspace_dir, data_dir.join("data"));

        // SAFETY: tests serialize on `env_override_lock()`.
        unsafe { std::env::remove_var("DX_AGENTS_DATA_DIR") };
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    #[test]
    async fn default_path_under_config_dir_respects_dx_agent_config_dir() {
        let _env_guard = env_override_lock().await;
        let custom_dir = std::env::temp_dir().join("zeroclaw-test-profile");
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DX_AGENT_CONFIG_DIR", &custom_dir) };
        ...
        assert_eq!(
            ...
            "expected path under DX_AGENT_CONFIG_DIR, got: {result}"
        );
    }
    ...
    async fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("zeroclaw_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let legacy_config_dir = temp_home.join(".dx-agent");
        ...
        // DX_AGENT_WORKSPACE env var (deprecated alias) resolved to the
...
"#,
        ...
        // `config.toml`), regardless of the DX_AGENT_WORKSPACE
        // (deprecated) override.
        assert_eq!(config.data_dir, legacy_config_dir.join("data"));
        assert_eq!(config.config_path, legacy_config_path);
        assert_eq!(
    ...
    async fn load_or_init_decrypts_feishu_channel_secrets() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("zeroclaw_test_home_{}", uuid::Uuid::new_v4()));
        let config_dir = temp_home.join(".dx-agent");
...
"#,
        ...
        // DX_AGENT_WORKSPACE env var (deprecated alias for
        // DX_AGENT_DATA_DIR) pinned the install root, so data_dir is
...
[security.estop]
enabled = true
state_file = "~/.dx-agent/estop-state.json"
require_otp_to_resume = true
"#,
        );
...
schema_version = 3
workspace_dir = "/dx-agent-data/workspace"
config_path = "/dx-agent-data/.dx-agent/config.toml"
api_key = ""
default_model_provider = "openrouter"
...
"#;
    ...
    async fn apply_env_overrides_rejects_schema_version() {
        let _env_guard = env_override_lock().await;
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("DX_AGENT_schema_version", "99") };
        let mut config = Config::default();
        let result = crate::env_overrides::apply_env_overrides(&mut config);
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("DX_AGENT_schema_version") };
    ...
    async fn env_override_save_cycle_preserves_on_disk_secret() {
        ...
        // `DX_AGENT_*` env override for the same path and triggers any
    ...
    #[test]
    async fn init_defaults_output_deserializes_disabled_gateway_tls() {
        let mut config = Config::default();
        let initialized = config.init_defaults(None);
        assert!(
            initialized.contains(&"gateway.tls"),
            "full init should materialize gateway.tls"
        );

        let toml_src = toml::to_string_pretty(&config).expect("serialize initialized config");
        let reloaded: Config =
            toml::from_str(&toml_src).expect("initialized config must deserialize");
        let tls = reloaded
            .gateway
            .tls
            .as_ref()
            .expect("gateway.tls should remain materialized");
        assert!(!tls.enabled);
    ...
    #[test]
    async fn init_defaults_minimal_disabled_blocks_deserialize() {
        let toml_src = r#"
schema_version = 3
...
"#;
        ...
        let reloaded: Config =
            toml::from_str(toml_src).expect("minimal disabled init blocks must deserialize");
        let tls = reloaded
            .gateway
            .tls
            .expect("gateway.tls should deserialize");
        assert!(!tls.enabled);
        assert!(tls.cert_path.is_empty());
        assert!(tls.key_path.is_empty());
        assert_eq!(reloaded.tunnel.tunnel_provider, "none");
        let openvpn = reloaded
            .tunnel
            .openvpn
            .expect("tunnel.openvpn should deserialize");
        assert!(openvpn.config_file.is_empty());
        let local_whisper = reloaded
            .transcription
            .local_whisper
            .expect("transcription.local_whisper should deserialize");
        assert!(local_whisper.url.is_empty());
    }
...
"#;
    ...
    async fn credential_shaped_prop_fields_have_explicit_classification() {
        ...
        config.file_upload.headers = HashMap::from([(
            "Authorization".to_string(),
            "Bearer upload-secret".to_string(),
        )]);
        config.file_upload_bundle.headers = HashMap::from([(
            "Authorization".to_string(),
            "Bearer bundle-secret".to_string(),
        )]);
        config.file_download.headers = HashMap::from([(
            "Authorization".to_string(),
            "Bearer download-secret".to_string(),
        )]);
        config.channels.gmail_push.insert(
            "default".into(),
            crate::scattered_types::GmailPushConfig {
                webhook_secret: "gmail-webhook-secret".into(),
                ..Default::default()
            },
        );
        config
        ...
        config.channels.voice_call.insert(
            "default".into(),
            crate::scattered_types::VoiceCallConfig {
                auth_token: "voice-auth-secret".into(),
                ..Default::default()
            },
        );
        config
    ...
    async fn prop_fields_carry_credential_classification_from_schema_fields() {
        ...
        let has_secret_field = |name: &str| secret_fields.iter().any(|field| field.name == name);
        assert!(has_secret_field("file_upload.headers"));
        assert!(has_secret_field("file-upload-bundle.headers"));
        assert!(has_secret_field("file-download.headers"));
    }

    #[test]
    async fn config_field_entries_preserve_credential_classification() {
        let mut config = Config::default();
        config.init_defaults(None);
        config
            .providers
        ...
        let entries: Vec<crate::traits::ConfigFieldEntry> = config
            .prop_fields()
            .into_iter()
            .map(|field| crate::traits::ConfigFieldEntry::from_prop_field(field, false))
            .collect();
        let entry_for = |path: &str| {
            entries
                .iter()
                .find(|entry| entry.path == path)
                .unwrap_or_else(|| panic!("missing config entry for {path}"))
        };

        let api_key = entry_for("providers.tts.openai.default.api_key");
        assert!(api_key.is_secret);
        assert!(api_key.value.is_none());
        assert_eq!(
            api_key.credential_class,
            Some(crate::config::CredentialSurfaceClass::EncryptedSecret)
        );
    }

    #[test]
    async fn route_object_array_surfaces_mask_nested_api_keys() {
        let mut config = Config::default();
        config.model_routes.push(ModelRouteConfig {
            hint: "fast".into(),
            model_provider: "openrouter.worker".into(),
            model: "openai/gpt-4o-mini".into(),
            api_key: Some("model-route-display-secret".into()),
        });
        config.embedding_routes.push(EmbeddingRouteConfig {
            hint: "semantic".into(),
            model_provider: "openrouter.worker".into(),
            model: "text-embedding-3-small".into(),
            dimensions: Some(1536),
            api_key: Some("embedding-route-display-secret".into()),
        });

        let fields = config.prop_fields();
        for path in ["model_routes", "embedding_routes"] {
            let field = fields
                .iter()
                .find(|field| field.name == path)
                .unwrap_or_else(|| panic!("missing prop field for {path}"));
            assert_eq!(field.kind, crate::config::PropKind::ObjectArray);
            assert!(!field.display_value.contains("route-display-secret"));
            assert!(field.display_value.contains(crate::traits::MASKED_SECRET));

            let entry = crate::traits::ConfigFieldEntry::from_prop_field(field.clone(), false);
            let value = entry
                .value
                .as_ref()
                .and_then(serde_json::Value::as_str)
                .expect("object-array config entry should carry a display string");
            assert!(!value.contains("route-display-secret"));
            assert!(value.contains(crate::traits::MASKED_SECRET));

            let prop = config
                .get_prop(path)
                .expect("route prop should be readable");
            assert!(!prop.contains("route-display-secret"));
            assert!(prop.contains(crate::traits::MASKED_SECRET));
        }

        let mut masked = config.clone();
        masked.mask_route_api_keys();
        assert_eq!(
            masked.model_routes[0].api_key.as_deref(),
            Some(crate::traits::MASKED_SECRET)
        );
        assert_eq!(
        ...
        let mut echoed = masked.clone();
        echoed.restore_route_api_keys_from(&config);
        assert_eq!(
            echoed.model_routes[0].api_key.as_deref(),
            Some("model-route-display-secret")
        );
        assert_eq!(
            echoed.embedding_routes[0].api_key.as_deref(),
            Some("embedding-route-display-secret")
        );
    }

    #[test]
    async fn route_api_key_restore_handles_duplicate_hints() {
        let mut current = Config::default();
        current.model_routes.push(ModelRouteConfig {
            hint: "shared".into(),
            model_provider: "openrouter.worker".into(),
            model: "openai/gpt-4o-mini".into(),
            api_key: Some("first-model-secret".into()),
        });
        current.model_routes.push(ModelRouteConfig {
            hint: "shared".into(),
            model_provider: "groq.worker".into(),
            model: "llama-3.3-70b-versatile".into(),
            api_key: Some("second-model-secret".into()),
        });
        current.embedding_routes.push(EmbeddingRouteConfig {
            hint: "shared".into(),
            model_provider: "openrouter.worker".into(),
            model: "text-embedding-3-small".into(),
            dimensions: Some(1536),
            api_key: Some("first-embedding-secret".into()),
        });
        current.embedding_routes.push(EmbeddingRouteConfig {
            hint: "shared".into(),
            model_provider: "local.worker".into(),
            model: "nomic-embed-text".into(),
            dimensions: Some(768),
            api_key: Some("second-embedding-secret".into()),
        });

        let mut reordered = current.clone();
        reordered.model_routes.swap(0, 1);
        reordered.embedding_routes.swap(0, 1);
        reordered.mask_route_api_keys();
        reordered.restore_route_api_keys_from(&current);

        assert_eq!(
            reordered.model_routes[0].api_key.as_deref(),
            Some("second-model-secret")
        );
        assert_eq!(
            reordered.model_routes[1].api_key.as_deref(),
            Some("first-model-secret")
        );
        assert_eq!(
            reordered.embedding_routes[0].api_key.as_deref(),
            Some("second-embedding-secret")
        );
        assert_eq!(
            reordered.embedding_routes[1].api_key.as_deref(),
            Some("first-embedding-secret")
        );

        let mut edited = current.clone();
        edited.model_routes[0].model = "edited-model".into();
        edited.embedding_routes[0].model = "edited-embedding-model".into();
        edited.mask_route_api_keys();
        edited.restore_route_api_keys_from(&current);

        assert_eq!(
            edited.model_routes[0].api_key.as_deref(),
            Some("first-model-secret")
        );
        assert_eq!(
            edited.embedding_routes[0].api_key.as_deref(),
            Some("first-embedding-secret")
        );
    }
