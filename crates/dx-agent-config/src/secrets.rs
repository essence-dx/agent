...
// in `~/.dx-agent/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.dx-agent/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(dx_agent_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: dx_agent_dir.join(".secret_key"),
            enabled,
        }
    }
    ...
    fn decrypt_chacha20(&self, hex_str: &str) -> Result<String> {
        ...
        let plaintext_bytes = cipher
            ...
            .map_err(|_| {
                ::dx_agent_log::record!(ERROR, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail).with_outcome(::dx_agent_log::EventOutcome::Failure).with_attrs(::serde_json::json!({"key_path": self.key_path.display().to_string()})), "enc2: decryption failed. `.secret_key` is missing or does not match the key used to encrypt this value. \
