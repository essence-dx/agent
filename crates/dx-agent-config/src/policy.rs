...
// Re-export from dx-agent-config.
...
impl ActionTracker {
    ...
    fn prune(actions: &mut Vec<Instant>, now: Instant) {
    ...
    pub fn count(&self) -> usize {
        let mut actions = self.actions.lock();
        Self::prune(&mut actions, Instant::now());
        actions.len()
    }
}
...
impl PerSenderTracker {
    ...
    fn current_key() -> String {
        dx_agent_api::TOOL_LOOP_THREAD_ID
            .try_with(|v| v.clone())
            .ok()
            .flatten()
...
impl SecurityPolicy {
    ...
    pub fn runtime_config_violation_message(&self, resolved: &Path) -> String {
        format!(
            "Refusing to modify DX Agent runtime config/state file: {}. Use dedicated config tools or edit it manually outside the agent loop.",
            resolved.display()
        )
    }
...
mod tests {
    ...
    #[cfg(not(target_os = "windows"))]
    fn tp_ws() -> PathBuf {
        PathBuf::from("/home/user/.dx-agent/workspace")
    }
    #[cfg(target_os = "windows")]
    fn tp_ws() -> PathBuf {
        PathBuf::from("C:\\Users\\user\\.dx-agent\\workspace")
    }

    #[cfg(not(target_os = "windows"))]
    fn tp_ws_shared() -> PathBuf {
        PathBuf::from("/home/user/.dx-agent/shared")
    }
    #[cfg(target_os = "windows")]
    fn tp_ws_shared() -> PathBuf {
        PathBuf::from("C:\\Users\\user\\.dx-agent\\shared")
    }
    ...
    fn resolve_tool_path_normalizes_workspace_prefixed_relative_paths() {
        let p = SecurityPolicy {
            workspace_dir: PathBuf::from("/dx-agent-data/workspace"),
            ..SecurityPolicy::default()
        };
        let resolved = p.resolve_tool_path("dx-agent-data/workspace/scripts/daily.py");
        assert_eq!(
            resolved,
            PathBuf::from("/dx-agent-data/workspace/scripts/daily.py")
        );
    }
