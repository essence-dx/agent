...
fn route_api_key_display_regex() -> &'static regex::Regex {
    static REGEX: OnceLock<regex::Regex> = OnceLock::new();
    REGEX.get_or_init(|| {
        regex::Regex::new(r#"(?i)(("?api_key"?\s*(?::|=)\s*")([^"]*)("))"#)
            .expect("valid route api_key redaction regex")
    })
}

pub fn redact_route_api_keys_for_display(path: &str, display_value: String) -> String {
    if !matches!(path, "model_routes" | "embedding_routes") {
        return display_value;
    }

    route_api_key_display_regex()
        .replace_all(&display_value, |captures: &regex::Captures<'_>| {
            format!(
                "{}{}{}",
                &captures[2],
                crate::traits::MASKED_SECRET,
                &captures[4]
            )
        })
        .into_owned()
...
pub fn serde_get_prop<T: serde::Serialize>(
    ...
    let display = toml_value_to_display(table.as_table().and_then(|t| t.get(&serde_name)));
    Ok(redact_route_api_keys_for_display(name, display))
}
...
/// filesystem paths on Windows/macOS/Linux, and `DX_AGENT_*` env-var grammar.
///
...
/// forbidden so the bootstrap env-vars (`DX_AGENT_WORKSPACE`,
/// `DX_AGENT_CONFIG_DIR`) stay disambiguated by case.
