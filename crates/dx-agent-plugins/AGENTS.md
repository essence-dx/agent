...
WASM plugin host for DX Agent. Handles plugin discovery, manifest parsing,
Ed25519 signature verification, and Extism-based WASM execution. Bridges
plugin-exported functions into DX Agent's `Tool` and `Channel` traits so
plugins appear as native capabilities to the agent runtime.

## What this crate is allowed to depend on

- `dx-agent-api` (traits only — `Tool`, `Channel`, `ToolResult`)
- `extism` (WASM runtime)
...
- Concrete tool or channel implementations (those go in `dx-agent-tools` or
  `dx-agent-channels`, or in a WASM plugin)
- Plugin business logic (that belongs in the plugin's own crate)
- Config schema definitions (those go in `dx-agent-config`)
