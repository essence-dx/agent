...
//! and never see the enum. `Action` and `EventOutcome` from `dx_agent_log`
...
use std::path::Path;
use dx_agent_api::model_provider::{ChatMessage, ConversationMessage, ToolCall, ToolResultMessage};
use dx_agent_log::{Action, EventOutcome};
...
impl AcpSessionStore {
    ...
    /// values are used at the matching `dx_agent_log::record!` call site.
    pub fn append_event(
        &self,
        session_uuid: &str,
        action: Action,
...
mod tests {
    ...
    use dx_agent_api::model_provider::{ChatMessage, ToolCall, ToolResultMessage};
