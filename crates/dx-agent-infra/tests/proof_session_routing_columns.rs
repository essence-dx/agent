...
//! whenever `DX_AGENT_LIVE_SESSIONS_DB` is set so the migration runs
...
//!
//! Run with:
//!   DX_AGENT_LIVE_SESSIONS_DB=$HOME/.dx_agent/data/sessions/sessions.db \
//!     cargo test -p dx-agent-infra --test proof_session_routing_columns \
//!     -- --nocapture
...
use dx_agent_infra::session_backend::{SessionBackend, SessionContext};
use dx_agent_infra::session_sqlite::SqliteSessionBackend;
...
fn migration_adds_routing_columns_and_set_session_context_persists() {
    ...
    if let Ok(live) = std::env::var("DX_AGENT_LIVE_SESSIONS_DB") {
        ...
        println!("DX_AGENT_LIVE_SESSIONS_DB unset; running migration over empty file");
    }
