...
/// `impl_enum_prop_kind!` block in `crates/dx-agent-config/src/schema.rs`, or
...
/// in `crates/dx-agent-config/src/schema.rs`.
#[proc_macro_derive(
    Configurable,
    attributes(
        secret,
...
pub fn derive_configurable(input: TokenStream) -> TokenStream {
    ...
    for field in fields {
        ...
        // `crates/dx-agent-config/src/traits.rs` — adding a new shape is a
                    ...
                    let display_value: String = match #inner_value_expr {
                        ...
                        Some(v) => match <#inner_ty as crate::config::HasPropKind>::PROP_KIND {
                            crate::config::PropKind::ObjectArray => {
                                crate::config::redact_route_api_keys_for_display(
                                    #full_name_lit,
