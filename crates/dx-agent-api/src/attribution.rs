...
//! their work; the `LogCaptureLayer` in `dx-agent-log` walks the span
...
impl Role {
    ...
    /// Closest [`dx_agent_log::EventCategory`] for this role, used by
    /// the layer to default `event.category` when the call site doesn't
    /// override. Returned as a `&'static str` to keep `dx-agent-api`
    /// free of a back-dep on `dx-agent-log`.
    #[must_use]
