...
/// Map tool name aliases from various LLM model_providers to DX Agent tool names.
    ...
    if calls.is_empty() {
        ...
        for cap in MD_TOOL_NAME_RE.captures_iter(response) {
            ...
            if json_values.is_empty() {
                ...
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"tool_name": tool_name, "inner": inner.chars().take(100).collect::<String>()})), "Found ```tool <name> block but could not parse JSON arguments");
            } else {
                for value in json_values {
...
Let me check the result."#;
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Additional parser internals tests (moved from dx-agent-runtime to keep
