...
pub(crate) fn filter_models(catalog: &Catalog, provider_key: &str) -> Result<Vec<String>> {
    ...
    let mut ids: Vec<String> = entry
        .models
        .values()
        .filter(|model| model_is_available_without_provider_key(provider_key, model))
        .map(|m| m.id.clone())
        .collect();
    ids.sort();
    ids.dedup();
...
fn model_is_available_without_provider_key(provider_key: &str, model: &ModelEntry) -> bool {
    if provider_key != "opencode" {
        return true;
    }
    crate::provider_metadata::metadata_for_provider_id("opencode")
        .is_some_and(|metadata| metadata.free_model_ids.contains(&model.id.as_str()))
}
...
mod tests {
    ...
    #[test]
    fn filter_opencode_keeps_only_verified_public_models() {
        let raw = r#"{
            "opencode": {
                "models": {
                    "free": {
