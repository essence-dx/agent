...
//! daemon heartbeat path (`crates/dx-agent-runtime/src/daemon/mod.rs`) calls
...
async fn scheduled_run_does_not_leak_conversation_memory_into_provider_request() {
    ...
    // Matches `crates/dx-agent-runtime/src/daemon/mod.rs:476` / `:599`:
    //   crate::agent::run(
    ...
    let run_result = dx_agent_runtime::agent::run(
        config,
        "default",
        Some(prompt),
        None,
