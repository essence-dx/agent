...
use dx_agent_config::helpers::kebab_to_snake;
...
pub fn validate_only_with_surface(
    ...
    if ok {
        ::dx_agent_log::record!(
            INFO,
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Validate)
...
pub async fn apply_with_surface(
    ...
    ::dx_agent_log::record!(
        DEBUG,
        ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Write).with_attrs(
            merge_attrs(
                ctx.base_attrs(),
    ...
    match &write_result {
        Ok(_) => ::dx_agent_log::record!(
            INFO,
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Write)
...
pub struct QuickstartState {
    ...
    /// registry in `dx_agent_providers::list_model_providers()` — the
    ...
    /// Risk presets from `dx_agent_config::presets::RISK_PRESETS`.
    pub risk_presets: &'static [dx_agent_config::presets::RiskPreset],
    /// Runtime presets from `dx_agent_config::presets::RUNTIME_PRESETS`.
    pub runtime_presets: &'static [dx_agent_config::presets::RuntimePreset],
...
pub struct QuickstartTypeOption {
    ...
    pub local: bool,
    /// Provider identity metadata when this row represents a model
    /// provider. Channel rows omit it.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub identity: Option<dx_agent_providers::provider_metadata::ProviderIdentity>,
    /// Free-tier metadata when the provider has a documented freemium
    /// surface. Channel rows omit it.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub freemium: Option<dx_agent_providers::provider_metadata::FreemiumMetadata>,
}

/// Build a [`QuickstartState`] snapshot from the live config.
///
/// The two `*_types` lists are populated from the canonical sources
/// (`provider_catalog::list_model_provider_catalog_entries()` for providers,
...
pub fn snapshot_state(cfg: &Config) -> QuickstartState {
    let model_provider_types =
        dx_agent_providers::provider_catalog::list_model_provider_catalog_entries()
            .into_iter()
            .map(|entry| {
                let identity = entry.identity();
                let freemium = entry.freemium();
    ...
    QuickstartState {
        ...
        channel_types,
        risk_presets: dx_agent_config::presets::RISK_PRESETS,
        runtime_presets: dx_agent_config::presets::RUNTIME_PRESETS,
...
fn memory_kind_keys() -> Vec<String> {
    use dx_agent_config::multi_agent::MemoryBackendKind as M;
    [
        M::Sqlite,
        M::Markdown,
        M::Postgres,
...
fn build_channel_type_options(
    channels_cfg: &dx_agent_config::schema::ChannelsConfig,
...
#[derive(Debug, Clone)]
struct ResolvedModelProviderType {
    config_family: String,
    catalog_only: bool,
    default_uri: Option<String>,
}

fn resolve_model_provider_type(type_key: &str) -> Option<ResolvedModelProviderType> {
    let entry =
        dx_agent_providers::provider_catalog::find_model_provider_catalog_entry(type_key.trim())?;
    let catalog_only = !entry.is_native();
    let default_uri = if catalog_only {
        dx_agent_providers::provider_catalog::find_dx_provider_catalog_summary(&entry.name)
            .ok()
            .and_then(|provider| provider.api_url)
    } else {
        None
    };
    Some(ResolvedModelProviderType {
        config_family: if catalog_only {
            "custom".to_string()
        } else {
            entry.name
        },
...
pub struct FieldDescriptor {
    ...
    /// `PropKind::wire_name`.
    pub kind: dx_agent_config::traits::PropKind,
...
pub fn field_shape(section: FieldSection, type_key: &str) -> Vec<FieldDescriptor> {
    ...
    let mut catalog_only_model_provider = None;
    let (section_path, essentials) = match section {
        FieldSection::ModelProvider => {
            let Some(provider) = resolve_model_provider_type(type_key) else {
                return Vec::new();
            };
            if provider.catalog_only {
                catalog_only_model_provider = Some(provider.default_uri.clone());
            }
            (
                format!("providers.models.{}", provider.config_family),
    ...
    for info in probe.prop_fields() {
        ...
        let default = if info.is_secret {
            ...
            if raw.is_empty() || raw == dx_agent_config::traits::UNSET_DISPLAY {
                None
            } else {
                Some(raw.to_string())
            }
        ...
        if field_path == "uri"
            && let Some(default_uri) = catalog_only_model_provider.as_ref()
        {
            required = true;
            default = default_uri.clone();
        }
...
fn set_model_provider_field(
    ...
    if let Err(err) = config.set_prop_persistent(&format!("{prefix}.{field}"), value) {
        errors.push(QuickstartError::new(
            QuickstartStep::ModelProvider,
            dx_agent_config::helpers::kebab_to_snake(field),
            err.to_string(),
        ));
        return false;
    }
...
fn apply_model_provider(
    ...
    match choice {
        ...
        SelectorChoice::Fresh(choice) => {
            ...
            // offers canonical catalog keys, but a hand-typed or
            // whitespace-padded value (e.g. "llamacpp ", "llama.cpp") would
            // otherwise reach `create_map_key` verbatim. Catalog-only provider
            // rows intentionally resolve to the typed `custom` slot instead of
            // creating 184 schema families.
            let provider_type = choice.provider_type.trim();
            let Some(resolved_provider) = resolve_model_provider_type(provider_type) else {
                errors.push(QuickstartError::new(
                    QuickstartStep::ModelProvider,
                    "provider_type",
                    format!(
            ...
            let alias = choice.alias.trim();
            let model = choice.model.trim();
            let config_family = resolved_provider.config_family.as_str();
            let submitted_uri = submitted_field_value(&choice.fields, "uri");
            let default_uri = resolved_provider
                .default_uri
                .as_deref()
                .map(str::trim)
                .filter(|value| !value.is_empty());
            if resolved_provider.catalog_only && submitted_uri.is_none() && default_uri.is_none() {
                errors.push(QuickstartError::new(
                    QuickstartStep::ModelProvider,
                    "uri",
                    format!(
                        "catalog provider `{provider_type}` needs an endpoint URI before it can be saved as `custom.{alias}`"
                    ),
                ));
                return None;
            }
            if section_has_alias(config, "providers.models", config_family, alias) {
                errors.push(QuickstartError::new(
                    QuickstartStep::ModelProvider,
                    "alias",
                    format!("alias `{}.{}` already exists", config_family, alias),
                ));
                return None;
            }
            let prefix = format!("providers.models.{config_family}.{alias}");
            if let Err(err) =
                config.create_map_key(&format!("providers.models.{config_family}"), alias)
            {
                errors.push(QuickstartError::new(
                    QuickstartStep::ModelProvider,
                    "provider_type",
            ...
            if !set_model_provider_field(config, &prefix, "model", model, errors) {
                return None;
            }
            if resolved_provider.catalog_only {
                if !set_model_provider_field(config, &prefix, "kind", "openai-compatible", errors) {
                    return None;
                }
                if submitted_uri.is_none()
                    && let Some(uri) = default_uri
                    && !set_model_provider_field(config, &prefix, "uri", uri, errors)
                {
                    return None;
                }
            }
            ...
            for (key, value) in entries {
                ...
                if !set_model_provider_field(config, &prefix, key, value, errors) {
                    return None;
                }
            }
            Some(format!("{config_family}.{alias}"))
        }
    }
}
...
fn apply_peer_groups(
    config: &mut Config,
    peer_groups: &[dx_agent_config::presets::QuickstartPeerGroup],
...
fn apply_personality_files(
    config: &Config,
    agent_alias: &str,
    files: &[dx_agent_config::presets::QuickstartPersonalityFile],
...
pub async fn model_catalog(model_provider: &str) -> (Vec<String>, bool) {
    if let Ok(handle) = dx_agent_providers::create_model_provider(model_provider, None)
        && let Ok(models) = handle.list_models().await
        && !models.is_empty()
    {
        return (models, true);
    }
    match dx_agent_providers::catalog::list_models_for_family(model_provider).await {
        Ok(models) if !models.is_empty() => (models, true),
        _ => match dx_agent_providers::provider_catalog::list_models_for_catalog_provider(
            model_provider,
        ) {
            Ok(models) if !models.is_empty() => (models, true),
            _ => (Vec::new(), false),
...
pub fn model_provider_is_local(model_provider: &str) -> bool {
    dx_agent_providers::provider_catalog::find_model_provider_catalog_entry(model_provider)
...
mod tests {
    ...
    #[test]
    fn snapshot_state_preserves_opencode_provider_metadata() {
        let cfg = Config::default();
        ...
        let opencode = state
            ...
            .expect("opencode provider quickstart row");

        assert_eq!(
            opencode
                .identity
                .as_ref()
                .expect("opencode identity metadata")
                .canonical_id,
            "opencode-zen"
        );
        assert!(
    ...
    #[tokio::test]
    async fn model_catalog_reads_dx_archive_for_catalog_only_provider_when_available() {
        if dx_agent_providers::provider_catalog::discover_dx_providers_catalog().is_none() {
            return;
        }

        let (models, live) = model_catalog("aiml").await;

        assert!(live, "DX catalog-only providers should expose model rows");
        assert!(
            !models.is_empty(),
            "aiml should have models in the copied DX providers catalog"
        );
    }

    #[test]
    fn field_shape_catalog_only_provider_uses_custom_slot_when_available() {
        let Some((provider_type, _model, default_uri)) = catalog_only_quickstart_provider() else {
            return;
        };

        let rows = super::field_shape(super::FieldSection::ModelProvider, &provider_type);
        let keys: Vec<&str> = rows.iter().map(|row| row.key.as_str()).collect();
        let uri = rows
            .iter()
            .find(|row| row.key == "uri")
            .expect("catalog-only provider should expose custom uri row");

        assert!(
            keys.contains(&"model"),
            "custom model row missing: {keys:?}"
        );
        assert!(
            keys.contains(&"api_key"),
            "custom api_key row missing: {keys:?}"
        );
        assert!(uri.required, "catalog-only provider uri must be required");
        assert_eq!(uri.default.as_deref(), Some(default_uri.as_str()));
    }

    #[test]
    fn apply_catalog_only_provider_writes_custom_alias_when_available() {
        let Some((provider_type, model, default_uri)) = catalog_only_quickstart_provider() else {
            return;
        };
        ...
        let provider = cfg
            ...
            .models
            .find("custom", "catalog")
            .expect("catalog-only provider should land in custom slot");
        assert_eq!(provider.kind.as_deref(), Some("openai-compatible"));
        assert_eq!(provider.uri.as_deref(), Some(default_uri.as_str()));
        let agent = cfg.agents.get("bot").expect("agent created");
        assert_eq!(agent.model_provider.as_str(), "custom.catalog");
    }
    ...
    fn catalog_only_quickstart_provider() -> Option<(String, String, String)> {
        if dx_agent_providers::provider_catalog::discover_dx_providers_catalog().is_none() {
            return None;
        }
        ...
        dx_agent_providers::provider_catalog::list_model_provider_catalog_entries()
            ...
            .filter(|entry| {
                entry.source
                    == dx_agent_providers::provider_catalog::ModelProviderCatalogSource::DxProvidersCatalog
            })
            .find_map(|entry| {
                let provider =
                    dx_agent_providers::provider_catalog::find_dx_provider_catalog_summary(
                        &entry.name,
                    )
                    .ok()?;
                let default_uri = provider.api_url?;
                let model = provider
                    .models
                    .into_iter()
                    .find(|model| !model.id.trim().is_empty())?
                    .id;
                Some((entry.name, model, default_uri))
            })
    }
    ...
    fn validate_only_accepts_every_builtin_risk_preset() {
        let cfg = Config::default();
        for p in dx_agent_config::presets::RISK_PRESETS {
            let mut submission = fresh_submission("bot");
    ...
    fn field_shape_openai_includes_codex_auth_fields() {
        ...
        for row in &rows {
            assert_ne!(
                ...
                Some(dx_agent_config::traits::UNSET_DISPLAY),
