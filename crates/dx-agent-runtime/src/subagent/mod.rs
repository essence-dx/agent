...
//! [`dx_agent_memory::Memory::ensure_agent_uuid`] first — SQL-backed
...
pub struct SubAgentOverrides {
    ...
    /// [`dx_agent_memory::Memory::ensure_agent_uuid`] before passing
    /// them to the wrapper (SQL backends use UUIDs; Markdown / Qdrant
    /// / None use the alias verbatim per the trait default). The
    /// in-tree consumer today is `dx_agent_memory::create_memory_for_agent`,
