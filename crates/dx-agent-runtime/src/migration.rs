...
pub async fn migrate_openclaw_memory(
    ...
    if paths_equal(&source_workspace, &config.data_dir) {
        bail!("Source workspace matches current DX Agent workspace; refusing self-migration");
    }
...
fn target_memory_backend(config: &Config) -> Result<Box<dyn Memory>> {
    dx_agent_memory::create_memory_for_migration(&config.memory.backend, &config.data_dir)
}
...
mod tests {
    ...
    use dx_agent_config::schema::{Config, MemoryConfig};
    use dx_agent_memory::SqliteMemory;
