...
impl GitHubScout {
    pub fn new(token: Option<String>) -> Self {
        ...
        headers.insert(
            ...
            "DX Agent-SkillForge/0.1".parse().expect("valid header"),
        );
    ...
    async fn discover(&self) -> Result<Vec<ScoutResult>> {
        ...
        for query in &self.queries {
            ...
            let resp = match self.client.get(&url).send().await {
                Ok(r) => r,
                Err(e) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"query": query.as_str(), "error": format!("{}", e)})), "GitHub API request failed, skipping query");
                    continue;
                }
            };

            if !resp.status().is_success() {
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"status": resp.status().to_string(), "query": query.as_str()})), "GitHub search returned non-200");
                continue;
            }
            ...
            let body: serde_json::Value = match resp.json().await {
                Ok(v) => v,
                Err(e) => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"query": query.as_str(), "error": format!("{}", e)})), "Failed to parse GitHub response, skipping query");
                    continue;
                }
            };
