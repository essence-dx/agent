...
fn load_ftl_with_reader(
    ...
    let path = dx_agent_config::schema::ftl_locale_dir(locale)
        .ok()
...
fn read_config_table() -> Option<toml::Table> {
    ...
    // it at a known dir without the host's real ~/.dx-agent/config.toml leaking
    ...
    if let Some(base) = directories::BaseDirs::new() {
        candidates.push(base.home_dir().join(".dx-agent/config.toml"));
...
mod tests {
    ...
    fn zh_cn_wechat_translations_preserve_machine_facing_tokens() {
        ...
        assert!(success.contains("DX Agent"));
    }
