...
use dx_agent_api::jsonrpc::error_codes::*;
use dx_agent_api::jsonrpc::{
    JSONRPC_VERSION, JsonRpcError, JsonRpcNotification, JsonRpcRequest, JsonRpcResponse,
    RpcOutbound,
};
use dx_agent_api::model_provider::ChatMessage;
...
impl RpcDispatcher {
    ...
    async fn handle_session_cancel(&self, params: &Value) -> RpcResult {
        ...
        if !allowed {
            ...
            ::dx_agent_log::record!(
                WARN,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                    .with_category(::dx_agent_log::EventCategory::Channel)
                    .with_outcome(::dx_agent_log::EventOutcome::Failure)
                    .with_attrs(::serde_json::json!({
    ...
    async fn handle_session_list(&self, params: &Value) -> RpcResult {
        ...
        let all = if let Some(ref keyword) = req.query {
            if keyword.trim().is_empty() {
                ...
                use dx_agent_infra::session_backend::SessionQuery;
    ...
    async fn handle_session_messages(&self, params: &Value) -> RpcResult {
        ...
        let mut raw: Vec<dx_agent_api::model_provider::ChatMessage> = Vec::new();
    ...
    fn handle_session_approve(&self, params: &Value) -> RpcResult {
        ...
        let response = match p.decision.as_str() {
            "allow_once" => dx_agent_api::channel::ChannelApprovalResponse::Approve,
            "allow_always" => dx_agent_api::channel::ChannelApprovalResponse::AlwaysApprove,
            "reject" | "reject_once" => dx_agent_api::channel::ChannelApprovalResponse::Deny,
            "reject_with_edit" => {
                let replacement = p.replacement.unwrap_or_default();
                dx_agent_api::channel::ChannelApprovalResponse::DenyWithEdit { replacement }
            }
            other => {
                return Err(rpc_err(
                    INVALID_PARAMS,
    ...
    async fn handle_config_set(&self, params: &Value) -> RpcResult {
        let req: ConfigSetParams = parse_params(params)?;
        {
            ...
            let value_str = match &req.value {
                ...
                other => dx_agent_config::typed_value::coerce_for_set_prop(
                    other,
    ...
    fn handle_config_list(&self, params: &Value) -> RpcResult {
        ...
        use dx_agent_config::traits::ConfigFieldEntry;
    ...
    fn handle_personality_templates(&self, params: &Value) -> RpcResult {
        ...
        let ctx = crate::agent::personality_templates::TemplateContext {
            agent: config
                ...
                .unwrap_or_else(|| "DX Agent".to_string()),
        ...
        let sections: Vec<ConfigSectionEntry> = ordered
            ...
            .map(|key| {
                ...
                let completed = wizard
                    .map(|w| dx_agent_config::sections::section_has_signal(&config, w))
                    .unwrap_or(false);
    ...
    fn handle_config_status(&self) -> RpcResult {
        use dx_agent_config::sections::QUICKSTART_SECTIONS;
        let config = self.ctx.config.read().clone();
        let missing: Vec<String> = QUICKSTART_SECTIONS
            .iter()
            .filter(|&&s| !dx_agent_config::sections::section_has_signal(&config, s))
    ...
    fn handle_config_catalog(&self) -> RpcResult {
        ...
        to_result(CatalogResponse::from(listing))
    }
    ...
    async fn handle_logs_query(&self, params: &Value) -> RpcResult {
        ...
        let path = dx_agent_log::current_log_path()
            .ok_or_else(|| rpc_err(INTERNAL_ERROR, "Log persistence is not enabled"))?;

        let filter = dx_agent_log::LogFilter {
            since_ts: p.since_ts,
            until_ts: p.until_ts,
            until_id: p.until_id,
            action: p.action,
        ...
        let page = dx_agent_log::load_page(&path, &filter, limit)
    ...
    async fn handle_logs_get(&self, params: &Value) -> RpcResult {
        ...
        let path = dx_agent_log::current_log_path()
            .ok_or_else(|| rpc_err(INTERNAL_ERROR, "Log persistence is not enabled"))?;
        let event = dx_agent_log::find_event_by_id(&path, &p.id)
            .map_err(|e| rpc_err(INTERNAL_ERROR, format!("Log read failed: {e:#}")))?;
        match event {
            Some(evt) => {
    ...
    // `dx-agent-gateway`. All business logic lives in
    // `dx_agent_runtime::quickstart`; these handlers are call-the-runtime
    ...
    fn signal_daemon_reload(&self) -> bool {
        ...
        ::dx_agent_log::record!(
            INFO,
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Start).with_attrs(
                ::serde_json::json!({
                    "surface": crate::quickstart::Surface::Tui.as_str(),
                })
            ),
...
fn truncate_memory_previews(
    mut entries: Vec<dx_agent_api::memory_traits::MemoryEntry>,
) -> Vec<dx_agent_api::memory_traits::MemoryEntry> {
...
mod tests {
    ...
    fn make_acp_test_config(tmp: &tempfile::TempDir) -> dx_agent_config::schema::Config {
        use std::collections::HashMap;
        use dx_agent_config::schema::{AliasedAgentConfig, RiskProfileConfig};

        let workspace_dir = tmp.path().join("workspace");
        std::fs::create_dir_all(&workspace_dir).unwrap();

        let mut providers = dx_agent_config::providers::Providers::default();
        {
            let base = providers
                .models
    ...
    fn make_acp_test_dispatcher(
        ...
        use dx_agent_infra::session_queue::SessionActorQueue;
    ...
    fn make_persistence_test_dispatcher(
        ...
        Arc<dx_agent_infra::session_sqlite::SqliteSessionBackend>,
        Arc<dx_agent_infra::acp_session_store::AcpSessionStore>,
    ) {
        use dx_agent_infra::session_queue::SessionActorQueue;
        let queue = Arc::new(SessionActorQueue::new(4, 10, 60));
        let sessions = Arc::new(crate::rpc::session::SessionStore::new(16, queue));
        let chat_backend =
            Arc::new(dx_agent_infra::session_sqlite::SqliteSessionBackend::new(data_dir).unwrap());
        let acp_store =
            Arc::new(dx_agent_infra::acp_session_store::AcpSessionStore::new(data_dir).unwrap());
        let ctx = RpcContext::for_persistence_tests(
            config,
            Arc::clone(&sessions),
            Some(chat_backend.clone() as Arc<dyn dx_agent_infra::session_backend::SessionBackend>),
    ...
    fn make_config_set_test_dispatcher(config: dx_agent_config::schema::Config) -> RpcDispatcher {
        use dx_agent_infra::session_queue::SessionActorQueue;
        let queue = Arc::new(SessionActorQueue::new(4, 10, 60));
        let sessions = Arc::new(crate::rpc::session::SessionStore::new(16, queue));
        let ctx = RpcContext::minimal(config, Arc::clone(&sessions));
    ...
    /// `default_config_and_data_dirs()` and clobbers `~/.dx-agent/config.toml`.
    fn make_secret_test_config(tmp: &tempfile::TempDir) -> dx_agent_config::schema::Config {
    ...
    async fn config_set_rejects_masked_secret_value() {
        ...
        for masked in [dx_agent_config::traits::MASKED_SECRET, "****", ""] {
            let params = json!({
                "prop": "providers.models.anthropic.default.api_key",
                "value": masked
            });
