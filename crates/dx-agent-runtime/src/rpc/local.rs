...
//! its own endpoint. `$DX_AGENT_SOCKET` overrides the endpoint path on
//! both platforms.
...
///
/// Returns `$DX_AGENT_SOCKET` when set, otherwise a per-`data_dir`
...
mod tests {
    ...
    use dx_agent_api::jsonrpc::{JSONRPC_VERSION, JsonRpcRequest};
    ...
    async fn socket_rejects_before_initialize() {
        ...
        assert_eq!(
            resp["error"]["code"],
            dx_agent_api::jsonrpc::error_codes::AUTH_REQUIRED
        );
    ...
    async fn session_approve_resolves_pending_approval() {
        ...
        let (pending_tx, mut pending_rx) =
            tokio::sync::oneshot::channel::<dx_agent_api::channel::ChannelApprovalResponse>();
        ...
        assert_eq!(
            decision,
            dx_agent_api::channel::ChannelApprovalResponse::Approve
        );
