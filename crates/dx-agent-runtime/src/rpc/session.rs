...
impl SessionStore {
    ...
    pub async fn seed_history(&self, id: &str, msgs: &[dx_agent_api::model_provider::ChatMessage]) {
    ...
    pub async fn seed_conversation_history(
        &self,
        id: &str,
        msgs: Vec<dx_agent_api::model_provider::ConversationMessage>,
    ) {
        if let Some(s) = self.sessions.lock().await.get(id) {
    ...
    pub async fn history_slice_from(
        ...
        id: &str,
        from: usize,
    ) -> Option<Vec<dx_agent_api::model_provider::ConversationMessage>> {
        let sessions = self.sessions.lock().await;
        let s = sessions.get(id)?;
...
mod tests {
    ...
    fn make_agent() -> Agent {
        ...
        let mem_cfg = dx_agent_config::schema::MemoryConfig {
            backend: "none".into(),
            ..dx_agent_config::schema::MemoryConfig::default()
        };
        let mem = Arc::from(
            dx_agent_memory::create_memory(&mem_cfg, &std::env::temp_dir(), None).unwrap(),
        );
    ...
    impl dx_agent_providers::ModelProvider for StubProvider {
        ...
        async fn chat(
            &self,
            _: dx_agent_providers::ChatRequest<'_>,
            _: &str,
            _: Option<f64>,
        ) -> anyhow::Result<dx_agent_providers::ChatResponse> {
    ...
    impl dx_agent_api::attribution::Attributable for StubProvider {
