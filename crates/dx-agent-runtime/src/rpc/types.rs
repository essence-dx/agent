//! Shared request/response types for the DX Agent RPC + gateway API surface.
//!
...
// Consumers can `use dx_agent_runtime::rpc::types::*` and get everything.
...
pub use dx_agent_api::memory_traits::{MemoryCategory, MemoryEntry};
pub use dx_agent_config::cost::types::CostSummary;
pub use dx_agent_config::traits::{ConfigFieldEntry, PropKind};
...
rpc_type! {
    /// Owned wire representation of a [`dx_agent_config::traits::MapKeySection`].
    ...
    pub struct ConfigTemplateEntry {
        pub path: String,
        pub kind: dx_agent_config::traits::MapKeyKind,
        pub value_type: String,
        pub description: String,
    }
}

impl From<dx_agent_config::traits::MapKeySection> for ConfigTemplateEntry {
    fn from(s: dx_agent_config::traits::MapKeySection) -> Self {
        Self {
...
rpc_type! {
    ...
    pub struct CatalogModelProvider {
        ...
        pub local: bool,
        #[serde(default, skip_serializing_if = "Option::is_none")]
        pub source: Option<dx_agent_providers::provider_catalog::ModelProviderCatalogSource>,
        #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl From<dx_agent_providers::ModelProviderInfo> for CatalogModelProvider {
    fn from(provider: dx_agent_providers::ModelProviderInfo) -> Self {
        ...
        Self {
            ...
            source: Some(dx_agent_providers::provider_catalog::ModelProviderCatalogSource::Native),
            model_count: None,
            identity,
            freemium,
        }
...
impl From<dx_agent_providers::provider_catalog::ModelProviderCatalogEntry>
    for CatalogModelProvider
{
    fn from(provider: dx_agent_providers::provider_catalog::ModelProviderCatalogEntry) -> Self {
        let identity = provider.identity();
        let freemium = provider.freemium();
        let model_count = (provider.model_count > 0).then_some(provider.model_count);
...
rpc_type! {
    pub struct ProviderCatalogDiagnostic {
        pub loaded: bool,
        #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl From<dx_agent_providers::provider_catalog::DxProvidersCatalogLoadDiagnostic>
    for ProviderCatalogDiagnostic
{
    fn from(
        diagnostic: dx_agent_providers::provider_catalog::DxProvidersCatalogLoadDiagnostic,
    ) -> Self {
        Self {
...
rpc_type! {
    ...
    pub struct CatalogResponse {
        ...
        #[serde(default, skip_serializing_if = "Option::is_none")]
        pub catalog: Option<ProviderCatalogDiagnostic>,
    }
}

impl From<dx_agent_providers::provider_catalog::ModelProviderCatalogListing> for CatalogResponse {
    fn from(listing: dx_agent_providers::provider_catalog::ModelProviderCatalogListing) -> Self {
        Self {
            model_providers: listing
                .entries
                .into_iter()
                .map(CatalogModelProvider::from)
                .collect(),
            catalog: listing
                .dx_providers_catalog
                .map(ProviderCatalogDiagnostic::from),
        }
    }
}

#[cfg(test)]
mod catalog_model_provider_tests {
    use super::*;

    #[test]
    fn catalog_provider_row_preserves_opencode_freemium_metadata() {
        let provider = dx_agent_providers::list_model_providers()
            .into_iter()
            .find(|provider| provider.name == "opencode")
            .expect("opencode provider row");

        let row = CatalogModelProvider::from(provider);

        assert_eq!(
            row.identity
                .as_ref()
                .expect("opencode identity")
                .canonical_id,
            "opencode-zen"
        );
        assert!(
    ...
    #[test]
    fn catalog_provider_row_marks_dx_catalog_source_and_model_count() {
        let row = CatalogModelProvider::from(
            dx_agent_providers::provider_catalog::ModelProviderCatalogEntry {
                name: "example-labs".to_string(),
                display_name: "Example Labs".to_string(),
                local: false,
                source: dx_agent_providers::provider_catalog::ModelProviderCatalogSource::DxProvidersCatalog,
                model_count: 42,
            },
        );

        assert_eq!(row.name, "example-labs");
        assert_eq!(
            row.source,
            Some(dx_agent_providers::provider_catalog::ModelProviderCatalogSource::DxProvidersCatalog)
        );
        assert_eq!(row.model_count, Some(42));
    }

    #[test]
    fn catalog_response_preserves_dx_provider_catalog_diagnostic() {
        let listing = dx_agent_providers::provider_catalog::ModelProviderCatalogListing {
            entries: vec![dx_agent_providers::provider_catalog::ModelProviderCatalogEntry {
                name: "example-labs".to_string(),
                display_name: "Example Labs".to_string(),
                local: false,
                source: dx_agent_providers::provider_catalog::ModelProviderCatalogSource::DxProvidersCatalog,
            ...
            dx_providers_catalog: Some(
                dx_agent_providers::provider_catalog::DxProvidersCatalogLoadDiagnostic {
                    path: std::path::PathBuf::from(r"G:\Dx\providers\data\missing.rkyv"),
                    provider_count: None,
                    model_count: None,
        ...
        let response = CatalogResponse::from(listing);
        let catalog = response
            .catalog
            .as_ref()
            .expect("catalog diagnostic should be preserved");

        assert_eq!(response.model_providers.len(), 1);
        ...
        let json = serde_json::to_value(&response).expect("catalog response should serialize");
        assert_eq!(
            json.pointer("/catalog/error")
...
rpc_type! {
    ...
    pub struct ConfigSectionEntry {
        ...
        /// backend picker).
        #[serde(default, skip_serializing_if = "Option::is_none")]
        pub shape: Option<dx_agent_config::sections::SectionShape>,
    }
}
...
// `dx-agent-gateway`. The wire shapes are deliberately identical so the
...
pub use dx_agent_config::presets::BuilderSubmission;

rpc_type! {
    /// Mirrors `dx_agent_gateway::api_quickstart::QuickstartState`.
    pub struct QuickstartStateResult {
        ...
        /// the canonical `dx_agent_providers::list_model_providers()`
