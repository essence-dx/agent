...
use dx_agent_api::model_provider::{ChatMessage, ChatResponse};
use dx_agent_api::tool::ToolResult;
        ...
        for h in &self.handlers {
                ...
                .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "before_model_resolve cancelled by hook");
    ...
    pub async fn run_before_prompt_build(&self, mut prompt: String) -> HookResult<String> {
        for h in &self.handlers {
                ...
                .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "before_prompt_build cancelled by hook");
                    return HookResult::Cancel(reason);
                }
                Err(_) => {
                    ::dx_agent_log::record!(
    ...
    pub async fn run_before_llm_call(
        ...
        for h in &self.handlers {
                ...
                .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "before_llm_call cancelled by hook");
                    return HookResult::Cancel(reason);
                }
                Err(_) => {
                    ::dx_agent_log::record!(
    ...
    pub async fn run_before_tool_call(
        ...
        for h in &self.handlers {
                ...
                .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "before_tool_call cancelled by hook");
                    return HookResult::Cancel(reason);
                }
                Err(_) => {
                    ::dx_agent_log::record!(
    ...
    pub async fn run_on_message_received(
        ...
        for h in &self.handlers {
                ...
                .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "on_message_received cancelled by hook");
                    return HookResult::Cancel(reason);
                }
                Err(_) => {
                    ::dx_agent_log::record!(
    ...
    pub async fn run_on_message_sending(
        ...
        for h in &self.handlers {
            ...
            .await
            {
                ...
                Ok(HookResult::Cancel(reason)) => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"hook": hook_name, "reason": reason.to_string()})), "on_message_sending cancelled by hook");
                    return HookResult::Cancel(reason);
                }
                Err(_) => {
                    ::dx_agent_log::record!(
