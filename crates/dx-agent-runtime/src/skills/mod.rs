...
const OPEN_SKILLS_SYNC_MARKER: &str = ".dx-agent-open-skills-sync";
...
const SKILLS_REGISTRY_SYNC_MARKER: &str = ".dx-agent-skills-registry-sync";
const SKILLS_REGISTRY_SYNC_INTERVAL_SECS: u64 = 60 * 60 * 24;

/// A skill is a user-defined or community-built capability.
/// Skills live in `~/.dx-agent/workspace/skills/<name>/SKILL.md`
...
impl ::dx_agent_api::attribution::Attributable for Skill {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Skill
    }
    fn alias(&self) -> &str {
        &self.name
    }
...
/// `crates/dx-agent-runtime/src/skillforge/integrate.rs`). Lives at the
...
/// [`dx_agent_config::skill_bundles::resolve_directory`] helper, and unions
...
pub fn load_skills_for_agent(
    ...
    for bundle_alias in &agent.skill_bundles {
        let bundle = match config.skill_bundles.get(bundle_alias) {
            Some(b) => b,
            None => {
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": agent_alias, "bundle": bundle_alias, "bundle_alias": bundle_alias})), "skipping skill bundle: [skill_bundles.] is not configured");
                continue;
            }
        };
        let dir = match dx_agent_config::skill_bundles::resolve_directory(
            config,
            &install_root,
            bundle_alias,
        ) {
            Ok(d) => d,
            Err(e) => {
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": agent_alias, "bundle": bundle_alias, "e": e.to_string()})), "skipping skill bundle: ");
                continue;
            }
        };
    ...
    if let Some(raw) = env_override {
        ...
        if !raw.trim().is_empty() {
            ::dx_agent_log::record!(
                ...
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                    .with_outcome(::dx_agent_log::EventOutcome::Unknown),
                "Ignoring invalid DX_AGENT_OPEN_SKILLS_ENABLED (valid: 1|0|true|false|yes|no|on|off)"
            );
        }
    }
...
fn open_skills_enabled(config_open_skills_enabled: Option<bool>) -> bool {
    let env_override = std::env::var("DX_AGENT_OPEN_SKILLS_ENABLED").ok();
...
fn resolve_open_skills_dir(config_open_skills_dir: Option<&str>) -> Option<PathBuf> {
    let env_dir = std::env::var("DX_AGENT_OPEN_SKILLS_DIR").ok();
...
pub fn skills_to_prompt_with_mode(
    skills: &[Skill],
    workspace_dir: &Path,
    mode: dx_agent_config::schema::SkillsPromptInjectionMode,
) -> String {
    use std::fmt::Write;
    ...
    let mut prompt = match mode {
        dx_agent_config::schema::SkillsPromptInjectionMode::Full => String::from(
            "## Available Skills\n\n\
             Skill instructions and tool metadata are preloaded below.\n\
             Follow these instructions directly; do not read skill files at runtime unless the user asks.\n\n\
             <available_skills>\n",
        ),
        dx_agent_config::schema::SkillsPromptInjectionMode::Compact => String::from(
            "## Available Skills\n\n\
...
) -> Vec<Box<dyn dx_agent_api::tool::Tool>> {
...
fn resolve_elevated_tool(
    ...
    tool: &SkillTool,
    kind_label: &str,
    resolution_registry: &[std::sync::Arc<dyn dx_agent_api::tool::Tool>],
) -> Option<Box<dyn dx_agent_api::tool::Tool>> {
...
pub fn skills_to_tools_with_context(
    skills: &[Skill],
    security: std::sync::Arc<crate::security::SecurityPolicy>,
    unfiltered_registry: &[std::sync::Arc<dyn dx_agent_api::tool::Tool>],
) -> Vec<Box<dyn dx_agent_api::tool::Tool>> {
    let mut tools: Vec<Box<dyn dx_agent_api::tool::Tool>> = Vec::new();
    for skill in skills {
        for tool in &skill.tools {
            match tool.kind.as_str() {
                "shell" | "script" => {
                    ...
                    tools.push(Box::new(dx_agent_tools::wrappers::RateLimitedTool::new(
                        inner,
                        security.clone(),
                    )));
                }
...
#[cfg(feature = "plugins-wasm")]
pub fn load_plugin_skills_from_config(config: &dx_agent_config::schema::Config) -> Vec<Skill> {
    ...
    let signature_mode = dx_agent_plugins::host::PluginHost::parse_signature_mode(
        &config.plugins.security.signature_mode,
    );
    let trusted_keys = config.plugins.security.trusted_publisher_keys.clone();

    let host = match dx_agent_plugins::host::PluginHost::with_security(
        &parent,
        signature_mode,
        trusted_keys,
    ) {
...
mod registry_tests {
    ...
    fn test_is_registry_source_rejects_paths() {
        ...
        assert!(!is_registry_source("~/.dx-agent/skills/foo"));
    }
    ...
    fn build_install_tier_banner_community_warns() {
        ...
        assert!(banner.contains("not audited by DX Agent"));
