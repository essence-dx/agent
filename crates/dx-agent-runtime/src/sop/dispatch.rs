...
pub async fn dispatch_sop_event(
    ...
    for run in &started_runs {
        let span = dx_agent_log::attribution_span!(run);
        let run_id = run.run_id.clone();
        if let Err(e) = dx_agent_log::scope!(
            session_key: run_id,
            =>
        ...
        match result {
            DispatchResult::Started {
                ...
                SopRunAction::Failed { reason, .. } => {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"run_id": run_id, "sop_name": sop_name, "reason": reason.to_string()})), "SOP headless dispatch: run  ('') failed: ");
                }
            },
            DispatchResult::Skipped { sop_name, reason } => {
                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"sop_name": sop_name, "reason": reason.to_string()})), "SOP headless dispatch: skipped '': ");
            }
...
mod tests {
    ...
    use dx_agent_config::schema::{MemoryConfig, SopConfig};
    use dx_agent_memory::traits::Memory;
