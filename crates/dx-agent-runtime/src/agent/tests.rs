...
use dx_agent_config::schema::{AliasedAgentConfig, MemoryConfig};
use dx_agent_memory::{self, Memory};

dx_agent_api::mock_tool_attribution!(CountingTool, EchoTool, FailingTool, PanickingTool);
...
impl ::dx_agent_api::attribution::Attributable for ToolSpecCaptureModelProvider {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        ::dx_agent_api::attribution::Role::Provider(
            ::dx_agent_api::attribution::ProviderKind::Model(
                ::dx_agent_api::attribution::ModelProviderKind::Custom,
...
fn make_memory() -> Arc<dyn Memory> {
    ...
    Arc::from(dx_agent_memory::create_memory(&cfg, &std::env::temp_dir(), None).unwrap())
}
...
fn make_sqlite_memory() -> (Arc<dyn Memory>, tempfile::TempDir) {
    ...
    let mem = Arc::from(dx_agent_memory::create_memory(&cfg, tmp.path(), None).unwrap());
    (mem, tmp)
}
