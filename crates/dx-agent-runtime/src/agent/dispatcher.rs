...
use std::fmt::Write;
use dx_agent_providers::{ChatMessage, ChatResponse, ConversationMessage, ToolResultMessage};
        ...
        let calls = response
            ...
            .map(|tc| ParsedToolCall {
                ...
                arguments: serde_json::from_str(&tc.arguments).unwrap_or_else(|e| {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"tool": tc.name, "error": format!("{}", e)})), "Failed to parse native tool call arguments as JSON; defaulting to empty object");
