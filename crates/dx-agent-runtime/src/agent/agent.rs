...
use dx_agent_memory::{self, Memory, MemoryCategory};
use dx_agent_providers::{
    self, ChatMessage, ChatRequest, ConversationMessage, ModelProvider, ToolResultMessage,
};
use dx_agent_tool_call_parser::strip_think_tags;

// Re-export TurnEvent from zeroclaw-types for backwards compatibility.
pub use dx_agent_api::agent::TurnEvent;
...
pub struct Agent {
    ...
    memory_strategy: Arc<dyn dx_agent_api::memory_traits::MemoryStrategy>,
    config: dx_agent_config::schema::AliasedAgentConfig,
    multimodal_config: dx_agent_config::schema::MultimodalConfig,
    model_name: String,
    ...
    identity_config: dx_agent_config::schema::IdentityConfig,
    skills: Vec<crate::skills::Skill>,
    skills_prompt_mode: dx_agent_config::schema::SkillsPromptInjectionMode,
    auto_save: bool,
    memory_session_id: Option<String>,
    history: Vec<ConversationMessage>,
    classification_config: dx_agent_config::schema::QueryClassificationConfig,
    available_hints: Vec<String>,
    route_model_by_hint: HashMap<String, String>,
    #[allow(dead_code)] // WIP: stored for future runtime tool filtering
    allowed_tools: Option<Vec<String>>,
    response_cache: Option<Arc<dx_agent_memory::response_cache::ResponseCache>>,
    ...
    image_cache: dx_agent_providers::multimodal::LocalImageCache,
}
    ...
    fn drop(&mut self) {
        ::dx_agent_log::record!(
            ...
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note)
                .with_category(::dx_agent_log::EventCategory::Agent)
...
impl AgentChannelHandles {
    ...
    pub fn register_channel(
        ...
        channel: Arc<dyn dx_agent_api::channel::Channel>,
    ) {
        let name = name.into();
    ...
    pub fn get_channel(&self, name: &str) -> Option<Arc<dyn dx_agent_api::channel::Channel>> {
        for handle in self.populated_handles().into_iter().flatten() {
...
pub struct AgentBuilder {
    ...
    memory_strategy: Option<Arc<dyn dx_agent_api::memory_traits::MemoryStrategy>>,
    config: Option<dx_agent_config::schema::AliasedAgentConfig>,
    multimodal_config: Option<dx_agent_config::schema::MultimodalConfig>,
    model_name: Option<String>,
    model_provider_name: Option<String>,
    temperature: Option<f64>,
    workspace_dir: Option<std::path::PathBuf>,
    agent_workspace_dir: Option<std::path::PathBuf>,
    identity_config: Option<dx_agent_config::schema::IdentityConfig>,
    skills: Option<Vec<crate::skills::Skill>>,
    skills_prompt_mode: Option<dx_agent_config::schema::SkillsPromptInjectionMode>,
    auto_save: Option<bool>,
    memory_session_id: Option<String>,
    classification_config: Option<dx_agent_config::schema::QueryClassificationConfig>,
...
impl AgentBuilder {
    ...
    pub fn config(mut self, config: dx_agent_config::schema::AliasedAgentConfig) -> Self {
    ...
    pub fn response_cache(
        mut self,
        cache: Option<Arc<dx_agent_memory::response_cache::ResponseCache>>,
    ) -> Self {
    ...
    pub fn build(self) -> Result<Agent> {
        ...
        let memory: Arc<dyn Memory> = if exclude_memory {
            Arc::new(dx_agent_memory::NoneMemory::new("none"))
        } else {
        ...
        Ok(Agent {
            ...
            memory_strategy: memory_strategy.unwrap_or_else(|| {
                Arc::new(
                    crate::agent::memory_strategy::DefaultMemoryStrategy::with_config(
                        memory.clone(),
                        dx_agent_config::schema::MemoryConfig::default(),
                        workspace_dir.clone(),
                    ),
                )
            }),
            ...
            exclude_memory,
            image_cache: dx_agent_providers::multimodal::LocalImageCache::new(),
        })
    }
}
...
impl Agent {
    ...
    pub fn populate_channels(
        &self,
        channel_map: &std::collections::HashMap<String, Arc<dyn dx_agent_api::channel::Channel>>,
    ) -> Vec<String> {
    ...
    fn response_cache_key_for_messages(
        ...
        Some(dx_agent_memory::response_cache::ResponseCache::cache_key(
            effective_model,
            system,
            &transcript,
        ))
    ...
    fn synthesize_cancelled_tool_results(
        ...
        remaining: &[dx_agent_providers::ToolCall],
        new_msgs: &mut Vec<ConversationMessage>,
    ) {
    ...
    fn parse_response_for_effective_tools(
        &self,
        response: &dx_agent_providers::ChatResponse,
    ...
    async fn from_config_with_session_cwd_and_mcp_approval_mode(
        ...
        if let Err(e) = tokio::fs::create_dir_all(&agent_workspace).await {
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": agent_alias, "workspace": agent_workspace.display().to_string(), "e": e.to_string()})), "Failed to create per-agent workspace dir (continuing): ");
        }
        ...
        // identity to load.
        if let Err(e) = dx_agent_config::schema::ensure_bootstrap_files(&agent_workspace).await {
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"agent": agent_alias, "workspace": agent_workspace.display().to_string(), "e": e.to_string()})), "Failed to ensure per-agent bootstrap files (continuing with whatever exists): ");
        }
        ...
        let memory: Arc<dyn Memory> = dx_agent_memory::create_memory_for_agent(
            config,
            agent_alias,
        ...
        let provider_runtime_options = dx_agent_providers::provider_runtime_options_for_alias(
            config,
            provider_name,
            provider_alias,
        );
        ...
        let model_provider: Box<dyn ModelProvider> =
            dx_agent_providers::create_routed_model_provider_with_options(
                config,
                &provider_ref,
                agent_model_provider.and_then(|e| e.api_key.as_deref()),
        ...
        let response_cache = if config.memory.response_cache_enabled {
            dx_agent_memory::response_cache::ResponseCache::with_hot_cache(
                &config.data_dir,
    ...
    fn trim_history(&mut self) {
        ...
        if other_messages.len() > max {
            ...
            ::dx_agent_log::record!(
                DEBUG,
                ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Complete)
                    .with_category(::dx_agent_log::EventCategory::Agent)
    ...
    async fn prepare_provider_messages(
        ...
        let prepared = dx_agent_providers::multimodal::prepare_messages_for_provider_cached(
            messages,
    ...
    async fn execute_tool_call(&self, call: &ParsedToolCall) -> ToolExecutionResult {
        ...
        if let Some(ref hooks) = self.hook_runner {
                ...
                .await
            {
                ...
                crate::hooks::HookResult::Cancel(reason) => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"tool": call.name, "reason": reason.to_string()})), "tool call cancelled by hook");
            ...
            && approval_requirement == ApprovalRequirement::Prompt
        {
            ...
            let (decision, decision_channel) = if mgr.is_non_interactive() {
                ...
                let ch_request = dx_agent_api::channel::ChannelApprovalRequest {
                    tool_name: request.tool_name.clone(),
                    arguments_summary: crate::approval::summarize_args(&request.arguments),
                    raw_arguments: Some(request.arguments.clone()),
                };
                let mut channel_decision: Option<dx_agent_api::channel::ChannelApprovalResponse> =
                    None;
                ...
                let channels: Vec<(String, Arc<dyn dx_agent_api::channel::Channel>)> = self
                    .channel_handles
                    .ask_user
                    .as_ref()
                    .map(|h| {
                ...
                for (ch_name, ch) in &channels {
                    match ch.request_approval("", &ch_request).await {
                        ...
                        Ok(None) => continue,
                        Err(e) => {
                            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"tool": tool_name, "channel": ch_name, "error": format!("{}", e)})), "channel approval request failed");
                        }
                    }
                }
                let approval = match channel_decision {
                    Some(dx_agent_api::channel::ChannelApprovalResponse::Approve) => {
                        ApprovalResponse::Yes
                    }
                    Some(dx_agent_api::channel::ChannelApprovalResponse::AlwaysApprove) => {
                        ApprovalResponse::Always
                    }
                    Some(dx_agent_api::channel::ChannelApprovalResponse::Deny) => {
                        ApprovalResponse::No
                    }
                    Some(dx_agent_api::channel::ChannelApprovalResponse::DenyWithEdit {
                        replacement,
        ...
        ::dx_agent_log::record!(
            DEBUG,
            ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Invoke)
                .with_category(::dx_agent_log::EventCategory::Tool)
                .with_attrs(::serde_json::json!({
                    "tool": tool_name,
    ...
    fn classify_model(&self, user_message: &str) -> String {
            ...
            && self.available_hints.contains(&decision.hint)
        {
            let resolved_model = self
                ...
                .map(String::as_str)
                .unwrap_or("unknown");
            ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"hint": decision.hint.as_str(), "model": resolved_model, "rule_priority": decision.priority, "message_length": user_message.len()})), "Classified message route");
        ...
        if let Some(ref ac) = self.config.resolved.auto_classify {
            ...
            {
                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"hint": hint, "complexity": format!("{:?}", tier), "message_length": user_message.len()})), "Auto-classified by complexity");
    ...
    pub async fn turn(&mut self, user_message: &str) -> Result<String> {
        ...
        for _ in 0..self.config.resolved.max_tool_iterations {
                ...
                .await
            {
                ...
                Err(err) => {
                    let safe_error = dx_agent_providers::sanitize_api_error(&err.to_string());
                    self.observer.record_event(&ObserverEvent::LlmResponse {
                        model_provider: self.model_provider_name.clone(),
                        model: effective_model.clone(),
                        duration: llm_started_at.elapsed(),
    ...
    pub async fn turn_streamed_with_steering_state(
        ...
        for _ in 0..self.config.resolved.max_tool_iterations {
            ...
            let stream_opts = dx_agent_providers::traits::StreamOptions::new(
                self.model_provider.supports_streaming(),
            );
            let mut stream = self.model_provider.stream_chat(
                dx_agent_providers::ChatRequest {
                    messages: &prepared_messages,
                    tools: if self.should_send_tool_specs() {
                        Some(&self.tool_specs)
                    } else {
            ...
            let mut streamed_tool_calls: Vec<dx_agent_providers::traits::ToolCall> = Vec::new();
            let mut streamed_usage: Option<dx_agent_providers::traits::TokenUsage> = None;
            ...
            loop {
                ...
                match item {
                    Ok(event) => match event {
                        dx_agent_providers::traits::StreamEvent::TextDelta(chunk) => {
                        ...
                        dx_agent_providers::traits::StreamEvent::PreExecutedToolResult {
                            name,
                            output,
                        } => {
                        ...
                        dx_agent_providers::traits::StreamEvent::Usage(usage) => {
                            streamed_usage = Some(usage);
                        }
                        dx_agent_providers::traits::StreamEvent::Final => break,
                    },
                    Err(error) => {
            ...
            if stream_error.is_some() && visible_streamed_output {
                ...
                let safe_error = dx_agent_providers::sanitize_api_error(
            ...
            let response = if got_stream && stream_error.is_none() {
                ...
                dx_agent_providers::ChatResponse {
                    ...
                    ::dx_agent_log::record!(
                        ...
                        ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Fail)
                            ...
                            .with_attrs(::serde_json::json!({
                                ...
                                "error": dx_agent_providers::sanitize_api_error(error),
                            })),
                ...
                match chat_result {
                    Ok(resp) => resp,
                    Err(error) => {
                        let safe_error = dx_agent_providers::sanitize_api_error(&error.to_string());
                        self.observer.record_event(&ObserverEvent::LlmResponse {
                            model_provider: self.model_provider_name.clone(),
                            model: effective_model.clone(),
                            duration: llm_started_at.elapsed(),
    ...
    pub async fn run_interactive(&mut self) -> Result<()> {
        ...
        let listen_handle = dx_agent_spawn::spawn!(async move {
            let _ = dx_agent_api::channel::Channel::listen(&*cli, tx).await;
        });
...
mod tests {
    ...
    use dx_agent_api::observability_traits::ObserverMetric;

    dx_agent_api::mock_tool_attribution!(
        CountingTool,
        NamedMockTool,
        MockTool,
    ...
    struct MockModelProvider {
        responses: Mutex<Vec<dx_agent_providers::ChatResponse>>,
    }
    ...
    impl ModelProvider for MockModelProvider {
        ...
        async fn chat(
            ...
            _model: &str,
            _temperature: Option<f64>,
        ) -> Result<dx_agent_providers::ChatResponse> {
            let mut guard = self.responses.lock();
            if guard.is_empty() {
                return Ok(dx_agent_providers::ChatResponse {
                    text: Some("done".into()),
                    tool_calls: vec![],
                    usage: None,
    ...
    impl ::dx_agent_api::attribution::Attributable for TranscriptCaptureModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ModelProvider for StreamingSteeringModelProvider {
        ...
        async fn chat(
            ...
            Ok(dx_agent_providers::ChatResponse {
        ...
        fn stream_chat(
            ...
            _model: &str,
            _temperature: Option<f64>,
            _options: dx_agent_providers::traits::StreamOptions,
        ) -> futures_util::stream::BoxStream<
            'static,
            dx_agent_providers::traits::StreamResult<dx_agent_providers::traits::StreamEvent>,
        > {
                ...
                async move {
                    match step {
                        0 if should_fail => Some((
                            Err(dx_agent_providers::traits::StreamError::ModelProvider(
                                "synthetic provider failure".into(),
                            )),
                            1,
                        )),
                        0 => Some((
                            Ok(dx_agent_providers::traits::StreamEvent::TextDelta(
                                dx_agent_providers::traits::StreamChunk {
                                    delta,
                                    is_final: false,
                                    reasoning: None,
                                    token_count: 0,
                        ...
                        1 => {
                            ...
                            Some((Ok(dx_agent_providers::traits::StreamEvent::Final), 2))
                        }
                        _ => None,
                    }
                }
    ...
    impl ::dx_agent_api::attribution::Attributable for StreamingSteeringModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ModelProvider for MultimodalCaptureProvider {
        ...
        > {
            ...
            if self.streamed {
                let chunk = dx_agent_providers::traits::StreamEvent::TextDelta(
                    dx_agent_providers::traits::StreamChunk {
                ...
                stream::iter(vec![
                    Ok(chunk),
                    Ok(dx_agent_providers::traits::StreamEvent::Final),
                ])
                .boxed()
            } else {
                stream::iter(vec![Ok(dx_agent_providers::traits::StreamEvent::Final)]).boxed()
            }
        }
    ...
    impl ::dx_agent_api::attribution::Attributable for MultimodalCaptureProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    struct ApprovalChannel {
        response: dx_agent_api::channel::ChannelApprovalResponse,
        requests: Arc<AtomicUsize>,
    }

    impl ::dx_agent_api::attribution::Attributable for ApprovalChannel {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
    ...
    #[async_trait]
    impl dx_agent_api::channel::Channel for ApprovalChannel {
        fn name(&self) -> &str {
            "acp"
        }
        ...
        async fn request_approval(
            &self,
            _recipient: &str,
            _request: &dx_agent_api::channel::ChannelApprovalRequest,
        ) -> anyhow::Result<Option<dx_agent_api::channel::ChannelApprovalResponse>> {
    ...
    async fn turn_without_tools_returns_text() {
        let model_provider = Box::new(MockModelProvider {
            responses: Mutex::new(vec![dx_agent_providers::ChatResponse {
        ...
        let memory_cfg = dx_agent_config::schema::MemoryConfig {
            backend: "none".into(),
            ..dx_agent_config::schema::MemoryConfig::default()
        };
        let mem: Arc<dyn Memory> = Arc::from(
            dx_agent_memory::create_memory(&memory_cfg, std::path::Path::new("/tmp"), None)
    ...
    async fn direct_agent_strict_tool_parsing_ignores_xml_dispatcher_calls() {
        ...
        let agent_config = dx_agent_config::schema::AliasedAgentConfig {
            resolved: dx_agent_config::schema::ResolvedRuntime {
                strict_tool_parsing: true,
                ..Default::default()
            },
            ..dx_agent_config::schema::AliasedAgentConfig::default()
        };
        let mut agent = Agent::builder()
    ...
    fn native_agent_prompt_omits_duplicate_tools_section() {
        ...
        let mem: Arc<dyn Memory> = Arc::from(
            dx_agent_memory::create_memory(&memory_cfg, workspace.path(), None)
                .expect("memory creation should succeed with valid config"),
        );
        let observer: Arc<dyn Observer> = Arc::from(crate::observability::NoopObserver {});
    ...
    async fn direct_agent_tool_execution_requests_acp_approval() {
        ...
        let approval_cfg = dx_agent_config::schema::RiskProfileConfig {
        ...
        let channel: Arc<dyn dx_agent_api::channel::Channel> = Arc::new(ApprovalChannel {
            response: dx_agent_api::channel::ChannelApprovalResponse::Approve,
    ...
    async fn direct_agent_tool_execution_denies_when_acp_rejects() {
        ...
        agent.channel_handles.ask_user = Some(Arc::clone(&handle));
        let channel: Arc<dyn dx_agent_api::channel::Channel> = Arc::new(ApprovalChannel {
            response: dx_agent_api::channel::ChannelApprovalResponse::Deny,
            requests: Arc::clone(&approval_requests),
        });
        agent.channel_handles().register_channel("acp", channel);
    ...
    async fn direct_agent_shell_does_not_trust_model_supplied_approved_arg() {
        ...
        let approval_cfg = dx_agent_config::schema::RiskProfileConfig::default();
        let mut agent = Agent::builder()
            .model_provider(provider)
    ...
    async fn direct_agent_shell_keeps_runtime_approval_from_always_allowlist() {
        ...
        agent.channel_handles.ask_user = Some(Arc::clone(&handle));
        let channel: Arc<dyn dx_agent_api::channel::Channel> = Arc::new(ApprovalChannel {
            response: dx_agent_api::channel::ChannelApprovalResponse::AlwaysApprove,
            requests: Arc::clone(&approval_requests),
        });
        agent.channel_handles().register_channel("acp", channel);
        ...
        let model_provider = Box::new(MockModelProvider {
            responses: Mutex::new(vec![
                dx_agent_providers::ChatResponse {
                    ...
                    tool_calls: vec![dx_agent_providers::ToolCall {
                        id: "tc1".into(),
                        name: "echo".into(),
                        arguments: "{}".into(),
                        extra_content: None,
    ...
    async fn from_config_passes_extra_headers_to_custom_provider() {
        ...
        let server_handle = dx_agent_spawn::spawn!(async move {
        ...
        let mut config = dx_agent_config::schema::Config {
    ...
    fn session_cwd_keeps_workspace_in_allowed_roots() {
        ...
        let risk_profile = dx_agent_config::schema::RiskProfileConfig::default();
    ...
    fn seed_conversation_history_preserves_tool_call_variants() {
        use dx_agent_api::model_provider::{
    ...
    impl ModelProvider for StreamToolCaptureModelProvider {
        ...
        > {
            ...
            *count += 1;
            if *count == 1 {
                let tc = dx_agent_providers::traits::StreamEvent::ToolCall(
                    dx_agent_providers::ToolCall {
                        id: "00000000-0000-0000-0000-000000000001".into(),
                        name: "echo".into(),
                        arguments: "{}".into(),
                        extra_content: None,
    ...
    impl ::dx_agent_api::attribution::Attributable for StreamToolCaptureModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ModelProvider for TwoToolCallStreamModelProvider {
        ...
        > {
            ...
            *count += 1;
            if *count == 1 {
                stream::iter(vec![
                    Ok(dx_agent_providers::traits::StreamEvent::ToolCall(
                        dx_agent_providers::ToolCall {
                            id: "00000000-0000-0000-0000-000000000001".into(),
                            name: "echo".into(),
                            arguments: "{}".into(),
    ...
    impl ::dx_agent_api::attribution::Attributable for TwoToolCallStreamModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    impl ::dx_agent_api::attribution::Attributable for PreExecutedToolModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    async fn turn_normalizes_user_image_markers_before_provider_call() {
        ...
        let mut agent = Agent::builder()
            ...
            .tool_dispatcher(Box::new(NativeToolDispatcher))
            .workspace_dir(std::path::PathBuf::from("/tmp"))
            .multimodal_config(dx_agent_config::schema::MultimodalConfig::default())
            .build()
            .expect("agent builder should succeed with valid config");
    ...
    fn trim_history_does_not_leave_orphan_tool_results() {
        use dx_agent_providers::{ToolCall, ToolResultMessage};
    ...
    fn cancel_synthesizes_paired_tool_results_for_orphaned_calls() {
        use dx_agent_providers::ToolCall;
        ...
        let mut agent = Agent::builder()
            ...
            .tool_dispatcher(Box::new(NativeToolDispatcher))
            .workspace_dir(std::path::PathBuf::from("/tmp"))
            .config(dx_agent_config::schema::AliasedAgentConfig::default())
            .build()
            .expect("agent builder should succeed with valid config");
    ...
    impl ::dx_agent_api::attribution::Attributable for NarrationStreamModelProvider {
        fn role(&self) -> ::dx_agent_api::attribution::Role {
            ::dx_agent_api::attribution::Role::Provider(
                ::dx_agent_api::attribution::ProviderKind::Model(
                    ::dx_agent_api::attribution::ModelProviderKind::Custom,
    ...
    async fn response_cache_key_uses_full_provider_visible_transcript() {
        ...
        let cache = Arc::new(
            dx_agent_memory::response_cache::ResponseCache::new(tmp.path(), 60, 100)
    ...
    async fn turn_streamed_with_steering_commits_streamed_output_before_continuing() {
        ...
        let handle = dx_agent_spawn::spawn!(async move {
            agent
    ...
    async fn turn_streamed_cancel_during_stream_emits_llm_response_failure() {
        ...
        let canceller = dx_agent_spawn::spawn!(async move {
