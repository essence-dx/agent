...
use dx_agent_api::model_provider::{ChatMessage, ModelProvider};
use dx_agent_memory::traits::Memory;
use dx_agent_providers::multimodal;

pub use dx_agent_config::scattered_types::ContextCompressionConfig;
        ...
        if let Some(ref memory) = self.memory {
            ...
            if let Err(e) = memory
                .store(
                    &facts_key,
                    &summary,
                    dx_agent_memory::traits::MemoryCategory::Daily,
                    None,
                )
                .await
            {
...
mod tests {
    ...
    impl ModelProvider for CaptureSummarizerModelProvider {
        ...
        async fn chat(
            &self,
            _request: dx_agent_api::model_provider::ChatRequest<'_>,
            _model: &str,
            _temperature: Option<f64>,
        ) -> Result<dx_agent_api::model_provider::ChatResponse> {
    ...
    impl ::dx_agent_api::attribution::Attributable for CaptureSummarizerModelProvider {
    ...
    fn test_build_summarizer_transcript_strips_all_attachment_kinds_for_non_vision_provider() {
        ...
        // `crates/dx-agent-channels/src/util.rs`. Regression: a `[PHOTO:]`
