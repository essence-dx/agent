use std::sync::Arc;
use dx_agent_api::memory_traits::{Memory, MemoryStrategy};
...
pub struct DefaultMemoryStrategy {
    ...
    memory_config: dx_agent_config::schema::MemoryConfig,
...
impl MemoryStrategy for DefaultMemoryStrategy {
    ...
    async fn consolidate_turn(
        ...
        dx_agent_memory::consolidation::consolidate_turn(
            provider,
            model,
            temperature,
            self.memory.as_ref(),
    ...
    async fn run_governance(&self) -> anyhow::Result<()> {
        ...
        dx_agent_memory::hygiene::run_if_due(&self.memory_config, &self.workspace_dir)
    }
}
