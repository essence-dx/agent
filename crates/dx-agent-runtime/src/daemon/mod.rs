...
use dx_agent_memory::{MEMORY_CONTEXT_CLOSE, MEMORY_CONTEXT_OPEN};
...
async fn wait_for_exit_signal(
    ...
    #[cfg(unix)]
    {
        ...
        loop {
            tokio::select! {
                _ = sigint.recv() => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "Received SIGINT, shutting down...");
                    return Ok(DaemonExit::Shutdown);
                }
                _ = sigterm.recv() => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "Received SIGTERM, shutting down...");
                    return Ok(DaemonExit::Shutdown);
                }
                _ = sighup.recv() => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "Received SIGHUP, ignoring (daemon stays running)");
                }
                changed = reload_rx.changed() => {
                    if changed.is_err() {
                        ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown), "Reload sender dropped; shutting down");
                        return Ok(DaemonExit::Shutdown);
                    }
                    if *reload_rx.borrow_and_update() {
                        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "Reload requested via /admin/reload");
                        return Ok(DaemonExit::Reload);
                    }
                }
                _ = &mut ephemeral_shutdown => {
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "Ephemeral daemon: no clients remaining, shutting down");
                    return Ok(DaemonExit::Shutdown);
                }
            }
        }
        ...
        loop {
            tokio::select! {
                res = tokio::signal::ctrl_c() => {
                    res?;
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note), "Received Ctrl+C, shutting down...");
                    return Ok(DaemonExit::Shutdown);
                }
                changed = reload_rx.changed() => {
                    if changed.is_err() {
...
pub struct DaemonSubsystems {
    ...
    pub mqtt_start: Option<
        Box<
            dyn Fn(
                    dx_agent_config::schema::MqttConfig,
                ) -> std::pin::Pin<Box<dyn Future<Output = Result<()>> + Send>>
                + Send
                + Sync,
        >,
...
pub async fn run(
    ...
    let rpc_ctx = if need_rpc_ctx {
        ...
        let session_backend = dx_agent_infra::make_session_backend(
            &config.data_dir,
        ...
        let rpc_memory: Option<std::sync::Arc<dyn dx_agent_api::memory_traits::Memory>> = if config
            .agents
            .is_empty()
        {
            None
        ...
        let acp_session_store: Option<
            std::sync::Arc<dx_agent_infra::acp_session_store::AcpSessionStore>,
        > = match dx_agent_infra::acp_session_store::AcpSessionStore::new(&config.data_dir) {
    ...
    println!("🧠 DX Agent daemon started");
...
async fn run_heartbeat_worker(config: Config) -> Result<()> {
    ...
    loop {
        ...
        let tasks_to_run = if two_phase {
            ...
            match phase1_result {
                Ok(response) => {
                    ...
                    ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"selected": indices.len(), "total": tasks.len()})), "heartbeat phase 1: running task subset");
                    indices
                        .into_iter()
        ...
        let heartbeat_memory: Option<Box<dyn dx_agent_memory::Memory>> =
            dx_agent_memory::create_memory(
                &config.memory,
                &config.data_dir,
                config
                    ...
                    Ok(entries) if !entries.is_empty() => {
                        let ctx: String = entries
                            ...
                            .filter(|e| {
                                !matches!(
                                    e.category,
                                    dx_agent_memory::traits::MemoryCategory::Conversation
                                )
                            })
                        ...
                        && let Some(ref mem) = heartbeat_memory
                    {
                        ...
                        let _ = mem
                            .store(
                                ...
                                dx_agent_memory::MemoryCategory::Daily,
                                None,
                            )
                            .await;
                    }
...
fn load_heartbeat_session_context(config: &Config) -> Option<String> {
    use dx_agent_providers::traits::ChatMessage;
...
fn load_jsonl_messages(path: &std::path::Path) -> Vec<dx_agent_providers::traits::ChatMessage> {
    ...
    let mut window: VecDeque<dx_agent_providers::traits::ChatMessage> =
    ...
    for line in reader.lines() {
        ...
        if let Ok(msg) = serde_json::from_str::<dx_agent_providers::traits::ChatMessage>(trimmed) {
...
// run_mqtt_sop_listener has been moved to dx-agent-channels::orchestrator::mqtt.
...
mod tests {
    ...
    fn all_disabled_channels_not_supervised() {
        ...
        config.channels.discord.insert(
            "clamps".to_string(),
            dx_agent_config::schema::DiscordConfig {
                enabled: false,
                ...
                mention_only: true,
                stream_mode: dx_agent_config::schema::StreamMode::default(),
    ...
    fn detects_dingtalk_as_supervised_channel() {
        ...
        config.channels.dingtalk.insert(
            "default".to_string(),
            dx_agent_config::schema::DingTalkConfig {
                enabled: true,
    ...
    fn detects_qq_as_supervised_channel() {
        ...
        config.channels.qq.insert(
            "default".to_string(),
            dx_agent_config::schema::QQConfig {
                enabled: true,
                app_id: "app-id".into(),
    ...
    async fn sighup_does_not_shut_down_daemon() {
        ...
        let handle = dx_agent_spawn::spawn!(wait_for_exit_signal(reload_rx, false, count));
    ...
    async fn ephemeral_does_not_exit_before_client_connects() {
        ...
        let (_reload_tx, reload_rx) = tokio::sync::watch::channel(false);
        let count = std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(0));
        let handle = dx_agent_spawn::spawn!(wait_for_exit_signal(reload_rx, true, count));
    ...
    async fn ephemeral_exits_after_client_disconnects() {
        ...
        let handle = dx_agent_spawn::spawn!(wait_for_exit_signal(reload_rx, true, count2));
    ...
    async fn ephemeral_grace_period_resets_on_reconnect() {
        ...
        let count = std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(0));
        let count2 = count.clone();
        let mut handle = dx_agent_spawn::spawn!(wait_for_exit_signal(reload_rx, true, count2));
