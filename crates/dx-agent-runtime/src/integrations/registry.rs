...
//! - AI providers: `dx_agent_providers::list_providers()` (each
...
fn evaluate_model_provider_activation(
    config: &Config,
    info: &dx_agent_providers::ModelProviderInfo,
) -> IntegrationStatus {
    bool_to_status(
        config
            .providers
...
pub fn all_integrations(config: &Config) -> Vec<IntegrationEntry> {
    ...
    let providers = dx_agent_providers::list_model_providers()
        .into_iter()
        .map(|info| {
...
mod tests {
    use super::*;
    use dx_agent_config::schema::Config;
    use dx_agent_config::schema::{IMessageConfig, MatrixConfig, StreamMode, TelegramConfig};
    ...
    fn cron_active_when_any_job_configured() {
        ...
        config.cron.insert(
            ...
            dx_agent_config::schema::CronJobDecl::default(),
        );
        assert!(matches!(
            toggle_status(&config, |n| n == "Cron"),
            IntegrationStatus::Active
    ...
    fn populated_typed_slot_activates_corresponding_ai_integration() {
        ...
        for info in dx_agent_providers::list_model_providers() {
            let mut config = Config::default();
            assert!(
                config
                    .providers
