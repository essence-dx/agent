...
mod tests {
    ...
    use dx_agent_tools::wrappers::{PathGuardedTool, RateLimitedTool};
    ...
    async fn shell_does_not_leak_api_key() {
        ...
        let _g2 = EnvGuard::set("DX_AGENT_API_KEY", "sk-test-secret-67890");
        ...
        assert!(
            ...
            "DX_AGENT_API_KEY leaked to shell command output"
        );
    }
    ...
    async fn shell_allows_configured_env_passthrough() {
        let _guard = EnvGuard::set("DX_AGENT_TEST_PASSTHROUGH", "db://unit-test");
        let tool = ShellTool::new(
            test_security_with_env_passthrough(&["DX_AGENT_TEST_PASSTHROUGH"]),
            test_runtime(),
        );
        ...
        assert!(
            result
                .output
                .contains("DX_AGENT_TEST_PASSTHROUGH=db://unit-test")
        );
    }
