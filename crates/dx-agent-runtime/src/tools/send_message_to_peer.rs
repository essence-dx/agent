...
//!   `dx_agent_spawn::spawn!`, so the sender's `ToolResult.success = true`
    ...
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        if target_is_agent {
            ...
            dx_agent_spawn::spawn!(async move {
                ...
                {
                    ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"sender": sender, "recipient": recipient_alias, "error": format!("{}", e)})), "peer-message in-process delivery failed");
                }
            });
