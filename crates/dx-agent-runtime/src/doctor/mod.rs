...
pub async fn run(config: &Config) -> Result<()> {
    ...
    println!("🩺 DX Agent Doctor (enhanced)");
    println!();
...
fn create_doctor_model_provider(
    config: &Config,
    provider_name: &str,
) -> anyhow::Result<Box<dyn dx_agent_api::model_provider::ModelProvider>> {
    ...
    match provider_name.split_once('.') {
        Some((family, alias)) => dx_agent_providers::create_model_provider_for_alias(
            config, family, alias, api_key, &options,
        ),
        None => {
            dx_agent_providers::create_model_provider_with_options(provider_name, api_key, &options)
        }
    }
}
...
pub async fn run_models(
    ...
    println!("🩺 DX Agent Doctor — Model Catalog Probe");
...
fn provider_validation_error(name: &str) -> Option<String> {
    match dx_agent_providers::create_model_provider(name, None) {
        Ok(_) => None,
        Err(err) => Some(
            err.to_string()
                .lines()
...
mod tests {
    ...
    fn config_validation_warns_empty_model_route() {
        let config = Config {
            model_routes: vec![dx_agent_config::schema::ModelRouteConfig {
                hint: "fast".into(),
    ...
    fn config_validation_warns_empty_embedding_route_model() {
        let config = Config {
            embedding_routes: vec![dx_agent_config::schema::EmbeddingRouteConfig {
                hint: "semantic".into(),
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(
            first
                ...
                .is_some_and(|name| name.starts_with(".dx-agent_doctor_probe_"))
        );
    }
