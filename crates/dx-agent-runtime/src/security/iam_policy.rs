...
/// Maps a single Nevis role to DX Agent permissions.
...
/// IAM policy engine that maps Nevis roles to DX Agent tool permissions.
///
...
impl IamPolicy {
    ...
    pub fn evaluate_tool_access(
        ...
        for role in &identity.roles {
            ...
            {
                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"user_id": crate::security::redact(&identity.user_id), "role": key, "tool": normalized_tool})), "IAM policy: tool access ALLOWED");
        ...
        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"user_id": crate::security::redact(&identity.user_id), "tool": normalized_tool})), "IAM policy: tool access DENIED");
    ...
    pub fn evaluate_workspace_access(
        ...
        for role in &identity.roles {
            ...
            {
                ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"user_id": crate::security::redact(&identity.user_id), "role": key, "workspace": normalized_ws})), "IAM policy: workspace access ALLOWED");
                return PolicyDecision::Allow;
            }
        }
        ...
        ::dx_agent_log::record!(INFO, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_attrs(::serde_json::json!({"user_id": crate::security::redact(&identity.user_id), "workspace": normalized_ws})), "IAM policy: workspace access DENIED");
        PolicyDecision::Deny(reason)
    }
...
mod tests {
    ...
    fn test_mappings() -> Vec<RoleMapping> {
        vec![
            RoleMapping {
                ...
                dx_agent_permissions: vec!["all".into()],
                workspace_access: vec!["all".into()],
            },
            RoleMapping {
                nevis_role: "operator".into(),
                dx_agent_permissions: vec![
                    "shell".into(),
                    "file_read".into(),
                    "file_write".into(),
                    "memory_search".into(),
            ...
            RoleMapping {
                ...
                dx_agent_permissions: vec!["file_read".into(), "memory_search".into()],
    ...
    fn duplicate_normalized_roles_are_rejected() {
        let mappings = vec![
            ...
            RoleMapping {
                nevis_role: " ADMIN ".into(),
                dx_agent_permissions: vec!["file_read".into()],
