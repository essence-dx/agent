...
/// - `X-DX Agent-Timestamp` — unix epoch seconds
/// - `X-DX Agent-Nonce` — random UUID v4
/// - `X-DX Agent-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-DX Agent-Timestamp", timestamp.to_string())
            .header("X-DX Agent-Nonce", &nonce)
            .header("X-DX Agent-Signature", &signature)
