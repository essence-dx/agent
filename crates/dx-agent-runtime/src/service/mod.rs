...
const SERVICE_LABEL: &str = "com.dx-agent.daemon";
const WINDOWS_TASK_NAME: &str = "DX Agent Daemon";
...
/// Returns whether the DX Agent daemon service is currently running.
...
fn render_macos_launch_agent_plist(
    ...
    // When running under Homebrew, inject DX_AGENT_CONFIG_DIR and
...
fn resolve_invoking_user_config_dir() -> Option<PathBuf> {
        ...
        && output.status.success()
    {
        ...
        if fields.len() >= 6 {
            return Some(PathBuf::from(fields[5]).join(".dx-agent"));
        }
    }
...
// integration dependencies and should run in every dx-agent-runtime test build.
#[cfg(test)]
...
mod tests {
    ...
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "DX Agent Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        assert!(script.contains("description=\"DX Agent daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/zeroclaw\""));
        assert!(script.contains("command_args=\"--config-dir /etc/zeroclaw daemon\""));
        assert!(!script.contains("env DX_AGENT_CONFIG_DIR"));
        assert!(!script.contains("env DX_AGENT_WORKSPACE"));
