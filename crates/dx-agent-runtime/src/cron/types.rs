...
impl ::dx_agent_api::attribution::Attributable for CronJob {
    fn role(&self) -> ::dx_agent_api::attribution::Role {
        let kind = match self.schedule {
            Schedule::Cron { .. } => ::dx_agent_api::attribution::CronKind::Cron,
            Schedule::At { .. } => ::dx_agent_api::attribution::CronKind::At,
            Schedule::Every { .. } => ::dx_agent_api::attribution::CronKind::Interval,
        };
        ::dx_agent_api::attribution::Role::Cron(kind)
    }
    fn alias(&self) -> &str {
        &self.id
    }
