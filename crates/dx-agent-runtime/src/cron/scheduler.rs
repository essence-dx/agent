...
use dx_agent_config::schema::{CronJobDecl, CronScheduleDecl};
...
pub async fn execute_job_now(config: &Config, job: &CronJob) -> (bool, String) {
    ...
    let span = dx_agent_log::attribution_span!(job);
...
async fn process_due_jobs(
    ...
    let mut in_flight = stream::iter(jobs.into_iter().filter_map(|job| {
        ...
        let Some(agent_alias) = resolve_owning_agent(config, &job) else {
            ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"job_id": job.id})), "Cron job has no owning agent; add the alias to an [agents.<x>].cron_jobs list");
            return None;
        };
        ...
        let security = match SecurityPolicy::for_agent(config, &agent_alias) {
            Ok(s) => Arc::new(s),
            Err(e) => {
                ::dx_agent_log::record!(WARN, ::dx_agent_log::Event::new(module_path!(), ::dx_agent_log::Action::Note).with_outcome(::dx_agent_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"job_id": job.id, "agent": agent_alias, "error": format!("{}", e)})), "Cron job: failed to build SecurityPolicy for owning agent");
                return None;
            }
        };
...
async fn run_agent_job(
    ...
    let memory_context = if !job.uses_memory {
        ...
        match dx_agent_memory::create_memory_for_agent(
            config,
            agent_alias,
            config
    ...
    let subagent_span = dx_agent_log::info_span!(
        "subagent",
        category = "cron",
    ...
    match run_result {
        ...
        Err(e) => {
            ...
            let mem_session_key = dx_agent_api::session_keys::sanitize_session_key(&format!(
                "cli:{}",
                session_path.display()
            ));
            if let Ok(mem) = dx_agent_memory::create_memory_for_agent(
                config,
                agent_alias,
                config
                    .model_provider_for_agent(agent_alias)
...
///
/// Uses `sh -c <command>` (non-login shell). On Windows, DX Agent users
...
mod tests {
    ...
    async fn test_config(tmp: &TempDir) -> Config {
        ...
        config.runtime_profiles.insert(
            TEST_AGENT.to_string(),
            dx_agent_config::schema::RuntimeProfileConfig::default(),
        );
    ...
    async fn persist_job_result_delivery_stubbed_succeeds() {
        // Delivery is stubbed (moved to dx-agent-channels orchestrator).
    ...
    // scan_and_redact_output tests moved to dx-agent-channels orchestrator
