...
pub fn sync_declarative_jobs(
    config: &Config,
    decls: &std::collections::HashMap<String, dx_agent_config::schema::CronJobDecl>,
) -> Result<()> {
    use dx_agent_config::schema::CronScheduleDecl;
        ...
        for (id, decl) in decls {
            ...
            if exists {
                ...
                let schedule_changed =
                    current_schedule_raw.as_deref() != Some(schedule_json.as_str());
...
fn validate_decl(id: &str, decl: &dx_agent_config::schema::CronJobDecl) -> Result<()> {
    if id.trim().is_empty() {
...
fn convert_schedule_decl(decl: &dx_agent_config::schema::CronScheduleDecl) -> Result<Schedule> {
    use dx_agent_config::schema::CronScheduleDecl;
    match decl {
...
fn convert_delivery_decl(decl: &dx_agent_config::schema::DeliveryConfigDecl) -> DeliveryConfig {
    DeliveryConfig {
...
mod tests {
    ...
    fn make_shell_decl(
        ...
        expr: &str,
        cmd: &str,
    ) -> (String, dx_agent_config::schema::CronJobDecl) {
        (
            id.to_string(),
            dx_agent_config::schema::CronJobDecl {
                name: Some(format!("decl-{id}")),
                job_type: "shell".to_string(),
                schedule: dx_agent_config::schema::CronScheduleDecl::Cron {
                    expr: expr.to_string(),
                    tz: None,
                },
    ...
    fn decls_map(
        items: Vec<(String, dx_agent_config::schema::CronJobDecl)>,
    ) -> std::collections::HashMap<String, dx_agent_config::schema::CronJobDecl> {
    ...
    fn sync_every_schedule_works() {
        ...
        let decl = dx_agent_config::schema::CronJobDecl {
            name: None,
            job_type: "shell".to_string(),
            schedule: dx_agent_config::schema::CronScheduleDecl::Every { every_ms: 60000 },
...
schedule = { kind = "every", every_ms = 300000 }
        ...
        struct Wrap {
            cron: std::collections::HashMap<String, dx_agent_config::schema::CronJobDecl>,
        }
        ...
        assert!(matches!(
            report.schedule,
            dx_agent_config::schema::CronScheduleDecl::Cron { ref expr, .. } if expr == "0 9 * * *"
        ));
        ...
        assert!(matches!(
            health.schedule,
            dx_agent_config::schema::CronScheduleDecl::Every { every_ms: 300_000 }
        ));
    }
}
