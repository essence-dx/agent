...
//! direct `dx_agent_log::record!` invocations carrying typed attribution
...
//! directly to `dx_agent_log::record!`.

use std::path::Path;

use dx_agent_log::LogEvent;

pub use dx_agent_log::{LogEvent as RuntimeTraceEvent, LogFilter, LogPage};

fn to_log_config(config: &dx_agent_config::schema::ObservabilityConfig) -> dx_agent_log::LogConfig {
    dx_agent_log::LogConfig {
...
pub fn init_from_config(
    config: &dx_agent_config::schema::ObservabilityConfig,
    workspace_dir: &Path,
) {
    dx_agent_log::init_from_config(&to_log_config(config), workspace_dir);
}
...
pub fn resolve_trace_path(
    ...
    let policy = dx_agent_log::ResolvedPolicy::from_config(&to_log_config(config), workspace_dir);
    policy.path
}
...
pub fn find_event_by_id(path: &Path, id: &str) -> anyhow::Result<Option<LogEvent>> {
    dx_agent_log::find_event_by_id(path, id)
}
