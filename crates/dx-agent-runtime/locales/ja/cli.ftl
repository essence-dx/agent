...
cli-config-about = DX Agent設定を管理
cli-update-about = DX Agent更新を確認・適用
cli-self-test-about = 診断自己テストを実行
cli-completions-about = シェル補完スクリプトを生成
cli-desktop-about = DX Agentコンパニオンデスクトップアプリを起動
...
cli-wechat-bound-success = ✅ WeChatアカウントが正常にバインドされました。これで DX Agent と会話できます。
...
cli-skills-install-tier-community =
    ...
    このスキルは DX Agent による監査を受けていません。スキルの内容を確認し、
...
cli-peripheral-flash-about = Arduino ボードに DX Agent ファームウェアをフラッシュ
cli-sop-list-about = ロードされた SOP を一覧表示
cli-sop-validate-about = SOP 定義を検証
cli-sop-show-about = SOP の詳細を表示
cli-migrate-openclaw-about = OpenClaw ワークスペースからこの DX Agent ワークスペースにメモリをインポート
...
cli-daemon-long-about =
    ...
    完全な DX Agent ランタイムを起動します: ゲートウェイサーバー、すべての設定されたチャネル（Telegram、Discord、Slack など）、ハートビートモニター、および cron スケジューラー。これは本番環境またはオンアシスタントとして DX Agent を実行する推奨方法です。
...
cli-channel-long-about =
    ...
    DX Agent をメッセージングプラットフォームに接続するチャネルを追加、削除、一覧表示、送信、およびヘルスチェックします。サポートされるチャネルタイプ: telegram、discord、slack、whatsapp、matrix、imessage、email。
...
cli-self-test-long-about =
    診断自己テストを実行して DX Agent インストールを検証します。
...
cli-desktop-long-about =
    DX Agent コンパニオンデスクトップアプリを起動します。
...
cli-skills-create-hint = {"  "}作成: mkdir -p ~/.dx_agent/workspace/skills/my-skill
...
cli-otp-initialized = DX Agent用のOTPシークレットを初期化しました。
...
cli-status-title = 🦀 DX Agent ステータス
...
cli-desktop-not-installed = DX Agent コンパニオンアプリがインストールされていません。
