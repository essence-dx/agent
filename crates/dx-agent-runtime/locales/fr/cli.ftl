...
cli-config-about = Gérer la configuration de DX Agent
cli-update-about = Vérifier et appliquer les mises à jour de DX Agent
cli-self-test-about = Exécuter les tests d'autodiagnostic
cli-completions-about = Générer des scripts d'achèvement de shell
cli-desktop-about = Lancer l'application de bureau companion DX Agent
...
cli-wechat-bound-success = ✅ Compte WeChat lié avec succès. Vous pouvez maintenant parler à DX Agent.
...
cli-skills-install-tier-community =
    ...
    Ce skill n'est pas audité par DX Agent. Examinez le contenu du skill
...
cli-peripheral-flash-about = Flasher le firmware de DX Agent sur une carte Arduino
...
cli-migrate-openclaw-about = Importer la mémoire d'un espace de travail OpenClaw vers cet espace de travail DX Agent
...
cli-daemon-long-about =
    ...
    Lance l'exécution Runtime complète de DX Agent : serveur gateway, tous les canaux configurés (Telegram, Discord, Slack, etc., moniteur de cœur et planificateur cron. C'est la méthode recommandée pour exécuter DX Agent en production ou comme assistant toujours actif.
...
cli-channel-long-about =
    ...
    Ajouter, supprimer, lister, envoyer et vérifier la santé des canaux qui connectent DX Agent aux plateformes de messagerie. Types de canaux pris en charge : telegram, discord, slack, whatsapp, matrix, imessage, email.
...
cli-config-long-about =
    Gérer la configuration de DX Agent.
...
cli-update-long-about =
    Vérifie et applique les mises à jour de DX Agent.
...
cli-self-test-long-about =
    Exécute les tests d'auto-diagnostic pour vérifier l'installation de DX Agent.
...
cli-desktop-long-about =
    Lance l'application de bureau compagnon DX Agent.
...
cli-skills-create-hint = {"  "}Créez-en une : mkdir -p ~/.dx_agent/workspace/skills/my-skill
...
cli-otp-initialized = Secret OTP initialisé pour DX Agent.
...
cli-status-title = 🦀 État de DX Agent
...
cli-desktop-download = Téléchargez l'application compagnon DX Agent :
...
cli-desktop-launching = Lancement de l'application compagnon DX Agent...
...
cli-desktop-not-installed = L'application compagnon DX Agent n'est pas installée.
