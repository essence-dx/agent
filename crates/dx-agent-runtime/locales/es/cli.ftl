...
cli-config-about = Gestiona la configuración de DX Agent
cli-update-about = Comprueba y aplica las actualizaciones de DX Agent
cli-self-test-about = Ejecuta autopruebas de diagnóstico
cli-completions-about = Genera scripts de autocompletado del shell
cli-desktop-about = Inicia la aplicación de escritorio complementaria de DX Agent
...
cli-wechat-bound-success = ✅ Cuenta de WeChat vinculada correctamente. Ya puedes hablar con DX Agent.
...
cli-skills-install-tier-community =
    ...
    Esta skill no está auditada por DX Agent. Revisa el contenido de la skill
...
cli-peripheral-flash-about = Flashea el firmware de DX Agent a una placa Arduino
...
cli-migrate-openclaw-about = Importa memoria de un espacio de trabajo OpenClaw a este espacio de trabajo DX Agent
...
cli-daemon-long-about =
    ...
    Lanza el entorno de ejecución completo de DX Agent: servidor de gateway, todos los canales configurados (Telegram, Discord, Slack, etc.), monitor de heartbeat y el programador cron. Esta es la forma recomendada de ejecutar DX Agent en producción o como un asistente siempre activo.
...
cli-channel-long-about =
    ...
    Agrega, elimina, lista, envía y verifica el estado de los canales que conectan DX Agent con plataformas de mensajería. Tipos de canal admitidos: telegram, discord, slack, whatsapp, matrix, imessage, email.
...
cli-config-long-about =
    Gestiona la configuración de DX Agent.
...
cli-update-long-about =
    Comprueba y aplica actualizaciones de DX Agent.
...
cli-self-test-long-about =
    Ejecuta autodiagnósticos para verificar la instalación de DX Agent.
...
cli-desktop-long-about =
    Lanza la aplicación de escritorio complementaria de DX Agent.
...
cli-skills-create-hint = {"  "}Cree uno: mkdir -p ~/.dx_agent/workspace/skills/my-skill
...
cli-otp-initialized = Secreto OTP inicializado para DX Agent.
...
cli-status-title = 🦀 Estado de DX Agent
...
cli-desktop-download = Descarga la aplicación complementaria de DX Agent:
...
cli-desktop-launching = Iniciando la aplicación complementaria de DX Agent...
...
cli-desktop-not-installed = La aplicación complementaria de DX Agent no está instalada.
