...
cli-config-about = 管理 DX Agent 配置
cli-update-about = 检查并应用 DX Agent 更新
cli-self-test-about = 运行诊断自检
cli-completions-about = 生成 shell 补全脚本
cli-desktop-about = 启动 DX Agent 伴侣桌面应用
...
cli-wechat-bound-success = ✅ WeChat 账号绑定成功。现在可以和 DX Agent 对话了。
...
cli-peripheral-flash-about = 将 DX Agent 固件刷写到 Arduino 开发板
cli-sop-list-about = 列出已加载的 SOP
cli-sop-validate-about = 验证 SOP 定义
cli-sop-show-about = 显示 SOP 的详细信息
cli-migrate-openclaw-about = 将 OpenClaw 工作区中的记忆导入到此 DX Agent 工作区
...
cli-daemon-long-about =
    ...
    启动完整的 DX Agent 运行时：网关服务器、所有已配置的通道（Telegram、Discord、Slack 等）、心跳监视器以及 cron 调度器。这是在生产环境中或作为始终在线助手运行 DX Agent 的推荐方式。
...
cli-channel-long-about =
    ...
    添加、删除、列出、发送以及对将 DX Agent 连接到消息平台的通道进行健康检查。支持的通道类型：telegram、discord、slack、whatsapp、matrix、imessage、email。
...
cli-skills-create-hint = {"  "}创建一个: mkdir -p ~/.dx_agent/workspace/skills/my-skill
...
cli-otp-initialized = 已为 DX Agent 初始化 OTP 密钥。
...
cli-desktop-not-installed = 未安装 DX Agent 配套应用。
