...
cli-try-quickstart = Try `dx-agents quickstart` to create your first agent.
...
cli-config-about = Manage DX Agents configuration
cli-update-about = Check for and apply DX Agents updates
cli-self-test-about = Run diagnostic self-tests
cli-completions-about = Generate shell completion scripts
cli-desktop-about = Launch the DX Agents companion desktop app
...
cli-wechat-bound-success = ✅ WeChat account bound successfully. You can talk to DX Agent now.
...
cli-skills-install-tier-community =
    ...
    This skill is not audited by DX Agent. Review the skill content
...
cli-skills-bundle-add-prompt =
    To create skill-bundle '{$alias}' with directory '{$dir}', run:
      dx-agents config map-key skill-bundles {$alias}
      dx-agents config set skill-bundles.{$alias}.directory {$dir}

    (Direct bundle creation through `dx-agents skills bundle add` would duplicate the config mutation surface.)

cli-skills-bundle-remove-prompt =
    To remove skill-bundle '{$alias}', run:
      dx-agents config map-key-delete skill-bundles {$alias}
...
cli-skills-bundle-list-empty =
    No skill bundles configured.
      Create one: dx-agents config set skill-bundles.default.directory shared/skills/default
...
cli-peripheral-flash-about = Flash DX Agent firmware to an Arduino board
...
cli-migrate-openclaw-about = Import memory from an OpenClaw workspace into this DX Agent workspace
...
cli-daemon-long-about =
    ...
    Launches the full DX Agents runtime: gateway server, all configured channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and the cron scheduler. This is the recommended way to run DX Agents in production or as an always-on assistant.

    Use 'dx-agents service install' to register the daemon as an OS service (systemd/launchd) for auto-start on boot.
...
cli-channel-long-about =
    ...
    Add, remove, list, send, and health-check channels that connect DX Agents to messaging platforms. Supported channel types: telegram, discord, slack, whatsapp, matrix, imessage, email.
...
# `crates/dx-agent-runtime/src/doctor/mod.rs` and `src/commands/self_test.rs`.
...
cli-skills-create-hint = {"  "}Create one: mkdir -p ~/.dx_agent/workspace/skills/my-skill
...
cli-onboard-deprecated = `dx-agents onboard` is deprecated — use `dx-agents quickstart`.
cli-otp-initialized = Initialized OTP secret for DX Agents.
...
cli-status-title = DX Agents Status
...
cli-desktop-download = Download the DX Agents companion app:
...
cli-desktop-launching = Launching DX Agents companion app...
...
cli-desktop-not-installed = DX Agents companion app is not installed.
...
cli-config-review-hint = Run `dx-agents config list` to review, then set required fields.
...
cli-channels-start-hint = To start channels: dx-agents channel start
cli-channels-doctor-hint = To check health:    dx-agents channel doctor
cli-channels-configure-hint = To configure:      zeroclaw onboard
