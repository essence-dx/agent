...
fn update_repository() -> String {
    std::env::var(UPDATE_REPOSITORY_ENV)
        .ok()
        .and_then(|value| normalize_repository_slug(&value))
        .unwrap_or_else(|| DEFAULT_UPDATE_REPOSITORY.to_string())
}

fn normalize_repository_slug(value: &str) -> Option<String> {
...
fn github_releases_latest_url_for_repo(repository: &str) -> String {
    let repository = normalize_repository_slug(repository)
        .unwrap_or_else(|| DEFAULT_UPDATE_REPOSITORY.to_string());
    format!("https://api.github.com/repos/{repository}/releases/latest")
}

fn github_releases_tag_url_for_repo(repository: &str, tag: &str) -> String {
    let repository = normalize_repository_slug(repository)
        .unwrap_or_else(|| DEFAULT_UPDATE_REPOSITORY.to_string());
    format!("https://api.github.com/repos/{repository}/releases/tags/{tag}")
}

fn update_user_agent(version: &str) -> String {
    format!("{PRIMARY_BINARY_NAME}/{version}")
}
...
pub async fn check(target_version: Option<&str>) -> Result<UpdateInfo> {
    ...
    let client = reqwest::Client::builder()
        .user_agent(update_user_agent(&current))
    ...
    let url = match target_version {
        Some(v) => {
            ...
            github_releases_tag_url_for_repo(&repository, &tag)
        }
        None => github_releases_latest_url_for_repo(&repository),
    };
...
pub async fn run(target_version: Option<&str>) -> Result<()> {
    ...
    let download_path = temp_dir.path().join("dx_agents_new");
    download_binary(
        &download_url,
...
fn find_asset_url(release: &serde_json::Value) -> Option<String> {
    ...
    find_asset_url_with_prefix(assets, target, PRIMARY_BINARY_NAME)
        .or_else(|| find_asset_url_with_prefix(assets, target, LEGACY_BINARY_NAME))
}

fn find_asset_url_with_prefix(
    ...
    assets.iter().find_map(|asset| {
        ...
        if !is_installable_release_asset_with_prefix(name, target, prefix) {
            return None;
        }
...
fn is_installable_release_asset(name: &str, target: &str) -> bool {
    is_installable_release_asset_with_prefix(name, target, PRIMARY_BINARY_NAME)
        || is_installable_release_asset_with_prefix(name, target, LEGACY_BINARY_NAME)
}

fn is_installable_release_asset_with_prefix(name: &str, target: &str, prefix: &str) -> bool {
    name == format!("{prefix}-{target}.tar.gz")
        || name == format!("{prefix}-{target}.tgz")
        || (target.contains("-windows-") && name == format!("{prefix}-{target}.zip"))
}
...
async fn download_binary(url: &str, sha256sums_url: Option<&str>, dest: &Path) -> Result<()> {
    let client = reqwest::Client::builder()
        .user_agent(update_user_agent(env!("CARGO_PKG_VERSION")))
    ...
    // Release assets are archives containing the primary CLI binary.
    // Extract the binary from the archive instead of writing the raw tarball.
    let asset_name = asset_name_from_url(url).unwrap_or_else(|| url.to_string());
...
fn extract_tar_gz(archive_bytes: &[u8], dest: &Path) -> Result<()> {
    ...
    for entry in archive.entries().context("failed to read tar entries")? {
        ...
        if is_expected_binary_file_name(file_name) {
            let mut buf = Vec::new();
            entry
                .read_to_end(&mut buf)
    ...
    bail!("archive does not contain a '{PRIMARY_BINARY_NAME}' binary")
}
...
fn extract_zip(archive_bytes: &[u8], dest: &Path) -> Result<()> {
    ...
    bail!("zip archive does not contain a '{PRIMARY_BINARY_NAME}' binary")
}
...
async fn validate_binary(path: &Path) -> Result<()> {
    ...
    if !version_output_identifies_expected_binary(&stdout) {
        bail!("downloaded binary does not appear to be {PRIMARY_BINARY_NAME}");
    }

    Ok(())
}

fn version_output_identifies_expected_binary(stdout: &str) -> bool {
    ...
    matches!(
        first_token.as_str(),
        PRIMARY_BINARY_NAME | LEGACY_BINARY_NAME
    )
}
...
mod tests {
    ...
    #[test]
    fn release_api_urls_default_to_dx_agents_repository() {
        assert_eq!(
            github_releases_latest_url_for_repo(DEFAULT_UPDATE_REPOSITORY),
            "https://api.github.com/repos/millercarla211-ctrl/dx-agents/releases/latest"
        );
        assert_eq!(
            github_releases_tag_url_for_repo(DEFAULT_UPDATE_REPOSITORY, "v1.2.3"),
            "https://api.github.com/repos/millercarla211-ctrl/dx-agents/releases/tags/v1.2.3"
        );
    }
    ...
    #[test]
    fn find_asset_url_prefers_dx_agents_asset_over_legacy_zeroclaw_asset() {
        let target = current_target_triple().expect("supported test platform");
        let release = make_release(&[
            &format!("zeroclaw-{target}.tar.gz"),
    ...
    #[test]
    fn extract_tar_gz_finds_dx_agents_binary() {
        use flate2::Compression;
        use flate2::write::GzEncoder;
        use std::io::Write;
        ...
        let tmp = tempfile::tempdir().unwrap();
        let dest = tmp.path().join("dx_agents_extracted");
        extract_tar_gz(&gz_buf, &dest).unwrap();
    ...
    #[test]
    fn extract_zip_finds_dx_agents_binary() {
        use std::io::Write;

        let fake_binary = b"@echo off\r\necho dx-agents\r\n";
        ...
        let tmp = tempfile::tempdir().unwrap();
        let dest = tmp.path().join("dx_agents_extracted.exe");
        extract_zip(&archive_bytes, &dest).unwrap();
