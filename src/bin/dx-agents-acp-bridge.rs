...
async fn load_acp_bridge_target() -> Result<BridgeTarget> {
    ...
    let pair_code = pair_code_from_args(args)?.or_else(pair_code_from_env);
...
fn token_from_env() -> Option<String> {
    env_value_from_keys(ACP_BRIDGE_TOKEN_ENV, LEGACY_ACP_BRIDGE_TOKEN_ENV)
}

fn pair_code_from_env() -> Option<String> {
    env_value_from_keys(
        ACP_BRIDGE_PAIRING_CODE_ENV,
        LEGACY_ACP_BRIDGE_PAIRING_CODE_ENV,
    )
}

fn env_value_from_keys(primary: &str, legacy: &str) -> Option<String> {
    env_value(primary).or_else(|| env_value(legacy))
}
...
mod tests {
    use super::{
        ...
        pair_code_from_args, pair_code_from_env, read_cached_token, token_from_env,
        write_cached_token, write_frame,
    };
    use std::sync::Mutex;
    ...
    fn missing_config_error_matches_acp_client_guidance() {
        assert_eq!(
            ...
            "ERROR: config.toml not found. Are you sure the bridge and DX Agents are running on the same host? Tool use will not work remotely."
        );
    }
