...
#[cfg(feature = "agent-runtime")]
mod cli_input;
mod cli_provider_json;
mod commands;
#[cfg(feature = "agent-runtime")]
mod rag {
    pub use dx_agents::rag::*;
}
#[cfg(feature = "agent-runtime")]
mod browse;
mod config;
...
enum Commands {
    ...
    Quickstart {
        /// Provider type (anthropic / gemini / groq / openai / openrouter / ollama).
        #[arg(long)]
        ...
        /// Environment variable that contains the API key for the new provider entry.
        #[arg(long = "api-key-env", value_name = "ENV_VAR")]
...
Examples:
  ...
  dx-agent agent -a dx --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
        ...
        /// Model provider to use (anthropic, gemini, groq, openai, openai-codex, openrouter)
...
Launches the full DX Agents runtime: gateway server, all configured \
...
Examples:
  ...
  dx-agent cron update TASK_ID --expression '0 8 * * *' --tz Europe/London")]
    ...
    Providers {
        #[command(subcommand)]
        provider_command: Option<ProviderCommands>,

        /// Print Zed-safe provider discovery JSON.
        #[arg(long)]
        json: bool,
    },
...
Add, remove, list, send, and health-check channels that connect DX Agents \
...
Examples:
  ...
  zeroclaw hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: dx_agents::HardwareCommands,
    },
...
Examples:
  ...
  zeroclaw peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: dx_agents::PeripheralCommands,
    },
...
Examples:
  ...
  zeroclaw memory clear --category conversation --yes")]
    ...
    // i18n-exempt: clap derive help — framework requires a compile-time literal
    #[command(long_about = "\
Manage DX Agents configuration.
...
Property path tab completion is included automatically in `dx-agent completions <shell>`.")]
    ...
    // i18n-exempt: clap derive help — framework requires a compile-time literal
    #[command(long_about = "\
Check for and apply DX Agents updates.
...
Examples:
  ...
  dx-agent update --version 0.6.0     # install specific version")]
    ...
    // i18n-exempt: clap derive help — framework requires a compile-time literal
    #[command(long_about = "\
Run diagnostic self-tests to verify the DX Agents installation.
...
Examples (Windows PowerShell):
  ...
  dx-agent completions powershell > $PROFILE.CurrentUserAllHosts")]
    ...
    // i18n-exempt: clap derive help — framework requires a compile-time literal
    #[command(long_about = "\
Launch the DX Agents companion desktop app.
...
#[cfg(feature = "agent-runtime")]
fn resolve_quickstart_api_key_env<F>(
...
{
    let Some(name) = api_key_env.map(str::trim).filter(|name| !name.is_empty()) else {
        return Ok(None);
    };

    if name.contains('=') {
        anyhow::bail!("--api-key-env expects an environment variable name, not an assignment");
    }

    let value = get_env(name).map_err(|_| {
        anyhow::Error::msg(format!(
            "--api-key-env {name} is not set or is not valid Unicode"
        ))
    })?;
    let value = value.trim().to_string();
    if value.is_empty() {
        anyhow::bail!("--api-key-env {name} is set but empty");
    }
    Ok(Some(value))
}
...
/// functions.
///
/// Flag pre-fills (`--model-provider`, `--model`, `--api-key-env`,
...
async fn run_quickstart_cli(
    ...
    let api_key =
        resolve_quickstart_api_key_env(api_key_env.as_deref(), |name| std::env::var(name))?;
    ...
    let should_auto_apply = model_provider.is_some()
        && model.is_some()
        && agent.is_some()
        && form.provider_done()
        && form.agent_done();
    if model_provider.is_some() && model.is_some() && agent.is_some() && !form.provider_done() {
        anyhow::bail!(
            "quickstart flags did not resolve to a complete model provider; check --model-provider, --model, and --api-key-env"
        );
    }
    if should_auto_apply {
        form.risk = Some(PresetChoice::Fresh("balanced"));
        form.runtime = Some(PresetChoice::Fresh("balanced"));
        form.memory = Some(MemoryChoice::Sqlite);
...
enum ModelCommands {
    ...
    List {
        ...
        /// Print Zed-safe model discovery JSON.
        #[arg(long)]
        json: bool,
    },
...
enum ProviderCommands {
    ...
    /// Build a provider health matrix
    Health {
        /// Probe a specific provider only.
        #[arg(long)]
        provider: Option<String>,

        /// Run a bounded live model-list probe for the selected provider.
        #[arg(long)]
        live: bool,

        /// Print Zed-safe provider health JSON.
        #[arg(long)]
        json: bool,
    },
}
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!(
                ...
                t(
                    "cli-otp-initialized",
                    "Initialized OTP secret for DX Agents."
                )
            );
            println!(
                "{}",
    ...
    #[cfg(not(feature = "agent-runtime"))]
    {
        ...
        match cli.command {
            Commands::Agent {
                ...
                let model_provider = dx_agents::providers::create_model_provider(
                    provider_name,
    ...
    match cli.command {
        ...
        Commands::Gateway { gateway_command } => {
            match gateway_command {
                ...
                Some(dx_agents::GatewayCommands::GetPaircode { new, port, host }) => {
                    let (port, host) = resolve_gateway_addr(&config, port, host);
                ...
                Some(dx_agents::GatewayCommands::Start { port, host }) => {
                    let (port, host) = resolve_gateway_addr(&config, port, host);
                    log_gateway_start(&host, port);
                    Box::pin(run_gateway_if_enabled(&host, port, config, None)).await
                }
        ...
        Commands::Status { format } => {
            ...
            println!("{}", t("cli-status-title", "DX Agents Status"));
            println!();
            println!(
                "{}",
                ta(
        ...
        Commands::Models { model_command } => match model_command {
            ModelCommands::List {
                model_provider,
                json: true,
            } => cli_provider_json::print_models_json(&config, model_provider.as_deref()),
            model_command => {
        ...
        Commands::Providers {
            ...
            Some(ProviderCommands::Health {
                provider,
                live,
                json,
            }) => {
                if json {
                    return dx_agents::cli_provider_health::print_providers_health_json(
                        &config,
                        provider.as_deref(),
                        live,
                    )
                ...
                dx_agents::cli_provider_health::print_providers_health(
                    &config,
                    provider.as_deref(),
                    live,
                )
            ...
            Some(ProviderCommands::List { json }) => {
                if json {
                    return cli_provider_json::print_providers_json(&config);
                }
            ...
            if do_install {
                println!(
                    ...
                    t(
                        ...
                        "Download the DX Agents companion app:"
                    )
                );
                println!();
                #[cfg(target_os = "macos")]
                ...
                Some(bin) => {
                    println!(
                        ...
                        t(
                            ...
                            "Launching DX Agents companion app..."
                        )
                    );
                ...
                None => {
                    println!(
                        ...
                        t(
                            ...
                            "DX Agents companion app is not installed."
                        )
                    );
                    println!();
                    println!(
        ...
        Commands::Plugin { plugin_command } => match plugin_command {
            PluginCommands::List => {
                let host = dx_agents::plugins::host::PluginHost::new(&config.data_dir)?;
            ...
            PluginCommands::Install { source } => {
                let mut host = dx_agents::plugins::host::PluginHost::new(&config.data_dir)?;
                host.install(&source)?;
                println!(
                    "{}",
                    ta(
...
fn write_shell_completion<W: Write>(shell: CompletionShell, writer: &mut W) -> Result<()> {
    ...
    let completion_fallback_fn = format!("{completion_fn}_clap_orig");

    match shell {
        CompletionShell::Bash => {
            generate(shells::Bash, &mut cmd, bin_name.clone(), writer);
            // Wrap clap's generated function to inject dynamic config path completion.
            writeln!(
                writer,
                r#"
# Dynamic completion for {bin_name} config get/set paths
if type {completion_fn} &>/dev/null; then
    ...
    eval "$(declare -f {completion_fn} | sed '1s/{completion_fn}/{completion_fallback_fn}/')"
    {completion_fn}() {{
        local cur="${{COMP_WORDS[COMP_CWORD]}}"
        if [[ "${{COMP_WORDS[*]}}" =~ "config "(get|set)" " ]]; then
            COMPREPLY=($(compgen -W "$({bin_name} config complete "$cur" 2>/dev/null)" -- "$cur"))
            return
        fi
        {completion_fallback_fn} "$@"
    }}
fi"#
            )?;
        }
        CompletionShell::Fish => {
            ...
            writeln!(
                ...
                r#"
# Dynamic completion for {bin_name} config get/set paths
complete -c {bin_name} -n '__fish_seen_subcommand_from config; and __fish_seen_subcommand_from get set' \
    -a '({bin_name} config complete (commandline -ct) 2>/dev/null)' -f"#
            )?;
        }
        CompletionShell::Zsh => {
...
if (( $+functions[{completion_fn}] )); then
    functions[{completion_fallback_fn}]=$functions[{completion_fn}]
    {completion_fn}() {{
        if [[ "${{words[*]}}" == *"config "(get|set)* ]] && (( CURRENT > 3 )); then
            local -a props
            props=(${{(f)"$({bin_name} config complete "$words[CURRENT]" 2>/dev/null)"}})
            compadd -a props
            return
        fi
        {completion_fallback_fn} "$@"
...
async fn run_gateway_if_enabled(
    host: &str,
    port: u16,
    config: dx_agents::config::Config,
...
async fn run_gateway_if_enabled(
    _host: &str,
    _port: u16,
    _config: dx_agents::config::Config,
...
mod tests {
    ...
    fn quickstart_cli_rejects_value_bearing_api_key_flag() {
        let result = Cli::try_parse_from([
            ...
            "gemini-2.0-flash",
            "--api-key",
            "secret-should-not-enter-cli-state",
        ]);

        assert!(
            result.is_err(),
            "quickstart should not accept API keys directly in argv"
        );
    }
    ...
    fn quickstart_cli_accepts_api_key_env_flag() {
        ...
        .expect("quickstart should accept an API key environment variable name");

        match cli.command {
            Commands::Quickstart { api_key_env, .. } => {
    ...
    fn providers_cli_accepts_list_json_subcommand() {
        ...
        match cli.command {
            Commands::Providers {
                provider_command: Some(ProviderCommands::List { json }),
                ..
            } => assert!(json),
            other => panic!("expected providers list command, got {other:?}"),
        }
    }
    ...
    fn providers_cli_accepts_health_json_subcommand() {
        ...
        .expect("providers health should accept JSON output");
        ...
        match cli.command {
            Commands::Providers {
                ...
                ..
            } => assert_eq!(provider.as_deref(), Some("nvidia")),
            other => panic!("expected providers health command, got {other:?}"),
        }
    }
    ...
    fn providers_cli_accepts_health_live_json_subcommand() {
        ...
        .expect("providers health should accept bounded live JSON output");
        ...
        match cli.command {
            ...
            other => panic!("expected providers live health command, got {other:?}"),
        }
    }
    ...
    fn models_list_cli_accepts_json_flag() {
        ...
        .expect("models list should accept JSON output");
        ...
        match cli.command {
            Commands::Models {
                ...
                assert_eq!(model_provider.as_deref(), Some("groq"));
                assert!(json);
            }
    ...
    #[test]
    #[cfg(feature = "agent-runtime")]
    fn quickstart_api_key_env_resolver_reads_named_variable() {
        let value = resolve_quickstart_api_key_env(Some(" GOOGLE_API_KEY "), |name| match name {
            "GOOGLE_API_KEY" => Ok(" secret-from-env ".to_string()),
            other => panic!("unexpected env lookup: {other}"),
        })
        .expect("env resolver should read configured variable");

        assert_eq!(value.as_deref(), Some("secret-from-env"));
    }

    #[test]
    #[cfg(feature = "agent-runtime")]
    fn quickstart_api_key_env_resolver_rejects_missing_or_empty_values() {
        let missing = resolve_quickstart_api_key_env(Some("GOOGLE_API_KEY"), |_| {
            Err(std::env::VarError::NotPresent)
        });
        assert!(missing.is_err());

        let empty = resolve_quickstart_api_key_env(Some("GOOGLE_API_KEY"), |_| Ok(" ".to_string()));
        assert!(empty.is_err());
    }

    #[test]
    #[cfg(feature = "agent-runtime")]
    fn quickstart_api_key_env_resolver_rejects_assignment_syntax() {
        let result = resolve_quickstart_api_key_env(Some("GOOGLE_API_KEY=secret"), |_| {
            unreachable!("assignment syntax should be rejected before env lookup")
        });
    ...
    #[test]
    #[cfg(feature = "agent-runtime")]
    fn bash_completion_dynamic_config_wrapper_uses_dx_agents_name() {
        let mut output = Vec::new();
        write_shell_completion(CompletionShell::Bash, &mut output)
            .expect("completion generation should succeed");
        let script = String::from_utf8(output).expect("completion output should be valid utf-8");
        ...
        assert!(
            ...
            "bash completion should not reference the upstream binary name"
        );
    }

    #[test]
    #[cfg(feature = "agent-runtime")]
    fn fish_completion_dynamic_config_wrapper_uses_dx_agents_name() {
        let mut output = Vec::new();
        write_shell_completion(CompletionShell::Fish, &mut output)
            .expect("completion generation should succeed");
        let script = String::from_utf8(output).expect("completion output should be valid utf-8");
        assert!(
        ...
        assert!(
            ...
            "fish completion should not query the upstream binary name"
        );
    }

    #[test]
    #[cfg(feature = "agent-runtime")]
    fn zsh_completion_dynamic_config_wrapper_uses_dx_agents_name() {
        let mut output = Vec::new();
        write_shell_completion(CompletionShell::Zsh, &mut output)
            .expect("completion generation should succeed");
        let script = String::from_utf8(output).expect("completion output should be valid utf-8");
        assert!(
        ...
        assert!(
            !script.contains("_zeroclaw") && !script.contains("zeroclaw config complete"),
            "zsh completion should not reference the upstream binary name"
        );
    }
    ...
    fn gateway_get_paircode_cli_accepts_port_and_host_overrides() {
        ...
        match cli.command {
            Commands::Gateway {
                gateway_command: Some(dx_agents::GatewayCommands::GetPaircode { new, port, host }),
            } => {
                assert!(new);
